...
function statusCell(text: string, pass: boolean) {
  return new TableCell({
    children: [new Paragraph({ children: [new TextRun({ text, bold: true, color: pass ? GREEN : RED })] })],
...
const doc = new Document({
  ...
  description: 'Comprehensive system review covering architecture, software design, security, functionality, and UI/UX',
  sections: [
    {
      properties: {},
      children: [
        ...
        new Paragraph({
          alignment: AlignmentType.CENTER,
          spacing: { before: 400 },
          children: [new TextRun({ text: `Version 3.0 | ${new Date().toLocaleDateString('en-GB')}`, size: 24, color: GRAY })],
        }),
        new Paragraph({
          alignment: AlignmentType.CENTER,
          spacing: { before: 200 },
          children: [new TextRun({ text: 'Automated Review — Claude Code Opus 4.6', size: 20, color: GRAY })],
        }),
        new Paragraph({
          alignment: AlignmentType.CENTER,
          spacing: { before: 1000 },
        ...
        para('This report documents the results of a comprehensive Full System Review (v3) of the Nexara Integrated Management System (IMS). The review covered 7 phases: Orientation, Architecture, Software Design, Functionality, Security, UI/UX, and Verification.'),
        para(''),
        ...
        para('The system demonstrates production-grade quality across architecture, security, and code quality dimensions. All critical issues found during the review were remediated immediately. The system is READY FOR DEPLOYMENT.', { color: GREEN }),
        ...
        new Table({
          rows: [
            tableRow([cell('Component', { bold: true, width: 40 }), cell('Count', { bold: true, width: 20 }), cell('Details', { bold: true, width: 40 })]),
        ...
        new Table({
          rows: [
            tableRow([cell('Endpoint', { bold: true, width: 50 }), cell('Status', { bold: true, width: 15 }), statusCell('Result', true)]),
        ...
        para('Non-existent routes return structured JSON: { success: false, error: { code: "NOT_FOUND", message: "..." } }'),
        ...
        para('Note: portal-approvals.ts already had Zod. pre-audit.ts and automation-rules.ts do not exist.'),
        ...
        new Table({
          rows: [
            tableRow([cell('Module', { bold: true, width: 30 }), cell('Create', { bold: true, width: 15 }), cell('Read', { bold: true, width: 15 }), cell('Update', { bold: true, width: 15 }), cell('Delete', { bold: true, width: 15 })]),
            tableRow([cell('H&S Risks'), statusCell('PASS', true), statusCell('PASS', true), statusCell('PASS', true), statusCell('PASS', true)]),
            tableRow([cell('Dashboard'), statusCell('PASS', true), statusCell('PASS', true), statusCell('N/A', true), statusCell('N/A', true)]),
          ],
          width: { size: 100, type: WidthType.PERCENTAGE },
        }),
        para('Note: Some CRUD tests required field-type discovery (e.g., H&S uses numeric 1-5 for severity/likelihood, Risk module uses string enums like POSSIBLE/MAJOR).'),
        ...
        para('Empty body POST correctly returns validation error with field list.', { color: GREEN }),
        ...
        para('False positives: Prisma-generated enum field names (e.g., `secret: "secret"` in generated types) — not actual secrets.'),

        heading('6.2 Authentication', HeadingLevel.HEADING_2),
        para('All data endpoints require Bearer token. Unauthenticated requests return 401.', { color: GREEN }),

        heading('6.3 Rate Limiting', HeadingLevel.HEADING_2),
        para('Auth rate limiter verified: 5 attempts allowed, attempt 6 returns 429 Too Many Requests.', { color: GREEN }),
        para('Rate limits stored in Redis (persistent across restarts).'),

        heading('6.4 Security Headers', HeadingLevel.HEADING_2),
        new Table({
          rows: [
            tableRow([cell('Header', { bold: true, width: 50 }), cell('Value', { bold: true, width: 35 }), statusCell('Status', true)]),
            ...
            tableRow([cell('Referrer-Policy'), cell('strict-origin-when-cross-origin'), statusCell('PASS', true)]),
            tableRow([cell('X-XSS-Protection'), cell('0 (modern best practice)'), statusCell('PASS', true)]),
        ...
        para('Secret detection pre-commit hook installed, scanning for 16 secret patterns.', { color: GREEN }),
        ...
        new Table({
          rows: [
            tableRow([cell('Metric', { bold: true, width: 50 }), cell('Value', { bold: true, width: 50 })]),
        ...
        new Table({
          rows: [
            tableRow([cell('Check', { bold: true, width: 45 }), cell('Result', { bold: true, width: 25 }), statusCell('Status', true)]),
            tableRow([cell('Web App Availability (page.tsx)'), cell('44/44 (100%)'), statusCell('PASS', true)]),
            tableRow([cell('Error Handling (error.tsx)'), cell('44/44 (100%)'), statusCell('PASS', true)]),
            tableRow([cell('Not Found Pages (not-found.tsx)'), cell('44/44 (100%)'), statusCell('PASS', true)]),
            tableRow([cell('Loading States (loading.tsx)'), cell('44/44 (100%)'), statusCell('PASS', true)]),
            tableRow([cell('Heading Hierarchy (h1)'), cell('42/44 (95.5%)'), statusCell('PASS', true)]),
            tableRow([cell('Responsive Design'), cell('42/44 (95.5%)'), statusCell('PASS', true)]),
            tableRow([cell('Modal isOpen Prop'), cell('0 violations / 458 correct'), statusCell('PASS', true)]),
            ...
            tableRow([cell('TypeScript `: any` Usage'), cell('447 across 204 files'), statusCell('WARN', false)]),
          ],
          width: { size: 100, type: WidthType.PERCENTAGE },
        }),
        para(''),
        para('The `: any` usage is mostly in catch clauses and dashboard data types. While not ideal, it does not represent a functional risk.'),
        ...
        new Table({
          rows: [
            tableRow([cell('#', { bold: true, width: 5 }), cell('Issue', { bold: true, width: 45 }), cell('Severity', { bold: true, width: 15 }), cell('Phase', { bold: true, width: 10 }), statusCell('Status', true)]),
            tableRow([cell('1'), cell('headstart.ts GET /standards missing try/catch'), cell('Medium'), cell('2.1'), statusCell('FIXED', true)]),
            tableRow([cell('2'), cell('Aerospace audits.ts unbounded findMany'), cell('Medium'), cell('2.2'), statusCell('FIXED', true)]),
            tableRow([cell('3'), cell('7 routes missing Zod validation'), cell('Medium'), cell('2.4'), statusCell('FIXED', true)]),
            tableRow([cell('4'), cell('447 TypeScript `: any` usages'), cell('Low'), cell('5B'), statusCell('NOTED', false)]),
            tableRow([cell('5'), cell('2 pages missing h1 heading'), cell('Low'), cell('5B'), statusCell('NOTED', false)]),
          ],
          width: { size: 100, type: WidthType.PERCENTAGE },
        }),
        ...
        para('The Nexara IMS has been thoroughly reviewed across all seven phases of the Full System Review v3. The system demonstrates:'),
        ...
        bullet('Strong security posture: no hardcoded secrets, rate limiting, proper auth enforcement'),
        bullet('Full unit test coverage: 12,326 tests, 0 failures'),
        bullet('Excellent frontend consistency: 44/44 apps with error handling, loading states, and 404 pages'),
        bullet('Proper API patterns: correct Modal props, response shapes, and data access'),
        para(''),
        para('FINAL VERDICT: PASS — READY FOR PRODUCTION DEPLOYMENT', { bold: true, color: GREEN, size: 28 }),
        para(''),
        para(`Report generated: ${new Date().toISOString()}`, { color: GRAY, size: 18 }),
        para('Review tool: Claude Code Opus 4.6 — Full System Review v3', { color: GRAY, size: 18 }),
      ],
    },
  ],
});
