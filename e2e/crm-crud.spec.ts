...
test.describe('CRM CRUD', () => {
  ...
  test('should list leads via API', async ({ page }) => {
    const response = await page.request.get('http://localhost:4000/api/crm/leads', {
      headers: {
  ...
  test('should list contacts via API', async ({ page }) => {
    const response = await page.request.get('http://localhost:4000/api/crm/contacts', {
      headers: {
        Authorization: `Bearer ${await page.evaluate(() => localStorage.getItem('token'))}`,
      },
    });
  ...
  test('should list deals via API', async ({ page }) => {
