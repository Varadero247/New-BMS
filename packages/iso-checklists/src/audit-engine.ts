...
export interface AuditClauseStatus {
  ...
  evidence: string[];
  mandatory: boolean;
  status: 'NOT_STARTED' | 'IN_PROGRESS' | 'CONFORMING' | 'MINOR_NC' | 'MAJOR_NC' | 'OBSERVATION' | 'NOT_APPLICABLE';
  findings: string[];
  objectiveEvidence: string[];
  auditorNotes: string;
}

export function createAuditPlan(standard: string, auditType: AuditPlan['auditType'], title: string, scope: string): AuditPlan | null {
...
export function calculateAuditScore(plan: AuditPlan): {
  ...
  return { total, assessed, conforming, minorNCs, majorNCs, observations, notApplicable, conformanceRate };
}

export function getClausesByStatus(plan: AuditPlan, status: AuditClauseStatus['status']): AuditClauseStatus[] {
  return plan.clauses.filter(c => c.status === status);
}

export function getMandatoryGaps(plan: AuditPlan): AuditClauseStatus[] {
  return plan.clauses.filter(c => c.mandatory && c.status !== 'CONFORMING' && c.status !== 'NOT_APPLICABLE');
}
