...
export type ServiceStatus = 'operational' | 'degraded' | 'partial-outage' | 'major-outage' | 'unknown';
...
const statusConfig: Record<ServiceStatus, { color: string; label: string; bg: string }> = {
  ...
  'partial-outage': { color: 'bg-orange-500', label: 'Partial Outage', bg: 'bg-orange-50 text-orange-700' },
...
export function StatusIndicator({ status, label, showLabel = true, size = 'md', pulse = false, className }: StatusIndicatorProps) {
  ...
  return (
    <div className={cn('inline-flex items-center gap-1.5', className)}>
      ...
      {showLabel && (
        <span className={cn('font-medium', sizeStyle.text, status === 'operational' ? 'text-green-700 dark:text-green-400' : status === 'degraded' ? 'text-yellow-700' : status === 'major-outage' ? 'text-red-700' : 'text-gray-600')}>
          {label || config.label}
        </span>
      )}
    </div>
