...
// ─── Algorithm puzzle phases (ph217dpx–ph218dpx) ──────────────────────────────
function moveZeroes217dpx(nums:number[]):number{let k=0;for(const n of nums)if(n!==0)nums[k++]=n;while(k<nums.length)nums[k++]=0;return nums[0];}
describe('ph217dpx_mz',()=>{
  it('a',()=>{expect(moveZeroes217dpx([0,1,0,3,12])).toBe(1);});
  it('b',()=>{expect(moveZeroes217dpx([0,0,1])).toBe(1);});
  it('c',()=>{expect(moveZeroes217dpx([1])).toBe(1);});
  it('d',()=>{expect(moveZeroes217dpx([0,0,0,1])).toBe(1);});
  it('e',()=>{expect(moveZeroes217dpx([4,2,0,0,3])).toBe(4);});
});
function missingNumber218dpx(nums:number[]):number{const n=nums.length;return n*(n+1)/2-nums.reduce((a,b)=>a+b,0);}
describe('ph218dpx_mn',()=>{
  it('a',()=>{expect(missingNumber218dpx([3,0,1])).toBe(2);});
  it('b',()=>{expect(missingNumber218dpx([0,1])).toBe(2);});
  it('c',()=>{expect(missingNumber218dpx([9,6,4,2,3,5,7,0,1])).toBe(8);});
  it('d',()=>{expect(missingNumber218dpx([0])).toBe(1);});
  it('e',()=>{expect(missingNumber218dpx([1])).toBe(0);});
});
