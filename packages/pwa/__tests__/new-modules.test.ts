...
function moveZeroes217g(nums:number[]):number{let k=0;for(const n of nums)if(n!==0)nums[k++]=n;while(k<nums.length)nums[k++]=0;return nums[0];}
describe('ph217g_mz',()=>{
  it('a',()=>{expect(moveZeroes217g([0,1,0,3,12])).toBe(1);});
  it('b',()=>{expect(moveZeroes217g([0,0,1])).toBe(1);});
  it('c',()=>{expect(moveZeroes217g([1])).toBe(1);});
  it('d',()=>{expect(moveZeroes217g([0,0,0,1])).toBe(1);});
  it('e',()=>{expect(moveZeroes217g([4,2,0,0,3])).toBe(4);});
});

function missingNumber218g(nums:number[]):number{const n=nums.length;return n*(n+1)/2-nums.reduce((a,b)=>a+b,0);}
describe('ph218g_mn',()=>{
  it('a',()=>{expect(missingNumber218g([3,0,1])).toBe(2);});
  it('b',()=>{expect(missingNumber218g([0,1])).toBe(2);});
  it('c',()=>{expect(missingNumber218g([9,6,4,2,3,5,7,0,1])).toBe(8);});
  it('d',()=>{expect(missingNumber218g([0])).toBe(1);});
  it('e',()=>{expect(missingNumber218g([1])).toBe(0);});
});

function countBits219g(n:number):number[]{const r=Array(n+1).fill(0);for(let i=1;i<=n;i++)r[i]=r[i>>1]+(i&1);return r;}
describe('ph219g_cb',()=>{
  it('a',()=>{expect(countBits219g(2)).toEqual([0,1,1]);});
  it('b',()=>{expect(countBits219g(5)).toEqual([0,1,1,2,1,2]);});
  it('c',()=>{expect(countBits219g(0)).toEqual([0]);});
  it('d',()=>{expect(countBits219g(1)).toEqual([0,1]);});
  it('e',()=>{expect(countBits219g(4)[4]).toBe(1);});
});

function hammingDistance221g(x:number,y:number):number{let xor=x^y,dist=0;while(xor){dist+=xor&1;xor>>=1;}return dist;}
describe('ph221g_hd',()=>{
  it('a',()=>{expect(hammingDistance221g(1,4)).toBe(2);});
  it('b',()=>{expect(hammingDistance221g(3,1)).toBe(1);});
  it('c',()=>{expect(hammingDistance221g(0,0)).toBe(0);});
  it('d',()=>{expect(hammingDistance221g(0,15)).toBe(4);});
  it('e',()=>{expect(hammingDistance221g(7,3)).toBe(1);});
});

function climbStairs224g(n:number):number{if(n<=2)return n;let a=1,b=2;for(let i=3;i<=n;i++){const c=a+b;a=b;b=c;}return b;}
describe('ph224g_cs',()=>{
  it('a',()=>{expect(climbStairs224g(2)).toBe(2);});
  it('b',()=>{expect(climbStairs224g(3)).toBe(3);});
  it('c',()=>{expect(climbStairs224g(1)).toBe(1);});
  it('d',()=>{expect(climbStairs224g(5)).toBe(8);});
  it('e',()=>{expect(climbStairs224g(10)).toBe(89);});
});

function singleNumber226g(nums:number[]):number{return nums.reduce((a,b)=>a^b,0);}
describe('ph226g_sn',()=>{
  it('a',()=>{expect(singleNumber226g([2,2,1])).toBe(1);});
  it('b',()=>{expect(singleNumber226g([4,1,2,1,2])).toBe(4);});
  it('c',()=>{expect(singleNumber226g([1])).toBe(1);});
  it('d',()=>{expect(singleNumber226g([0,1,0])).toBe(1);});
  it('e',()=>{expect(singleNumber226g([3,5,3])).toBe(5);});
});
