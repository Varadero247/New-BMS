...
router.get('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const [data, total] = await Promise.all([
      prisma.femFireRiskAssessment.findMany({
        where,
        skip,
    ...
    logger.error('Failed to fetch FRAs', { error: (error as Error).message });
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch fire risk assessments' },
    });
  }
});
...
router.get('/overdue', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to fetch overdue FRAs', { error: (error as Error).message });
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch overdue FRAs' },
    });
  }
});
...
router.post('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const overallRiskScore = calculateRiskScore(
      ...
      parsed.data.consequenceRating
    );
    const overallRiskLevel = calculateFireRiskLevel(
      parsed.data.likelihoodRating,
      parsed.data.consequenceRating
    );
    ...
    logger.error('Failed to create FRA', { error: (error as Error).message });
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to create fire risk assessment' },
    });
  }
});
...
router.get('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    if (!item)
      return res
        .status(404)
        .json({ success: false, error: { code: 'NOT_FOUND', message: 'FRA not found' } });
    res.json({ success: true, data: item });
  } catch (error: unknown) {
    logger.error('Failed to fetch FRA', { error: (error as Error).message });
    res
      .status(500)
      .json({ success: false, error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch FRA' } });
  }
});
...
router.put('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    if (parsed.data.assessmentDate)
      updateData.assessmentDate = new Date(parsed.data.assessmentDate);
    if (parsed.data.nextReviewDate)
      updateData.nextReviewDate = new Date(parsed.data.nextReviewDate);
    if (parsed.data.staffInformedDate)
      updateData.staffInformedDate = new Date(parsed.data.staffInformedDate);
    if (parsed.data.likelihoodRating && parsed.data.consequenceRating) {
      updateData.overallRiskScore = calculateRiskScore(
        parsed.data.likelihoodRating,
        parsed.data.consequenceRating
      );
      updateData.overallRiskLevel = calculateFireRiskLevel(
        parsed.data.likelihoodRating,
        parsed.data.consequenceRating
      );
    }
    ...
    logger.error('Failed to update FRA', { error: (error as Error).message });
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to update fire risk assessment' },
    });
  }
});
...
router.post('/:id/approve', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to approve FRA', { error: (error as Error).message });
    res.status(500).json({
      success: false,
      error: { code: 'APPROVE_ERROR', message: 'Failed to approve fire risk assessment' },
    });
  }
});
...
router.get('/:id/action-plan', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to fetch action plan', { error: (error as Error).message });
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch action plan' },
    });
  }
});
