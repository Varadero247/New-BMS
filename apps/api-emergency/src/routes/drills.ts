...
router.get('/analytics', authenticate, async (_req: Request, res: Response) => {
  try {
    const drills = await (prisma as any).femEvacuationDrill.findMany({
  ...
  } catch (error: any) { logger.error('Failed to fetch drill analytics', { error: error.message }); res.status(500).json({ success: false, error: { code: 'FETCH_ERROR', message: 'Failed to fetch drill analytics' } }); }
});

// GET /api/drills/premises/:id — drill history for premises
router.get('/premises/:id', authenticate, async (req: Request, res: Response) => {
  try {
    const data = await (prisma as any).femEvacuationDrill.findMany({
      where: { premisesId: req.params.id },
      orderBy: { drillDate: 'desc' },
    });
    res.json({ success: true, data });
  } catch (error: any) { logger.error('Failed to fetch drills', { error: error.message }); res.status(500).json({ success: false, error: { code: 'FETCH_ERROR', message: 'Failed to fetch drills' } }); }
});
...
router.post('/premises/:id', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const data = await (prisma as any).femEvacuationDrill.create({
      data: { ...rest, premisesId: req.params.id, drillDate: new Date(drillDate), createdBy: (req as any).user?.id },
    });
    res.status(201).json({ success: true, data });
  } catch (error: any) { logger.error('Failed to create drill', { error: error.message }); res.status(400).json({ success: false, error: { code: 'CREATE_ERROR', message: error.message } }); }
});
...
router.put('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    if (!parsed.success) return res.status(400).json({ success: false, error: { code: 'VALIDATION_ERROR', message: parsed.error.errors[0].message } });
    const existing = await (prisma as any).femEvacuationDrill.findUnique({ where: { id: req.params.id } });
    ...
    const data = await (prisma as any).femEvacuationDrill.update({ where: { id: req.params.id }, data: updateData });
    res.json({ success: true, data });
  } catch (error: any) { logger.error('Failed to update drill', { error: error.message }); res.status(500).json({ success: false, error: { code: 'UPDATE_ERROR', message: error.message } }); }
});
