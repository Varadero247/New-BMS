...
async function generateBcpRef(orgId: string): Promise<string> {
  ...
  const count = await (prisma as any).femBusinessContinuityPlan.count({
...
router.get('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const [data, total] = await Promise.all([
      (prisma as any).femBusinessContinuityPlan.findMany({ where, skip, take: parseInt(limit), orderBy: { createdAt: 'desc' }, include: { _count: { select: { exercises: true } } } }),
      (prisma as any).femBusinessContinuityPlan.count({ where }),
    ]);
    res.json({ success: true, data, pagination: { page: parseInt(page), limit: parseInt(limit), total, pages: Math.ceil(total / parseInt(limit)) } });
  } catch (error: any) { logger.error('Failed to fetch BCPs', { error: error.message }); res.status(500).json({ success: false, error: { code: 'FETCH_ERROR', message: 'Failed to fetch BCPs' } }); }
});
...
router.get('/due-review', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const data = await (prisma as any).femBusinessContinuityPlan.findMany({
      where: { organisationId: orgId, reviewDate: { lt: thirtyDaysFromNow } },
      orderBy: { reviewDate: 'asc' },
    });
    res.json({ success: true, data });
  } catch (error: any) { logger.error('Failed to fetch BCPs due review', { error: error.message }); res.status(500).json({ success: false, error: { code: 'FETCH_ERROR', message: 'Failed to fetch BCPs due review' } }); }
});
...
router.post('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const data = await (prisma as any).femBusinessContinuityPlan.create({
      data: {
        ...rest,
        planReference,
  ...
  } catch (error: any) { logger.error('Failed to create BCP', { error: error.message }); res.status(400).json({ success: false, error: { code: 'CREATE_ERROR', message: error.message } }); }
});

// GET /api/bcp/:id — get BCP with exercises
router.get('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    const item = await (prisma as any).femBusinessContinuityPlan.findUnique({
  ...
  } catch (error: any) { logger.error('Failed to fetch BCP', { error: error.message }); res.status(500).json({ success: false, error: { code: 'FETCH_ERROR', message: 'Failed to fetch BCP' } }); }
});
...
router.put('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    if (!parsed.success) return res.status(400).json({ success: false, error: { code: 'VALIDATION_ERROR', message: parsed.error.errors[0].message } });
    const existing = await (prisma as any).femBusinessContinuityPlan.findUnique({ where: { id: req.params.id } });
    ...
    const data = await (prisma as any).femBusinessContinuityPlan.update({ where: { id: req.params.id }, data: updateData });
    res.json({ success: true, data });
  } catch (error: any) { logger.error('Failed to update BCP', { error: error.message }); res.status(500).json({ success: false, error: { code: 'UPDATE_ERROR', message: error.message } }); }
});
...
router.post('/:id/activate', authenticate, async (req: Request, res: Response) => {
  try {
    const existing = await (prisma as any).femBusinessContinuityPlan.findUnique({ where: { id: req.params.id } });
    if (!existing) return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'BCP not found' } });
    const data = await (prisma as any).femBusinessContinuityPlan.update({
      where: { id: req.params.id },
      data: { status: 'ACTIVE' },
    });
    res.json({ success: true, data });
  } catch (error: any) { logger.error('Failed to activate BCP', { error: error.message }); res.status(500).json({ success: false, error: { code: 'ACTIVATE_ERROR', message: error.message } }); }
});
...
router.post('/:id/exercise', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const data = await (prisma as any).femBcpExercise.create({
      data: { ...rest, scheduledDate: new Date(scheduledDate), bcpId: req.params.id, createdBy: (req as any).user?.id },
    });
    res.status(201).json({ success: true, data });
  } catch (error: any) { logger.error('Failed to create exercise', { error: error.message }); res.status(400).json({ success: false, error: { code: 'CREATE_ERROR', message: error.message } }); }
});
...
router.put('/:bcpId/exercise/:id', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const parsed = schema.safeParse(req.body);
    if (!parsed.success) return res.status(400).json({ success: false, error: { code: 'VALIDATION_ERROR', message: parsed.error.errors[0].message } });
    const existing = await (prisma as any).femBcpExercise.findUnique({ where: { id: req.params.id } });
    ...
    const data = await (prisma as any).femBcpExercise.update({ where: { id: req.params.id }, data: updateData });
    // Update parent BCP test info
    if (parsed.data.outcome) {
      await (prisma as any).femBusinessContinuityPlan.update({
        where: { id: req.params.bcpId },
        data: { lastTestedDate: new Date(), lastTestOutcome: parsed.data.outcome },
      });
    }
    res.json({ success: true, data });
  } catch (error: any) { logger.error('Failed to update exercise', { error: error.message }); res.status(500).json({ success: false, error: { code: 'UPDATE_ERROR', message: error.message } }); }
});
