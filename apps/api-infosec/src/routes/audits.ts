  ...
  try {
    ...
    const audit = await prisma.isAudit.create({
      data: {
        ...
        auditees: parsed.data.auditTeam || [],
    ...
    logger.error('Failed to create ISMS audit', {
      error: error instanceof Error ? error.message : 'Unknown error',
    });
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to create ISMS audit' },
    });
  }
});
...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to list ISMS audits', {
      error: error instanceof Error ? error.message : 'Unknown error',
    });
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to list ISMS audits' },
    });
  }
});
...
router.get('/vulnerability-scans', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to list vulnerability scans', {
      error: error instanceof Error ? error.message : 'Unknown error',
    });
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to list vulnerability scans' },
    });
  }
});
  ...
  try {
    ...
    const scan = await prisma.isVulnerabilityScan.create({
      data: {
        ...
        scanner: parsed.data.scanName || parsed.data.scanner || null,
        targetSystem: (parsed.data.targetSystems || []).join(', ') || null,
        criticalFindings: parsed.data.criticalCount || 0,
        highFindings: parsed.data.highCount || 0,
        mediumFindings: parsed.data.mediumCount || 0,
        lowFindings: parsed.data.lowCount || 0,
        totalFindings: (parsed.data.criticalCount || 0) + (parsed.data.highCount || 0) + (parsed.data.mediumCount || 0) + (parsed.data.lowCount || 0),
        report: parsed.data.reportUrl || null,
        createdBy: authReq.user?.id || 'system',
      },
    });
    ...
    logger.error('Failed to log vulnerability scan', {
      error: error instanceof Error ? error.message : 'Unknown error',
    });
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to log vulnerability scan' },
    });
  }
});
...
router.get('/penetration-tests', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to list penetration tests', {
      error: error instanceof Error ? error.message : 'Unknown error',
    });
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to list penetration tests' },
    });
  }
});
  ...
  try {
    ...
    const test = await prisma.isPenetrationTest.create({
      data: {
        ...
        tester: parsed.data.tester || parsed.data.testName,
        methodology: parsed.data.methodology || null,
        scope: parsed.data.scope || null,
        totalFindings: parsed.data.findingsCount || 0,
    ...
    logger.error('Failed to log penetration test', {
      error: error instanceof Error ? error.message : 'Unknown error',
    });
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to log penetration test' },
    });
  }
});
...
router.get('/:id', async (req: Request, res: Response, next) => {
  ...
  try {
    ...
    if (!audit) {
      return res
        .status(404)
        .json({ success: false, error: { code: 'NOT_FOUND', message: 'ISMS audit not found' } });
    }

    res.json({ success: true, data: audit });
  } catch (error: unknown) {
    logger.error('Failed to get ISMS audit', {
      error: error instanceof Error ? error.message : 'Unknown error',
      id: req.params.id,
    });
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to get ISMS audit' },
    });
  }
});
...
router.get('/:id/checklist', async (req: Request, res: Response, next) => {
  ...
  try {
    ...
    logger.error('Failed to get audit checklist', {
      error: error instanceof Error ? error.message : 'Unknown error',
      id: req.params.id,
    });
    res.status(500).json({
...
router.post('/:id/findings', async (req: Request, res: Response, next) => {
  ...
  try {
    ...
    logger.error('Failed to add audit finding', {
      error: error instanceof Error ? error.message : 'Unknown error',
      auditId: req.params.id,
    });
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to add audit finding' },
    });
  }
});
...
router.put('/:id/complete', async (req: Request, res: Response, next) => {
  ...
  try {
    ...
    logger.error('Failed to complete ISMS audit', {
      error: error instanceof Error ? error.message : 'Unknown error',
      id: req.params.id,
    });
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to complete ISMS audit' },
    });
  }
});
