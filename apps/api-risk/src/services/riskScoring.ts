...
export function getAppetiteStatus(
  residualScore: number,
  appetiteStatement: { maximumTolerableScore: number; acceptableResidualScore: number } | null
): string {
...
export function getControlEffectivenessOverall(controls: Array<{ effectiveness: string }>): string {
  ...
  const avg =
    controls.reduce((sum, c) => sum + (EFFECTIVENESS_MAP[c.effectiveness] || 1), 0) /
    controls.length;
