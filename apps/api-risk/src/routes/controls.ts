...
router.post('/:id/controls', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const control = await prisma.riskControl.create({
      data: { ...parsed.data, riskId: req.params.id },
    });
    // Update overall control effectiveness on parent risk
    const allControls = await prisma.riskControl.findMany({
    ...
    logger.error('Failed to add control', { error: (error as Error).message });
    res
      .status(500)
...
router.get('/:id/controls', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to fetch controls', { error: (error as Error).message });
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch controls' },
    });
  }
});
...
router.put('/:riskId/controls/:id', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const allControls = await prisma.riskControl.findMany({
      where: { riskId: req.params.riskId, isActive: true },
      take: 1000,
    });
    const overall = getControlEffectivenessOverall(allControls);
    await prisma.riskRegister.update({
    ...
    logger.error('Failed to update control', { error: (error as Error).message });
    res
      .status(500)
      .json({ success: false, error: { code: 'INTERNAL_ERROR', message: 'Operation failed' } });
  }
...
router.delete('/:riskId/controls/:id', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to delete control', { error: (error as Error).message });
    res
      .status(500)
      .json({ success: false, error: { code: 'INTERNAL_ERROR', message: 'Operation failed' } });
  }
...
router.post('/:riskId/controls/:id/test', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to test control', { error: (error as Error).message });
    res
      .status(500)
      .json({ success: false, error: { code: 'INTERNAL_ERROR', message: 'Operation failed' } });
  }
