...
router.post('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const data = await (prisma as any).riskRegister.create({ data: { ...req.body, orgId, referenceNumber, createdBy: (req as any).user?.id, updatedBy: (req as any).user?.id } });
...
router.put('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const data = await (prisma as any).riskRegister.update({ where: { id: req.params.id }, data: { ...req.body, updatedBy: (req as any).user?.id } });
