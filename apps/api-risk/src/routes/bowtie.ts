...
router.get('/:id/bowtie', authenticate, async (req: Request, res: Response) => {
  try {
    const bowtie = await (prisma as any).riskBowtie.findUnique({ where: { riskId: req.params.id } });
    res.json({ success: true, data: bowtie });
  } catch (error: any) { logger.error('Failed to fetch bowtie', { error: error.message }); res.status(500).json({ success: false, error: { code: 'FETCH_ERROR', message: 'Failed to fetch bowtie' } }); }
});
...
router.post('/:id/bowtie', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const existing = await (prisma as any).riskBowtie.findUnique({ where: { riskId: req.params.id } });
    let bowtie;
    if (existing) {
      bowtie = await (prisma as any).riskBowtie.update({
        where: { riskId: req.params.id },
        data: { ...parsed.data, version: String(parseFloat(existing.version) + 0.1), createdBy: (req as any).user?.id },
      });
    } else {
      bowtie = await (prisma as any).riskBowtie.create({
  ...
  } catch (error: any) { logger.error('Failed to save bowtie', { error: error.message }); res.status(500).json({ success: false, error: { code: 'CREATE_ERROR', message: error.message } }); }
});

// GET /api/risks/bowtie/all
router.get('/bowtie/all', authenticate, async (req: Request, res: Response) => {
  try {
    const bowties = await (prisma as any).riskBowtie.findMany({
  ...
  } catch (error: any) { logger.error('Failed to fetch bowties', { error: error.message }); res.status(500).json({ success: false, error: { code: 'FETCH_ERROR', message: 'Failed to fetch bowties' } }); }
});
