...
const kriSchema = z.object({
  ...
  nextMeasurementDue: z.string().trim().datetime({ offset: true }).optional().or(z.string().trim().datetime({ offset: true }).optional()),
});
...
router.put('/:riskId/kri/:id', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    if (!parsed.success) return res.status(400).json({ success: false, error: { code: 'VALIDATION_ERROR', message: parsed.error.errors[0].message } });
    const existing = await prisma.riskKri.findFirst({ where: { id: req.params.id, riskId: req.params.riskId, deletedAt: null, risk: { orgId } } as any });
...
router.post('/:riskId/kri/:id/reading', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const kri = await prisma.riskKri.findFirst({ where: { id: req.params.id, riskId: req.params.riskId, deletedAt: null, risk: { orgId } } as any });
