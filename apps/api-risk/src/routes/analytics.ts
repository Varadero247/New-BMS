  ...
  try {
    ...
    const [
      ...
      prisma.riskRegister.groupBy({
        ...
        where: { ...baseWhere, residualRiskLevel: { not: null } },
        _count: true,
      }),
      prisma.riskRegister.count({ where: { ...baseWhere, appetiteStatus: 'EXCEEDS' } }),
      prisma.riskRegister.count({
        where: { ...baseWhere, nextReviewDate: { lt: new Date() }, status: { not: 'CLOSED' } },
      }),
      ...
      prisma.riskRegister.findMany({
        where: { ...baseWhere, status: { not: 'CLOSED' } },
    ...
    const moduleBreakdownRaw = await prisma.riskRegister.groupBy({
      by: ['sourceModule'],
      where: baseWhere,
      _count: true,
    });
    ...
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to generate analytics dashboard' },
    });
  }
});
...
router.get('/analytics/by-module', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to fetch by-module', { error: (error as Error).message });
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch module breakdown' },
    });
  }
});
