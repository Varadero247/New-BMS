...
router.get('/appetite', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to fetch appetite', { error: (error as Error).message });
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch appetite statements' },
    });
  }
});
...
router.post('/appetite', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    if (existing) {
      statement = await prisma.riskAppetiteStatement.update({
        ...
        data: {
          ...
          approvedAt: parsed.data.approvedBy ? new Date() : undefined,
        },
      });
    } else {
    ...
    logger.error('Failed to save appetite', { error: (error as Error).message });
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to save appetite statement' },
    });
  }
});
...
router.get('/framework', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to fetch framework', { error: (error as Error).message });
    res.status(500).json({
      success: false,
...
router.put('/framework', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    let framework;
    if (existing) {
      framework = await prisma.riskFramework.update({
        ...
        data: {
          ...
          consequenceScale: parsed.data.consequenceScale || [
            ...
            },
            {
              ...
              examples: ['Short delay', 'Minor cost <£10k'],
            },
            {
              score: 3,
              label: 'Moderate',
              description: 'Significant impact requiring management intervention',
              examples: ['Project delay', 'Cost £10k-£100k'],
            },
            {
              score: 4,
              label: 'Major',
              description: 'Major impact, substantial loss',
              examples: ['Major project failure', 'Cost £100k-£1M', 'Serious injury'],
            },
            {
              score: 5,
              label: 'Catastrophic',
              description: 'Extreme impact, threatens viability',
              examples: ['Business failure', 'Cost >£1M', 'Death or permanent disability'],
            },
          ],
          riskLevelBands: parsed.data.riskLevelBands || [
            {
              ...
              level: 'LOW',
              colour: '#22C55E',
              action: 'Manage by routine procedures',
            },
            {
              min: 4,
              max: 6,
    ...
    logger.error('Failed to save framework', { error: (error as Error).message });
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to save framework' },
    });
  }
});
