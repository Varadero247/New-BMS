...
describe('GET /api/risks/:id/bowtie', () => {
  it('should return bowtie or null', async () => {
    ...
    const res = await request(app).get('/api/risks/r1/bowtie');
...
describe('POST /api/risks/:id/bowtie', () => {
  it('should create bowtie for HIGH risk', async () => {
    (prisma as any).riskRegister.findFirst.mockResolvedValue({ id: 'r1', residualRiskLevel: 'HIGH', inherentRiskLevel: 'HIGH' });
    (prisma as any).riskBowtie.findUnique.mockResolvedValue(null);
    (prisma as any).riskBowtie.create.mockResolvedValue({ id: 'b1', ...validBowtie });
    const res = await request(app).post('/api/risks/r1/bowtie').send(validBowtie);
    expect(res.status).toBe(201);
    expect(res.body.success).toBe(true);
  });

  it('should reject bowtie for LOW risk', async () => {
    (prisma as any).riskRegister.findFirst.mockResolvedValue({ id: 'r1', residualRiskLevel: 'LOW', inherentRiskLevel: 'LOW' });
    const res = await request(app).post('/api/risks/r1/bowtie').send(validBowtie);
  ...
  it('should reject bowtie for MEDIUM risk', async () => {
    (prisma as any).riskRegister.findFirst.mockResolvedValue({ id: 'r1', residualRiskLevel: 'MEDIUM', inherentRiskLevel: 'MEDIUM' });
    const res = await request(app).post('/api/risks/r1/bowtie').send(validBowtie);
    expect(res.status).toBe(400);
  });

  it('should update existing bowtie', async () => {
    (prisma as any).riskRegister.findFirst.mockResolvedValue({ id: 'r1', residualRiskLevel: 'CRITICAL' });
