...
jest.mock('../src/prisma', () => ({
  prisma: {
    ...
    riskBowtie: { findUnique: jest.fn(), create: jest.fn(), update: jest.fn(), findMany: jest.fn() },
  },
  Prisma: {},
}));
...
const validBowtie = {
  ...
  preventionBarriers: [{ id: 'pb1', description: 'Bunded storage', type: 'PREVENTIVE', effectiveness: 'ADEQUATE' }],
  consequences: [{ id: 'c1', description: 'Environmental contamination', severity: 4 }],
  mitigationBarriers: [{ id: 'mb1', description: 'Spill response team', type: 'REACTIVE', effectiveness: 'STRONG' }],
};
...
describe('POST /api/risks/:id/bowtie', () => {
  it('should create bowtie for HIGH risk', async () => {
    (prisma as any).riskRegister.findFirst.mockResolvedValue({ id: '00000000-0000-0000-0000-000000000001', residualRiskLevel: 'HIGH', inherentRiskLevel: 'HIGH' });
    (prisma as any).riskBowtie.findUnique.mockResolvedValue(null);
    (prisma as any).riskBowtie.create.mockResolvedValue({ id: 'b1', ...validBowtie });
    const res = await request(app).post('/api/risks/00000000-0000-0000-0000-000000000001/bowtie').send(validBowtie);
    expect(res.status).toBe(201);
    expect(res.body.success).toBe(true);
  });

  it('should reject bowtie for LOW risk', async () => {
    (prisma as any).riskRegister.findFirst.mockResolvedValue({ id: '00000000-0000-0000-0000-000000000001', residualRiskLevel: 'LOW', inherentRiskLevel: 'LOW' });
    const res = await request(app).post('/api/risks/00000000-0000-0000-0000-000000000001/bowtie').send(validBowtie);
  ...
  it('should reject bowtie for MEDIUM risk', async () => {
    (prisma as any).riskRegister.findFirst.mockResolvedValue({ id: '00000000-0000-0000-0000-000000000001', residualRiskLevel: 'MEDIUM', inherentRiskLevel: 'MEDIUM' });
    const res = await request(app).post('/api/risks/00000000-0000-0000-0000-000000000001/bowtie').send(validBowtie);
    expect(res.status).toBe(400);
  });

  it('should update existing bowtie', async () => {
    (prisma as any).riskRegister.findFirst.mockResolvedValue({ id: '00000000-0000-0000-0000-000000000001', residualRiskLevel: 'CRITICAL' });
    (prisma as any).riskBowtie.findUnique.mockResolvedValue({ id: 'b1', version: '1.0' });
    (prisma as any).riskBowtie.update.mockResolvedValue({ id: 'b1', ...validBowtie, version: '1.1' });
    const res = await request(app).post('/api/risks/00000000-0000-0000-0000-000000000001/bowtie').send(validBowtie);
    expect(res.status).toBe(200);
  });
