...
describe('GET /api/risks', () => {
  it('should return risks', async () => {
    (prisma as any).riskRegister.findMany.mockResolvedValue([{ id: '00000000-0000-0000-0000-000000000001', title: 'Test' }]);
...
describe('GET /api/risks/:id', () => {
  ...
  it('should return item by id with relations', async () => {
    (prisma as any).riskRegister.findFirst.mockResolvedValue({ id: '00000000-0000-0000-0000-000000000001', riskControls: [], keyRiskIndicators: [], treatmentActions: [], reviews: [] });
...
describe('POST /api/risks', () => {
  it('should create with auto-calculated scores', async () => {
    (prisma as any).riskRegister.count.mockResolvedValue(0);
    (prisma as any).riskRegister.create.mockResolvedValue({ id: '00000000-0000-0000-0000-000000000001', title: 'New', inherentScore: 9, inherentRiskLevel: 'HIGH' });
  ...
  it('should accept numeric likelihood/consequence', async () => {
    (prisma as any).riskRegister.count.mockResolvedValue(0);
    (prisma as any).riskRegister.create.mockResolvedValue({ id: '00000000-0000-0000-0000-000000000001', inherentScore: 12 });
    (prisma as any).riskAppetiteStatement.findFirst.mockResolvedValue(null);
    const res = await request(app).post('/api/risks').send({
...
describe('PUT /api/risks/:id', () => {
  it('should update', async () => {
    (prisma as any).riskRegister.findFirst.mockResolvedValue({ id: '00000000-0000-0000-0000-000000000001', category: 'OPERATIONAL', residualScore: 8 });
    (prisma as any).riskRegister.update.mockResolvedValue({ id: '00000000-0000-0000-0000-000000000001', title: 'Updated' });
    (prisma as any).riskAppetiteStatement.findFirst.mockResolvedValue(null);
    const res = await request(app).put('/api/risks/00000000-0000-0000-0000-000000000001').send({ title: 'Updated' });
    expect(res.status).toBe(200);
  });

  it('should auto-check appetite status on update', async () => {
    (prisma as any).riskRegister.findFirst.mockResolvedValue({ id: '00000000-0000-0000-0000-000000000001', category: 'HEALTH_SAFETY', residualScore: 12 });
    (prisma as any).riskAppetiteStatement.findFirst.mockResolvedValue({ maximumTolerableScore: 6, acceptableResidualScore: 4 });
    (prisma as any).riskRegister.update.mockResolvedValue({ id: '00000000-0000-0000-0000-000000000001', appetiteStatus: 'EXCEEDS' });
    const res = await request(app).put('/api/risks/00000000-0000-0000-0000-000000000001').send({ residualLikelihoodNum: 3, residualConsequenceNum: 4 });
    expect(res.status).toBe(200);
  });
});
...
describe('DELETE /api/risks/:id', () => {
  it('should soft delete', async () => {
    ...
    (prisma as any).riskRegister.update.mockResolvedValue({ id: '00000000-0000-0000-0000-000000000001' });
...
describe('PUT /api/risks/:id - error paths', () => {
  it('should return 404 when risk not found', async () => {
    (prisma as any).riskRegister.findFirst.mockResolvedValue(null);
    const res = await request(app).put('/api/risks/00000000-0000-0000-0000-000000000099').send({ title: 'Updated' });
    expect(res.status).toBe(404);
  });
...
describe('GET /api/risks/register', () => {
  it('should return full register with relations', async () => {
    (prisma as any).riskRegister.findMany.mockResolvedValue([{ id: '00000000-0000-0000-0000-000000000001' }]);
    (prisma as any).riskRegister.count.mockResolvedValue(1);
...
describe('GET /api/risks/heatmap', () => {
  it('should return 5x5 heatmap data', async () => {
    (prisma as any).riskRegister.findMany.mockResolvedValue([
      { id: '00000000-0000-0000-0000-000000000001', residualLikelihoodNum: 3, residualConsequenceNum: 4, title: 'Test', referenceNumber: 'RISK-2026-0001' },
    ]);
...
describe('GET /api/risks/overdue-review', () => {
  it('should return overdue risks', async () => {
    (prisma as any).riskRegister.findMany.mockResolvedValue([{ id: '00000000-0000-0000-0000-000000000001', nextReviewDate: '2025-01-01' }]);
...
describe('POST /api/risks/from-coshh/:coshhId', () => {
  it('should create risk from COSHH', async () => {
    (prisma as any).riskRegister.count.mockResolvedValue(0);
    (prisma as any).riskRegister.create.mockResolvedValue({ id: '00000000-0000-0000-0000-000000000001', sourceModule: 'CHEMICAL_COSHH' });
    const res = await request(app).post('/api/risks/from-coshh/00000000-0000-0000-0000-000000000001').send({
      id: '00000000-0000-0000-0000-000000000001', chemicalName: 'Benzene', activity: 'Lab work', inherentLikelihood: 3, inherentSeverity: 4,
    });
    expect(res.status).toBe(201);
  });
...
describe('POST /api/risks/from-fra/:fraId', () => {
  it('should create risk from FRA', async () => {
    (prisma as any).riskRegister.count.mockResolvedValue(0);
    (prisma as any).riskRegister.create.mockResolvedValue({ id: '00000000-0000-0000-0000-000000000001', sourceModule: 'FIRE_EMERGENCY' });
    const res = await request(app).post('/api/risks/from-fra/00000000-0000-0000-0000-000000000001').send({
      id: '00000000-0000-0000-0000-000000000001', premisesName: 'HQ', likelihoodRating: 3, consequenceRating: 4,
    });
    expect(res.status).toBe(201);
  });
});
...
describe('POST /api/risks/from-incident/:id', () => {
  it('should create risk from incident', async () => {
    (prisma as any).riskRegister.count.mockResolvedValue(0);
    (prisma as any).riskRegister.create.mockResolvedValue({ id: '00000000-0000-0000-0000-000000000001', sourceModule: 'HEALTH_SAFETY' });
    const res = await request(app).post('/api/risks/from-incident/00000000-0000-0000-0000-000000000001').send({
      id: '00000000-0000-0000-0000-000000000001', title: 'Slip and fall', severity: 'MAJOR',
    });
    expect(res.status).toBe(201);
  });
});
...
describe('POST /api/risks/from-audit/:id', () => {
  it('should create risk from audit finding', async () => {
    (prisma as any).riskRegister.count.mockResolvedValue(0);
    (prisma as any).riskRegister.create.mockResolvedValue({ id: '00000000-0000-0000-0000-000000000001', sourceModule: 'AUDIT_MOD', category: 'COMPLIANCE' });
    const res = await request(app).post('/api/risks/from-audit/00000000-0000-0000-0000-000000000001').send({ id: '00000000-0000-0000-0000-000000000001', title: 'Missing documentation' });
    expect(res.status).toBe(201);
  });
});
