...
async function generateRef(orgId: string): Promise<string> {
  ...
  const c = await prisma.ptwToolboxTalk.count({
    where: { orgId, referenceNumber: { startsWith: `PTT-${y}` } },
  });
...
router.get('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const [data, total] = await Promise.all([
      prisma.ptwToolboxTalk.findMany({
        where,
        skip,
    ...
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch toolbox talks' },
    });
  }
});
...
router.get('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    const orgId = ((req as AuthRequest).user as any)?.orgId || 'default';
    const item = await prisma.ptwToolboxTalk.findFirst({
      where: { id: req.params.id, orgId, deletedAt: null } as any,
    });
    if (!item)
      return res
        .status(404)
        .json({ success: false, error: { code: 'NOT_FOUND', message: 'toolbox talk not found' } });
    ...
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch toolbox talk' },
    });
  }
});
...
router.put('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    const orgId = ((req as AuthRequest).user as any)?.orgId || 'default';
    const existing = await prisma.ptwToolboxTalk.findFirst({
      where: { id: req.params.id, orgId, deletedAt: null } as any,
    });
    if (!existing)
      return res
    ...
    const data = await prisma.ptwToolboxTalk.update({
  ...
  try {
    ...
    if (!existing)
      return res
        .status(404)
        .json({ success: false, error: { code: 'NOT_FOUND', message: 'toolbox talk not found' } });
    await prisma.ptwToolboxTalk.update({
      where: { id: req.params.id },
