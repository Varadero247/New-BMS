...
const createSchema = z.object({
  ...
  conductedDate: z.string().refine(s => !isNaN(Date.parse(s)), 'Invalid date format').optional(),
...
async function generateRef(orgId: string): Promise<string> {
  ...
  const c = await prisma.ptwToolboxTalk.count({ where: { orgId, referenceNumber: { startsWith: `PTT-${y}` } } });
...
router.get('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const [data, total] = await Promise.all([
      prisma.ptwToolboxTalk.findMany({ where, skip, take: Math.min(Math.max(1, parseInt(limit, 10) || 20), 100), orderBy: { createdAt: 'desc' } }),
    ...
    res.status(500).json({ success: false, error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch toolbox talks' } });
  }
});
...
router.get('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    const orgId = ((req as AuthRequest).user as any)?.orgId || 'default';
    const item = await prisma.ptwToolboxTalk.findFirst({ where: { id: req.params.id, orgId, deletedAt: null } as any });
    ...
    res.status(500).json({ success: false, error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch toolbox talk' } });
  }
});
...
router.post('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const data = await prisma.ptwToolboxTalk.create({
      data: {
...
router.put('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    const orgId = ((req as AuthRequest).user as any)?.orgId || 'default';
    const existing = await prisma.ptwToolboxTalk.findFirst({ where: { id: req.params.id, orgId, deletedAt: null } as any });
