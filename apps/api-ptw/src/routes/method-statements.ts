...
router.get('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    const item = await prisma.ptwMethodStatement.findFirst({ where: { id: req.params.id, deletedAt: null } });
...
router.put('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    const existing = await prisma.ptwMethodStatement.findFirst({ where: { id: req.params.id, deletedAt: null } });
