...
export default function KPIsPage() {
  ...
  const filtered = kpis.filter((k) => {
    const matchesSearch =
      !searchTerm || JSON.stringify(k).toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const atRisk = kpis.filter(
    (k) => k.target && k.value / k.target >= 0.75 && k.value / k.target < 0.95
  ).length;
  ...
  function openEdit(k: KPI) {
    ...
    setForm({
      name: k.name || '',
      category: k.category || 'RELIABILITY',
  ...
  async function handleEdit() {
    ...
    setError('');
    try {
      await api.put(`/kpis/${selected!.id}`, {
        ...form,
        value: parseFloat(form.value) || 0,
        target: parseFloat(form.target) || 0,
      });
  ...
  async function handleDelete() {
    setSaving(true);
    try {
      await api.delete(`/kpis/${selected!.id}`);
      setDeleteOpen(false);
      await load();
    } catch (e) {
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="col-span-2">
          ...
          <textarea
            ...
            placeholder="KPI description and measurement method..."
          />
        </div>
      </div>
    </div>
  ...
  return (
    <div className="p-8">
                          ...
                          <td className="py-3 px-4">
                            <div className="flex items-center gap-2">
                              <div className="w-24 h-2 bg-gray-200 rounded-full overflow-hidden">
                                <div
                                  className={`h-2 rounded-full ${perf.bar}`}
                                  style={{ width: `${pct}%` }}
                                />
                              </div>
      ...
      <Modal isOpen={createOpen} onClose={() => setCreateOpen(false)} title="Add KPI" size="lg">
        <FormFields />
        <ModalFooter>
          <button
      ...
      <Modal isOpen={editOpen} onClose={() => setEditOpen(false)} title="Edit KPI" size="lg">
        <FormFields />
        <ModalFooter>
          <button
      ...
      <Modal isOpen={deleteOpen} onClose={() => setDeleteOpen(false)} title="Delete KPI" size="sm">
        <div className="flex items-start gap-3">
          <div className="flex-shrink-0 p-2 bg-red-100 rounded-full">
            <Ban className="h-5 w-5 text-red-600" />
          </div>
          <p className="text-sm text-gray-700 dark:text-gray-300">
            Delete KPI <span className="font-semibold">{selected?.name}</span>? This cannot be
            undone.
          </p>
        </div>
        <ModalFooter>
