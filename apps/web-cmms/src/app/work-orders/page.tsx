...
export default function WorkOrdersPage() {
  ...
  const filtered = orders.filter((o) => {
    const matchesSearch =
      !searchTerm || JSON.stringify(o).toLowerCase().includes(searchTerm.toLowerCase());
  ...
  function openDelete(wo: WorkOrder) {
    setSelected(wo);
    setDeleteOpen(true);
  }
                    ...
                    {filtered.map((wo) => {
                      ...
                      return (
                        <tr key={wo.id} className="border-b hover:bg-gray-50 dark:bg-gray-800">
                          ...
                          <td className="py-3 px-4 text-gray-900 dark:text-gray-100 font-medium max-w-[200px] truncate">
                            {wo.title}
                          </td>
                          <td className="py-3 px-4 text-gray-600">{wo.type}</td>
                          <td className="py-3 px-4">
                            <span
                              className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${priorityColors[wo.priority] || 'bg-gray-100 dark:bg-gray-800 text-gray-700'}`}
                            >
                              {wo.priority}
                            </span>
                          </td>
                          ...
                          <td className="py-3 px-4">
                            <span
                              className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${statusColors[wo.status] || 'bg-gray-100 dark:bg-gray-800 text-gray-700'}`}
                            >
      ...
      >
        <div className="flex items-start gap-3">
          ...
          <p className="text-sm text-gray-700 dark:text-gray-300">
            ...
            <span className="font-semibold">{selected?.title}</span>? This action cannot be undone.
          </p>
        </div>
        <ModalFooter>
          <button
