...
export default function VendorsPage() {
  ...
  const filtered = vendors.filter((v) => {
    const matchesSearch =
      !searchTerm || JSON.stringify(v).toLowerCase().includes(searchTerm.toLowerCase());
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setError(axios.isAxiosError(e) && e.response?.data?.error || 'Failed to create vendor');
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleEdit() {
    ...
    try {
      ...
      setError(axios.isAxiosError(e) && e.response?.data?.error || 'Failed to update vendor');
    } finally {
      setSaving(false);
    }
  }
                  ...
                  <tbody>
                    {filtered.map((v) => (
                      <tr key={v.id} className="border-b hover:bg-gray-50 dark:bg-gray-800">
                        ...
                        <td className="py-3 px-4">
                          <span
                            className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${v.status === 'ACTIVE' ? 'bg-green-100 text-green-700' : v.status === 'BLACKLISTED' ? 'bg-red-100 text-red-700' : 'bg-gray-100 dark:bg-gray-800 text-gray-700'}`}
                          >
                            {v.status}
                          </span>
                        </td>
