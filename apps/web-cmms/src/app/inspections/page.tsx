...
export default function InspectionsPage() {
  ...
  const stats = {
    ...
    overdue: inspections.filter(
      (i) => i.scheduledDate && new Date(i.scheduledDate) < new Date() && i.status === 'SCHEDULED'
    ).length,
  };
  ...
  function openEdit(i: Inspection) {
    ...
    setForm({
      ...
      scheduledDate: i.scheduledDate ? i.scheduledDate.slice(0, 10) : '',
      completedDate: i.completedDate ? i.completedDate.slice(0, 10) : '',
      result: i.result || '',
      findings: i.findings || '',
  ...
  function openDelete(i: Inspection) {
    setSelected(i);
    setDeleteOpen(true);
  }
  ...
  return (
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">Manage equipment inspections</p>
          </div>
          <button
            onClick={openCreate}
            className="bg-amber-600 text-white px-4 py-2 rounded-lg hover:bg-amber-700 flex items-center gap-2"
          >
            <Plus className="h-5 w-5" /> Schedule Inspection
          </button>
        </div>
          ...
          <CardContent>
            {filtered.length > 0 ? (
                    ...
                    {filtered.map((insp) => (
                      <tr key={insp.id} className="border-b hover:bg-gray-50 dark:bg-gray-800">
                        ...
                        <td className="py-3 px-4">
                          {insp.result ? (
                            <span
                              className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${resultColors[insp.result] || 'bg-gray-100 dark:bg-gray-800 text-gray-700'}`}
                            >
                              {insp.result}
                            </span>
                          ) : (
                        ...
                        <td className="py-3 px-4">
                          <span
                            className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${statusColors[insp.status] || 'bg-gray-100 dark:bg-gray-800 text-gray-700'}`}
                          >
              ...
              <div className="text-center py-12 text-gray-500 dark:text-gray-400">
                ...
                <p className="text-sm mt-1">Schedule your first inspection to get started</p>
              </div>
            )}
          </CardContent>
        </Card>
      ...
      <Modal isOpen={editOpen} onClose={() => setEditOpen(false)} title="Edit Inspection" size="lg">
        <FormFields />
        <ModalFooter>
          <button
      ...
      >
        <div className="flex items-start gap-3">
          ...
          <p className="text-sm text-gray-700 dark:text-gray-300">
            Delete <span className="font-semibold">{selected?.title}</span>? This cannot be undone.
          </p>
        </div>
        <ModalFooter>
          <button
