...
export default function LocationsPage() {
  ...
  const filtered = locations.filter((l) => {
    const matchesSearch =
      !searchTerm || JSON.stringify(l).toLowerCase().includes(searchTerm.toLowerCase());
  ...
  async function handleEdit() {
    ...
    setError('');
    try {
      await api.put(`/locations/${selected!.id}`, form);
      setEditOpen(false);
      await load();
    } catch (e) {
  ...
  return (
    <div className="p-8">
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b">
                      {['Code', 'Name', 'Type', 'Parent', 'Assets', 'Status', 'Actions'].map(
                        (h) => (
                          <th
                            key={h}
                    ...
                    {filtered.map((loc) => (
                      <tr key={loc.id} className="border-b hover:bg-gray-50 dark:bg-gray-800">
                        ...
                        <td className="py-3 px-4">
                          <span
                            className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${loc.status === 'ACTIVE' ? 'bg-green-100 text-green-700' : 'bg-gray-100 dark:bg-gray-800 text-gray-700'}`}
                          >
                            {loc.status}
                          </span>
                        </td>
      ...
      <Modal isOpen={editOpen} onClose={() => setEditOpen(false)} title="Edit Location" size="lg">
        <FormFields />
        <ModalFooter>
          <button
