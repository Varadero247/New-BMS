...
export default function DowntimePage() {
  ...
  const filtered = events.filter((e) => {
    const matchesSearch =
      !searchTerm || JSON.stringify(e).toLowerCase().includes(searchTerm.toLowerCase());
  ...
  function openEdit(e: DowntimeEvent) {
    ...
    setForm({
      asset: e.asset || '',
      category: e.category || 'MECHANICAL',
      reason: e.reason || '',
  ...
  function openDelete(e: DowntimeEvent) {
    setSelected(e);
    setDeleteOpen(true);
  }
  ...
  async function handleEdit() {
    ...
    setError('');
    try {
      await api.put(`/downtime/${selected!.id}`, {
        ...form,
  ...
  async function handleDelete() {
    setSaving(true);
    try {
      await api.delete(`/downtime/${selected!.id}`);
      setDeleteOpen(false);
      await load();
    } catch (e) {
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="col-span-2">
          ...
          <textarea
            ...
            onChange={(e) => setForm((f) => ({ ...f, corrective: e.target.value }))}
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">
              Track and analyse equipment downtime events
            </p>
          </div>
          <button
            onClick={openCreate}
          ...
          >
            <Plus className="h-5 w-5" /> Log Downtime
          </button>
        </div>
          ...
          <CardContent className="pt-5">
            <div className="flex flex-wrap gap-4 items-center">
              <div className="flex-1 min-w-[200px] relative">
                ...
                <input
                  type="text"
                  aria-label="Search downtime events..."
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              Downtime Events ({filtered.length})
            </CardTitle>
          </CardHeader>
          <CardContent>
            {filtered.length > 0 ? (
                    ...
                    {filtered.map((evt) => (
                      <tr key={evt.id} className="border-b hover:bg-gray-50 dark:bg-gray-800">
                        ...
                        <td className="py-3 px-4 text-gray-600 text-xs">
                          {evt.startTime ? new Date(evt.startTime).toLocaleString() : '-'}
                        </td>
                        <td className="py-3 px-4 text-gray-600 text-xs">
                          {evt.endTime ? (
                        ...
                        <td className="py-3 px-4">
                          <span
                            className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${impactColors[evt.impact] || 'bg-gray-100 dark:bg-gray-800 text-gray-700'}`}
                          >
                            {evt.impact}
                          </span>
                        </td>
                        <td className="py-3 px-4">
                          <span
                            className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${statusColors[evt.status] || 'bg-gray-100 dark:bg-gray-800 text-gray-700'}`}
                          >
              ...
              <div className="text-center py-12 text-gray-500 dark:text-gray-400">
                ...
                <p className="font-medium">No downtime events found</p>
                <p className="text-sm mt-1">Log a downtime event to start tracking</p>
              </div>
            )}
          </CardContent>
        </Card>
      ...
      >
        <div className="flex items-start gap-3">
          ...
          <p className="text-sm text-gray-700 dark:text-gray-300">
            Delete downtime event for <span className="font-semibold">{selected?.asset}</span>? This
            cannot be undone.
          </p>
        </div>
        <ModalFooter>
