...
export default function ChecklistsPage() {
  ...
  const filtered = checklists.filter((c) => {
    const matchesSearch =
      !searchTerm || JSON.stringify(c).toLowerCase().includes(searchTerm.toLowerCase());
  ...
  function openDelete(c: Checklist) {
    setSelected(c);
    setDeleteOpen(true);
  }
  ...
  async function handleEdit() {
    ...
    setError('');
    try {
      await api.put(`/checklists/${selected!.id}`, form);
      setEditOpen(false);
      await load();
    } catch (e) {
  ...
  async function handleDelete() {
    setSaving(true);
    try {
      await api.delete(`/checklists/${selected!.id}`);
      setDeleteOpen(false);
      await load();
    } catch (e) {
    ...
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          ...
          >
            <Plus className="h-5 w-5" /> Create Checklist
          </button>
        </div>
              ...
              <CardContent className="pt-5">
                <div className="flex items-center justify-between">
                  ...
                  <div className={`p-3 rounded-full ${card.bg}`}>
                    <CheckSquare className={`h-6 w-6 ${card.color}`} />
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            {filtered.length > 0 ? (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {filtered.map((cl) => {
                      const progress = cl.items
                        ? Math.round((cl.completedItems / cl.items) * 100)
                        : 0;
                      return (
                          ...
                          <td className="py-3 px-4">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-xs text-gray-500 dark:text-gray-400">
                                {cl.completedItems || 0}/{cl.items || 0}
                              </span>
                            </div>
                          </td>
                          <td className="py-3 px-4 text-gray-600">{cl.assignedTo || '-'}</td>
                          <td className="py-3 px-4 text-gray-600">
                            {cl.dueDate ? new Date(cl.dueDate).toLocaleDateString() : '-'}
                          </td>
                          <td className="py-3 px-4">
                            <span
                              className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${statusColors[cl.status] || 'bg-gray-100 dark:bg-gray-800 text-gray-700'}`}
                            >
      ...
      <Modal isOpen={editOpen} onClose={() => setEditOpen(false)} title="Edit Checklist" size="lg">
        <FormFields />
        <ModalFooter>
          <button
