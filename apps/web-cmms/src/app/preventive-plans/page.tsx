...
import { Plus, Search, CalendarCheck, Edit2, Trash2, CheckCircle, AlertCircle, Clock, Ban } from 'lucide-react';
...
const EMPTY_FORM = {
  name: '', description: '', asset: '', frequency: '1', frequencyUnit: 'MONTHS',
  nextDue: '', assignedTo: '', estimatedHours: '', status: 'ACTIVE',
};
...
export default function PreventivePlansPage() {
  ...
  function openEdit(p: PreventivePlan) {
    setSelected(p);
    setForm({
      name: p.name || '', description: p.description || '', asset: p.asset || '',
      frequency: p.frequency?.toString() || '1', frequencyUnit: p.frequencyUnit || 'MONTHS',
  ...
  function openDelete(p: PreventivePlan) { setSelected(p); setDeleteOpen(true); }
    ...
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-4 gap-4 mb-6">
          {[
            { label: 'Total Plans', value: stats.total, icon: CalendarCheck, color: 'text-blue-600', bg: 'bg-blue-50' },
            ...
            { label: 'Due This Week', value: stats.dueThisWeek, icon: Clock, color: 'text-amber-600', bg: 'bg-amber-50' },
          ].map(card => {
        ...
        <Card className="mb-6"><CardContent className="pt-5">
          <div className="flex flex-wrap gap-4 items-center">
            <div className="flex-1 min-w-[200px] relative">
              ...
              <input type="text" aria-label="Search plans..." placeholder="Search plans..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-amber-500" />
            </div>
