...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to list releases' },
    });
  }
});
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const _refPrefix = 'QMS-REL';
    const _refYear = new Date().getFullYear();
    const _refCount = await prisma.qualRelease.count({ where: { referenceNumber: { startsWith: `${_refPrefix}-${_refYear}` } } });
...
router.put('/:id/authorise', async (req: Request, res: Response) => {
  try {
    ...
    if (!existing)
      return res
        .status(404)
        .json({ success: false, error: { code: 'NOT_FOUND', message: 'Release not found' } });
    ...
    logger.error('Failed to authorise release', {
      error: error instanceof Error ? error.message : 'Unknown error',
    });
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to authorise release' },
    });
  }
});
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to get release' },
    });
  }
});
...
router.delete('/:id', async (req: Request, res: Response) => {
  try {
    ...
    await prisma.qualRelease.update({
      where: { id: req.params.id },
