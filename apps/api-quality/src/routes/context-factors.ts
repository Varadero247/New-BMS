...
const createSchema = z.object({
  ...
  status: z.enum(['OPEN', 'UNDER_REVIEW', 'TREATED', 'MONITORED', 'CLOSED']).optional().default('OPEN'),
...
router.get('/', requirePermission('quality', 'read' as any), async (req: Request, res: Response) => {
  try {
    ...
    const data = items.map((item: Record<string, unknown>) => ({
      ...
      factorType: item.bias === 'RISK' ? 'INTERNAL' : item.bias === 'OPPORTUNITY' ? 'EXTERNAL' : 'INTERNAL',
    ...
    logger.error('Failed to list context factors', { error: error instanceof Error ? error.message : 'Unknown error' });
    res.status(500).json({ success: false, error: { code: 'INTERNAL_ERROR', message: 'Failed to list context factors' } });
  }
});
...
router.post('/', requirePermission('quality', 'write' as any), async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to create context factor', { error: error instanceof Error ? error.message : 'Unknown error' });
    res.status(500).json({ success: false, error: { code: 'INTERNAL_ERROR', message: 'Failed to create context factor' } });
  }
});
...
router.get('/:id', requirePermission('quality', 'read' as any), async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to get context factor', { error: error instanceof Error ? error.message : 'Unknown error' });
    res.status(500).json({ success: false, error: { code: 'INTERNAL_ERROR', message: 'Failed to get context factor' } });
  }
});
...
router.put('/:id', requirePermission('quality', 'write' as any), async (req: Request, res: Response) => {
  try {
    ...
    if (parsed.data.factorType !== undefined) data.bias = parsed.data.factorType === 'INTERNAL' ? 'RISK' : 'OPPORTUNITY';
    ...
    logger.error('Failed to update context factor', { error: error instanceof Error ? error.message : 'Unknown error' });
    res.status(500).json({ success: false, error: { code: 'INTERNAL_ERROR', message: 'Failed to update context factor' } });
  }
});
...
router.delete('/:id', requirePermission('quality', 'delete' as any), async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to delete context factor', { error: error instanceof Error ? error.message : 'Unknown error' });
    res.status(500).json({ success: false, error: { code: 'INTERNAL_ERROR', message: 'Failed to delete context factor' } });
  }
});
