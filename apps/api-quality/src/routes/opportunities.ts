...
router.post('/', async (req: Request, res: Response) => {
  try {
    const schema = z.object({
      ...
      interestedParties: z.string().trim().optional(),
      ...
      actionToExploit: z.string().trim().optional(),
    ...
    const _refPrefix = 'QMS-OPP';
    const _refYear = new Date().getFullYear();
    const _refCount = await prisma.qualOpportunity.count({ where: { referenceNumber: { startsWith: `${_refPrefix}-${_refYear}` } } });
    ...
    try {
      ...
      const schema = z.object({
        process: z
          ...
          .optional(),
        interestedParties: z.string().trim().nullable().optional(),
        ...
        actionToExploit: z.string().trim().nullable().optional(),
