...
router.put('/:id', checkOwnership(prisma.qualLegal), async (req: AuthRequest, res: Response) => {
  try {
    ...
    const schema = z.object({
      ...
      lastAssessmentDate: z.string().nullable().refine(s => !isNaN(Date.parse(s)), 'Invalid date format').optional(),
      nextAssessmentDate: z.string().nullable().refine(s => !isNaN(Date.parse(s)), 'Invalid date format').optional(),
      ...
      effectiveDate: z.string().nullable().refine(s => !isNaN(Date.parse(s)), 'Invalid date format').optional(),
