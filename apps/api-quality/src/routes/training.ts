...
async function generateRefNumber(): Promise<string> {
  ...
  const count = await (prisma as any).qualTraining.count({
...
router.get('/overdue', async (req: Request, res: Response) => {
  try {
    ...
    const [items, total] = await Promise.all([
      (prisma as any).qualTraining.findMany({ where, skip, take: limit, orderBy: { dueDate: 'asc' } }),
      (prisma as any).qualTraining.count({ where }),
    ]);
...
router.get('/stats', async (_req: Request, res: Response) => {
  try {
    const [total, completed, inProgress, overdue, byType] = await Promise.all([
      (prisma as any).qualTraining.count({ where: { deletedAt: null } }),
      (prisma as any).qualTraining.count({ where: { deletedAt: null, status: 'COMPLETED' } }),
      (prisma as any).qualTraining.count({ where: { deletedAt: null, status: 'IN_PROGRESS' } }),
      (prisma as any).qualTraining.count({ where: { deletedAt: null, dueDate: { lt: new Date() }, status: { not: 'COMPLETED' } } }),
      (prisma as any).qualTraining.groupBy({ by: ['trainingType'], where: { deletedAt: null }, _count: { id: true } }),
    ]);
    const completionRate = total > 0 ? Math.round((completed / total) * 100) : 0;
    res.json({ success: true, data: { total, completed, inProgress, overdue, completionRate, byType: byType.map((t: any) => ({ trainingType: t.trainingType, count: t._count.id })) } });
  } catch (error: unknown) {
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const item = await (prisma as any).qualTraining.create({
      data: {
        referenceNumber,
        ...parsed.data,
...
router.put('/:id/complete', async (req: Request, res: Response) => {
  try {
    ...
    const existing = await (prisma as any).qualTraining.findFirst({ where: { id: req.params.id, deletedAt: null } });
    ...
    const item = await (prisma as any).qualTraining.update({
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    const item = await (prisma as any).qualTraining.findFirst({ where: { id: req.params.id, deletedAt: null } });
...
router.put('/:id', async (req: Request, res: Response) => {
  try {
    ...
    const item = await (prisma as any).qualTraining.update({ where: { id: req.params.id }, data });
    res.json({ success: true, data: item });
  } catch (error: unknown) {
...
router.delete('/:id', async (req: Request, res: Response) => {
  try {
    ...
    await (prisma as any).qualTraining.update({ where: { id: req.params.id }, data: { deletedAt: new Date() } });
