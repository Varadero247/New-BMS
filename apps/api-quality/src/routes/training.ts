...
const updateSchema = createSchema.partial().extend({
  status: z
    .enum(['ASSIGNED', 'IN_PROGRESS', 'COMPLETED', 'FAILED', 'OVERDUE', 'CANCELLED'])
    .optional(),
  completedDate: z
    .string()
    .refine((s) => !isNaN(Date.parse(s)), 'Invalid date format')
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const _refPrefix = 'QMS-TRN';
    const _refYear = new Date().getFullYear();
    const _refCount = await prisma.qualTraining.count({ where: { referenceNumber: { startsWith: `${_refPrefix}-${_refYear}` } } });
...
router.delete('/:id', async (req: Request, res: Response) => {
  try {
    ...
    await prisma.qualTraining.update({
      where: { id: req.params.id },
