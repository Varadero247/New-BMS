...
router.get('/', requirePermission('quality', 'read' as any), async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to get QMS scope', { error: error instanceof Error ? error.message : 'Unknown error' });
    res.status(500).json({ success: false, error: { code: 'INTERNAL_ERROR', message: 'Failed to get QMS scope' } });
  }
});
...
router.put('/', requirePermission('quality', 'write' as any), async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to update QMS scope', { error: error instanceof Error ? error.message : 'Unknown error' });
    res.status(500).json({ success: false, error: { code: 'INTERNAL_ERROR', message: 'Failed to update QMS scope' } });
  }
});
