...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      logger.error('Failed to get Quality Policy', {
        error: error instanceof Error ? error.message : 'Unknown error',
      });
      res.status(500).json({
        success: false,
        error: { code: 'INTERNAL_ERROR', message: 'Failed to get Quality Policy' },
      });
    }
  }
);
...
router.put(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      logger.error('Failed to update Quality Policy', {
        error: error instanceof Error ? error.message : 'Unknown error',
      });
      res.status(500).json({
        success: false,
        error: { code: 'INTERNAL_ERROR', message: 'Failed to update Quality Policy' },
      });
    }
  }
);
