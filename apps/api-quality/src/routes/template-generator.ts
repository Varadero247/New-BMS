...
const TEMPLATE_CATEGORIES: Record<string, { prefix: string; isoStandards: string[]; defaultSections: string[] }> = {
  POLICY: {
    ...
    defaultSections: ['Purpose', 'Scope', 'Policy Statement', 'Objectives', 'Responsibilities', 'Communication', 'Review', 'Related Documents', 'Approval'],
  },
  PROCEDURE: {
    prefix: 'PRO',
    isoStandards: ['ISO 9001:2015'],
    defaultSections: ['Purpose', 'Scope', 'Definitions', 'Responsibilities', 'Procedure', 'Records', 'Related Documents', 'Review'],
  },
  SWP: {
    prefix: 'SWP',
    isoStandards: ['ISO 45001:2018'],
    defaultSections: ['Purpose', 'Scope', 'Definitions', 'Hazards & Risks', 'Control Measures', 'PPE Requirements', 'Emergency Procedures', 'Training', 'Related Documents'],
  },
  FORM: {
    prefix: 'FRM',
    isoStandards: ['ISO 9001:2015'],
    defaultSections: ['Form Header', 'Details Section', 'Assessment Section', 'Actions Section', 'Sign-Off'],
  },
  REGISTER: {
    prefix: 'REG',
    isoStandards: ['ISO 9001:2015'],
  ...
  PLAN: {
    prefix: 'PLN',
    isoStandards: ['ISO 9001:2015'],
    defaultSections: ['Purpose', 'Scope', 'Objectives', 'Schedule', 'Resources', 'Responsibilities', 'Monitoring', 'Review'],
  },
  REPORT: {
    prefix: 'RPT',
    isoStandards: ['ISO 9001:2015'],
    defaultSections: ['Executive Summary', 'Methodology', 'Findings', 'Analysis', 'Recommendations', 'Appendices'],
  },
  AUDIT: {
    prefix: 'AUD',
...
const ISO_KEYWORDS: Record<string, { standard: string; clause?: string; owner: string }> = {
  ...
  'information security': { standard: 'ISO/IEC 27001:2022', owner: '[Information Security Manager]' },
...
function buildTitle(prompt: string, category: string): string {
  ...
  const title = words.slice(0, 6).map(w => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase()).join(' ');

  const categoryLabel = category === 'SWP' ? 'Safe Working Procedure' :
...
function buildSections(prompt: string, category: string, iso: { standard: string; owner: string }): unknown[] {
  ...
  if (category === 'POLICY') {
    sections.push(
      { heading: '1. Purpose', content: `This policy establishes [COMPANY NAME]'s commitment and approach to ${extractTopic(prompt)}. It provides the framework for setting objectives and targets in compliance with ${iso.standard}.` },
      { heading: '2. Scope', content: `This policy applies to all employees, contractors, and relevant interested parties of [COMPANY NAME]. It covers all activities, products, and services within the scope of the management system.` },
      ...
      { heading: '4. Objectives', content: `The following objectives support this policy:\n\na) [Objective 1 — SMART: Specific, Measurable, Achievable, Relevant, Time-bound]\nb) [Objective 2]\nc) [Objective 3]` },
      { heading: '5. Responsibilities', content: `[Managing Director]: Overall accountability for this policy and provision of resources.\n\n${iso.owner}: Day-to-day implementation, monitoring performance, and reporting to management.\n\n[All Employees]: Compliance with this policy and reporting of concerns.` },
      { heading: '6. Communication', content: `This policy shall be:\n\na) Communicated to all employees during induction and annually thereafter\nb) Made available to interested parties on request\nc) Displayed in common areas and on the company intranet\nd) Reviewed for continuing suitability at each management review` },
      { heading: '7. Review', content: 'This policy shall be reviewed annually or when significant changes occur to the organisation, its context, or applicable legal requirements.' },
      { heading: '8. Related Documents', bullets: ['Management System Manual', 'Objectives & Targets Register (REG-002)', 'Management Review Minutes (FRM-009)'] }
    );
  } else if (category === 'PROCEDURE') {
    sections.push(
      { heading: '1. Purpose', content: `This procedure defines the process for ${extractTopic(prompt)} within [COMPANY NAME]'s Integrated Management System, ensuring compliance with ${iso.standard}.` },
      { heading: '2. Scope', content: `This procedure applies to all relevant personnel and activities related to ${extractTopic(prompt)} across [COMPANY NAME].` },
      { heading: '3. Definitions', bullets: ['[Term 1]: [Definition]', '[Term 2]: [Definition]', '[Term 3]: [Definition]'] },
      { heading: '4. Responsibilities', content: `${iso.owner}: Responsible for the implementation and maintenance of this procedure.\n\n[Department Managers]: Ensure compliance within their areas.\n\n[All Staff]: Follow this procedure and report any deviations.` },
      { heading: '5. Procedure', level: 1, content: '' },
      { heading: '5.1 Planning', level: 2, content: 'a) [Step 1 — planning requirements]\nb) [Step 2 — resource allocation]\nc) [Step 3 — communication of requirements]' },
      { heading: '5.2 Implementation', level: 2, content: 'a) [Step 1 — execution details]\nb) [Step 2 — monitoring requirements]\nc) [Step 3 — recording requirements]' },
      { heading: '5.3 Review & Improvement', level: 2, content: 'a) [Step 1 — review frequency and criteria]\nb) [Step 2 — corrective action process]\nc) [Step 3 — lessons learned]' },
      { heading: '6. Records', content: 'The following records shall be maintained:', bullets: ['[Record type 1] — retained for [X] years', '[Record type 2] — retained for [X] years'] },
      { heading: '7. Related Documents', bullets: ['[Related procedure/form 1]', '[Related procedure/form 2]', `${iso.standard}`] },
      { heading: '8. Review', content: 'This procedure shall be reviewed annually or when significant changes occur.' }
    );
  } else if (category === 'SWP') {
    sections.push(
      { heading: '1. Purpose', content: `To establish safe working practices for ${extractTopic(prompt)}, ensuring compliance with ${iso.standard} and applicable legal requirements.` },
      { heading: '2. Scope', content: 'This safe working procedure applies to all employees, contractors, and visitors who may be involved in or affected by these activities.' },
      { heading: '3. Hazards & Risks', bullets: ['[Hazard 1] — [Associated risk and potential consequence]', '[Hazard 2] — [Associated risk and potential consequence]', '[Hazard 3] — [Associated risk and potential consequence]', '[Hazard 4] — [Associated risk and potential consequence]'] },
      { heading: '4. Control Measures', level: 1, content: '' },
      { heading: '4.1 Pre-Work Requirements', level: 2, bullets: ['Complete risk assessment (FRM-002)', 'Obtain Permit to Work where required (FRM-006)', 'Verify training and competence of all personnel', 'Check and inspect all equipment before use', 'Brief all workers on specific hazards and controls'] },
      { heading: '4.2 During Work', level: 2, bullets: ['[Control measure 1]', '[Control measure 2]', '[Control measure 3]', 'Stop work immediately if unsafe conditions develop'] },
      { heading: '4.3 PPE Requirements', level: 2, bullets: ['[PPE item 1 — specification]', '[PPE item 2 — specification]', '[PPE item 3 — specification]'] },
      { heading: '5. Emergency Procedures', content: 'In the event of an emergency:\n\na) Stop work immediately and make the area safe\nb) Raise the alarm and contact emergency services if required\nc) Administer first aid if trained to do so\nd) Report the incident to the H&S Manager\ne) Preserve the scene for investigation' },
      { heading: '6. Training Requirements', bullets: ['[Required training/certification 1]', '[Required training/certification 2]', 'Refresher frequency: [annually/every 3 years]'] },
      { heading: '7. Related Documents', bullets: ['FRM-002: Hazard & Risk Assessment', 'FRM-006: Permit to Work', 'PRO-007: Incident Investigation', iso.standard] }
    );
  } else if (category === 'FORM') {
    sections.push(
      { heading: '1. Form Information', table: { headers: ['Field', 'Detail'], rows: [
        ['Form Number', '[Auto-generated]'], ['Date', '[DD/MM/YYYY]'],
        ['Completed By', '[Name]'], ['Department', '[Department]'],
        ['Reference', `${iso.standard}`]
      ]}},
      { heading: '2. Details', table: { headers: ['Item', 'Description', 'Status', 'Notes'], rows: [
        ['[Item 1]', '', '', ''], ['[Item 2]', '', '', ''],
        ['[Item 3]', '', '', ''], ['[Item 4]', '', '', '']
      ]}},
      { heading: '3. Assessment / Findings', content: '[Space for assessment details, findings, observations, or measurements]' },
      { heading: '4. Actions Required', table: { headers: ['Action', 'Responsible', 'Due Date', 'Status'], rows: [
        ['', '', '', ''], ['', '', '', ''], ['', '', '', '']
      ]}},
      { heading: '5. Sign-Off', content: 'Completed By: _________________________ Date: ___/___/______\n\nReviewed By: _________________________ Date: ___/___/______\n\nApproved By: _________________________ Date: ___/___/______' }
    );
  } else if (category === 'REGISTER') {
    sections.push(
      { heading: '1. Register Information', table: { headers: ['Field', 'Detail'], rows: [
        ['Register Title', extractTopic(prompt)], ['Owner', iso.owner],
        ['Review Frequency', 'Annually'], ['ISO Reference', iso.standard]
      ]}},
      { heading: '2. Register', table: { headers: ['Ref #', 'Description', 'Category', 'Status', 'Owner', 'Date', 'Review Date', 'Notes'], rows: [
        ['001', '', '', '', '', '', '', ''], ['002', '', '', '', '', '', '', ''],
        ['003', '', '', '', '', '', '', '']
      ]}},
      { heading: '3. Review History', table: { headers: ['Review Date', 'Reviewer', 'Changes Made', 'Next Review'], rows: [
        ['', '', '', ''], ['', '', '', '']
      ]}}
    );
  } else if (category === 'PLAN') {
    sections.push(
      { heading: '1. Purpose', content: `This plan establishes the approach for ${extractTopic(prompt)} in compliance with ${iso.standard}.` },
      { heading: '2. Scope', content: 'This plan covers [define scope — departments, activities, timeframe].' },
      { heading: '3. Objectives', bullets: ['[Objective 1]', '[Objective 2]', '[Objective 3]'] },
      { heading: '4. Schedule', table: { headers: ['Activity', 'Responsible', 'Start Date', 'End Date', 'Status'], rows: [
        ['', '', '', '', ''], ['', '', '', '', ''], ['', '', '', '', ''],
        ['', '', '', '', ''], ['', '', '', '', '']
      ]}},
      { heading: '5. Resources', content: 'The following resources are required:\n\na) Personnel: [roles and time commitments]\nb) Equipment: [specific equipment/tools]\nc) Budget: [estimated budget]\nd) External support: [consultants, training providers]' },
      { heading: '6. Monitoring & Review', content: 'Progress will be monitored through:\n\na) [Weekly/monthly] status meetings\nb) KPI tracking against objectives\nc) Formal review at [frequency]\nd) Reporting to management review' },
      { heading: '7. Related Documents', bullets: ['[Related plan/procedure]', iso.standard] }
    );
  } else if (category === 'REPORT') {
    sections.push(
      { heading: '1. Executive Summary', content: `This report presents the findings of ${extractTopic(prompt)} for the period [start date] to [end date]. [Summary of key findings and conclusions].` },
      { heading: '2. Methodology', content: 'The following methodology was used:\n\na) [Data collection method]\nb) [Analysis approach]\nc) [Criteria/standards used]\nd) [Limitations and assumptions]' },
      { heading: '3. Findings', content: '[Detailed findings organised by topic/area]' },
      { heading: '4. Analysis', table: { headers: ['Area', 'Finding', 'Risk Level', 'Trend', 'Action Required'], rows: [
        ['', '', '', '', ''], ['', '', '', '', ''], ['', '', '', '', '']
      ]}},
      { heading: '5. Recommendations', bullets: ['[Recommendation 1 — priority, responsible party, timeframe]', '[Recommendation 2]', '[Recommendation 3]'] },
      { heading: '6. Conclusion', content: '[Overall conclusion and next steps]' },
      { heading: '7. Appendices', content: '[Reference to supporting data, evidence, and supplementary information]' }
    );
  } else if (category === 'AUDIT') {
    sections.push(
      { heading: '1. Audit Information', table: { headers: ['Field', 'Detail'], rows: [
        ['Audit Number', '[AUD-YYYY-NNN]'], ['Standard', iso.standard],
        ['Audit Date', '[DD/MM/YYYY]'], ['Lead Auditor', '[Name]'],
        ['Auditee', '[Name / Department]'], ['Scope', extractTopic(prompt)]
      ]}},
      { heading: '2. Instructions', content: 'For each requirement, record:\n• C = Conforming\n• NC = Non-Conforming (raise NCR)\n• OFI = Opportunity for Improvement\n• N/A = Not Applicable (justify)\n\nRecord objective evidence in the Evidence column.' },
      { heading: '3. Audit Checklist', table: { headers: ['Requirement', 'Evidence', 'Rating', 'Finding', 'NCR Ref'], rows: [
      ...
      { heading: '4. Summary', table: { headers: ['Metric', 'Count'], rows: [
        ['Conforming', ''], ['Non-Conforming', ''], ['OFI', ''], ['N/A', '']
      ]}},
...
function extractTopic(prompt: string): string {
  return prompt
    .replace(/create|generate|make|build|write|draft|a|an|the|for|me|please|template|document|policy|procedure|form|register|plan|report|audit|checklist|swp|safe working/gi, '')
    .replace(/\s+/g, ' ')
...
router.get('/', authenticate as any, async (req: Request, res: Response) => {
  try {
    ...
    res.json({ success: true, data: templates, pagination: { page: parseInt(page as string, 10) || 1, limit: parseInt(limit as string, 10) || 20, total } });
  } catch (error: unknown) {
...
router.post('/', authenticate as any, async (req: Request, res: Response) => {
  try {
    ...
    const { prompt, category: requestedCategory, isoStandard: requestedStandard, title: requestedTitle } = parsed.data;
    ...
    if (!catDef) {
      return res.status(400).json({
        success: false,
        error: { code: 'INVALID_CATEGORY', message: `Invalid category. Valid: ${Object.keys(TEMPLATE_CATEGORIES).join(', ')}` },
      });
    }
