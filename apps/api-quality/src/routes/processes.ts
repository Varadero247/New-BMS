...
router.put('/:id', checkOwnership(prisma.qualProcess), async (req: AuthRequest, res: Response) => {
  try {
    ...
    const schema = z.object({
      ...
      processOwner: z.string().trim().min(1).optional(),
      version: z.string().optional(),
      status: z.enum(['DRAFT', 'ACTIVE', 'UNDER_REVIEW', 'RETIRED']).optional(),
      // Turtle Diagram
      purposeScope: z.string().trim().min(1).optional(),
      inputs: z.string().trim().min(1).optional(),
      outputs: z.string().trim().min(1).optional(),
