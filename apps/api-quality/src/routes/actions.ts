...
router.put('/:id', checkOwnership(prisma.qualAction), async (req: AuthRequest, res: Response) => {
  try {
    ...
    const schema = z.object({
      ...
      completionDate: z.string().nullable().refine(s => !isNaN(Date.parse(s)), 'Invalid date format').optional(),
      percentComplete: z.number().min(0).max(100).optional(),
      verificationMethod: z.enum(['DOCUMENT_REVIEW', 'INSPECTION', 'AUDIT', 'TEST', 'SIGN_OFF']).nullable().optional(),
      verifiedBy: z.string().nullable().optional(),
      verificationDate: z.string().nullable().refine(s => !isNaN(Date.parse(s)), 'Invalid date format').optional(),
