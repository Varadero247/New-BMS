...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to list job notes', {
      error: error instanceof Error ? error.message : 'Unknown error',
    });
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to list job notes' },
    });
  }
});
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to create job note', {
      error: error instanceof Error ? error.message : 'Unknown error',
    });
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to create job note' },
    });
  }
});
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    ...
    if (!data) {
      return res
        .status(404)
        .json({ success: false, error: { code: 'NOT_FOUND', message: 'Job note not found' } });
    }
    res.json({ success: true, data });
  } catch (error: unknown) {
    logger.error('Failed to get job note', {
      error: error instanceof Error ? error.message : 'Unknown error',
    });
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to get job note' },
    });
  }
});
...
router.put('/:id', async (req: Request, res: Response) => {
  try {
    const existing = await prisma.fsSvcJobNote.findFirst({
      where: { id: req.params.id, deletedAt: null },
    });
    if (!existing) {
      return res
    ...
    logger.error('Failed to update job note', {
      error: error instanceof Error ? error.message : 'Unknown error',
    });
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to update job note' },
    });
  }
});
...
router.delete('/:id', async (req: Request, res: Response) => {
  try {
    ...
    await prisma.fsSvcJobNote.update({
      where: { id: req.params.id },
      data: { deletedAt: new Date() },
    });
    res.json({ success: true, data: { message: 'Job note deleted' } });
  } catch (error: unknown) {
    logger.error('Failed to delete job note', {
      error: error instanceof Error ? error.message : 'Unknown error',
    });
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to delete job note' },
    });
  }
});
