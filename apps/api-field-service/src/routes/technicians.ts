...
const technicianCreateSchema = z.object({
  ...
  skills: z.array(z.string().trim()).or(z.record(z.any())),
  certifications: z.array(z.string().trim()).or(z.record(z.any())).optional().nullable(),
...
const technicianUpdateSchema = z.object({
  ...
  phone: z.string().trim().max(30).optional().nullable(),
  skills: z.array(z.string().trim()).or(z.record(z.any())).optional(),
  certifications: z.array(z.string().trim()).or(z.record(z.any())).optional().nullable(),
  zone: z.string().trim().max(100).optional().nullable(),
  status: z.enum(['AVAILABLE', 'ON_JOB', 'OFF_DUTY', 'ON_LEAVE']).optional(),
  currentLocation: z.record(z.any()).optional().nullable(),
