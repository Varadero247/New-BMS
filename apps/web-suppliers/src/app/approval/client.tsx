...
export default function ApprovalClient() {
  ...
  const pendingCount = items.filter(i => i.status === 'PROSPECTIVE' || i.status === 'CONDITIONAL').length;
  const approvedCount = items.filter(i => i.status === 'APPROVED').length;
  const suspendedCount = items.filter(i => i.status === 'SUSPENDED' || i.status === 'BLACKLISTED').length;
    ...
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">Supplier Approval</h1>
            <p className="text-gray-500 dark:text-gray-400 mt-1">Approve, suspend, or manage supplier status</p>
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <Card><CardContent className="pt-6 text-center"><p className="text-3xl font-bold">{items.length}</p><p className="text-sm text-gray-500 dark:text-gray-400">Total Suppliers</p></CardContent></Card>
          <Card><CardContent className="pt-6 text-center"><p className="text-3xl font-bold text-blue-600">{pendingCount}</p><p className="text-sm text-gray-500 dark:text-gray-400">Pending Approval</p></CardContent></Card>
          <Card><CardContent className="pt-6 text-center"><p className="text-3xl font-bold text-green-600">{approvedCount}</p><p className="text-sm text-gray-500 dark:text-gray-400">Approved</p></CardContent></Card>
          <Card><CardContent className="pt-6 text-center"><p className="text-3xl font-bold text-red-600">{suspendedCount}</p><p className="text-sm text-gray-500 dark:text-gray-400">Suspended / Blocked</p></CardContent></Card>
        </div>
                  ...
                  <TableBody>
                    {items.map(item => (
                      <TableRow key={item.id}>
                        ...
                        <TableCell className="text-sm">{item.approvedDate ? new Date(item.approvedDate).toLocaleDateString() : '-'}</TableCell>
                        <TableCell>
                          <div className="flex gap-2">
                            {item.status !== 'APPROVED' && (
                              <Button size="sm" variant="outline" onClick={() => handleApprove(item.id)} disabled={actionLoading === item.id}
                                className="text-green-600 hover:bg-green-50 dark:hover:bg-green-900/20">
                                {actionLoading === item.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <><CheckCircle className="h-4 w-4 mr-1" />Approve</>}
                              </Button>
                            )}
                            {item.status !== 'SUSPENDED' && item.status !== 'BLACKLISTED' && (
                              <Button size="sm" variant="outline" onClick={() => handleSuspend(item.id)} disabled={actionLoading === item.id}
                                className="text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20">
                                {actionLoading === item.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <><XCircle className="h-4 w-4 mr-1" />Suspend</>}
                              </Button>
                            )}
                          </div>
                        </TableCell>
