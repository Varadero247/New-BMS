...
async function generateRef(orgId: string): Promise<string> {
  ...
  const c = await prisma.assetInspection.count({
    where: { orgId, referenceNumber: { startsWith: `AIN-${y}` } },
  });
  return `AIN-${y}-${String(c + 1).padStart(4, '0')}`;
}
router.get('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    if (search) where.referenceNumber = { contains: search, mode: 'insensitive' };
    const skip =
    ...
    const [data, total] = await Promise.all([
      ...
      prisma.assetInspection.count({ where }),
    ]);
    res.json({
      success: true,
      data,
    ...
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch inspections' },
    });
  }
});
router.get('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    const orgId = ((req as AuthRequest).user as any)?.orgId || 'default';
    const item = await prisma.assetInspection.findFirst({
    ...
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch inspection' },
    });
  }
});
router.post('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const data = await prisma.assetInspection.create({
      data: {
        assetId,
        inspector,
        inspectorName,
...
router.put('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const orgId = ((req as AuthRequest).user as any)?.orgId || 'default';
    const existing = await prisma.assetInspection.findFirst({
      where: { id: req.params.id, orgId, deletedAt: null } as any,
    });
    if (!existing)
      return res
    ...
    const data = await prisma.assetInspection.update({
