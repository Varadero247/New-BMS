...
async function generateRef(orgId: string): Promise<string> {
  ...
  const c = await prisma.assetWorkOrder.count({
    where: { orgId, referenceNumber: { startsWith: `AWO-${y}` } },
  });
  return `AWO-${y}-${String(c + 1).padStart(4, '0')}`;
}
...
router.get('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    const orgId = ((req as AuthRequest).user as { orgId?: string })?.orgId || 'default';
    const item = await prisma.assetWorkOrder.findFirst({
    ...
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch work order' },
    });
  }
});
router.post('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const data = await prisma.assetWorkOrder.create({
      data: {
        assetId,
        title,
        description,
...
router.put('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const orgId = ((req as AuthRequest).user as { orgId?: string })?.orgId || 'default';
    const existing = await prisma.assetWorkOrder.findFirst({
      where: { id: req.params.id, orgId, deletedAt: null } as any,
    });
    if (!existing)
      return res
    ...
    const data = await prisma.assetWorkOrder.update({
