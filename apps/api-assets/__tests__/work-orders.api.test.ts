...
describe('GET /api/work-orders', () => {
  it('should return work orders', async () => {
    (prisma as any).assetWorkOrder.findMany.mockResolvedValue([{ id: '1', title: 'Test' }]);
...
describe('GET /api/work-orders/:id', () => {
  it('should return 404 if not found', async () => {
    ...
    const res = await request(app).get('/api/work-orders/nope');
    expect(res.status).toBe(404);
  });
  it('should return item by id', async () => {
    (prisma as any).assetWorkOrder.findFirst.mockResolvedValue({ id: '1' });
    const res = await request(app).get('/api/work-orders/1');
    expect(res.status).toBe(200);
...
describe('PUT /api/work-orders/:id', () => {
  it('should update', async () => {
    (prisma as any).assetWorkOrder.findFirst.mockResolvedValue({ id: '1' });
    (prisma as any).assetWorkOrder.update.mockResolvedValue({ id: '1', title: 'Updated' });
    const res = await request(app).put('/api/work-orders/1').send({ title: 'Updated' });
    expect(res.status).toBe(200);
  });
});
...
describe('DELETE /api/work-orders/:id', () => {
  it('should soft delete', async () => {
    (prisma as any).assetWorkOrder.findFirst.mockResolvedValue({ id: '1' });
    (prisma as any).assetWorkOrder.update.mockResolvedValue({ id: '1' });
