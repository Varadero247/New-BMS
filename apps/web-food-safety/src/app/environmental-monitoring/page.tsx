...
const initialForm = {
  location: '', testType: 'SWAB', organism: '', sampleDate: '', result: '',
  value: '', unit: 'CFU/cm²', limit: '', performedBy: '', notes: '', status: 'PENDING',
};
...
export default function EnvironmentalMonitoringPage() {
  ...
  async function handleSave() {
    ...
    setSubmitting(true);
    try {
      const payload: any = { location: form.location, testType: form.testType, status: form.status };
      ...
      if (editing) { await api.put(`/environmental-monitoring/${editing.id}`, payload); }
      else { await api.post('/environmental-monitoring', payload); }
  ...
  async function handleDelete(id: string) {
    ...
    try { await api.delete(`/environmental-monitoring/${id}`); load(); } catch { alert('Failed'); }
  }

  const filtered = items.filter(i => {
    const loc = i.location || i.zone || '';
    return loc.toLowerCase().includes(search.toLowerCase()) || (i.organism || '').toLowerCase().includes(search.toLowerCase());
  });
  ...
  return (
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        ...
        {fails.length > 0 && (
          <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-lg flex items-center gap-3">
            ...
            <p className="text-red-700 font-medium">{fails.length} sample{fails.length > 1 ? 's' : ''} failed or require retest — immediate investigation required</p>
          </div>
        )}
        ...
        <Card className="mb-6"><CardContent className="pt-6">
          <div className="flex gap-4">
            <div className="flex-1 relative">
              ...
              <input type="text" aria-label="Search by location or organism..." placeholder="Search by location or organism..." value={search} onChange={e => setSearch(e.target.value)}
        ...
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2"><Microscope className="h-5 w-5 text-orange-600" />Environmental Samples ({filtered.length})</CardTitle></CardHeader>
          <CardContent>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Sample Record' : 'Log Environmental Sample'} size="lg">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input value={form.location} onChange={e => setForm({...form, location: e.target.value})}
                className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:outline-none" placeholder="e.g. Processing Line 1 - Zone 2" />
            </div>
            <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input value={form.organism} onChange={e => setForm({...form, organism: e.target.value})}
                className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:outline-none" placeholder="e.g. Listeria, TVC, E.coli" />
            </div>
            <div>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <input value={form.value} onChange={e => setForm({...form, value: e.target.value})}
                className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:outline-none" placeholder="e.g. 50" />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Unit</label>
              <input value={form.unit} onChange={e => setForm({...form, unit: e.target.value})}
                className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:outline-none" placeholder="CFU/cm²" />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Limit</label>
              <input value={form.limit} onChange={e => setForm({...form, limit: e.target.value})}
                className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:outline-none" placeholder="e.g. 100" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
        ...
        <ModalFooter>
          ...
          <Button className="bg-orange-600 hover:bg-orange-700" onClick={handleSave} disabled={submitting}>{submitting ? 'Saving...' : editing ? 'Update' : 'Log Sample'}</Button>
        </ModalFooter>
      </Modal>
    </div>
  );
