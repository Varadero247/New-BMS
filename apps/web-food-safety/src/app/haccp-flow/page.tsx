...
const initialForm = {
  processStep: '', stepNumber: '', isCCP: 'false', hazards: '', controlMeasures: '',
  criticalLimit: '', monitoringProcedure: '', correctiveAction: '', verificationProcedure: '',
  recordKeeping: '', notes: '', status: 'ACTIVE',
};
...
export default function HaccpFlowPage() {
  ...
  async function load() {
    try {
      ...
      setItems(data.sort((a: HaccpStep, b: HaccpStep) => ((a.step || a.stepNumber || 0) - (b.step || b.stepNumber || 0))));
  ...
  async function handleSave() {
    setFormError('');
    if (!form.processStep.trim()) { setFormError('Process step is required'); return; }
    setSubmitting(true);
    try {
      const payload: any = {
      ...
      if (editing) { await api.put(`/haccp-flow/${editing.id}`, payload); }
      else { await api.post('/haccp-flow', payload); }
  ...
  async function handleDelete(id: string) {
    ...
    try { await api.delete(`/haccp-flow/${id}`); load(); } catch { alert('Failed'); }
  }
  ...
  return (
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <Card className="mb-6"><CardContent className="pt-6">
          <div className="relative">
            ...
            <input type="text" aria-label="Search process steps..." placeholder="Search process steps..." value={search} onChange={e => setSearch(e.target.value)}
        ...
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2"><GitBranch className="h-5 w-5 text-orange-600" />HACCP Steps ({filtered.length})</CardTitle></CardHeader>
          <CardContent>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit HACCP Step' : 'Add HACCP Step'} size="lg">
        <div className="space-y-4 max-h-[65vh] overflow-y-auto pr-1">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="col-span-2">
              ...
              <input value={form.processStep} onChange={e => setForm({...form, processStep: e.target.value})}
                className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:outline-none" placeholder="e.g. Pasteurisation, Cooling, Packaging" />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Step Number</label>
              <input type="number" value={form.stepNumber} onChange={e => setForm({...form, stepNumber: e.target.value})}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select value={form.isCCP} onChange={e => setForm({...form, isCCP: e.target.value})}
                className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:outline-none">
          ...
          <div>
            ...
            <textarea value={form.hazards} onChange={e => setForm({...form, hazards: e.target.value})}
              rows={2} className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:outline-none" placeholder="Biological, chemical, physical hazards at this step" />
          </div>
          <div>
            <label className="block text-sm font-medium mb-1">Control Measures</label>
            <textarea value={form.controlMeasures} onChange={e => setForm({...form, controlMeasures: e.target.value})}
          ...
          {form.isCCP === 'true' && <>
            <div>
              ...
              <input value={form.criticalLimit} onChange={e => setForm({...form, criticalLimit: e.target.value})}
                className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:outline-none" placeholder="e.g. ≥72°C for 15 seconds" />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Monitoring Procedure</label>
              <textarea value={form.monitoringProcedure} onChange={e => setForm({...form, monitoringProcedure: e.target.value})}
                rows={2} className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:outline-none" />
            </div>
            <div>
            ...
            <div>
              ...
              <textarea value={form.verificationProcedure} onChange={e => setForm({...form, verificationProcedure: e.target.value})}
                rows={2} className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:outline-none" />
            </div>
          </>}
        </div>
