...
export default function HazardsPage() {
  ...
  const filtered = items.filter(
    (i) =>
      i.name.toLowerCase().includes(search.toLowerCase()) && (!typeFilter || i.type === typeFilter)
  );
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">
              HACCP hazard identification and analysis
            </p>
          </div>
            ...
            <Card key={type}>
              <CardContent className="pt-6">
                <div className="flex items-center justify-between">
                  ...
                  <AlertTriangle
                    className={`h-8 w-8 ${type === 'BIOLOGICAL' ? 'text-blue-500' : type === 'CHEMICAL' ? 'text-red-500' : type === 'PHYSICAL' ? 'text-gray-500 dark:text-gray-400' : 'text-orange-500'}`}
                  />
                </div>
              </CardContent>
            </Card>
                  ...
                  <tbody>
                    {filtered.map((h) => (
                      <tr key={h.id} className="border-b hover:bg-gray-50 dark:bg-gray-800">
                        ...
                        <td className="py-3 px-4">
                          <span
                            className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${severityColors[h.severity] || 'bg-gray-100 dark:bg-gray-800 text-gray-700'}`}
                          >
                            {h.severity}
                          </span>
                        </td>
                        ...
                        <td className="py-3 px-4">
                          <span
                            className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${severityColors[h.riskLevel] || 'bg-gray-100 dark:bg-gray-800 text-gray-700'}`}
                          >
                            {h.riskLevel}
                          </span>
                        </td>
