...
const initialForm = { name: '', processStep: '', hazardType: 'BIOLOGICAL', criticalLimit: '', monitoringFrequency: 'CONTINUOUS', correctiveAction: '', status: 'ACTIVE' };
...
export default function CCPsPage() {
  ...
  async function load() {
    try { const res = await api.get('/ccps'); setItems(res.data.data || []); }
  ...
  function openEdit(c: CCP) {
    setEditing(c);
    setForm({ name: c.name, processStep: c.processStep || '', hazardType: c.hazardType, criticalLimit: c.criticalLimit || '', monitoringFrequency: c.monitoringFrequency || 'CONTINUOUS', correctiveAction: c.correctiveAction || '', status: c.status });
  ...
  return (
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Table */}
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2"><Crosshair className="h-5 w-5 text-orange-600" />CCPs ({filtered.length})</CardTitle></CardHeader>
          <CardContent>
                    ...
                    {filtered.map(c => (
                      <tr key={c.id} className="border-b hover:bg-gray-50">
                        ...
                        <td className="py-3 px-4">
                          <Badge className={c.status === 'ACTIVE' || c.status === 'IN_CONTROL' ? 'bg-green-100 text-green-700' : c.status === 'DEVIATION' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700'}>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit CCP' : 'Add CCP'} size="lg">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input value={form.processStep} onChange={e => setForm({...form, processStep: e.target.value})}
                className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:outline-none" placeholder="e.g. Cooking, Cooling" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium mb-1">Hazard Type</label>
              <select value={form.hazardType} onChange={e => setForm({...form, hazardType: e.target.value})}
                className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:outline-none">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input value={form.criticalLimit} onChange={e => setForm({...form, criticalLimit: e.target.value})}
                className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:outline-none" placeholder="e.g. ≥75°C for 15s" />
            </div>
            <div>
