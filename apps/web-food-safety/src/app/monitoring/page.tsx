...
const initialForm = { ccpId: '', parameter: '', value: '', unit: '', recordedBy: '', notes: '', status: 'IN_CONTROL' };
...
export default function MonitoringPage() {
  ...
  async function load() {
    try {
      ...
      const [monRes, ccpRes] = await Promise.all([api.get(`/monitoring${params}`), api.get('/ccps')]);
  ...
  async function handleSave() {
    setFormError('');
    if (!form.parameter.trim()) { setFormError('Parameter is required'); return; }
    setSubmitting(true);
    try {
      ...
      setModalOpen(false); setForm(initialForm); load();
  ...
  async function handleDelete(id: string) {
    ...
    try { await api.delete(`/monitoring/${id}`); load(); } catch (e: any) { alert('Failed'); }
  }
  ...
  return (
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        ...
        {deviations.length > 0 && (
          <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-lg flex items-center gap-3">
            ...
            <p className="text-red-700 font-medium">{deviations.length} deviation{deviations.length > 1 ? 's' : ''} detected — immediate corrective action required</p>
          </div>
        )}
        ...
        <Card className="mb-6"><CardContent className="pt-6">
          <div className="flex gap-4">
            <div className="flex-1 relative">
              ...
              <input type="text" aria-label="Search records..." placeholder="Search records..." value={search} onChange={e => setSearch(e.target.value)}
        ...
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2"><Activity className="h-5 w-5 text-orange-600" />Records ({filtered.length})</CardTitle></CardHeader>
          <CardContent>
                  ...
                  <tbody>
                    {filtered.map(r => (
                      <tr key={r.id} className={`border-b hover:bg-gray-50 dark:bg-gray-800 ${r.status === 'DEVIATION' ? 'bg-red-50' : ''}`}>
                        ...
                        <td className="py-3 px-4"><div className="flex justify-end"><Button variant="ghost" size="sm" onClick={() => handleDelete(r.id)}><Trash2 className="h-4 w-4 text-red-500" /></Button></div></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title="Log Monitoring Reading" size="lg">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select value={form.ccpId} onChange={e => setForm({...form, ccpId: e.target.value})}
            ...
            <div>
              ...
              <input value={form.parameter} onChange={e => setForm({...form, parameter: e.target.value})}
                className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:outline-none" placeholder="e.g. Temperature" />
            </div>
          </div>
          <div className="grid grid-cols-3 gap-4">
            <div className="col-span-2">
              ...
              <input value={form.value} onChange={e => setForm({...form, value: e.target.value})}
                className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:outline-none" placeholder="e.g. 76.5" />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Unit</label>
              <input value={form.unit} onChange={e => setForm({...form, unit: e.target.value})}
                className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:outline-none" placeholder="°C" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
        ...
        <ModalFooter>
          ...
          <Button className="bg-orange-600 hover:bg-orange-700" onClick={handleSave} disabled={submitting}>{submitting ? 'Saving...' : 'Log Reading'}</Button>
        </ModalFooter>
      </Modal>
    </div>
  );
