...
export default function AllergensPage() {
  ...
  async function handleSave() {
    ...
    try {
      if (editing) {
        await api.put(`/allergens/${editing.id}`, form);
      } else {
        await api.post('/allergens', form);
      }
      setModalOpen(false);
      load();
    } catch (e) {
  ...
  return (
    <div className="p-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">
              Track and manage the 14 EU allergens across products and processes
            </p>
          </div>
          ...
          <CardContent>
            <div className="flex flex-wrap gap-2">
              {EU_ALLERGENS.map((a) => (
                <span
                  key={a}
                  className={`px-2 py-1 rounded text-xs font-medium ${items.some((i) => (i.category || '').toUpperCase() === a) ? 'bg-orange-100 text-orange-700 border border-orange-200' : 'bg-gray-100 dark:bg-gray-800 text-gray-500'}`}
                >
                  {a.replace(/_/g, ' ')}
                </span>
              ))}
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-600">
                    {items.filter((i) => i.status === 'ACTIVE' || i.status === 'MANAGED').length}
                  </p>
                </div>
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-gray-500 dark:text-gray-400">Allergen Types</p>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              Allergen Records ({filtered.length})
            </CardTitle>
          </CardHeader>
          <CardContent>
            {loading ? (
                    ...
                    {filtered.map((a) => (
                      <tr key={a.id} className="border-b hover:bg-gray-50 dark:bg-gray-800">
                        ...
                        <td className="py-3 px-4">
                          <Badge
                            className={
                              a.status === 'ACTIVE' || a.status === 'MANAGED'
      ...
      <Modal
        ...
        title={editing ? 'Edit Allergen' : 'Add Allergen'}
        size="md"
      >
