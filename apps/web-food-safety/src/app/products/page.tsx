...
const initialForm = {
  name: '', sku: '', category: 'FINISHED_GOODS', description: '', ingredients: '',
  shelfLife: '', shelfLifeUnit: 'days', storageConditions: '', allergens: '', certifications: '', status: 'ACTIVE',
};
...
export default function ProductsPage() {
  ...
  async function handleDelete(id: string) {
    ...
    try { await api.delete(`/products/${id}`); load(); } catch { alert('Failed'); }
  }
  ...
  return (
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2"><Package className="h-5 w-5 text-orange-600" />Products ({filtered.length})</CardTitle></CardHeader>
          <CardContent>
        ...
        <div className="space-y-4 max-h-[65vh] overflow-y-auto pr-1">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input value={form.sku} onChange={e => setForm({...form, sku: e.target.value})}
                className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:outline-none font-mono" placeholder="SKU-001" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="col-span-2">
              ...
              <input type="number" value={form.shelfLife} onChange={e => setForm({...form, shelfLife: e.target.value})}
                className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:outline-none" />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Unit</label>
              <select value={form.shelfLifeUnit} onChange={e => setForm({...form, shelfLifeUnit: e.target.value})}
                className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:outline-none">
          ...
          <div>
            ...
            <input value={form.storageConditions} onChange={e => setForm({...form, storageConditions: e.target.value})}
              className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:outline-none" placeholder="e.g. Store at 2-8°C, dry conditions" />
          </div>
          <div>
            ...
            <input value={form.allergens} onChange={e => setForm({...form, allergens: e.target.value})}
              className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:outline-none" placeholder="e.g. Contains: milk, wheat, eggs" />
          </div>
          <div>
            <label className="block text-sm font-medium mb-1">Certifications</label>
            <input value={form.certifications} onChange={e => setForm({...form, certifications: e.target.value})}
              className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:outline-none" placeholder="e.g. Organic, Halal, Kosher" />
          </div>
        </div>
        <ModalFooter>
