...
const initialForm = {
  title: '', area: '', threatType: 'INTENTIONAL', vulnerability: '', likelihood: 'LOW',
  severity: 'MEDIUM', mitigation: '', responsible: '', reviewDate: '', notes: '', status: 'OPEN',
};
...
export default function FoodDefensePage() {
  ...
  async function handleSave() {
    ...
    setSubmitting(true);
    try {
      const payload: any = { title: form.title, threatType: form.threatType, likelihood: form.likelihood, severity: form.severity, status: form.status };
      ...
      if (editing) { await api.put(`/food-defense/${editing.id}`, payload); }
      else { await api.post('/food-defense', payload); }
  ...
  async function handleDelete(id: string) {
    ...
    try { await api.delete(`/food-defense/${id}`); load(); } catch { alert('Failed'); }
  }
  ...
  const filtered = items.filter(i => {
    ...
    return title.toLowerCase().includes(search.toLowerCase()) || (i.area || '').toLowerCase().includes(search.toLowerCase());
  });

  const highRisk = items.filter(i => getRiskLevel(i.likelihood || 'LOW', i.severity || 'LOW').label === 'HIGH');
  ...
  return (
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        ...
        {highRisk.length > 0 && (
          <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-lg flex items-center gap-3">
            ...
            <p className="text-red-700 font-medium">{highRisk.length} high-risk threat{highRisk.length > 1 ? 's' : ''} identified — mitigation actions required</p>
          </div>
        )}
        ...
        <Card className="mb-6"><CardContent className="pt-6">
          <div className="flex gap-4">
            <div className="flex-1 relative">
              ...
              <input type="text" aria-label="Search by title or area..." placeholder="Search by title or area..." value={search} onChange={e => setSearch(e.target.value)}
        ...
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2"><Shield className="h-5 w-5 text-orange-600" />Threat Assessments ({filtered.length})</CardTitle></CardHeader>
          <CardContent>
                    ...
                    {filtered.map(r => {
                      ...
                      return (
                        <tr key={r.id} className="border-b hover:bg-gray-50 dark:bg-gray-800">
                          ...
                          <td className="py-3 px-4"><Badge className={r.status === 'MITIGATED' || r.status === 'CLOSED' ? 'bg-green-100 text-green-700' : r.status === 'OPEN' ? 'bg-yellow-100 text-yellow-700' : 'bg-blue-100 text-blue-700'}>{r.status}</Badge></td>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Threat Assessment' : 'Add Threat Assessment'} size="lg">
        <div className="space-y-4 max-h-[65vh] overflow-y-auto pr-1">
          ...
          <div>
            ...
            <input value={form.title} onChange={e => setForm({...form, title: e.target.value})}
              className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:outline-none" placeholder="e.g. Raw material intake point vulnerability" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium mb-1">Area</label>
              <input value={form.area} onChange={e => setForm({...form, area: e.target.value})}
                className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:outline-none" placeholder="e.g. Receiving Dock" />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Threat Type</label>
              <select value={form.threatType} onChange={e => setForm({...form, threatType: e.target.value})}
          ...
          <div>
            ...
            <textarea value={form.vulnerability} onChange={e => setForm({...form, vulnerability: e.target.value})}
