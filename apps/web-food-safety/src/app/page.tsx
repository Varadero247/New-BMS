...
export default function FoodSafetyDashboard() {
  ...
  async function loadDashboard() { try { const res = await api.get('/dashboard'); setData(res.data.data); } catch (error) { console.error('Error:', error); setData({ totalHazards: 0, criticalCCPs: 0, activeMonitoring: 0, openNCRs: 0, pendingAudits: 0, activeRecalls: 0, supplierCount: 0, complianceScore: 0 }); } finally { setLoading(false); } }
  ...
  const kpiCards = [
    { title: 'Hazards Identified', value: String(data?.totalHazards || 0), icon: AlertTriangle, iconColor: 'text-red-500', bgColor: 'bg-red-50', valueColor: 'text-red-700', href: '/hazards' },
    { title: 'Critical CCPs', value: String(data?.criticalCCPs || 0), icon: Crosshair, iconColor: 'text-orange-500', bgColor: 'bg-orange-50', valueColor: 'text-orange-700', href: '/ccps' },
    { title: 'Active Monitoring', value: String(data?.activeMonitoring || 0), icon: Activity, iconColor: 'text-blue-500', bgColor: 'bg-blue-50', valueColor: 'text-blue-700', href: '/monitoring' },
    { title: 'Open NCRs', value: String(data?.openNCRs || 0), icon: AlertTriangle, iconColor: 'text-amber-500', bgColor: 'bg-amber-50', valueColor: 'text-amber-700', href: '/ncrs' },
    { title: 'Pending Audits', value: String(data?.pendingAudits || 0), icon: ClipboardCheck, iconColor: 'text-indigo-500', bgColor: 'bg-indigo-50', valueColor: 'text-indigo-700', href: '/audits' },
    { title: 'Active Recalls', value: String(data?.activeRecalls || 0), icon: RotateCcw, iconColor: 'text-red-500', bgColor: 'bg-red-50', valueColor: 'text-red-700', href: '/recalls' },
    { title: 'Approved Suppliers', value: String(data?.supplierCount || 0), icon: Package, iconColor: 'text-green-500', bgColor: 'bg-green-50', valueColor: 'text-green-700', href: '/suppliers' },
    { title: 'Compliance Score', value: `${data?.complianceScore || 0}%`, icon: Shield, iconColor: 'text-teal-500', bgColor: 'bg-teal-50', valueColor: 'text-teal-700', href: '/audits' },
  ];
