...
export default function SanitationPage() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (editing) {
        await api.put(`/sanitation/${editing.id}`, payload);
      } else {
        await api.post('/sanitation', payload);
      }
      setModalOpen(false);
      load();
    } catch (e: any) {
  ...
  return (
    <div className="p-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">
              Cleaning and sanitation schedule management
            </p>
          </div>
        ...
        <div className="grid grid-cols-4 gap-4 mb-6">
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-gray-500 dark:text-gray-400">Overdue/Failed</p>
                  <p className="text-2xl font-bold text-red-600">
                    {items.filter((i) => i.status === 'OVERDUE' || i.status === 'FAILED').length}
                  </p>
                </div>
          ...
          <CardContent className="pt-6">
            <div className="flex gap-4">
              <div className="flex-1 relative">
                ...
                <input
                  type="text"
                  aria-label="Search by area or procedure..."
                  placeholder="Search by area or procedure..."
                  value={search}
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              Sanitation Records ({filtered.length})
            </CardTitle>
          </CardHeader>
          <CardContent>
            {loading ? (
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {filtered.map((r) => (
                      <tr
                        key={r.id}
                        className={`border-b hover:bg-gray-50 dark:bg-gray-800 ${r.status === 'OVERDUE' || r.status === 'FAILED' ? 'bg-red-50' : ''}`}
                      >
      ...
      <Modal
        ...
        title={editing ? 'Edit Sanitation Record' : 'Add Sanitation Record'}
        size="lg"
      >
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                placeholder="e.g. Processing Area A"
              />
            </div>
            <div>
