...
const mainNav = [
  ...
  { name: 'HACCP Flow', href: '/haccp-flow', icon: GitBranch },
  { name: 'Hazards', href: '/hazards', icon: AlertTriangle },
  { name: 'CCPs', href: '/ccps', icon: Crosshair },
...
const qualityNav = [
  ...
  { name: 'Recalls', href: '/recalls', icon: RotateCcw },
];
const operationsNav = [
  { name: 'Products', href: '/products', icon: Package },
  { name: 'Allergens', href: '/allergens', icon: Sparkles },
  { name: 'Traceability', href: '/traceability', icon: Link2 },
  { name: 'Sanitation', href: '/sanitation', icon: Apple },
  { name: 'Training', href: '/training', icon: GraduationCap },
];
const complianceNav = [
  { name: 'Env Monitoring', href: '/environmental-monitoring', icon: Microscope },
  { name: 'Food Defense', href: '/food-defense', icon: Shield },
];
  ...
  return (
    <aside className="w-64 bg-card border-r border-border flex flex-col">
      <div className="p-6 border-b border-border bg-orange-50 dark:bg-orange-900/10">
        <h1 className="text-xl font-bold font-display text-orange-900 dark:text-orange-100">
          Food Safety
        </h1>
        <p className="text-xs text-orange-600 dark:text-orange-400 mt-1">HACCP Management System</p>
      </div>
      <nav className="flex-1 p-4 overflow-y-auto" aria-label="Module navigation">
        <NavSection items={mainNav} pathname={pathname} />
        <NavSection title="Quality" items={qualityNav} pathname={pathname} />
