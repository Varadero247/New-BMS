...
async function generateRefNumber(): Promise<string> {
  ...
  const prefix = `ENV-COMM-${yy}${mm}`;
  const count = await prisma.envCommunication.count({
...
router.post('/', async (req: AuthRequest, res: Response) => {
  try {
    ...
    const communication = await prisma.envCommunication.create({
      data: {
        refNumber,
...
router.get('/:id', checkOwnership(prisma.envCommunication), async (req: AuthRequest, res: Response) => {
  try {
...
router.put('/:id', checkOwnership(prisma.envCommunication), async (req: AuthRequest, res: Response) => {
  try {
    ...
    const communication = await prisma.envCommunication.update({
      where: { id: req.params.id },
      data: updateData,
    });
...
router.delete('/:id', checkOwnership(prisma.envCommunication), async (req: AuthRequest, res: Response) => {
  try {
    const existing = await prisma.envCommunication.findUnique({ where: { id: req.params.id } });
    if (!existing || existing.deletedAt) {
      return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'Communication not found' } });
