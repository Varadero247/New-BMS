...
describe('Environment Audits API Routes', () => {
  ...
  describe('GET /api/audits/schedule', () => {
    it('should return active audit schedules ordered by next due date', async () => {
      const mockSchedules = [
        { id: 'sched-1', title: 'Annual EMS Audit', type: 'SYSTEM', frequency: 'ANNUAL', nextDueDate: new Date('2026-06-01'), active: true },
        { id: 'sched-2', title: 'Compliance Check', type: 'COMPLIANCE', frequency: 'QUARTERLY', nextDueDate: new Date('2026-09-01'), active: true },
      ];
  ...
  describe('POST /api/audits/schedule', () => {
    ...
    it('should create an audit schedule successfully', async () => {
      const mockSchedule = { id: 'sched-1', ...schedulePayload, nextDueDate: new Date('2026-06-01'), active: true };
  ...
  describe('GET /api/audits/:id', () => {
    ...
    it('should return a single audit with findings', async () => {
      ...
      const response = await request(app)
        .get('/api/audits/audit-1')
        .set('Authorization', 'Bearer token');

      expect(response.status).toBe(200);
      expect(response.body.success).toBe(true);
      expect(response.body.data.id).toBe('audit-1');
  ...
  describe('DELETE /api/audits/:id', () => {
    it('should soft-delete audit successfully', async () => {
      (mockPrisma.envAudit.findUnique as jest.Mock).mockResolvedValueOnce({ id: 'audit-1' });
      (mockPrisma.envAudit.update as jest.Mock).mockResolvedValueOnce({ id: 'audit-1', deletedAt: new Date() });

      const response = await request(app)
        .delete('/api/audits/audit-1')
        .set('Authorization', 'Bearer token');
  ...
  describe('PUT /api/audits/:id/findings/:fid', () => {
    ...
    it('should update a finding successfully', async () => {
      ...
      const response = await request(app)
        .put('/api/audits/audit-1/findings/finding-1')
        .set('Authorization', 'Bearer token')
    ...
    it('should return 404 for non-existent finding', async () => {
      ...
      const response = await request(app)
        .put('/api/audits/audit-1/findings/00000000-0000-4000-a000-ffffffffffff')
        .set('Authorization', 'Bearer token')
  ...
  describe('GET /api/audits/:id/checklist', () => {
    it('should return clause checklist with statuses', async () => {
      ...
      const response = await request(app)
        .get('/api/audits/audit-1/checklist')
        .set('Authorization', 'Bearer token');

      expect(response.status).toBe(200);
      expect(response.body.success).toBe(true);
      expect(response.body.data.auditId).toBe('audit-1');
  ...
  describe('POST /api/audits/:id/complete', () => {
    ...
    it('should mark audit as complete with summary', async () => {
      ...
      const response = await request(app)
        .post('/api/audits/audit-1/complete')
        .set('Authorization', 'Bearer token')
