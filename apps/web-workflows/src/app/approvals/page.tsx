        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
          {chains.length === 0 ? (
            <div className="col-span-full rounded-lg bg-white p-12 text-center shadow">
              ...
              <h3 className="mt-4 text-lg font-medium text-gray-900">No approval chains</h3>
              <p className="mt-2 text-gray-500">Create an approval chain to get started.</p>
            </div>
          ) : (
            chains.map((chain) => (
              <div key={chain.id} className="rounded-lg bg-white p-6 shadow">
                <div className="flex items-start justify-between">
                  <div>
                    <h3 className="font-semibold text-gray-900">{chain.name}</h3>
                    <p className="text-sm text-gray-500">{chain.entityType}</p>
                  </div>
                  <span className={`inline-flex rounded-full px-2 py-1 text-xs font-medium ${
                    chain.isActive ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'
                  }`}>
                ...
                <div className="mt-4">
                  <p className="text-xs font-medium text-gray-500">APPROVAL LEVELS</p>
                  <div className="mt-2 space-y-2">
                    {chain.levels.map((level) => (
                      <div key={level.id} className="flex items-center space-x-2">
                        ...
                        <span className="text-xs text-gray-400">({level.approverType})</span>
                      </div>
                    ))}
                  </div>
                </div>
