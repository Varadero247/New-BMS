...
router.get('/low-stock', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const lowStock = items.filter((i: any) => i.quantityOnhand <= (i.minStockLevel || 0));
    res.json({ success: true, data: lowStock });
  } catch (error: any) {
    logger.error('Failed to fetch low stock', { error: error.message });
...
router.get('/expiring', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const data = await (prisma as any).chemInventory.findMany({
    ...
    logger.error('Failed to fetch expiring stock', { error: error.message });
...
router.get('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const where: any = { isActive: true, chemical: { orgId, isActive: true, deletedAt: null } };
    ...
    const [data, total] = await Promise.all([
      ...
      (prisma as any).chemInventory.count({ where }),
    ]);
...
router.get('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    const item = await (prisma as any).chemInventory.findFirst({
    ...
    logger.error('Failed to fetch inventory record', { error: error.message });
...
router.post('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const chemical = await (prisma as any).chemRegister.findFirst({ where: { id: parsed.data.chemicalId, deletedAt: null } });
    ...
    if (chemical.incompatibleWith && chemical.incompatibleWith.length > 0) {
      const conflicting = await (prisma as any).chemInventory.findMany({
        where: {
          isActive: true,
      ...
      if (conflicting.length > 0) {
        ...
        for (const c of conflicting) {
          await (prisma as any).chemIncompatAlert.create({
            data: {
    ...
    const data = await (prisma as any).chemInventory.create({ data: parsed.data });
...
router.put('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    if (!parsed.success) return res.status(400).json({ success: false, error: { code: 'VALIDATION_ERROR', message: parsed.error.errors[0].message } });
    const existing = await (prisma as any).chemInventory.findFirst({ where: { id: req.params.id, isActive: true } });
    if (!existing) return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'Inventory record not found' } });
    const data = await (prisma as any).chemInventory.update({ where: { id: req.params.id }, data: parsed.data });
    res.json({ success: true, data });
  } catch (error: any) {
...
router.post('/:id/inspect', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const data = await (prisma as any).chemInventory.update({
    ...
    logger.error('Failed to record inspection', { error: error.message });
    res.status(500).json({ success: false, error: { code: 'UPDATE_ERROR', message: error.message } });
  }
});
