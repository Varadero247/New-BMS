...
export default function MaterialityPage() {
  ...
  function openEdit(t: MaterialityTopic) {
    ...
    setForm({
      topic: t.topic,
      category: t.category,
      stakeholderImportance: t.stakeholderImportance,
  ...
  const avgStakeholder =
    topics.length > 0
      ? (topics.reduce((s, t) => s + (t.stakeholderImportance || 0), 0) / topics.length).toFixed(1)
      : '0';
  ...
  return (
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">
              Identify and prioritise material ESG topics by stakeholder importance and business
              impact
            </p>
          </div>
          <div className="flex gap-3">
            <div className="flex border rounded-lg overflow-hidden">
              <button
                ...
                className={`px-3 py-2 text-sm font-medium ${view === 'table' ? 'bg-green-600 text-white' : 'bg-white dark:bg-gray-900 text-gray-600 hover:bg-gray-50'}`}
              >
                Table
              </button>
              <button
                onClick={() => setView('matrix')}
                className={`px-3 py-2 text-sm font-medium ${view === 'matrix' ? 'bg-green-600 text-white' : 'bg-white dark:bg-gray-900 text-gray-600 hover:bg-gray-50'}`}
              >
                Matrix
              </button>
            </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          {[
            ...
            {
              label: 'Avg Stakeholder Score',
              value: `${avgStakeholder}/5`,
              color: 'text-blue-700',
              bg: 'bg-blue-50',
            },
          ].map((c) => (
            ...
            <CardContent>
              <div className="relative" style={{ height: '320px' }}>
                ...
                <div className="absolute left-0 bottom-0 top-0 w-8 flex items-center justify-center">
                  <span className="text-xs text-gray-500 dark:text-gray-400 -rotate-90 whitespace-nowrap">
                    Business Impact
                  </span>
                </div>
                ...
                <div className="absolute left-8 right-0 top-0 bottom-6 bg-gradient-to-tr from-green-50 via-yellow-50 to-red-50 rounded-lg border border-gray-200 dark:border-gray-700">
                  ...
                  <div className="absolute top-2 right-2 text-xs text-gray-400 dark:text-gray-500 text-right">
                    Manage Closely
                  </div>
                  <div className="absolute bottom-2 left-2 text-xs text-gray-400 dark:text-gray-500">
                    Low Priority
                  </div>
                  <div className="absolute bottom-2 right-2 text-xs text-gray-400 dark:text-gray-500 text-right">
                    Engage & Report
                  </div>
                  {/* Topics plotted */}
                  {topics.map((t) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`w-3 h-3 rounded-full ${dotColor} cursor-pointer ring-2 ring-white`}
                        />
              ...
              <div className="flex items-center gap-4 mt-3 justify-center">
                {[
                  ...
                  <div key={l.label} className="flex items-center gap-1.5">
                    ...
                    <span className="text-xs text-gray-600">{l.label} Priority</span>
                  </div>
                ))}
              </div>
            </CardContent>
            ...
            {filtered.length > 0 ? (
                  ...
                  <tbody>
                    {filtered.map((t) => (
                      <tr key={t.id} className="border-b hover:bg-gray-50 dark:bg-gray-800">
                        ...
                        <td className="py-3 px-4">
                          <span
                            className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${categoryColors[t.category] || 'bg-gray-100 dark:bg-gray-800 text-gray-700'}`}
                          >
                        ...
                        <td className="py-3 px-4 text-center">
                          <div className="flex items-center justify-center gap-1">
                            {[1, 2, 3, 4, 5].map((n) => (
                              <div
                                key={n}
                                className={`w-2 h-2 rounded-full ${n <= t.stakeholderImportance ? 'bg-blue-500' : 'bg-gray-200'}`}
                              />
                            ))}
                            <span className="ml-1 text-xs font-medium">
                              {t.stakeholderImportance}/5
                            </span>
                          </div>
                        </td>
                        <td className="py-3 px-4 text-center">
                          <div className="flex items-center justify-center gap-1">
                            {[1, 2, 3, 4, 5].map((n) => (
                              <div
                                key={n}
                                className={`w-2 h-2 rounded-full ${n <= t.businessImpact ? 'bg-green-500' : 'bg-gray-200'}`}
                              />
                            ))}
                        ...
                        <td className="py-3 px-4">
                          <span
                            className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${priorityColors[t.priority] || 'bg-gray-100 dark:bg-gray-800 text-gray-700'}`}
                          >
                            {t.priority}
                          </span>
                        </td>
                        <td className="py-3 px-4">
                          <span
                            className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${t.status === 'ACTIVE' ? 'bg-green-100 text-green-700' : 'bg-gray-100 dark:bg-gray-800 text-gray-700'}`}
                          >
                            {t.status}
                          </span>
                        </td>
              ...
              <div className="text-center py-12 text-gray-500 dark:text-gray-400">
                ...
                <p className="text-sm mt-1">
                  Click "Add Topic" to begin your materiality assessment
                </p>
              </div>
            )}
          </CardContent>
      ...
      <Modal
        ...
        title={editing ? 'Edit Topic' : 'Add Materiality Topic'}
        size="lg"
      >
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                placeholder="e.g. Climate Change, Data Privacy"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                Stakeholder Importance (1-5)
              </label>
              <input
                type="number"
                min="1"
                max="5"
                value={form.stakeholderImportance}
                onChange={(e) =>
                  setForm((f) => ({ ...f, stakeholderImportance: parseInt(e.target.value) || 1 }))
                }
                className="w-full border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setForm((f) => ({ ...f, businessImpact: parseInt(e.target.value) || 1 }))
                }
                className="w-full border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
              />
            </div>
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className="w-full border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
              placeholder="Describe the topic and its relevance..."
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
      ...
      <Modal isOpen={!!deleteId} onClose={() => setDeleteId(null)} title="Delete Topic" size="sm">
        <p className="text-sm text-gray-600">
          Are you sure you want to delete this materiality topic?
        </p>
        <div className="flex justify-end gap-3 mt-6">
          <button
