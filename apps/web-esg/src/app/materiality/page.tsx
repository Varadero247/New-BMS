      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-500 mt-1">Identify and prioritise material ESG topics by stakeholder importance and business impact</p>
          </div>
          <div className="flex gap-3">
            <div className="flex border rounded-lg overflow-hidden">
              <button onClick={() => setView('table')} className={`px-3 py-2 text-sm font-medium ${view === 'table' ? 'bg-green-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-50'}`}>Table</button>
              <button onClick={() => setView('matrix')} className={`px-3 py-2 text-sm font-medium ${view === 'matrix' ? 'bg-green-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-50'}`}>Matrix</button>
            </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          {[
            { label: 'Total Topics', value: topics.length, color: 'text-gray-800', bg: 'bg-gray-50' },
            ...
            <CardContent>
              <div className="relative" style={{ height: '320px' }}>
                ...
                <div className="absolute left-0 bottom-0 top-0 w-8 flex items-center justify-center">
                  <span className="text-xs text-gray-500 -rotate-90 whitespace-nowrap">Business Impact</span>
                </div>
                <div className="absolute bottom-0 left-8 right-0 h-6 flex items-center justify-center">
                  <span className="text-xs text-gray-500">Stakeholder Importance</span>
                </div>
                {/* Grid area */}
                <div className="absolute left-8 right-0 top-0 bottom-6 bg-gradient-to-tr from-green-50 via-yellow-50 to-red-50 rounded-lg border border-gray-200">
                  {/* Quadrant labels */}
                  <div className="absolute top-2 left-2 text-xs text-gray-400">Monitor</div>
                  <div className="absolute top-2 right-2 text-xs text-gray-400 text-right">Manage Closely</div>
                  <div className="absolute bottom-2 left-2 text-xs text-gray-400">Low Priority</div>
                  <div className="absolute bottom-2 right-2 text-xs text-gray-400 text-right">Engage & Report</div>
                  {/* Topics plotted */}
                  {topics.map(t => {
        ...
        <Card className="mb-6"><CardContent className="pt-4 pb-4">
          <div className="flex flex-wrap gap-3 items-center">
            <div className="flex-1 min-w-[200px] relative">
              ...
              <input type="text" placeholder="Search topics..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-green-500" />
            </div>
            <select value={priorityFilter} onChange={e => setPriorityFilter(e.target.value)} className="border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead><tr className="border-b">
                    ...
                    <th className="text-center py-3 px-4 font-medium text-gray-500">Stakeholder</th>
                  ...
                  <tbody>
                    {filtered.map(t => (
                      <tr key={t.id} className="border-b hover:bg-gray-50">
                        ...
                        <td className="py-3 px-4"><span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${categoryColors[t.category] || 'bg-gray-100 text-gray-700'}`}>{t.category?.replace(/_/g, ' ')}</span></td>
                        ...
                        <td className="py-3 px-4"><span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${priorityColors[t.priority] || 'bg-gray-100 text-gray-700'}`}>{t.priority}</span></td>
                        <td className="py-3 px-4"><span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${t.status === 'ACTIVE' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}`}>{t.status}</span></td>
                        <td className="py-3 px-4 text-right"><div className="flex justify-end gap-2">
                          <button onClick={() => openEdit(t)} className="text-gray-400 hover:text-green-600"><Pencil className="h-4 w-4" /></button>
                          <button onClick={() => setDeleteId(t.id)} className="text-gray-400 hover:text-red-600"><Trash2 className="h-4 w-4" /></button>
                        </div></td>
                      </tr>
                    ))}
                  </tbody>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Topic' : 'Add Materiality Topic'} size="lg">
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Topic Name *</label>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Stakeholder Importance (1-5)</label>
            ...
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Business Impact (1-5)</label>
