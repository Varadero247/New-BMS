...
import { Plus, Search, Cloud, Pencil, Trash2 } from 'lucide-react';
...
export default function EmissionsPage() {
  ...
  const scope1 = emissions
    ...
    .reduce((s, e) => s + (e.co2Equivalent || 0), 0);
  ...
  return (
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">
              Track Scope 1, 2 & 3 greenhouse gas emissions
            </p>
          </div>
          <button
            onClick={openCreate}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          {[
            {
              label: 'Total CO2e',
              value: `${totalCO2.toLocaleString(undefined, { maximumFractionDigits: 1 })} t`,
              color: 'text-gray-800',
              bg: 'bg-gray-50 dark:bg-gray-800',
            },
            {
              label: 'Scope 1 (Direct)',
              value: `${scope1.toLocaleString(undefined, { maximumFractionDigits: 1 })} t`,
              color: 'text-emerald-700',
              bg: 'bg-emerald-50',
            },
            {
              label: 'Scope 2 (Indirect)',
              value: `${scope2.toLocaleString(undefined, { maximumFractionDigits: 1 })} t`,
              color: 'text-amber-700',
              bg: 'bg-amber-50',
            },
            {
              label: 'Scope 3 (Value Chain)',
              value: `${scope3.toLocaleString(undefined, { maximumFractionDigits: 1 })} t`,
                    ...
                    {filtered.map((em) => (
                      <tr key={em.id} className="border-b hover:bg-gray-50 dark:bg-gray-800">
                        ...
                        <td className="py-3 px-4">
                          <span
                            className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${scopeColors[em.scope] || 'bg-gray-100 dark:bg-gray-800 text-gray-700'}`}
                          >
                            {em.scope?.replace('_', ' ')}
                          </span>
                        </td>
                        ...
                        <td className="py-3 px-4 text-right font-medium">
                          {em.co2Equivalent?.toFixed(2)}
                        </td>
                        <td className="py-3 px-4 text-gray-600">{em.period}</td>
                        <td className="py-3 px-4">
                          <span
                            className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${statusColors[em.status] || 'bg-gray-100 dark:bg-gray-800 text-gray-700'}`}
                          >
                            {em.status}
                          </span>
                        </td>
      ...
      <Modal
        ...
        title={editing ? 'Edit Emission Record' : 'Log Emission'}
        size="lg"
      >
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                placeholder="e.g. Fleet Vehicles"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input
                type="number"
                value={form.co2Equivalent}
                onChange={(e) =>
                  setForm((f) => ({ ...f, co2Equivalent: parseFloat(e.target.value) || 0 }))
                }
                className="w-full border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
              />
            </div>
