    ...
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          {[
            { label: 'Total CO2e', value: `${totalCO2.toLocaleString(undefined, { maximumFractionDigits: 1 })} t`, color: 'text-gray-800', bg: 'bg-gray-50' },
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b">
                      ...
                      <th className="text-right py-3 px-4 font-medium text-gray-500">CO2e (t)</th>
                  ...
                  <tbody>
                    {filtered.map(em => (
                      <tr key={em.id} className="border-b hover:bg-gray-50">
                        <td className="py-3 px-4 font-medium text-gray-900">{em.source}</td>
                        <td className="py-3 px-4">
                          <span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${scopeColors[em.scope] || 'bg-gray-100 text-gray-700'}`}>
                            {em.scope?.replace('_', ' ')}
                          </span>
                        </td>
                        ...
                        <td className="py-3 px-4">
                          <span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${statusColors[em.status] || 'bg-gray-100 text-gray-700'}`}>
                            {em.status}
                          </span>
                        </td>
                        <td className="py-3 px-4 text-right">
                          <div className="flex justify-end gap-2">
                            <button onClick={() => openEdit(em)} className="text-gray-400 hover:text-green-600 transition-colors"><Pencil className="h-4 w-4" /></button>
                            <button onClick={() => setDeleteId(em.id)} className="text-gray-400 hover:text-red-600 transition-colors"><Trash2 className="h-4 w-4" /></button>
                          </div>
                        </td>
                      </tr>
                    ))}
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Emission Record' : 'Log Emission'} size="lg">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">CO2e (tonnes)</label>
