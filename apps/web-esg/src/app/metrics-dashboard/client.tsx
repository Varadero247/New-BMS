    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      <div>
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
          Key performance indicators across Environmental, Social & Governance dimensions
        </p>
      </div>
            ...
            {filtered.map((m) => {
              ...
              const displayProgress = isInverse
                ? m.target === 0
                  ...
                  : Math.min(
                      100,
                      Math.round(
                        ((m.previousPeriod - m.value) / (m.previousPeriod - m.target)) * 100
                      )
                    )
                : progress;
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`px-2 py-0.5 rounded-full text-xs font-medium ${m.category === 'environmental' ? 'bg-green-100 text-green-700' : m.category === 'social' ? 'bg-blue-100 text-blue-700' : 'bg-purple-100 text-purple-700'}`}
                    >
                  ...
                  <td className="px-4 py-3">
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div
                        className={`h-2 rounded-full ${adjustedProgress >= 80 ? 'bg-green-500' : adjustedProgress >= 50 ? 'bg-amber-500' : 'bg-red-500'}`}
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`px-2 py-0.5 rounded-full text-xs font-medium ${m.status === 'on-track' ? 'bg-green-100 text-green-700' : m.status === 'at-risk' ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700'}`}
                    >
                      {m.status
                        .split('-')
                        .map((w) => w[0].toUpperCase() + w.slice(1))
