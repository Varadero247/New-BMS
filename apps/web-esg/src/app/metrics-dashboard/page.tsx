    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Key performance indicators across Environmental, Social and Governance dimensions</p>
        </div>
      ...
      <div className="flex gap-2 items-center">
        ...
        {[{ v: '', l: 'All Statuses' }, { v: 'ON_TRACK', l: 'On Track' }, { v: 'AT_RISK', l: 'At Risk' }, { v: 'OFF_TRACK', l: 'Off Track' }].map(s => (
          <button key={s.v} onClick={() => setStatusFilter(s.v)} className={`px-3 py-1.5 rounded-lg text-xs font-medium ${statusFilter === s.v ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>{s.l}</button>
        ))}
      </div>
                ...
                return (
                  <tr key={m.id} className="border-t border-gray-100 hover:bg-gray-50">
                    <td className="px-4 py-3">
                      ...
                      {m.disclosure && <p className="text-xs text-gray-400">{m.disclosure}</p>}
                    </td>
                    <td className="px-4 py-3">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${categoryBadge[m.category] || 'bg-gray-100 text-gray-700'}`}>
                        {m.category?.[0] || '?'}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${statusColors[m.status] || 'bg-gray-100 text-gray-700'}`}>
                        {m.status?.replace(/_/g, ' ')}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-xs text-gray-500">{m.framework}</td>
                    <td className="px-4 py-3 text-right"><div className="flex justify-end gap-2">
                      <button onClick={() => openEdit(m)} className="text-gray-400 hover:text-green-600 transition-colors"><Pencil className="h-3.5 w-3.5" /></button>
                      <button onClick={() => setDeleteId(m.id)} className="text-gray-400 hover:text-red-600 transition-colors"><Trash2 className="h-3.5 w-3.5" /></button>
                    </div></td>
                  </tr>
                );
              })}
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Metric' : 'Add ESG Metric'} size="lg">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Current Value</label>
            ...
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Previous Period</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Disclosure Reference</label>
