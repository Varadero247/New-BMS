...
export default function WastePage() {
  ...
  function openEdit(r: WasteRecord) {
    ...
    setForm({
      ...
      recycledPercentage: r.recycledPercentage,
      facility: r.facility,
  ...
  const recycled = records.filter(
    (r) => r.category === 'RECYCLABLE' || r.recycledPercentage > 50
  ).length;
  const hazardous = records.filter((r) => r.category === 'HAZARDOUS').length;
  const avgRecycled =
    records.length > 0
      ? Math.round(records.reduce((s, r) => s + (r.recycledPercentage || 0), 0) / records.length)
      : 0;
  ...
  return (
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">
              Track waste generation, disposal, and recycling rates
            </p>
          </div>
          <button
            onClick={openCreate}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          {[
            ...
            {
              label: 'Recyclable Records',
              value: recycled,
              color: 'text-green-700',
              bg: 'bg-green-50',
            },
            {
              label: 'Hazardous Records',
              value: hazardous,
              color: 'text-red-700',
              bg: 'bg-red-50',
            },
                    ...
                    {filtered.map((r) => (
                      <tr key={r.id} className="border-b hover:bg-gray-50 dark:bg-gray-800">
                        ...
                        <td className="py-3 px-4">
                          <span
                            className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${categoryColors[r.category] || 'bg-gray-100 dark:bg-gray-800 text-gray-700'}`}
                          >
                          ...
                          <div className="flex items-center justify-end gap-2">
                            <div className="w-16 bg-gray-200 rounded-full h-1.5">
                              <div
                                ...
                                style={{ width: `${r.recycledPercentage || 0}%` }}
                              />
                            </div>
      ...
      <Modal
        ...
        title={editing ? 'Edit Waste Record' : 'Log Waste Record'}
        size="lg"
      >
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                placeholder="e.g. Cardboard, Electronic waste"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setForm((f) => ({ ...f, recycledPercentage: parseFloat(e.target.value) || 0 }))
                }
                className="w-full border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
              />
            </div>
