...
export default function EnergyPage() {
  ...
  const filtered = records.filter((r) => {
    ...
    const matchesRenewable =
      renewableFilter === '' || (renewableFilter === 'true' ? r.renewable : !r.renewable);
    return matchesSearch && matchesRenewable;
  });

  const totalConsumption = records.reduce((s, r) => s + (r.consumption || 0), 0);
  const renewableConsumption = records
    .filter((r) => r.renewable)
    .reduce((s, r) => s + (r.consumption || 0), 0);
  const renewablePct =
    totalConsumption > 0 ? Math.round((renewableConsumption / totalConsumption) * 100) : 0;
  ...
  return (
    <div className="p-8">
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">
              Track energy usage, renewables, and costs
            </p>
          </div>
          <button
            onClick={openCreate}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          {[
            ...
            },
            {
              ...
              value: `${renewableConsumption.toLocaleString()} kWh`,
          ...
          <CardContent className="pt-4 pb-4">
            <div className="flex flex-wrap gap-3 items-center">
              <div className="flex-1 min-w-[200px] relative">
                ...
                <input
                  type="text"
                  aria-label="Search energy records..."
              ...
              <select
                ...
                onChange={(e) => setRenewableFilter(e.target.value)}
                    ...
                    {filtered.map((r) => (
                      <tr key={r.id} className="border-b hover:bg-gray-50 dark:bg-gray-800">
                        ...
                        <td className="py-3 px-4">
                          <span
                            className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${typeColors[r.type] || 'bg-gray-100 dark:bg-gray-800 text-gray-700'}`}
                          >
                            {r.type?.replace(/_/g, ' ')}
                          </span>
                        </td>
                        ...
                        <td className="py-3 px-4">
                          {r.renewable ? (
                            ...
                            <span className="inline-flex px-2 py-0.5 rounded text-xs font-medium bg-gray-100 dark:bg-gray-800 text-gray-600">
                              No
                            </span>
                          )}
                        </td>
                        <td className="py-3 px-4">
                          <span
                            className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${r.status === 'VERIFIED' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}
                          >
                            {r.status}
                          </span>
                        </td>
      ...
      <Modal
        ...
        title={editing ? 'Edit Energy Record' : 'Log Energy Consumption'}
        size="lg"
      >
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                placeholder="e.g. National Grid, Rooftop Solar"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setForm((f) => ({ ...f, consumption: parseFloat(e.target.value) || 0 }))
                }
                className="w-full border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
              />
            </div>
