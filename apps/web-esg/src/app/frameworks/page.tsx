      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-500 mt-1">Manage reporting frameworks, standards coverage, and compliance assessments</p>
          </div>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          {[
            { label: 'Frameworks Tracked', value: frameworks.length, color: 'text-gray-800', bg: 'bg-gray-50' },
        ...
        <Card className="mb-6"><CardContent className="pt-4 pb-4">
          <div className="relative">
            ...
            <input type="text" placeholder="Search frameworks..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-green-500" />
          </div>
        </CardContent></Card>
                    ...
                    {filtered.map(fw => (
                      <tr key={fw.id} className="border-b hover:bg-gray-50">
                        <td className="py-3 px-4">
                          ...
                          <p className="text-xs text-gray-400 truncate max-w-xs">{fw.description}</p>
                        </td>
                        ...
                        <td className="py-3 px-4 text-gray-600">
                          ...
                          <span className="text-gray-400"> / {fw.requirements || 0}</span>
                        </td>
                        <td className="py-3 px-4 text-gray-600">{fw.lastAssessed ? new Date(fw.lastAssessed).toLocaleDateString() : '-'}</td>
                        <td className="py-3 px-4"><span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${fw.status === 'ACTIVE' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}`}>{fw.status}</span></td>
                        <td className="py-3 px-4 text-right">
                          <button onClick={() => openEdit(fw)} className="text-gray-400 hover:text-green-600"><Pencil className="h-4 w-4" /></button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title="Update Framework Assessment" size="lg">
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Framework Name</label>
            ...
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Version</label>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Coverage %</label>
            ...
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Total Requirements</label>
            ...
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Met Requirements</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Last Assessed</label>
