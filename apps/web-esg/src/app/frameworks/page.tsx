...
export default function FrameworksPage() {
  ...
  function openEdit(f: Framework) {
    ...
    setForm({
      ...
      status: f.status,
      coverage: f.coverage,
      lastAssessed: f.lastAssessed?.split('T')[0] || '',
      requirements: f.requirements,
      metRequirements: f.metRequirements,
      website: f.website || '',
    });
    setModalOpen(true);
  }
  ...
  const filtered = frameworks.filter(
    (f) => !searchTerm || JSON.stringify(f).toLowerCase().includes(searchTerm.toLowerCase())
  );
  const active = frameworks.filter((f) => f.status === 'ACTIVE').length;
  const avgCoverage =
    frameworks.length > 0
      ? Math.round(frameworks.reduce((s, f) => s + (f.coverage || 0), 0) / frameworks.length)
      : 0;
  ...
  return (
    <div className="p-8">
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">
              Manage reporting frameworks, standards coverage, and compliance assessments
            </p>
          </div>
        </div>
              ...
              {frameworkData.map((fw) => (
                <div key={fw.name} className={`border rounded-xl p-4 ${fw.color}`}>
                  <div className="flex items-center justify-between mb-2">
                    <span className={`px-2 py-0.5 rounded text-xs font-medium ${fw.badge}`}>
                      {fw.name}
                    </span>
            ...
            {filtered.length > 0 ? (
                  ...
                  <tbody>
                    {filtered.map((fw) => (
                      <tr key={fw.id} className="border-b hover:bg-gray-50 dark:bg-gray-800">
                        ...
                        <td className="py-3 px-4 text-gray-600">
                          {fw.lastAssessed ? new Date(fw.lastAssessed).toLocaleDateString() : '-'}
                        </td>
                        <td className="py-3 px-4">
                          <span
                            className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${fw.status === 'ACTIVE' ? 'bg-green-100 text-green-700' : 'bg-gray-100 dark:bg-gray-800 text-gray-700'}`}
                          >
                            {fw.status}
                          </span>
                        </td>
              ...
              <div className="text-center py-12 text-gray-500 dark:text-gray-400">
                ...
                <p className="text-sm mt-1">
                  Contact your ESG team to configure reporting frameworks
                </p>
              </div>
            )}
          </CardContent>
      ...
      <Modal
        ...
        title="Update Framework Assessment"
        size="lg"
      >
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <input
                ...
                value={form.coverage}
                onChange={(e) =>
                  setForm((f) => ({ ...f, coverage: parseFloat(e.target.value) || 0 }))
                }
                className="w-full border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setForm((f) => ({ ...f, requirements: parseInt(e.target.value) || 0 }))
                }
                className="w-full border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
              />
            </div>
            <div>
              ...
              <input
                type="number"
                value={form.metRequirements}
                onChange={(e) =>
                  setForm((f) => ({ ...f, metRequirements: parseInt(e.target.value) || 0 }))
                }
                className="w-full border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
              />
            </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                type="date"
                value={form.lastAssessed}
                onChange={(e) => setForm((f) => ({ ...f, lastAssessed: e.target.value }))}
                className="w-full border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
              />
            </div>
            <div>
