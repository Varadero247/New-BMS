...
export default function WaterPage() {
  ...
  function openEdit(r: WaterRecord) {
    setEditing(r);
    setForm({ source: r.source, usage: r.usage, unit: r.unit, facility: r.facility, period: r.period, recycled: r.recycled, discharged: r.discharged, status: r.status });
    setModalOpen(true);
  }
  ...
  return (
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">Track water consumption, recycling, and discharge</p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          {[
            { label: 'Total Usage', value: `${totalUsage.toLocaleString()} m³`, color: 'text-blue-800', bg: 'bg-blue-50' },
            { label: 'Total Recycled', value: `${totalRecycled.toLocaleString()} m³`, color: 'text-green-700', bg: 'bg-green-50' },
            { label: 'Total Discharged', value: `${totalDischarged.toLocaleString()} m³`, color: 'text-amber-700', bg: 'bg-amber-50' },
            { label: 'Recycled Rate', value: `${recycledPct}%`, color: 'text-emerald-700', bg: 'bg-emerald-50' },
          ].map(c => (
        ...
        <Card className="mb-6"><CardContent className="pt-4 pb-4">
          <div className="flex flex-wrap gap-3 items-center">
            <div className="flex-1 min-w-[200px] relative">
              ...
              <input type="text" aria-label="Search water records..." placeholder="Search water records..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-green-500" />
            </div>
        ...
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2"><Droplets className="h-5 w-5 text-green-600" />Water Records ({filtered.length})</CardTitle></CardHeader>
          <CardContent>
            {filtered.length > 0 ? (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead><tr className="border-b">
                    ...
                    <th className="text-right py-3 px-4 font-medium text-gray-500 dark:text-gray-400">Usage</th>
                    ...
                    <th className="text-right py-3 px-4 font-medium text-gray-500 dark:text-gray-400">Recycled</th>
                    <th className="text-right py-3 px-4 font-medium text-gray-500 dark:text-gray-400">Discharged</th>
                  ...
                  <tbody>
                    {filtered.map(r => (
                      <tr key={r.id} className="border-b hover:bg-gray-50 dark:bg-gray-800">
                        ...
                        <td className="py-3 px-4 text-right">{r.usage} {r.unit}</td>
                        <td className="py-3 px-4 text-gray-600">{r.facility}</td>
                        <td className="py-3 px-4 text-gray-600">{r.period}</td>
                        <td className="py-3 px-4 text-right text-green-700 font-medium">{r.recycled} {r.unit}</td>
                        <td className="py-3 px-4 text-right text-amber-700">{r.discharged} {r.unit}</td>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Water Record' : 'Log Water Usage'} size="lg">
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Source *</label>
              <input value={form.source} onChange={e => setForm(f => ({ ...f, source: e.target.value }))} className="w-full border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500" placeholder="e.g. Mains supply, Borehole" /></div>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Usage</label>
              <input type="number" value={form.usage} onChange={e => setForm(f => ({ ...f, usage: parseFloat(e.target.value) || 0 }))} className="w-full border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500" /></div>
            <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Recycled</label>
              <input type="number" value={form.recycled} onChange={e => setForm(f => ({ ...f, recycled: parseFloat(e.target.value) || 0 }))} className="w-full border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500" /></div>
            <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Discharged</label>
              <input type="number" value={form.discharged} onChange={e => setForm(f => ({ ...f, discharged: parseFloat(e.target.value) || 0 }))} className="w-full border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500" /></div>
          </div>
          <div className="grid grid-cols-3 gap-4">
      ...
      <Modal isOpen={!!deleteId} onClose={() => setDeleteId(null)} title="Delete Water Record" size="sm">
