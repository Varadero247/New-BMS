...
export default function SocialPage() {
  ...
  function openEdit(m: SocialMetric) {
    setEditing(m);
    setForm({ category: m.category, metric: m.metric, value: m.value, unit: m.unit, period: m.period, target: m.target, description: m.description, status: m.status });
    setModalOpen(true);
  }
  ...
  return (
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <Card className="mb-6"><CardContent className="pt-4 pb-4">
          <div className="flex flex-wrap gap-3 items-center">
            <div className="flex-1 min-w-[200px] relative">
              ...
              <input type="text" aria-label="Search social metrics..." placeholder="Search social metrics..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-green-500" />
            </div>
        ...
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2"><Users className="h-5 w-5 text-green-600" />Social Metrics ({filtered.length})</CardTitle></CardHeader>
          <CardContent>
            {filtered.length > 0 ? (
