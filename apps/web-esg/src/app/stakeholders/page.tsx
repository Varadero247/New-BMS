...
export default function StakeholdersPage() {
  ...
  function openEdit(s: Stakeholder) {
    ...
    setForm({
      ...
      interest: s.interest,
      engagementMethod: s.engagementMethod,
      contactPerson: s.contactPerson,
      email: s.email || '',
      phone: s.phone || '',
      engagementFrequency: s.engagementFrequency || 'QUARTERLY',
      notes: s.notes || '',
      status: s.status,
    });
    setModalOpen(true);
  ...
  const filtered = stakeholders.filter((s) => {
    const matchesSearch =
      !searchTerm || JSON.stringify(s).toLowerCase().includes(searchTerm.toLowerCase());
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">
              Manage stakeholder engagement, influence mapping, and communication strategies
            </p>
          </div>
          <button
            onClick={openCreate}
          ...
          <div className="grid grid-cols-4 gap-3 mb-6">
            {[
              ...
              },
              {
                ...
                influence: 'LOW',
                interest: 'LOW',
                color: 'border-gray-200 dark:border-gray-700 bg-gray-50',
                badge: 'bg-gray-100 dark:bg-gray-800 text-gray-600',
              },
            ].map((quadrant) => {
              const count = stakeholders.filter(
                (s) => s.influence === quadrant.influence && s.interest === quadrant.interest
              ).length;
              return (
                <div key={quadrant.label} className={`border rounded-xl p-3 ${quadrant.color}`}>
                  <span
                    className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${quadrant.badge}`}
                  >
                    {count} stakeholders
                  </span>
            ...
            {filtered.length > 0 ? (
                  ...
                  <tbody>
                    {filtered.map((s) => (
                      <tr key={s.id} className="border-b hover:bg-gray-50 dark:bg-gray-800">
                        ...
                        <td className="py-3 px-4">
                          <span
                            className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${categoryColors[s.category] || 'bg-gray-100 dark:bg-gray-800 text-gray-700'}`}
                          >
                            {s.category}
                          </span>
                        </td>
                        <td className="py-3 px-4">
                          <span
                            className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${influenceColors[s.influence] || 'bg-gray-100 dark:bg-gray-800 text-gray-700'}`}
                          >
                            {s.influence}
                          </span>
                        </td>
                        <td className="py-3 px-4">
                          <span
                            className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${interestColors[s.interest] || 'bg-gray-100 dark:bg-gray-800 text-gray-700'}`}
                          >
                            {s.interest}
                          </span>
                        </td>
                        ...
                        <td className="py-3 px-4">
                          <span
                            className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${s.status === 'ACTIVE' ? 'bg-green-100 text-green-700' : 'bg-gray-100 dark:bg-gray-800 text-gray-600'}`}
                          >
                            {s.status}
                          </span>
                        </td>
              ...
              <div className="text-center py-12 text-gray-500 dark:text-gray-400">
                ...
                <p className="text-sm mt-1">
                  Click "Add Stakeholder" to map your first stakeholder
                </p>
              </div>
            )}
          </CardContent>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                value={form.engagementMethod}
                onChange={(e) => setForm((f) => ({ ...f, engagementMethod: e.target.value }))}
                className="w-full border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
                placeholder="e.g. Quarterly meetings, Surveys"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
              ...
              <select
                ...
                onChange={(e) => setForm((f) => ({ ...f, engagementFrequency: e.target.value }))}
                className="w-full border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
              >
          ...
          <div>
            ...
            <textarea
              ...
              rows={2}
              className="w-full border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
              placeholder="Key interests, concerns, or engagement notes..."
            />
          </div>
        </div>
