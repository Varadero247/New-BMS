      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-500 mt-1">Manage stakeholder engagement, influence mapping, and communication strategies</p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          {[
            { label: 'Total Stakeholders', value: stakeholders.length, color: 'text-gray-800', bg: 'bg-gray-50' },
        ...
        {stakeholders.length > 0 && (
          <div className="grid grid-cols-4 gap-3 mb-6">
            {[
              ...
              { label: 'Monitor', sub: 'Low influence, Low interest', influence: 'LOW', interest: 'LOW', color: 'border-gray-200 bg-gray-50', badge: 'bg-gray-100 text-gray-600' },
            ].map(quadrant => {
              ...
              return (
                <div key={quadrant.label} className={`border rounded-xl p-3 ${quadrant.color}`}>
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">{quadrant.sub}</p>
                </div>
              );
            })}
          </div>
                  ...
                  <tbody>
                    {filtered.map(s => (
                      <tr key={s.id} className="border-b hover:bg-gray-50">
                        ...
                        <td className="py-3 px-4"><span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${categoryColors[s.category] || 'bg-gray-100 text-gray-700'}`}>{s.category}</span></td>
                        <td className="py-3 px-4"><span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${influenceColors[s.influence] || 'bg-gray-100 text-gray-700'}`}>{s.influence}</span></td>
                        <td className="py-3 px-4"><span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${interestColors[s.interest] || 'bg-gray-100 text-gray-700'}`}>{s.interest}</span></td>
                        <td className="py-3 px-4 text-gray-600">
                          <p>{s.engagementMethod}</p>
                          {s.engagementFrequency && <p className="text-xs text-gray-400">{s.engagementFrequency}</p>}
                        </td>
                        <td className="py-3 px-4 text-gray-600">
                          <p className="font-medium">{s.contactPerson}</p>
                          {s.email && <p className="text-xs text-gray-400 truncate max-w-[160px]">{s.email}</p>}
                        </td>
                        <td className="py-3 px-4"><span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${s.status === 'ACTIVE' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}>{s.status}</span></td>
                        <td className="py-3 px-4 text-right"><div className="flex justify-end gap-2">
                          <button onClick={() => openEdit(s)} className="text-gray-400 hover:text-green-600 transition-colors"><Pencil className="h-4 w-4" /></button>
                          <button onClick={() => setDeleteId(s.id)} className="text-gray-400 hover:text-red-600 transition-colors"><Trash2 className="h-4 w-4" /></button>
                        </div></td>
                      </tr>
                    ))}
                  </tbody>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Stakeholder' : 'Add Stakeholder'} size="lg">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Influence</label>
            ...
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Interest</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Engagement Method</label>
            ...
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Engagement Frequency</label>
