...
export default function GovernancePage() {
  ...
  function openEdit(m: GovernanceMetric) {
    ...
    setForm({
      ...
      lastReview: m.lastReview?.split('T')[0] || '',
      nextReview: m.nextReview?.split('T')[0] || '',
  ...
  return (
    <div className="p-8">
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">
              Track governance metrics, board composition, and compliance indicators
            </p>
          </div>
          <button
            onClick={openCreate}
          ...
          <CardContent className="pt-4 pb-4">
            <div className="flex flex-wrap gap-3 items-center">
              <div className="flex-1 min-w-[200px] relative">
                ...
                <input
                  type="text"
                  aria-label="Search governance indicators..."
                  placeholder="Search governance indicators..."
                  value={searchTerm}
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              Governance Indicators ({filtered.length})
            </CardTitle>
          </CardHeader>
          <CardContent>
            {filtered.length > 0 ? (
                    ...
                    {filtered.map((m) => {
                      ...
                      return (
                        <tr key={m.id} className="border-b hover:bg-gray-50 dark:bg-gray-800">
                          ...
                          <td className="py-3 px-4">
                            <span
                              className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium ${m.compliance === 'COMPLIANT' ? 'bg-green-100 text-green-700' : m.compliance === 'PARTIAL' ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700'}`}
                            >
                          ...
                          <td className="py-3 px-4">
                            <span
                              className={`inline-flex items-center gap-1 text-xs ${isOverdue ? 'text-red-600 font-medium' : 'text-gray-600'}`}
                            >
                          ...
                          <td className="py-3 px-4">
                            <span
                              className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${m.status === 'ACTIVE' ? 'bg-green-100 text-green-700' : 'bg-gray-100 dark:bg-gray-800 text-gray-700'}`}
                            >
                              {m.status}
                            </span>
                          </td>
              ...
              <div className="text-center py-12 text-gray-500 dark:text-gray-400">
                ...
                <p className="text-sm mt-1">
                  Click "Add Indicator" to start tracking governance metrics
                </p>
              </div>
            )}
          </CardContent>
      ...
      <Modal
        ...
        title={editing ? 'Edit Governance Indicator' : 'Add Governance Indicator'}
        size="lg"
      >
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
                placeholder="e.g. Board Gender Diversity, Whistleblowing Policy"
              />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
                placeholder="e.g. 40%, Yes, 5 members"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
              ...
              <select
                ...
                onChange={(e) => setForm((f) => ({ ...f, compliance: e.target.value }))}
                className="w-full border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
              >
                <option value="COMPLIANT">Compliant</option>
                <option value="PARTIAL">Partial</option>
        ...
        <div className="flex justify-end gap-3 mt-6">
          ...
          <button
            onClick={handleSave}
            disabled={saving || !form.indicator}
      ...
      <Modal
        ...
        title="Delete Governance Indicator"
        size="sm"
      >
        <p className="text-sm text-gray-600">
          Are you sure you want to delete this governance indicator?
        </p>
        <div className="flex justify-end gap-3 mt-6">
          <button
