    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Track sustainability improvement projects across Environmental, Social and Governance pillars</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          ...
          {totalBudget > 0 && <p className="text-xs text-gray-400">{Math.round((totalSpent / totalBudget) * 100)}% utilised</p>}
        </div>
      </div>
      ...
      {initiatives.length > 0 && (
        <div className="grid grid-cols-3 gap-4">
          {(['ENVIRONMENTAL', 'SOCIAL', 'GOVERNANCE']).map(p => {
            ...
            const cfg = pillarConfig[p] || { label: p, color: 'bg-gray-100 text-gray-700', prog: 'bg-gray-500' };
            return (
              <button key={p} onClick={() => setPillarFilter(pillarFilter === p ? '' : p)}
                ...
                <div className="mt-3">
                  <div className="flex justify-between text-xs text-gray-500 mb-1"><span>Progress</span><span>{pillarProgress}%</span></div>
      ...
      {filtered.length > 0 ? (
        <div className="space-y-3">
          {filtered.map(ini => {
            ...
            const cfg = pillarConfig[ini.pillar] || { label: ini.pillar, color: 'bg-gray-100 text-gray-700', prog: 'bg-gray-500' };
            const priCfg = priorityConfig[ini.priority] || { label: ini.priority, color: 'bg-gray-100 text-gray-600' };
            const stCfg = statusConfig[ini.status] || { label: ini.status, color: 'bg-gray-100 text-gray-700' };
            return (
              <div key={ini.id} className="bg-white border border-gray-200 rounded-xl overflow-hidden">
                <div className="flex items-center w-full p-4 hover:bg-gray-50 transition-colors">
                  ...
                  <div className="flex-1 min-w-0 cursor-pointer" onClick={() => toggleExpand(ini.id)}>
                    ...
                    <p className="text-xs text-gray-500 mt-0.5 truncate">{ini.owner}{ini.targetDate ? ` · Due ${new Date(ini.targetDate).toLocaleDateString()}` : ''}</p>
                  </div>
                  <div className="flex items-center gap-4 ml-4">
                    <div className="w-24 hidden sm:block">
                      <div className="flex justify-between text-xs text-gray-500 mb-1"><span>{prog}%</span></div>
                    ...
                    <div className="flex gap-1 flex-shrink-0">
                      <button onClick={() => openEdit(ini)} className="text-gray-400 hover:text-green-600 transition-colors p-1"><Pencil className="h-3.5 w-3.5" /></button>
                      <button onClick={() => setDeleteId(ini.id)} className="text-gray-400 hover:text-red-600 transition-colors p-1"><Trash2 className="h-3.5 w-3.5" /></button>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="border-t border-gray-100 p-4 space-y-4">
                    ...
                    {(ini.tags || []).length > 0 && (
                      <div className="flex flex-wrap gap-1">
                        {(ini.tags || []).map(t => <span key={t} className="px-2 py-0.5 bg-gray-100 text-gray-600 rounded text-xs">{t}</span>)}
                      </div>
                    )}
                  </div>
                )}
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Initiative' : 'New ESG Initiative'} size="lg">
        <div className="space-y-4">
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Initiative Name *</label>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Pillar</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Progress (%)</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Budget Spent (£)</label>
          ...
          <div><label className="block text-sm font-medium text-gray-700 mb-1">SDG Numbers (comma-separated)</label>
