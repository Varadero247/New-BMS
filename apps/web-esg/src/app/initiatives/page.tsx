  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Track sustainability improvement projects across Environmental, Social and Governance
            pillars
          </p>
        </div>
        <button
        ...
        <div className="grid grid-cols-3 gap-4">
          {['ENVIRONMENTAL', 'SOCIAL', 'GOVERNANCE'].map((p) => {
            ...
            const pillarProgress = Math.round(
              pillarInits.reduce((s, i) => s + (i.progress || 0), 0) / pillarInits.length
            );
            ...
            return (
              <button
                key={p}
                onClick={() => setPillarFilter(pillarFilter === p ? '' : p)}
              ...
              >
                ...
                <div className="mt-3">
                  ...
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div
                      className={`h-2 rounded-full ${cfg.prog}`}
      ...
      {filtered.length > 0 ? (
        <div className="space-y-3">
          {filtered.map((ini) => {
            ...
            const budgetPct =
              ini.budget && ini.budget > 0 ? Math.round(((ini.spent || 0) / ini.budget) * 100) : 0;
                ...
                <div className="flex items-center w-full p-4 hover:bg-gray-50 dark:bg-gray-800 transition-colors">
                  ...
                  >
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span
                        className={`px-2 py-0.5 rounded-full text-xs font-medium ${priCfg.color}`}
                      >
                        {priCfg.label}
                      </span>
                      <span
                        className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${stCfg.color}`}
                      >
                        {stCfg.label}
                      </span>
                    </div>
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5 truncate">
                      {ini.owner}
                      {ini.targetDate
                        ? ` · Due ${new Date(ini.targetDate).toLocaleDateString()}`
                        : ''}
                    </p>
                  </div>
                  <div className="flex items-center gap-4 ml-4">
                    <div className="w-24 hidden sm:block">
                      ...
                      <div className="w-full bg-gray-200 rounded-full h-1.5">
                        <div
                          className={`h-1.5 rounded-full ${prog === 100 ? 'bg-green-500' : prog >= 50 ? 'bg-blue-500' : 'bg-amber-500'}`}
                          style={{ width: `${prog}%` }}
                        />
                      </div>
                    </div>
                    <div className="flex gap-1 flex-shrink-0">
                      <button
                        ...
                        className="text-gray-400 dark:text-gray-500 hover:text-green-600 transition-colors p-1"
                      >
                        <Pencil className="h-3.5 w-3.5" />
                      </button>
                      <button
                        onClick={() => setDeleteId(ini.id)}
                      ...
                      {ini.budget !== null && ini.budget !== undefined && ini.budget > 0 && (
                        <div>
                          ...
                          <p className="text-lg font-bold text-gray-900 dark:text-gray-100">
                            {ini.budget >= 1000000
                              ? `£${(ini.budget / 1000000).toFixed(1)}M`
                          ...
                          <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">
                            {ini.spent && ini.spent >= 1000
                              ? `£${(ini.spent / 1000).toFixed(0)}k`
                              : `£${ini.spent || 0}`}{' '}
                            spent ({budgetPct}%)
                          </p>
                        </div>
                      ...
                      <div>
                        ...
                        {ini.startDate && (
                          <p className="text-xs text-gray-600">
                            Start: {new Date(ini.startDate).toLocaleDateString()}
                          </p>
                        )}
                        {ini.targetDate && (
                          <p className="text-xs text-gray-600">
                            Target: {new Date(ini.targetDate).toLocaleDateString()}
                          </p>
                        )}
                      </div>
                    </div>
      ...
      <Modal
        ...
        title={editing ? 'Edit Initiative' : 'New ESG Initiative'}
        size="lg"
      >
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="e.g. Carbon Neutrality Programme"
            />
          </div>
          <div className="grid grid-cols-3 gap-4">
            <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                min="0"
                value={form.spent}
                onChange={(e) => setForm((f) => ({ ...f, spent: parseFloat(e.target.value) || 0 }))}
                className="w-full border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
              />
            </div>
          </div>
          <div>
            ...
            <input
              value={form.sdgList}
              onChange={(e) => setForm((f) => ({ ...f, sdgList: e.target.value }))}
              className="w-full border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
          ...
          <div>
            ...
            <input
              value={form.tagList}
              onChange={(e) => setForm((f) => ({ ...f, tagList: e.target.value }))}
              className="w-full border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
              placeholder="e.g. Net Zero, Science-Based Target"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            ...
            <textarea
              ...
              rows={2}
              className="w-full border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
              placeholder="Initiative objectives and approach..."
            />
          </div>
        </div>
