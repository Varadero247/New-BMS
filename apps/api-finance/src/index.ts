...
dotenv.config();
initSentry('api-finance');
initTracing({ serviceName: 'api-finance' });
...
app.get('/health', createHealthCheck('api-finance', prisma, '1.0.0'));
...
const server = app.listen(PORT, () => {
  logger.info(`Finance API server running on port ${PORT}`);
});
