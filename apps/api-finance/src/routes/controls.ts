...
router.get('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const [data, total] = await Promise.all([
      prisma.finControl.findMany({
        where,
        skip,
        take: Math.min(Math.max(1, parseInt(limit, 10) || 20), 100),
        orderBy: { createdAt: 'desc' },
      }),
      prisma.finControl.count({ where }),
    ]);
    res.json({
      success: true,
      data,
...
router.post('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const c = await prisma.finControl.count({ where: { orgId } });
    const {
      title,
      description,
      controlType,
    ...
    const data = await prisma.finControl.create({
      data: {
        title,
        description,
        controlType,
        ...
        notes,
        orgId,
        referenceNumber: `FCR-${y}-${String(c + 1).padStart(4, '0')}`,
...
router.put('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const data = await prisma.finControl.update({
...
router.delete('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    await prisma.finControl.update({
      where: { id: req.params.id },
