...
router.post('/', authenticate, async (req: Request, res: Response) => { try { const parsed = createIr35Schema.safeParse(req.body); if (!parsed.success) { return res.status(400).json({ success: false, error: { code: 'VALIDATION_ERROR', message: parsed.error.errors[0].message } }); } const orgId = (req as any).user?.orgId || 'default'; const y = new Date().getFullYear(); const c = await (prisma as any).finIr35Assessment.count({ where: { orgId } }); const { contractorName, contractorEmail, engagementDesc, clientName, determination, assessmentDate, assessedBy, reasoning, evidenceUrl, reviewDate, notes } = parsed.data; const data = await (prisma as any).finIr35Assessment.create({ data: { contractorName, contractorEmail, engagementDesc, clientName, determination, assessmentDate, assessedBy, reasoning, evidenceUrl, reviewDate, notes, orgId, referenceNumber: `IR35-${y}-${String(c+1).padStart(4,'0')}`, createdBy: (req as any).user?.id } }); res.status(201).json({ success: true, data }); } catch (error: any) { res.status(400).json({ success: false, error: { code: 'CREATE_ERROR', message: error.message } }); } });
