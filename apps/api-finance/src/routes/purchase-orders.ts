...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to list purchase orders', {
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    if (!supplier) {
      return res.status(404).json({
        success: false,
        error: { code: 'NOT_FOUND', message: 'Supplier not found or inactive' },
      });
    }
  ...
  try {
    ...
    if (existing.status === 'RECEIVED' || existing.status === 'CANCELLED') {
      return res.status(400).json({
        ...
        error: {
          code: 'INVALID_STATE',
          message: `Cannot update a ${existing.status} purchase order`,
        },
      });
    }
  ...
  try {
    ...
    if (existing.status !== 'DRAFT' && existing.status !== 'CANCELLED') {
      return res.status(409).json({
        success: false,
        error: { code: 'CONFLICT', message: `Cannot delete a ${existing.status} purchase order` },
      });
    }
    ...
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to delete purchase order' },
    });
  }
});
