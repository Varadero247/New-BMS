...
function CategoryDonut({ byCategory }: { byCategory: CategoryBreakdown[] }) {
  ...
  if (total === 0)
    return <p className="text-sm text-gray-400 dark:text-gray-500 text-center py-4">No data</p>;
    ...
    <div className="mt-3 space-y-2">
      ...
      {byCategory.length > 8 && (
        <p className="text-xs text-gray-400 dark:text-gray-500 text-right">
          +{byCategory.length - 8} more categories
        </p>
      )}
    </div>
  );
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-gray-500 dark:text-gray-400 mt-1">
                Enterprise risk portfolio analytics — ISO 31000
              </p>
            </div>
            <button
              onClick={loadData}
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-4 gap-4">
                {[...Array(4)].map((_, i) => (
                  <div
                    key={i}
                    className="animate-pulse h-24 bg-gray-200 dark:bg-gray-700 rounded-xl"
                  />
                ))}
              </div>
              <div className="grid grid-cols-3 gap-4">
                {[...Array(3)].map((_, i) => (
                  <div
                    key={i}
                    className="animate-pulse h-64 bg-gray-200 dark:bg-gray-700 rounded-xl"
                  />
                ))}
              </div>
            </div>
            ...
            <>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                {[
                  ...
                  {
                    label: 'KRI Breaches',
                    value: data.kriBreaches,
                    icon: Activity,
                    color: 'red',
                    sub: `${data.kriWarnings} warnings`,
                  },
                  {
                    ...
                    icon: TrendingUp,
                    color: 'yellow',
                    sub: `${data.overdueReview} reviews overdue`,
                  },
                ].map((kpi) => {
                  const Icon = kpi.icon;
                  return (
                    <Card key={kpi.label} className="hover:shadow-md transition-shadow">
                      <CardContent className="p-5">
                        <div className="flex items-center justify-between">
                          ...
                          <div
                            className={`p-3 bg-${kpi.color}-50 dark:bg-${kpi.color}-900/20 rounded-xl`}
                          >
                  ...
                  <CardContent className="p-5">
                      ...
                      <div className="flex-1">
                        ...
                        >
                          ...
                          {[1, 2, 3, 4, 5].map((c) => (
                            <div
                              key={c}
                              className="text-center text-xs text-gray-400 dark:text-gray-500 pb-1 font-medium"
                            >
                          ...
                          {[5, 4, 3, 2, 1].map((l) => (
                            <>
                              <div
                                key={`label-${l}`}
                                className="flex items-center justify-end pr-1.5 text-xs text-gray-400 dark:text-gray-500 font-medium"
                              >
                                {LIKELIHOOD_LABELS[l]}
                              </div>
                              {[1, 2, 3, 4, 5].map((c) => {
                                const cell = data.heatmapData?.find(
                                  (h) => h.likelihood === l && h.consequence === c
                                );
                        ...
                        <div className="text-center text-xs text-gray-500 dark:text-gray-400 font-medium mt-2">
                          CONSEQUENCE
                        </div>
                      </div>
                    </div>
                    <div className="mt-4 flex gap-4 justify-center text-xs">
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-gray-600 dark:text-gray-400">Low (1-4)</span>
                      </div>
                      <div className="flex items-center gap-1">
                        <div className="w-3 h-3 bg-yellow-400 rounded" />
                        <span className="text-gray-600 dark:text-gray-400">Medium (5-9)</span>
                      </div>
                      <div className="flex items-center gap-1">
                        <div className="w-3 h-3 bg-orange-400 rounded" />
                        <span className="text-gray-600 dark:text-gray-400">High (10-14)</span>
                      </div>
                      <div className="flex items-center gap-1">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                ...
                <Card>
                  <CardContent className="p-5">
                    ...
                    <p className="text-xs text-gray-400 dark:text-gray-500">
                      Risk lifecycle stage distribution
                    </p>
                ...
                <Card>
                  <CardContent className="p-5">
                    ...
                    <p className="text-xs text-gray-400 dark:text-gray-500">
                      Residual risk level distribution
                    </p>
                    <div className="mt-3 space-y-2">
                      {['CRITICAL', 'VERY_HIGH', 'HIGH', 'MEDIUM', 'LOW'].map((level) => {
                        ...
                        return (
                          <div key={level}>
                            <div className="flex justify-between text-xs mb-0.5">
                              <span
                                className={LEVEL_TEXT[level] || 'text-gray-600 dark:text-gray-400'}
                              >
                                {level.replace(/_/g, ' ')}
                              </span>
                            ...
                            <div className="w-full bg-gray-100 dark:bg-gray-800 rounded-full h-1.5">
                              <div
                                className={`h-1.5 rounded-full ${LEVEL_COLORS[level] || 'bg-gray-400'}`}
                                style={{ width: `${pct}%` }}
                              />
                            </div>
                          </div>
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 mb-0.5">
                                ...
                                <span
                                  className={`px-1.5 py-0.5 rounded text-xs font-medium bg-opacity-20 ${LEVEL_TEXT[risk.residualRiskLevel] || ''}`}
                                >
                            ...
                            <div className="shrink-0 text-right">
                              <p
                                className={`text-2xl font-bold ${LEVEL_TEXT[risk.residualRiskLevel] || 'text-gray-700 dark:text-gray-300'}`}
                              >
                                {risk.residualScore}
                              </p>
                        ...
                        {data.recentlyChanged.slice(0, 8).map((risk) => (
                          ...
                          >
                            <div className="flex items-center gap-2.5 min-w-0">
                              <div
                                className={`w-2 h-2 rounded-full shrink-0 ${LEVEL_COLORS[risk.residualRiskLevel] || 'bg-gray-400'}`}
                              />
            ...
            <Card>
              <CardContent className="py-16 text-center">
                ...
                <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">
                  Add risks to the register to see analytics
                </p>
              </CardContent>
            </Card>
          )}
