...
import { AlertTriangle, ShieldCheck, ClipboardCheck, TrendingUp } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const kpis = [
    { label: 'Total Risks', value: stats.totalRisks ?? 0, icon: AlertTriangle, color: 'red' },
    { label: 'Open CAPAs', value: stats.openCapas ?? 0, icon: ShieldCheck, color: 'amber' },
    { label: 'Pending Reviews', value: stats.pendingReviews ?? 0, icon: ClipboardCheck, color: 'blue' },
    { label: 'Risk Score Avg', value: stats.avgRiskScore ?? '-', icon: TrendingUp, color: 'green' },
  ];
    ...
    <div className="flex min-h-screen">
      ...
      <main className="flex-1 p-8">
        <div className="max-w-7xl mx-auto">
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">Risk & CAPA Dashboard</h1>
            <p className="text-gray-500 dark:text-gray-400 mt-1">ISO 31000 Risk Management Overview</p>
          </div>
          ...
          {Object.keys(stats).length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {Object.entries(stats).filter(([k]) => !['totalRisks', 'openCapas', 'pendingReviews', 'avgRiskScore'].includes(k)).map(([key, value]) => (
