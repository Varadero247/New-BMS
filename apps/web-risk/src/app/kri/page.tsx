...
function KriStatusBadge({ status }: { status?: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${STATUS_COLORS[s] || STATUS_COLORS.GREEN}`}
    >
...
export default function KriPage() {
  ...
  const [dashStats, setDashStats] = useState<{ red: number; amber: number; green: number }>({
    red: 0,
    amber: 0,
    green: 0,
  });
  ...
  async function loadData() {
    ...
    try {
      ...
      setError(
        axios.isAxiosError(e) && e.response?.status === 401 ? 'Session expired. Please log in.' : 'Failed to load KRI data.'
      );
    } finally {
      setLoading(false);
    }
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-gray-500 dark:text-gray-400 mt-1">
                Key Risk Indicator monitoring and threshold breaches
              </p>
            </div>
            <button
              onClick={loadData}
                ...
                {loading ? (
                  ...
                  <div className="py-12 text-center">
                    ...
                    <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">
                      All KRIs are within acceptable limits
                    </p>
                  </div>
                ) : (
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody className="divide-y divide-gray-50 dark:divide-gray-800/60">
                        {breaches.map((kri) => (
                          <tr
                            key={kri.id}
                            className={`hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors ${kri.currentStatus === 'RED' ? 'bg-red-50/30 dark:bg-red-900/10' : 'bg-amber-50/30 dark:bg-amber-900/10'}`}
                          >
                            ...
                            <td className="px-4 py-3">
                              <Link href={`/risks/${kri.riskId}`} className="hover:underline">
                                <span className="font-mono text-xs text-gray-500 dark:text-gray-400">
                                  {kri.risk?.referenceNumber}
                                </span>
                                <p className="text-gray-700 dark:text-gray-300 text-xs mt-0.5 max-w-[160px] truncate">
                                  {kri.risk?.title}
                                </p>
                              </Link>
                            </td>
                      ...
                      <tbody className="divide-y divide-gray-50 dark:divide-gray-800/60">
                        {due.map((kri) => {
                          ...
                          return (
                            <tr
                              key={kri.id}
                              className={`hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors ${overdue ? 'bg-red-50/20 dark:bg-red-900/10' : ''}`}
                            >
                              <td className="px-4 py-3">
                                <p className="font-medium text-gray-900 dark:text-gray-100">
                                  {kri.name}
                              ...
                              <td className="px-4 py-3">
                                {kri.nextMeasurementDue ? (
                                  <div>
                                    <p
                                      className={`font-medium ${overdue ? 'text-red-600 dark:text-red-400' : 'text-gray-900 dark:text-gray-100'}`}
                                    >
