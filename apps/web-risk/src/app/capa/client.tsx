...
export default function CapaClient() {
  ...
  function openEdit(capa: Capa) {
    setForm({
      ...
      type: capa.type,
      source: capa.source || 'INTERNAL_REVIEW',
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (editId) {
        await api.put(`/capa/${editId}`, payload);
      } else {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <Card>
            <CardContent className="pt-6 text-center">
              <p className="text-3xl font-bold">{capas.length}</p>
          ...
          <Card>
            <CardContent className="pt-6 text-center">
              <p className="text-3xl font-bold text-red-600">
                {capas.filter((c) => c.status === 'OPEN' || c.status === 'OVERDUE').length}
              </p>
              <p className="text-sm text-gray-500 dark:text-gray-400">Open / Overdue</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="pt-6 text-center">
                  ...
                  <TableBody>
                    {capas.map((capa) => (
                      <TableRow key={capa.id}>
                        ...
                        <TableCell>
                          <span
                            className={`px-2 py-1 rounded-full text-xs font-medium ${getPriorityColor(capa.priority)}`}
                          >
                            {capa.priority}
                          </span>
                        </TableCell>
                        ...
                        <TableCell>
                          <div className="flex gap-2">
                            <Button size="sm" variant="outline" onClick={() => openEdit(capa)}>
                              Edit
                            </Button>
                            <Button
                              size="sm"
        ...
        {modalOpen && (
          <Modal
            ...
            title={editId ? 'Edit CAPA' : 'Add CAPA'}
            size="lg"
          >
            <div className="space-y-4">
              ...
              <div>
                ...
                <Textarea
                  ...
                  rows={2}
                  placeholder="Planned corrective/preventive actions..."
                />
              </div>
              <div>
