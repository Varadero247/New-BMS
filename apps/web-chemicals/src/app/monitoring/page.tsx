        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-gray-500 dark:text-gray-400 mt-1">
                Workplace exposure measurements and WEL compliance
              </p>
            </div>
            <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            ...
            <Card className={exceedanceCount > 0 ? 'border-red-200 dark:border-red-800' : ''}>
              <CardContent className="p-4 flex items-center gap-3">
                <div
                  className={`p-2 rounded-lg ${exceedanceCount > 0 ? 'bg-red-50 dark:bg-red-900/20' : 'bg-green-50 dark:bg-green-900/20'}`}
                >
                  <AlertTriangle
                    className={`h-5 w-5 ${exceedanceCount > 0 ? 'text-red-600' : 'text-green-600'}`}
                  />
                </div>
                <div>
                  <p className="text-sm text-gray-500 dark:text-gray-400">WEL Exceedances</p>
                  <p
                    className={`text-xl font-bold ${exceedanceCount > 0 ? 'text-red-600' : 'text-green-600'}`}
                  >
                    {exceedanceCount}
                  </p>
                </div>
            ...
            <Card className={overdueCount > 0 ? 'border-amber-200 dark:border-amber-800' : ''}>
              <CardContent className="p-4 flex items-center gap-3">
                <div
                  className={`p-2 rounded-lg ${overdueCount > 0 ? 'bg-amber-50 dark:bg-amber-900/20' : 'bg-green-50 dark:bg-green-900/20'}`}
                >
                  <TrendingUp
                    className={`h-5 w-5 ${overdueCount > 0 ? 'text-amber-600' : 'text-green-600'}`}
                  />
                </div>
                <div>
                  <p className="text-sm text-gray-500 dark:text-gray-400">Overdue Monitoring</p>
                  <p
                    className={`text-xl font-bold ${overdueCount > 0 ? 'text-amber-600' : 'text-green-600'}`}
                  >
                    {overdueCount}
                  </p>
                </div>
                    ...
                    {records.length === 0 ? (
                      ...
                      records.map((r) => {
                        ...
                        return (
                          <tr
                            key={r.id}
                            className={`border-b border-gray-100 dark:border-gray-700/50 transition-colors ${exceedance ? 'bg-red-50/50 dark:bg-red-900/10' : 'hover:bg-gray-50 dark:hover:bg-gray-800/30'}`}
                          >
                            ...
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-2">
                                <Beaker
                                  className={`h-4 w-4 ${exceedance ? 'text-red-500' : 'text-gray-400'}`}
                                />
                            ...
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-2">
                                ...
                                <span
                                  className={`font-mono font-medium text-xs ${r.welPercentage >= 100 ? 'text-red-600' : r.welPercentage >= 75 ? 'text-amber-600' : 'text-green-600'}`}
                                >
                                  {r.welPercentage}%
                                </span>
                              </div>
        ...
        <div className="space-y-4 p-4">
              ...
              >
                ...
                {chemicals.map((c) => (
                  <option key={c.id} value={c.id}>
                    {c.name} {c.welLimit ? `(WEL: ${c.welLimit} mg/m3)` : ''}
                  </option>
                ))}
              </select>
            </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                type="text"
                value={form.sampler}
                onChange={(e) => setForm({ ...form, sampler: e.target.value })}
                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-red-500"
                placeholder="Name of person taking sample"
              />
            </div>
          </div>
          <div>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
            ...
            <button
              ...
              disabled={saving || !form.chemicalId || !form.location || !form.measuredValue}
