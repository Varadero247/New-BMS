          ...
          {group.items.map((item) => {
            ...
            return (
              <li key={item.name}>
                ...
                >
                  <Icon
                    className={`h-4 w-4 shrink-0 ${isActive ? 'text-amber-600 dark:text-amber-400' : 'text-gray-400 dark:text-gray-500'}`}
                  />
  ...
  return (
    <aside className="w-64 bg-card border-r border-border flex flex-col h-full min-h-screen">
      <div className="p-4 border-b border-border bg-amber-50 dark:bg-amber-900/10">
        <h1 className="text-lg font-bold font-display text-amber-900 dark:text-amber-100">
          Permit to Work
        </h1>
