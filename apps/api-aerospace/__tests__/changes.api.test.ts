  ...
  describe('POST /api/changes', () => {
    ...
    it('should create a change request successfully', async () => {
      ...
      mockPrisma.aeroChangeRequest.create.mockResolvedValueOnce({
        id: 'cr-new', refNumber: 'AERO-CR-2026-0001', ...validPayload, status: 'DRAFT', priority: 'MEDIUM',
      });

      const res = await request(app).post('/api/changes').set('Authorization', 'Bearer token').send(validPayload);
    ...
    it('should return 400 when title is missing', async () => {
      const res = await request(app).post('/api/changes').set('Authorization', 'Bearer token')
