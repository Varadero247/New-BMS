...
describe('Aerospace Configuration Baselines API', () => {
  ...
  describe('GET /api/baselines', () => {
    ...
    it('should filter by baselineType', async () => {
      ...
      await request(app).get('/api/baselines?baselineType=PRODUCT').set('Authorization', 'Bearer token');
      expect(mockPrisma.aeroConfigBaseline.findMany).toHaveBeenCalledWith(
    ...
    it('should support search', async () => {
      ...
      await request(app).get('/api/baselines?search=functional').set('Authorization', 'Bearer token');
      expect(mockPrisma.aeroConfigBaseline.findMany).toHaveBeenCalledWith(
  ...
  describe('POST /api/baselines', () => {
    ...
    it('should create a baseline successfully', async () => {
      ...
      mockPrisma.aeroConfigBaseline.create.mockResolvedValueOnce({
        id: 'b-new', refNumber: 'AERO-BL-2026-003', ...validPayload, status: 'DRAFT',
      });

      const res = await request(app).post('/api/baselines').set('Authorization', 'Bearer token').send(validPayload);
    ...
    it('should return 400 when title is missing', async () => {
      const res = await request(app).post('/api/baselines').set('Authorization', 'Bearer token')
        .send({ program: 'JSF' });
