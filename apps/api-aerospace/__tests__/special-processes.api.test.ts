...
describe('Aerospace Special Processes API', () => {
  ...
  describe('GET /api/special-processes', () => {
    ...
    it('should filter by processType', async () => {
      ...
      await request(app).get('/api/special-processes?processType=WELDING').set('Authorization', 'Bearer token');
    ...
    it('should filter by status', async () => {
      ...
      await request(app).get('/api/special-processes?status=ACTIVE').set('Authorization', 'Bearer token');
      expect(mockPrisma.aeroSpecialProcess.findMany).toHaveBeenCalledWith(
    ...
    it('should filter by approvalBody', async () => {
      ...
      await request(app).get('/api/special-processes?approvalBody=NADCAP').set('Authorization', 'Bearer token');
      expect(mockPrisma.aeroSpecialProcess.findMany).toHaveBeenCalledWith(
  ...
  describe('POST /api/special-processes', () => {
    ...
    it('should create a special process successfully', async () => {
      ...
      mockPrisma.aeroSpecialProcess.create.mockResolvedValueOnce({
        id: 'sp-new', refNumber: 'AERO-SP-2026-001', ...validPayload, status: 'ACTIVE',
      });

      const res = await request(app).post('/api/special-processes').set('Authorization', 'Bearer token').send(validPayload);
    ...
    it('should return 400 when title is missing', async () => {
      const res = await request(app).post('/api/special-processes').set('Authorization', 'Bearer token')
  ...
  describe('POST /api/special-processes/nadcap', () => {
    ...
    it('should record a NADCAP approval successfully', async () => {
      ...
      const res = await request(app).post('/api/special-processes/nadcap').set('Authorization', 'Bearer token').send(validPayload);
      expect(res.status).toBe(201);
      expect(res.body.success).toBe(true);
    ...
    it('should return 400 when supplier is missing', async () => {
      const res = await request(app).post('/api/special-processes/nadcap').set('Authorization', 'Bearer token')
        .send({ specialProcessId: 'sp1', commodity: 'Coatings', approvalDate: '2026-01-01', expiryDate: '2027-01-01' });
      expect(res.status).toBe(400);
      expect(res.body.error.code).toBe('VALIDATION_ERROR');
    });
    ...
    it('should return 400 when approvalDate is missing', async () => {
      const res = await request(app).post('/api/special-processes/nadcap').set('Authorization', 'Bearer token')
        .send({ specialProcessId: 'sp1', supplier: 'AeroCoat', commodity: 'Coatings', expiryDate: '2027-01-01' });
      expect(res.status).toBe(400);
      expect(res.body.error.code).toBe('VALIDATION_ERROR');
    });
