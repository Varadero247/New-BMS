  ...
  try {
    ...
    const item = await prisma.configItem.create({
      data: {
        ...
        status: (data.status as any) || 'CURRENT',
...
router.get('/status-accounting', async (req: AuthRequest, res: Response) => {
  try {
    const [
      ...
      prisma.configItem.count({ where: { deletedAt: null } as any }),
      prisma.configItem.count({ where: { deletedAt: null, status: 'CURRENT' } as any }),
      prisma.configItem.count({ where: { deletedAt: null, status: 'PENDING_CHANGE' } as any }),
