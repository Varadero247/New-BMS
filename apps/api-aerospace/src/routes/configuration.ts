...
router.get('/status-accounting', async (_req: Request, res: Response) => {
  try {
    const [
      ...
      prisma.engineeringChangeProposal.count({
        where: { deletedAt: null, status: 'CCB_APPROVED' },
      }),
      prisma.engineeringChangeProposal.count({
        where: { deletedAt: null, status: 'CCB_REJECTED' },
      }),
      prisma.engineeringChangeProposal.count({
        where: { deletedAt: null, status: 'IMPLEMENTING' },
      }),
      prisma.engineeringChangeProposal.count({
        where: { deletedAt: null, status: 'IMPLEMENTED' },
      }),
      prisma.engineeringChangeProposal.count({
