...
async function generateRef(orgId: string): Promise<string> {
  ...
  const c = await (prisma as any).audChecklist.count({
...
router.get('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const [data, total] = await Promise.all([
      (prisma as any).audChecklist.findMany({ where, skip, take: parseInt(limit), orderBy: { createdAt: 'desc' } }),
    ...
    res.status(500).json({ success: false, error: { code: 'FETCH_ERROR', message: 'Failed to fetch checklists' } });
  }
});

router.get('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    const item = await (prisma as any).audChecklist.findFirst({ where: { id: req.params.id, deletedAt: null } });
    if (!item) return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'checklist not found' } });
    res.json({ success: true, data: item });
  } catch (error: any) {
    res.status(500).json({ success: false, error: { code: 'FETCH_ERROR', message: 'Failed to fetch checklist' } });
  }
});
...
router.post('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const data = await (prisma as any).audChecklist.create({
      data: {
        ...parsed.data,
        orgId,
        referenceNumber,
...
router.put('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const existing = await (prisma as any).audChecklist.findFirst({ where: { id: req.params.id, deletedAt: null } });
    if (!existing) return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'checklist not found' } });
    const data = await (prisma as any).audChecklist.update({
...
router.delete('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    const existing = await (prisma as any).audChecklist.findFirst({ where: { id: req.params.id, deletedAt: null } });
    if (!existing) return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'checklist not found' } });
    await (prisma as any).audChecklist.update({
      where: { id: req.params.id },
