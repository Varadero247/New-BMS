...
async function generateRef(orgId: string): Promise<string> {
  ...
  const c = await (prisma as any).audAudit.count({
...
router.get('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const [data, total] = await Promise.all([
      (prisma as any).audAudit.findMany({ where, skip, take: parseInt(limit), orderBy: { createdAt: 'desc' } }),
...
router.get('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    const item = await (prisma as any).audAudit.findFirst({ where: { id: req.params.id, deletedAt: null } });
...
router.post('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const data = await (prisma as any).audAudit.create({
      data: {
        ...parsed.data,
        orgId,
        referenceNumber,
...
router.put('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const existing = await (prisma as any).audAudit.findFirst({ where: { id: req.params.id, deletedAt: null } });
    if (!existing) return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'audit not found' } });
    const data = await (prisma as any).audAudit.update({
...
router.delete('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    const existing = await (prisma as any).audAudit.findFirst({ where: { id: req.params.id, deletedAt: null } });
    if (!existing) return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'audit not found' } });
    await (prisma as any).audAudit.update({
      where: { id: req.params.id },
