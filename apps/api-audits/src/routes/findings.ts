...
router.get('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    const item = await prisma.audFinding.findFirst({ where: { id: req.params.id, deletedAt: null } });
...
router.put('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const existing = await prisma.audFinding.findFirst({ where: { id: req.params.id, deletedAt: null } });
