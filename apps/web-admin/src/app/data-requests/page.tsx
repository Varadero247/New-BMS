    ...
    <div className="min-h-screen bg-[#0A1628] text-white">
      ...
      <main className="ml-64 p-8">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-gray-400 text-sm">GDPR/DSAR management with SLA tracking</p>
            </div>
          </div>
          <button
          ...
          {['', 'RECEIVED', 'VERIFIED', 'PROCESSING', 'COMPLETED', 'REJECTED'].map((s) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-sm transition-colors ${
                statusFilter === s ? 'bg-blue-600 text-white' : 'bg-[#1B3A6B]/40 text-gray-400 hover:text-white'
              }`}
            >
              {s || 'All'}
            </button>
        ...
        <div className="bg-[#0F2440] rounded-xl border border-[#1B3A6B]/30 overflow-hidden">
          {loading ? (
            <div className="p-12 text-center text-gray-400">Loading data requests...</div>
          ) : requests.length === 0 ? (
            <div className="p-12 text-center text-gray-400">No data requests found.</div>
          ) : (
            <table className="w-full">
              <thead>
                <tr className="border-b border-[#1B3A6B]/30">
                  ...
                  <th className="text-left px-6 py-3 text-xs font-medium text-gray-400 uppercase">Requester</th>
                  <th className="text-left px-6 py-3 text-xs font-medium text-gray-400 uppercase">Status</th>
                  <th className="text-left px-6 py-3 text-xs font-medium text-gray-400 uppercase">Received</th>
                  <th className="text-left px-6 py-3 text-xs font-medium text-gray-400 uppercase">Deadline</th>
                  <th className="text-left px-6 py-3 text-xs font-medium text-gray-400 uppercase">Days Left</th>
