...
import {
  Shield, Search, Filter, Clock, User, Globe, Monitor,
  FileText, Edit3, Trash2, Plus, Eye, LogIn, LogOut, Key
} from 'lucide-react';

type ActionType = 'create' | 'update' | 'delete' | 'read' | 'login' | 'logout' | 'export' | 'permission-change';
...
const entries: AuditEntry[] = [
  { id: 'al-1', timestamp: '2026-02-13 14:32:15', user: 'admin@ims.local', role: 'ORG_ADMIN', action: 'update', module: 'Quality', resource: 'NCR', resourceId: 'NCR-2602-0045', details: 'Updated status from OPEN to IN_INVESTIGATION', ipAddress: '192.168.1.45', userAgent: 'Chrome 121 / Windows', status: 'success' },
  { id: 'al-2', timestamp: '2026-02-13 14:30:08', user: 'sarah.chen@ims.local', role: 'QUALITY_MANAGER', action: 'create', module: 'Quality', resource: 'CAPA', resourceId: 'CAPA-2602-0012', details: 'Created corrective action linked to NCR-2602-0045', ipAddress: '192.168.1.52', userAgent: 'Chrome 121 / macOS', status: 'success' },
  { id: 'al-3', timestamp: '2026-02-13 14:28:55', user: 'tom.parker@ims.local', role: 'HSE_OFFICER', action: 'create', module: 'Health & Safety', resource: 'Incident', resourceId: 'INC-2602-0078', details: 'Reported near-miss incident in Warehouse B', ipAddress: '192.168.1.78', userAgent: 'Safari / iPhone', status: 'success' },
  { id: 'al-4', timestamp: '2026-02-13 14:25:12', user: 'emma.clarke@ims.local', role: 'FINANCE_MANAGER', action: 'export', module: 'Finance', resource: 'Report', resourceId: 'RPT-PL-2026-01', details: 'Exported P&L report January 2026 as PDF', ipAddress: '192.168.1.30', userAgent: 'Chrome 121 / Windows', status: 'success' },
  { id: 'al-5', timestamp: '2026-02-13 14:22:00', user: 'mike.johnson@ims.local', role: 'SALES_REPRESENTATIVE', action: 'read', module: 'Finance', resource: 'Invoice', resourceId: 'INV-2602-0234', details: 'Attempted to view invoice — access denied (no Finance permission)', ipAddress: '192.168.1.90', userAgent: 'Chrome 121 / Windows', status: 'denied' },
  { id: 'al-6', timestamp: '2026-02-13 14:18:30', user: 'admin@ims.local', role: 'ORG_ADMIN', action: 'permission-change', module: 'Settings', resource: 'User', resourceId: 'USR-045', details: 'Granted AUDITOR role to lisa.chang@ims.local', ipAddress: '192.168.1.45', userAgent: 'Chrome 121 / Windows', status: 'success' },
  { id: 'al-7', timestamp: '2026-02-13 14:15:00', user: 'alex.kim@ims.local', role: 'INFOSEC_MANAGER', action: 'update', module: 'InfoSec', resource: 'Risk Assessment', resourceId: 'RA-2602-0015', details: 'Updated risk score from MEDIUM to HIGH for supply chain risk', ipAddress: '192.168.1.55', userAgent: 'Firefox 122 / Linux', status: 'success' },
  { id: 'al-8', timestamp: '2026-02-13 14:10:22', user: 'priya.patel@ims.local', role: 'HR_MANAGER', action: 'delete', module: 'HR', resource: 'Leave Request', resourceId: 'LR-2602-0089', details: 'Cancelled approved leave request (employee request)', ipAddress: '192.168.1.40', userAgent: 'Chrome 121 / macOS', status: 'success' },
  { id: 'al-9', timestamp: '2026-02-13 14:05:00', user: 'system@ims.local', role: 'SYSTEM', action: 'create', module: 'Workflows', resource: 'Automation', resourceId: 'AUTO-NCR-ESC', details: 'Auto-escalated NCR-2602-0042 (overdue 48hrs)', ipAddress: '127.0.0.1', userAgent: 'IMS Automation Engine', status: 'success' },
  { id: 'al-10', timestamp: '2026-02-13 14:00:00', user: 'david.hughes@ims.local', role: 'EXECUTIVE', action: 'login', module: 'Auth', resource: 'Session', resourceId: 'SES-20260213-DH', details: 'Logged in from new device', ipAddress: '82.45.120.5', userAgent: 'Chrome 121 / iPad', status: 'success' },
  { id: 'al-11', timestamp: '2026-02-13 13:55:00', user: 'unknown', role: '-', action: 'login', module: 'Auth', resource: 'Session', resourceId: '-', details: 'Failed login attempt — invalid credentials (admin@ims.local)', ipAddress: '203.0.113.45', userAgent: 'Python-requests/2.31', status: 'error' },
  { id: 'al-12', timestamp: '2026-02-13 13:50:30', user: 'james.park@ims.local', role: 'QUALITY_ENGINEER', action: 'update', module: 'Medical', resource: 'Design Control', resourceId: 'DC-2602-0008', details: 'Uploaded design verification test report', ipAddress: '192.168.1.67', userAgent: 'Chrome 121 / Windows', status: 'success' },
];
...
export default function AuditLogClient() {
  ...
  const filtered = entries.filter((e) => {
    ...
    const matchesSearch = !searchTerm || e.user.toLowerCase().includes(searchTerm.toLowerCase()) || e.details.toLowerCase().includes(searchTerm.toLowerCase()) || e.resourceId.toLowerCase().includes(searchTerm.toLowerCase());
              ...
              {filtered.map((entry) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-xs text-gray-600 line-clamp-1 max-w-xs">{entry.details}</td>
                  </tr>
                );
              })}
            </tbody>
