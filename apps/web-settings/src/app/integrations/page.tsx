...
const defaultIntegrations: Integration[] = [
  ...
  },
  {
    id: 'jira',
    name: 'Jira',
    description: 'Sync CAPA actions and non-conformances with Jira tickets',
  ...
  },
  {
    id: 'sharepoint',
    name: 'SharePoint',
    description: 'Sync documents and evidence with SharePoint',
    category: 'Storage',
    icon: 'S',
    connected: false,
    status: 'inactive',
  },
  {
    id: 'google-drive',
    name: 'Google Drive',
    description: 'Back up documents and reports to Google Drive',
    category: 'Storage',
    icon: 'G',
    connected: false,
    status: 'inactive',
  },
  {
    id: 'power-bi',
    name: 'Power BI',
    description: 'Export KPIs and analytics data to Power BI dashboards',
    category: 'Analytics',
    icon: 'P',
    connected: false,
    status: 'inactive',
  },
  {
    id: 'sap',
    name: 'SAP ERP',
    description: 'Sync inventory, finance, and HR data with SAP',
    category: 'ERP',
    icon: 'S',
    connected: false,
    status: 'inactive',
  ...
  },
  {
    id: 'docusign',
    name: 'DocuSign',
    description: 'Electronic signatures for approvals and reviews',
    category: 'Compliance',
    icon: 'D',
    connected: false,
    status: 'inactive',
  },
  {
    id: 'smtp',
    name: 'Custom SMTP',
    description: 'Use your own SMTP server for outgoing emails',
    category: 'Email',
    icon: '@',
    connected: true,
    status: 'active',
...
export default function IntegrationsPage() {
  ...
  const handleToggle = (id: string) => {
    setIntegrations((prev) =>
      prev.map((i) =>
        i.id === id
          ? { ...i, connected: !i.connected, status: i.connected ? 'inactive' : 'active' }
          : i
      )
    );
  };
    ...
    <div className="p-6 space-y-6 max-w-4xl">
      <div>
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
          Connect IMS with your existing tools and services
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filtered.map((integration) => (
          <div
            key={integration.id}
            className="bg-white dark:bg-gray-900 rounded-lg border p-4 flex gap-4"
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                <span
                  className={`inline-flex rounded-full px-2 py-0.5 text-[10px] font-medium ${statusColors[integration.status]}`}
                >
                  {integration.status}
                </span>
              </div>
