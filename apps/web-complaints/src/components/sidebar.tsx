          ...
          {group.items.map((item) => {
            ...
            return (
              <li key={item.name}>
                ...
                >
                  <Icon
                    className={`h-4 w-4 shrink-0 ${isActive ? 'text-orange-600 dark:text-orange-400' : 'text-gray-400 dark:text-gray-500'}`}
                  />
  ...
  return (
    <aside className="w-64 bg-card border-r border-border flex flex-col h-full min-h-screen">
      <div className="p-4 border-b border-border bg-orange-50 dark:bg-orange-900/10">
        <h1 className="text-lg font-bold font-display text-orange-900 dark:text-orange-100">
          Complaints
        </h1>
