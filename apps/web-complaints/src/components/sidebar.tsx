...
const navGroups: NavGroup[] = [
  ...
  },
  {
    ...
    collapsible: true,
    items: [
      { name: 'SLA Tracking', href: '/sla', icon: Clock },
...
function CollapsibleSection({ group, pathname }: { group: NavGroup; pathname: string }) {
  ...
  return (
    <div className={group.clause ? 'mt-3 pt-3 border-t border-gray-200 dark:border-gray-700' : group.collapsible ? 'mt-3 pt-3 border-t border-gray-200 dark:border-gray-700' : ''}>
      {group.collapsible ? (
        <button
          type="button"
          ...
          {group.items.map((item) => {
            ...
            return (
              <li key={item.name}>
                ...
                >
                  <Icon className={`h-4 w-4 shrink-0 ${isActive ? 'text-orange-600 dark:text-orange-400' : 'text-gray-400 dark:text-gray-500'}`} />
...
export default function Sidebar() {
  ...
  return (
    <aside className="w-64 bg-card border-r border-border flex flex-col h-full min-h-screen">
      <div className="p-4 border-b border-border bg-orange-50 dark:bg-orange-900/10">
        <h1 className="text-lg font-bold font-display text-orange-900 dark:text-orange-100">Complaints</h1>
        <p className="text-xs text-orange-600 dark:text-orange-400 mt-0.5">Complaint Management</p>
      </div>
      <nav className="flex-1 p-3 overflow-y-auto" aria-label="Complaints module navigation">
        {navGroups.map((group) => (
