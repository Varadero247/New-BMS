...
const emptyForm: ActionForm = {
  complaintId: '', action: '', assignee: '', dueDate: '', completedAt: '', status: 'OPEN', notes: '',
};
...
export default function ActionsClient() {
  ...
  function openEdit(item: Action) {
    setForm({
      complaintId: item.complaintId || '', action: item.action || '',
  ...
  async function handleDelete(id: string) {
    ...
    try { await api.delete(`/actions/${id}`); loadData(); } catch (err) { console.error(err); }
  }
    ...
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">Corrective Actions</h1>
            <p className="text-gray-500 dark:text-gray-400 mt-1">Track actions related to complaints</p>
          </div>
          <Button onClick={openCreate} className="flex items-center gap-2"><Plus className="h-4 w-4" />New Action</Button>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <Card><CardContent className="pt-6 text-center"><p className="text-3xl font-bold">{items.length}</p><p className="text-sm text-gray-500 dark:text-gray-400">Total Actions</p></CardContent></Card>
          <Card><CardContent className="pt-6 text-center"><p className="text-3xl font-bold text-blue-600">{items.filter(i => i.status === 'IN_PROGRESS').length}</p><p className="text-sm text-gray-500 dark:text-gray-400">In Progress</p></CardContent></Card>
          <Card><CardContent className="pt-6 text-center"><p className="text-3xl font-bold text-red-600">{items.filter(i => i.status === 'OVERDUE').length}</p><p className="text-sm text-gray-500 dark:text-gray-400">Overdue</p></CardContent></Card>
        ...
        <div className="flex gap-4 mb-6 flex-wrap items-center">
          <div className="relative flex-1 max-w-md">
            ...
            <input type="text" aria-label="Search actions" placeholder="Search actions..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
          ...
          <CardContent className="p-0">
            {loading ? (
                  ...
                  <TableBody>
                    {items.map(item => (
                      <TableRow key={item.id}>
                        ...
                        <TableCell className="font-medium max-w-[300px] truncate">{item.action}</TableCell>
              ...
              <div className="text-center py-12">
                ...
                <Button variant="outline" className="mt-4" onClick={openCreate}><Plus className="h-4 w-4 mr-2" />Add First Action</Button>
              </div>
            )}
          </CardContent>
        </Card>
        ...
        {modalOpen && (
          <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editId ? 'Edit Action' : 'New Action'} size="lg">
            <div className="space-y-4">
              ...
              <div><Label>Action *</Label><Textarea value={form.action} onChange={e => setForm(p => ({ ...p, action: e.target.value }))} rows={3} placeholder="Describe the corrective action..." /></div>
              <div className="grid grid-cols-2 gap-4">
                <div><Label>Assignee</Label><Input value={form.assignee} onChange={e => setForm(p => ({ ...p, assignee: e.target.value }))} placeholder="Assigned to" /></div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div><Label>Completed At</Label><Input type="date" value={form.completedAt} onChange={e => setForm(p => ({ ...p, completedAt: e.target.value }))} /></div>
              </div>
            ...
            <ModalFooter>
              ...
              <Button onClick={handleSubmit} disabled={saving || !form.complaintId || !form.action}>
                {saving ? <><Loader2 className="h-4 w-4 animate-spin mr-2" />Saving...</> : editId ? 'Update Action' : 'Create Action'}
              </Button>
            </ModalFooter>
          </Modal>
        )}
