...
export async function collectStripeMetrics(): Promise<{
  ...
  try {
    ...
    const rawSubs: any = await stripe.getSubscriptions(100);
    const subscriptions: any[] = rawSubs?.data || (Array.isArray(rawSubs) ? rawSubs : []);
...
export async function collectHubSpotMetrics(): Promise<{
  ...
  try {
    ...
    const rawDeals: any = await hs.getDeals(200);
    const deals: any[] = rawDeals?.results || (Array.isArray(rawDeals) ? rawDeals : []);
