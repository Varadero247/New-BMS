...
export async function runGdprMonitorJob(): Promise<void> {
  ...
  try {
    ...
    const compliant = categories.filter(
      (c: Record<string, unknown>) => c.complianceStatus === 'COMPLIANT'
    ).length;
    const atRisk =
      categories.filter((c: Record<string, unknown>) => c.complianceStatus === 'AT_RISK').length +
      flaggedCount;
