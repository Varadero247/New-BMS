...
describe('GET /api/kpis/modules/:module', () => {
  it('should return KPIs for a specific module', async () => {
    const kpis = [{ id: 'kpi-1', name: 'TRIR', module: 'HEALTH_SAFETY' }];
    (prisma as any).analyticsKpi.findMany.mockResolvedValue(kpis);
...
describe('GET /api/kpis/:id', () => {
  it('should return a KPI by ID', async () => {
    (prisma as any).analyticsKpi.findFirst.mockResolvedValue({ id: 'kpi-1', name: 'TRIR' });
  ...
  it('should return 404 for non-existent KPI', async () => {
    ...
    const res = await request(app).get('/api/kpis/nonexistent');
...
describe('PUT /api/kpis/:id', () => {
  it('should update a KPI', async () => {
    (prisma as any).analyticsKpi.findFirst.mockResolvedValue({ id: 'kpi-1' });
    (prisma as any).analyticsKpi.update.mockResolvedValue({ id: 'kpi-1', name: 'Updated' });

    const res = await request(app).put('/api/kpis/kpi-1').send({ name: 'Updated' });
  ...
  it('should return 404 for non-existent KPI', async () => {
    ...
    const res = await request(app).put('/api/kpis/nonexistent').send({ name: 'Updated' });
...
describe('DELETE /api/kpis/:id', () => {
  it('should soft delete a KPI', async () => {
    (prisma as any).analyticsKpi.findFirst.mockResolvedValue({ id: 'kpi-1' });
    (prisma as any).analyticsKpi.update.mockResolvedValue({ id: 'kpi-1', deletedAt: new Date() });
  ...
  it('should return 404 for non-existent KPI', async () => {
    ...
    const res = await request(app).delete('/api/kpis/nonexistent');
...
describe('POST /api/kpis/:id/calculate', () => {
  it('should recalculate a KPI', async () => {
    (prisma as any).analyticsKpi.findFirst.mockResolvedValue({ id: 'kpi-1', currentValue: 50 });
    (prisma as any).analyticsKpi.update.mockResolvedValue({ id: 'kpi-1', currentValue: 75, previousValue: 50, trend: 'UP', lastCalculated: new Date() });

    const res = await request(app).post('/api/kpis/kpi-1/calculate');
  ...
  it('should return 404 for non-existent KPI', async () => {
    ...
    const res = await request(app).post('/api/kpis/nonexistent/calculate');
