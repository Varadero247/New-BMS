...
describe('GET /api/board-packs', () => {
  it('lists board packs newest first', async () => {
    (prisma.boardPack.findMany as jest.Mock).mockResolvedValue([{ id: 'bp-1', title: 'Q1 2026' }]);
...
describe('GET /api/board-packs/:id', () => {
  it('returns a single board pack with sections', async () => {
    ...
    const res = await request(app).get('/api/board-packs/bp-1');
    expect(res.status).toBe(200);
  ...
  it('returns 404 for missing board pack', async () => {
    ...
    const res = await request(app).get('/api/board-packs/missing');
...
describe('PATCH /api/board-packs/:id', () => {
  it('transitions DRAFT to FINAL', async () => {
    (prisma.boardPack.findUnique as jest.Mock).mockResolvedValue({ id: 'bp-1', status: 'DRAFT' });
    (prisma.boardPack.update as jest.Mock).mockResolvedValue({ id: 'bp-1', status: 'FINAL' });

    const res = await request(app).patch('/api/board-packs/bp-1').send({ status: 'FINAL' });
    expect(res.status).toBe(200);
    expect(res.body.data.status).toBe('FINAL');
  });

  it('transitions FINAL to DISTRIBUTED', async () => {
    (prisma.boardPack.findUnique as jest.Mock).mockResolvedValue({ id: 'bp-1', status: 'FINAL' });
    (prisma.boardPack.update as jest.Mock).mockResolvedValue({ id: 'bp-1', status: 'DISTRIBUTED' });

    const res = await request(app).patch('/api/board-packs/bp-1').send({ status: 'DISTRIBUTED' });
    expect(res.status).toBe(200);
