...
export default function HumanReviewPage() {
  ...
  const [decisionForm, setDecisionForm] = useState({ decision: 'APPROVED' as string, justification: '' });
  ...
  useEffect(() => { fetchReviews(); fetchPending(); }, [fetchReviews, fetchPending]);
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">ISO 42001:2023 — Human oversight of AI decisions</p>
      </div>
      ...
      {pendingReviews.length > 0 && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
          <h3 className="text-sm font-semibold text-yellow-800">
            {pendingReviews.length} review{pendingReviews.length > 1 ? 's' : ''} awaiting your decision
          </h3>
          <div className="mt-2 space-y-2">
            {pendingReviews.slice(0, 3).map(r => (
              <div key={r.id} className="flex items-center justify-between bg-white dark:bg-gray-900 rounded border p-3">
                <div>
                  <div className="font-medium text-sm">{r.title}</div>
                  <div className="text-xs text-gray-500 dark:text-gray-400">AI Decision: {r.aiDecision}{r.aiConfidence != null && ` (${Math.round(r.aiConfidence * 100)}%)`}</div>
                </div>
                <button
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {['PENDING', 'APPROVED', 'REJECTED', 'ESCALATED', 'EXPIRED'].map(status => (
          <div key={status} className="bg-white dark:bg-gray-900 rounded-lg border p-4">
            <div className="text-xl font-bold text-gray-900 dark:text-gray-100">{reviews.filter(r => r.status === status).length}</div>
      ...
      <div className="flex gap-3">
        ...
        >
          ...
          {['PENDING', 'APPROVED', 'REJECTED', 'ESCALATED', 'EXPIRED'].map(s => <option key={s} value={s}>{s}</option>)}
        </select>
      </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 dark:bg-gray-800 border-b">
              <tr>
                ...
                <th className="text-left p-3 font-medium text-gray-700 dark:text-gray-300">AI Decision</th>
                <th className="text-left p-3 font-medium text-gray-700 dark:text-gray-300">Confidence</th>
                <th className="text-left p-3 font-medium text-gray-700 dark:text-gray-300">Status</th>
                <th className="text-left p-3 font-medium text-gray-700 dark:text-gray-300">Reviewer</th>
            ...
            <tbody className="divide-y">
              {loading ? (
                ...
                <tr><td colSpan={6} className="p-8 text-center text-gray-500 dark:text-gray-400">No reviews found</td></tr>
              ) : reviews.map(r => (
      ...
      {/* Decision modal */}
      {activeReview && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setActiveReview(null)}>
          <div className="bg-white dark:bg-gray-900 rounded-lg shadow-xl max-w-lg w-full mx-4 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="p-6 space-y-4">
              ...
              <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4 space-y-2">
                ...
                <p className="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase mt-2">AI Decision</p>
                ...
                {activeReview.aiReasoning && (
                  <><p className="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase mt-2">AI Reasoning</p><p className="text-sm text-gray-600">{activeReview.aiReasoning}</p></>
                )}
              </div>

              <div className="space-y-3">
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Your Decision</label>
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button onClick={() => setActiveReview(null)} className="px-4 py-2 text-sm rounded-md border">Cancel</button>
                <button
