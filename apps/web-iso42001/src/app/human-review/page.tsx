...
export default function HumanReviewPage() {
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Human Review Gate</h1>
        <p className="text-sm text-gray-500 mt-1">ISO 42001:2023 — Human oversight of AI decisions</p>
      </div>
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
          ...
          <div className="mt-2 space-y-2">
            {pendingReviews.slice(0, 3).map(r => (
              <div key={r.id} className="flex items-center justify-between bg-white rounded border p-3">
                <div>
                  <div className="font-medium text-sm">{r.title}</div>
                  <div className="text-xs text-gray-500">AI Decision: {r.aiDecision}{r.aiConfidence != null && ` (${Math.round(r.aiConfidence * 100)}%)`}</div>
                </div>
                <button
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {['PENDING', 'APPROVED', 'REJECTED', 'ESCALATED', 'EXPIRED'].map(status => (
          <div key={status} className="bg-white rounded-lg border p-4">
            <div className="text-xl font-bold text-gray-900">{reviews.filter(r => r.status === status).length}</div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b">
              <tr>
                ...
                <th className="text-left p-3 font-medium text-gray-700">AI Decision</th>
                <th className="text-left p-3 font-medium text-gray-700">Confidence</th>
                <th className="text-left p-3 font-medium text-gray-700">Status</th>
                <th className="text-left p-3 font-medium text-gray-700">Reviewer</th>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="bg-gray-50 rounded-lg p-4 space-y-2">
                ...
                {activeReview.aiReasoning && (
                  <><p className="text-xs font-medium text-gray-400 uppercase mt-2">AI Reasoning</p><p className="text-sm text-gray-600">{activeReview.aiReasoning}</p></>
                )}
              </div>
