...
export default function TemplatesPage() {
  ...
  async function loadTemplates() {
    try {
      setError(null);
      const res = await gatewayApi.get('/templates?module=ISO42001');
    ...
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-1">ISO 42001 document templates and forms</p>
        </div>
        ...
        {filteredTemplates.length > 0 ? (
            ...
            {filteredTemplates.map((template) => (
              <div key={template.id} className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 hover:shadow-md transition-shadow">
                ...
                <div className="flex items-center justify-between text-xs text-gray-400 dark:text-gray-500 mb-4">
                  ...
                  {template.isBuiltIn && <span className="bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded-full">Built-in</span>}
                  {template.category && <span>{template.category}</span>}
                </div>
                <div className="flex gap-3">
                  <button onClick={() => viewTemplateDetails(template)} className="text-sm text-indigo-600 hover:text-indigo-700">View</button>
          ...
          <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-12 text-center text-gray-500 dark:text-gray-400">
            ...
            <p>No templates found for ISO 42001 module</p>
          </div>
        )}
      </div>
          ...
          <div className="flex items-center justify-center min-h-screen px-4">
            ...
            <div className="relative bg-white dark:bg-gray-900 rounded-xl shadow-xl max-w-2xl w-full p-6">
              ...
              <div className="flex justify-end gap-3 pt-4">
                ...
                <button onClick={() => handleExport(viewTemplate.id)} className="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">Export</button>
              </div>
            </div>
          </div>
        </div>
