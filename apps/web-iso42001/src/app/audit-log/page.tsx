    ...
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
          ISO 42001:2023 — Transparent record of all AI decisions and actions
        </p>
      </div>
              ...
              {loading ? (
                ...
                entries.map((entry) => (
                  <tr key={entry.id} className="hover:bg-gray-50 dark:bg-gray-800">
                    <td className="p-3 text-gray-500 dark:text-gray-400 text-xs font-mono whitespace-nowrap">
                      {new Date(entry.createdAt).toLocaleString()}
                    </td>
                    <td className="p-3">
                      <span
                        className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${actionColors[entry.action] || 'bg-gray-100 dark:bg-gray-800 text-gray-700'}`}
                      >
