...
const navGroups: NavGroup[] = [
  ...
  },
  {
    ...
    collapsible: true,
    items: [
      { name: 'AI Extraction', href: '/extraction', icon: Sparkles },
    ],
  },
];
          ...
          {group.items.map((item) => {
            ...
            return (
              <li key={item.name}>
                ...
                >
                  <Icon className={`h-4 w-4 shrink-0 ${isActive ? 'text-violet-600 dark:text-violet-400' : 'text-gray-400 dark:text-gray-500'}`} />
...
export default function Sidebar() {
  ...
  return (
    <aside className="w-64 bg-card border-r border-border flex flex-col h-full min-h-screen">
      <div className="p-4 border-b border-border bg-violet-50 dark:bg-violet-900/10">
        <h1 className="text-lg font-bold font-display text-violet-900 dark:text-violet-100">Contracts</h1>
        <p className="text-xs text-violet-600 dark:text-violet-400 mt-0.5">Contract Lifecycle Management</p>
      </div>
      <nav className="flex-1 p-3 overflow-y-auto" aria-label="Contracts module navigation">
        {navGroups.map((group) => (
