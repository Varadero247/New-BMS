...
const risks: Risk[] = [
  { id: 'R-001', title: 'Ransomware attack on production systems', description: 'Threat actors deploy ransomware via phishing or vulnerability exploitation', category: 'Cyber', owner: 'SOC Lead', likelihood: 4, impact: 5, residualLikelihood: 2, residualImpact: 4, controls: ['EDR', 'Email filtering', 'Backup strategy', 'Incident response plan'], treatmentPlan: 'Implement immutable backups, quarterly DR drills' },
  { id: 'R-002', title: 'Data breach — customer PII', description: 'Unauthorised access or exfiltration of customer personal data', category: 'Data', owner: 'DPO', likelihood: 3, impact: 5, residualLikelihood: 2, residualImpact: 4, controls: ['Access controls', 'DLP', 'Encryption at rest', 'Audit logging'], treatmentPlan: 'Deploy advanced DLP, data classification rollout Q2' },
  { id: 'R-003', title: 'Third-party supply chain compromise', description: 'Compromise of a critical vendor or software dependency', category: 'Supply Chain', owner: 'CISO', likelihood: 3, impact: 4, residualLikelihood: 2, residualImpact: 3, controls: ['Vendor risk assessments', 'SCA scanning', 'SBOM tracking'], treatmentPlan: 'Expand vendor assessment coverage to Tier 2 suppliers' },
  { id: 'R-004', title: 'Insider threat — data exfiltration', description: 'Malicious or negligent employee leaking sensitive information', category: 'Insider', owner: 'HR Manager', likelihood: 2, impact: 4, residualLikelihood: 1, residualImpact: 3, controls: ['DLP', 'USB restrictions', 'Privileged access monitoring'], treatmentPlan: 'Implement UEBA, annual insider threat training' },
  { id: 'R-005', title: 'Cloud misconfiguration', description: 'Publicly exposed storage buckets, overly permissive IAM policies', category: 'Cloud', owner: 'Cloud Architect', likelihood: 4, impact: 4, residualLikelihood: 2, residualImpact: 3, controls: ['CSPM tool', 'IaC scanning', 'Least privilege reviews'], treatmentPlan: 'Monthly CSPM reviews, automated remediation' },
  { id: 'R-006', title: 'Phishing — credential compromise', description: 'Social engineering attacks targeting employee credentials', category: 'Cyber', owner: 'SOC Lead', likelihood: 5, impact: 3, residualLikelihood: 3, residualImpact: 2, controls: ['MFA', 'Security awareness training', 'Email filtering', 'Phishing simulations'], treatmentPlan: 'Increase phishing simulation frequency, conditional access policies' },
  { id: 'R-007', title: 'DDoS attack on public services', description: 'Volumetric or application-layer denial of service', category: 'Cyber', owner: 'Infra Lead', likelihood: 3, impact: 3, residualLikelihood: 2, residualImpact: 2, controls: ['CDN/WAF', 'Rate limiting', 'DDoS mitigation service'], treatmentPlan: 'Annual DDoS readiness test' },
  { id: 'R-008', title: 'Loss of key personnel', description: 'Critical security staff leaving without adequate knowledge transfer', category: 'People', owner: 'CISO', likelihood: 3, impact: 3, residualLikelihood: 2, residualImpact: 2, controls: ['Cross-training', 'Documentation', 'Succession planning'], treatmentPlan: 'Update succession plan, ensure 2-deep coverage' },
  { id: 'R-009', title: 'Regulatory non-compliance — GDPR', description: 'Failure to meet GDPR obligations leading to fines or enforcement', category: 'Compliance', owner: 'DPO', likelihood: 2, impact: 5, residualLikelihood: 1, residualImpact: 4, controls: ['DPIA process', 'Data retention policy', 'Subject access request procedure'], treatmentPlan: 'Quarterly compliance reviews' },
  { id: 'R-010', title: 'Physical security breach', description: 'Unauthorised access to data centre or server room', category: 'Physical', owner: 'Facilities', likelihood: 2, impact: 4, residualLikelihood: 1, residualImpact: 3, controls: ['Biometric access', 'CCTV', '24/7 security guard'], treatmentPlan: 'Upgrade to man-trap entry system' },
  { id: 'R-011', title: 'API vulnerability exploitation', description: 'Exploitation of insecure APIs exposing backend systems', category: 'Cyber', owner: 'Dev Lead', likelihood: 4, impact: 3, residualLikelihood: 2, residualImpact: 2, controls: ['API gateway', 'Rate limiting', 'OWASP testing', 'Input validation'], treatmentPlan: 'Implement API security testing in CI/CD' },
  { id: 'R-012', title: 'Backup failure', description: 'Backup jobs fail silently, no recovery possible in disaster', category: 'Operations', owner: 'Infra Lead', likelihood: 2, impact: 5, residualLikelihood: 1, residualImpact: 3, controls: ['Backup monitoring', 'Quarterly restore tests', 'Geo-redundant storage'], treatmentPlan: 'Automated backup verification, monthly restore test' },
];
...
export default function HeatMapClient() {
  ...
  const criticalCount = risks.filter((r) => {
    const score = viewMode === 'inherent' ? r.likelihood * r.impact : r.residualLikelihood * r.residualImpact;
    return score >= 15;
  }).length;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl p-6">
          ...
          <div className="flex">
            ...
            <div className="flex flex-col items-center justify-center mr-2">
              <span className="text-xs text-gray-500 dark:text-gray-400 font-medium writing-mode-vertical" style={{ writingMode: 'vertical-rl', transform: 'rotate(180deg)' }}>
                Likelihood →
              </span>
            </div>
                  ...
                  {[1, 2, 3, 4, 5].map((impact) => {
                    ...
                    return (
                      <button
                        key={impact}
                        onClick={() => {
                          setSelectedCell(cellRisks.length > 0 ? { l: likelihood, i: impact } : null);
        ...
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl p-4 overflow-y-auto max-h-[600px]">
          <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">
            {selectedCell ? `Risks at L${selectedCell.l} × I${selectedCell.i}` : selectedRisk ? 'Risk Details' : 'Select a cell'}
          </h2>
          {selectedCell && (
          ...
          {selectedRisk && (
            <div className="space-y-4">
              <button onClick={() => setSelectedRisk(null)} className="text-xs text-indigo-600 hover:underline">← Back</button>
              <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-red-50 rounded-lg p-3">
                  ...
                  <p className="text-lg font-bold text-red-700">{selectedRisk.likelihood * selectedRisk.impact}</p>
                  <p className="text-xs text-red-500">L{selectedRisk.likelihood} × I{selectedRisk.impact}</p>
                </div>
                <div className="bg-green-50 rounded-lg p-3">
                  <p className="text-xs text-green-600 font-medium">Residual</p>
                  <p className="text-lg font-bold text-green-700">{selectedRisk.residualLikelihood * selectedRisk.residualImpact}</p>
                  <p className="text-xs text-green-500">L{selectedRisk.residualLikelihood} × I{selectedRisk.residualImpact}</p>
                </div>
              </div>
              <div>
