    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">ISO 27001:2022 Annex A controls — applicability and implementation status</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          <p className="text-xs text-gray-500 uppercase font-medium">Total Controls</p>
          <p className="text-3xl font-bold text-gray-900 mt-1">{controls.length}</p>
        </div>
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          <p className="text-xs text-gray-500 uppercase font-medium">Applicable</p>
          <p className="text-3xl font-bold text-indigo-700 mt-1">{applicable}</p>
          <p className="text-xs text-gray-400">{notApplicable} excluded</p>
        </div>
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          <p className="text-xs text-gray-500 uppercase font-medium">Fully Implemented</p>
          <p className="text-3xl font-bold text-emerald-700 mt-1">{fullyImpl}</p>
        </div>
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          <p className="text-xs text-gray-500 uppercase font-medium">Partial / In Progress</p>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50">
              <tr>
                <th className="text-left px-4 py-3 font-medium text-gray-500 w-20">Clause</th>
                <th className="text-left px-4 py-3 font-medium text-gray-500">Control Title</th>
                <th className="text-center px-4 py-3 font-medium text-gray-500 w-24">Applicable</th>
                <th className="text-left px-4 py-3 font-medium text-gray-500 w-48">Justification</th>
                <th className="text-center px-4 py-3 font-medium text-gray-500 w-36">Status</th>
                <th className="text-left px-4 py-3 font-medium text-gray-500 w-24">Evidence</th>
                <th className="text-left px-4 py-3 font-medium text-gray-500 w-20">Risk Ref</th>
                <th className="text-left px-4 py-3 font-medium text-gray-500 w-24">Owner</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((c) => {
                ...
                return (
                  <tr key={c.id} className="border-t border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2.5 text-xs text-gray-500 max-w-[12rem] truncate">{c.justification}</td>
                    <td className="px-4 py-2.5 text-center">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${st.color}`}>{st.label}</span>
                    </td>
                    <td className="px-4 py-2.5 font-mono text-xs text-gray-500">{c.evidenceRef || '—'}</td>
                    <td className="px-4 py-2.5 font-mono text-xs text-gray-500">{c.riskRef || '—'}</td>
