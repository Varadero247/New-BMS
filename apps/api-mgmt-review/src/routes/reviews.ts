...
async function generateRef(orgId: string): Promise<string> {
  ...
  const c = await prisma.mgmtReview.count({
    where: { orgId, referenceNumber: { startsWith: `MGR-${y}` } },
  });
  return `MGR-${y}-${String(c + 1).padStart(4, '0')}`;
}
router.get('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const [data, total] = await Promise.all([
      prisma.mgmtReview.findMany({
        where,
        skip,
        take: Math.min(Math.max(1, parseInt(limit, 10) || 20), 100),
        orderBy: { createdAt: 'desc' },
      }),
      prisma.mgmtReview.count({ where }),
    ]);
    res.json({
      success: true,
      data,
    ...
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch management reviews' },
    });
  }
});
router.get('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    const orgId = ((req as AuthRequest).user as any)?.orgId || 'default';
    const item = await prisma.mgmtReview.findFirst({
    ...
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch management review' },
    });
  }
});
router.post('/', authenticate, async (req: Request, res: Response) => {
  try {
    const parsed = createMgmtReviewSchema.safeParse(req.body);
    if (!parsed.success)
    ...
    const data = await prisma.mgmtReview.create({
      data: {
        title,
        description,
        status,
...
router.put('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    const parsed = updateMgmtReviewSchema.safeParse(req.body);
    if (!parsed.success)
      return res.status(400).json({
        success: false,
        error: { code: 'VALIDATION_ERROR', message: parsed.error.errors[0].message },
    ...
    const orgId = ((req as AuthRequest).user as any)?.orgId || 'default';
    const existing = await prisma.mgmtReview.findFirst({
      where: { id: req.params.id, orgId, deletedAt: null } as any,
    });
    if (!existing)
      return res.status(404).json({
    ...
    const data = await prisma.mgmtReview.update({
...
router.delete('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    res.json({ success: true, data: { message: 'management review deleted successfully' } });
  } catch (error: unknown) {
    logger.error('Failed to process request', { error: (error as Error).message });
    res.status(500).json({
      success: false,
