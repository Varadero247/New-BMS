...
dotenv.config();
initSentry('api-mgmt-review');
initTracing({ serviceName: 'api-mgmt-review' });
...
app.use(metricsMiddleware('api-mgmt-review'));
...
app.get('/health', createHealthCheck('api-mgmt-review', prisma, '1.0.0'));
...
const server = app.listen(PORT, () => {
  logger.info(`Management Review API running on port ${PORT}`);
});
