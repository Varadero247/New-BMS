...
import { ClipboardCheck, AlertCircle, CalendarRange, TrendingUp } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const kpis = [
    { label: 'Total Audits', value: stats.totalAudits ?? 0, icon: ClipboardCheck, color: 'emerald' },
    { label: 'Open Findings', value: stats.totalFindings ?? 0, icon: AlertCircle, color: 'amber' },
    { label: 'Checklists', value: stats.totalChecklists ?? 0, icon: CalendarRange, color: 'blue' },
    { label: 'Completion Rate', value: stats.totalAudits > 0 ? Math.round(((stats.completedAudits ?? 0) / stats.totalAudits) * 100) + '%' : '-', icon: TrendingUp, color: 'green' },
  ];
    ...
    <div className="flex min-h-screen">
      ...
      <main className="flex-1 p-8">
        <div className="max-w-7xl mx-auto">
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">Audits Dashboard</h1>
            <p className="text-gray-500 dark:text-gray-400 mt-1">Audit Programme Management Overview</p>
          </div>
          ...
          {Object.keys(stats).length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {Object.entries(stats).filter(([k]) => !['totalAudits', 'totalFindings', 'totalChecklists', 'completedAudits'].includes(k)).map(([key, value]) => (
