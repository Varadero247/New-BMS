    ...
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">Anti-Bribery Policies</h1>
            <p className="text-gray-500 dark:text-gray-400 mt-1">Manage anti-bribery policies per ISO 37001</p>
          </div>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="space-y-3">
            {filtered.map((policy) => (
              <div key={policy.id} className={`bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden ${policy.status === 'ARCHIVED' ? 'opacity-60' : ''}`}>
                <div className="px-6 py-4 flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-1">
                      <h3 className="text-base font-semibold text-gray-900 dark:text-gray-100">{policy.title}</h3>
                      <span className={`inline-flex px-2 py-0.5 text-xs font-medium rounded-full ${categoryColors[policy.category] || 'bg-gray-100 dark:bg-gray-800 text-gray-700'}`}>
                        {categoryLabels[policy.category] || policy.category}
                      </span>
                      <span className={`inline-flex px-2 py-0.5 text-xs font-medium rounded-full ${statusColors[policy.status] || 'bg-gray-100 dark:bg-gray-800 text-gray-700'}`}>
                        {policy.status}
                      </span>
                    </div>
          ...
          <div className="flex items-center justify-center min-h-screen px-4">
            ...
            <div className="relative bg-white dark:bg-gray-900 rounded-xl shadow-xl max-w-2xl w-full p-6 max-h-[90vh] overflow-y-auto">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100">{editingPolicy ? 'Edit Policy' : 'Add Policy'}</h2>
