    ...
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-gray-500 mt-1">ISO 37001 document templates and forms</p>
        </div>
            ...
            {filteredTemplates.map((template) => (
              <div key={template.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${statusColors[template.status] || 'bg-gray-100 text-gray-700'}`}>
                    {template.status}
                  </span>
                </div>
                ...
                <div className="flex gap-3">
                  ...
                  <button onClick={() => handleClone(template.id)} className="text-sm text-gray-600 hover:text-gray-700">Clone</button>
                  <button onClick={() => handleExport(template.id)} className="text-sm text-gray-600 hover:text-gray-700">Export</button>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="flex items-center justify-center min-h-screen px-4">
            ...
            <div className="relative bg-white rounded-xl shadow-xl max-w-2xl w-full p-6">
              <h2 className="text-xl font-bold text-gray-900 mb-4">{viewTemplate.name}</h2>
              ...
              <div className="flex justify-end gap-3 pt-4">
                <button onClick={() => setViewModalOpen(false)} className="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">Close</button>
