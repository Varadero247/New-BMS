    ...
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-1">ISO 37001 document templates and forms</p>
        </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 mb-6 p-4">
          <div className="relative">
            ...
            <input type="text" aria-label="Search templates..." placeholder="Search templates..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-rose-500" />
          </div>
        </div>
        ...
        {filteredTemplates.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredTemplates.map((template) => (
              <div key={template.id} className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${statusColors[template.status] || 'bg-gray-100 dark:bg-gray-800 text-gray-700'}`}>
                    {template.status}
                  </span>
                </div>
                ...
                <div className="flex items-center justify-between text-xs text-gray-400 dark:text-gray-500 mb-4">
                  ...
                  {template.isBuiltIn && <span className="bg-rose-50 text-rose-600 px-2 py-0.5 rounded-full">Built-in</span>}
                  {template.category && <span>{template.category}</span>}
                </div>
                <div className="flex gap-3">
                  <button onClick={() => viewTemplateDetails(template)} className="text-sm text-rose-600 hover:text-rose-700">View</button>
                  <button onClick={() => handleClone(template.id)} className="text-sm text-gray-600 hover:text-gray-700 dark:text-gray-300">Clone</button>
                  <button onClick={() => handleExport(template.id)} className="text-sm text-gray-600 hover:text-gray-700 dark:text-gray-300">Export</button>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="flex items-center justify-center min-h-screen px-4">
            ...
            <div className="relative bg-white dark:bg-gray-900 rounded-xl shadow-xl max-w-2xl w-full p-6">
              <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-4">{viewTemplate.name}</h2>
              ...
              <div className="flex justify-end gap-3 pt-4">
                <button onClick={() => setViewModalOpen(false)} className="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:bg-gray-800">Close</button>
                <button onClick={() => handleExport(viewTemplate.id)} className="px-4 py-2 bg-rose-600 text-white rounded-lg hover:bg-rose-700">Export</button>
              </div>
            </div>
          </div>
        </div>
