...
export default function MatrixClient() {
  ...
  function openEdit(entry: MatrixEntry) {
    setForm({
      ...
      currentLevel: entry.currentLevel || 'NOT_STARTED',
      targetLevel: entry.targetLevel || 'COMPETENT',
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      console.error('Failed to save matrix entry:', err);
    } finally {
      setSaving(false);
    }
  }
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">
              Employee competency tracking and gap analysis
            </p>
          </div>
          ...
          <Card>
            <CardContent className="pt-6 text-center">
              <p className="text-3xl font-bold text-red-600">
                {entries.filter((e) => e.gap).length}
              </p>
              <p className="text-sm text-gray-500 dark:text-gray-400">With Gaps</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="pt-6 text-center">
              <p className="text-3xl font-bold text-green-600">
                {
                  entries.filter(
                    (e) => e.currentLevel === 'COMPETENT' || e.currentLevel === 'EXPERT'
                  ).length
                }
              </p>
          ...
          <Card>
            <CardContent className="pt-6 text-center">
              <p className="text-3xl font-bold text-amber-600">
                {entries.filter((e) => e.currentLevel === 'DEVELOPING').length}
              </p>
              <p className="text-sm text-gray-500 dark:text-gray-400">Developing</p>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="flex gap-4 mb-6 flex-wrap items-center">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              type="text"
              aria-label="Search matrix entries"
                  ...
                  <TableBody>
                    {entries.map((entry) => (
                      <TableRow key={entry.id}>
                        ...
                        <TableCell>
                          <span
                            className={`px-2 py-1 rounded-full text-xs font-medium ${getLevelColor(entry.currentLevel)}`}
                          >
                            {entry.currentLevel?.replace(/_/g, ' ') || '-'}
                          </span>
                        </TableCell>
                        <TableCell>
                          <span
                            className={`px-2 py-1 rounded-full text-xs font-medium ${getLevelColor(entry.targetLevel)}`}
                          >
                            {entry.targetLevel?.replace(/_/g, ' ') || '-'}
                          </span>
                        </TableCell>
                        <TableCell>
                          <Badge variant={entry.gap ? 'default' : 'secondary'}>
                            {entry.gap ? 'Yes' : 'No'}
                          </Badge>
                        </TableCell>
        ...
        {modalOpen && (
          <Modal
            ...
            title={editId ? 'Edit Matrix Entry' : 'Add Matrix Entry'}
            size="lg"
          >
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setForm((p) => ({ ...p, competencyId: e.target.value }))}
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setForm((p) => ({ ...p, assessedBy: e.target.value }))}
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  id="hasGap"
                  checked={form.gap}
                  onChange={(e) => setForm((p) => ({ ...p, gap: e.target.checked }))}
            ...
            <ModalFooter>
              ...
              <Button
                ...
                disabled={saving || !form.competencyId || !form.employeeId}
              >
                {saving ? (
                  <>
