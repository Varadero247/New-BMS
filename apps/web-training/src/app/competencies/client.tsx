      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">
              Define and manage competency requirements
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <Card>
            <CardContent className="pt-6 text-center">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">Total Competencies</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="pt-6 text-center">
              <p className="text-3xl font-bold text-purple-600">
                {competencies.filter((c) => c.requiredLevel === 'EXPERT').length}
              </p>
          ...
          <Card>
            <CardContent className="pt-6 text-center">
              <p className="text-3xl font-bold text-green-600">
                {competencies.filter((c) => c.requiredLevel === 'COMPETENT').length}
              </p>
              <p className="text-sm text-gray-500 dark:text-gray-400">Competent Level</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="pt-6 text-center">
              <p className="text-3xl font-bold text-green-600">
                {competencies.filter((c) => c.isActive !== false).length}
              </p>
                  ...
                  <TableBody>
                    {competencies.map((comp) => (
                      <TableRow key={comp.id}>
                        ...
                        <TableCell>
                          <span
                            className={`px-2 py-1 rounded-full text-xs font-medium ${getLevelColor(comp.requiredLevel)}`}
                          >
                            {comp.requiredLevel?.replace(/_/g, ' ') || '-'}
                          </span>
                        </TableCell>
                        <TableCell>
        ...
        {modalOpen && (
          <Modal
            ...
            title={editId ? 'Edit Competency' : 'Add Competency'}
            size="lg"
          >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Select
                    ...
                    onChange={(e) => setForm((p) => ({ ...p, requiredLevel: e.target.value }))}
                  >
                    {LEVELS.map((l) => (
                      <option key={l} value={l}>
                        {l.replace(/_/g, ' ')}
