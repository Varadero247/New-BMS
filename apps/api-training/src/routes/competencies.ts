...
async function generateRef(orgId: string): Promise<string> { const y = new Date().getFullYear(); const c = await prisma.trainCompetency.count({ where: { orgId, referenceNumber: { startsWith: `CMP-${y}` } } as any }); return `CMP-${y}-${String(c + 1).padStart(4, '0')}`; }
