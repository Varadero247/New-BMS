...
router.post('/', authenticate, async (req: Request, res: Response) => { try { const parsed = createSchema.safeParse(req.body); if (!parsed.success) return res.status(400).json({ success: false, error: { code: 'VALIDATION_ERROR', message: parsed.error.errors[0].message } }); const orgId = (req as AuthRequest).user?.orgId || 'default'; const referenceNumber = await generateRef(orgId); const { courseId, employeeId, employeeName, status, scheduledDate, completedDate, expiryDate, score, passed, trainer, trainerName, location, certificateUrl, feedback, notes } = parsed.data; const data = await prisma.trainRecord.create({ data: { courseId, employeeId, employeeName, status, scheduledDate, completedDate, expiryDate, score, passed, trainer, trainerName, location, certificateUrl, feedback, notes, orgId, referenceNumber, createdBy: (req as AuthRequest).user?.id, updatedBy: (req as AuthRequest).user?.id } }); res.status(201).json({ success: true, data }); } catch (error: unknown) { res.status(400).json({ success: false, error: { code: 'CREATE_ERROR', message: (error as Error).message } }); } });
