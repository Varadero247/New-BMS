...
dotenv.config();
initSentry('api-training');
initTracing({ serviceName: 'api-training' });
...
const logger = createLogger('api-training');

import recordsRouter from './routes/records';
import coursesRouter from './routes/courses';
import competenciesRouter from './routes/competencies';
import matrixRouter from './routes/matrix';
import tnaRouter from './routes/tna';
import dashboardRouter from './routes/dashboard';
import inductionsRouter from './routes/inductions';
...
app.use(metricsMiddleware('api-training'));
...
app.get('/health', createHealthCheck('api-training', prisma, '1.0.0'));
...
const server = app.listen(PORT, () => {
  logger.info(`Training API server running on port ${PORT}`);
});
