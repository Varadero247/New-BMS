...
router.get('/dashboard', async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to get monitoring dashboard' },
    });
  }
});
...
router.get('/system/:systemId', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to get system monitoring', {
      error: error instanceof Error ? error.message : 'Unknown error',
    });
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to get system monitoring' },
    });
  }
});
...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to list monitoring records', {
      error: error instanceof Error ? error.message : 'Unknown error',
    });
    res.status(500).json({
      success: false,
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to create monitoring record', {
      error: error instanceof Error ? error.message : 'Unknown error',
    });
    res.status(500).json({
      success: false,
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    const record = await prisma.aiMonitoring.findFirst({
    ...
    logger.error('Failed to get monitoring record', {
      error: error instanceof Error ? error.message : 'Unknown error',
    });
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to get monitoring record' },
    });
  }
});
...
router.put('/:id', async (req: Request, res: Response) => {
  try {
    ...
    const existing = await prisma.aiMonitoring.findFirst({
      where: { id: req.params.id, deletedAt: null } as any,
    });
    if (!existing)
      return res.status(404).json({
...
router.delete('/:id', async (req: Request, res: Response) => {
  try {
    ...
    await prisma.aiMonitoring.update({
      where: { id: req.params.id },
      data: { deletedAt: new Date() },
    });
    res.json({ success: true, data: { id: req.params.id, deleted: true } });
  } catch (error: unknown) {
    logger.error('Failed to delete monitoring record', {
      error: error instanceof Error ? error.message : 'Unknown error',
    });
    res.status(500).json({
      success: false,
