      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">
              Track document distribution and acknowledgement
            </p>
          </div>
          ...
          <Card>
            <CardContent className="pt-6 text-center">
              <p className="text-3xl font-bold text-gray-600">
                {receipts.filter((r) => r.status === 'UNREAD').length}
              </p>
          ...
          <Card>
            <CardContent className="pt-6 text-center">
              <p className="text-3xl font-bold text-blue-600">
                {receipts.filter((r) => r.status === 'READ').length}
              </p>
                    ...
                    {receipts.map((receipt) => (
                      <TableRow key={receipt.id}>
                        <TableCell className="font-mono text-xs">
                          {receipt.documentId?.slice(0, 8)}...
                        </TableCell>
                        <TableCell className="font-medium">
                          {receipt.userName || receipt.userId?.slice(0, 8)}
                        </TableCell>
                        <TableCell>
                          <span
                            className={`px-2 py-1 rounded-full text-xs font-medium ${getReceiptStatusColor(receipt.status)}`}
                          >
                            {receipt.status}
                          </span>
                        </TableCell>
                        <TableCell className="text-sm">
                          {receipt.readAt ? new Date(receipt.readAt).toLocaleDateString() : '-'}
                        </TableCell>
                        <TableCell className="text-sm">
                          {receipt.acknowledgedAt
                            ? new Date(receipt.acknowledgedAt).toLocaleDateString()
                            : '-'}
                        </TableCell>
                        <TableCell className="text-sm">
                          {receipt.createdAt
                        ...
                        <TableCell>
                          <div className="flex gap-2">
                            <Button size="sm" variant="outline" onClick={() => openEdit(receipt)}>
                              Edit
                            </Button>
                            <Button
                              size="sm"
        ...
        {modalOpen && (
          <Modal
            ...
            title={editId ? 'Edit Read Receipt' : 'Add Read Receipt'}
            size="lg"
          >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    type="date"
                    value={form.readAt}
                    onChange={(e) => setForm((p) => ({ ...p, readAt: e.target.value }))}
                  />
                </div>
                <div>
