...
import { FileText, GitBranch, CheckCircle2, BookOpen } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const kpis = [
    { label: 'Total Documents', value: stats.totalDocuments ?? 0, icon: FileText, color: 'indigo' },
    { label: 'Total Versions', value: stats.totalVersions ?? 0, icon: GitBranch, color: 'blue' },
    { label: 'Pending Approvals', value: stats.pendingApprovals ?? 0, icon: CheckCircle2, color: 'amber' },
    { label: 'Read Receipts', value: stats.totalReadReceipts ?? 0, icon: BookOpen, color: 'green' },
  ];
    ...
    <div className="flex min-h-screen">
      ...
      <main className="flex-1 p-8">
        <div className="max-w-7xl mx-auto">
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">Document Control Dashboard</h1>
            <p className="text-gray-500 dark:text-gray-400 mt-1">ISO 9001 Document Management Overview</p>
          </div>
          ...
          {Object.keys(stats).length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {Object.entries(stats).filter(([k]) => !['totalDocuments', 'totalVersions', 'pendingApprovals', 'totalReadReceipts'].includes(k)).map(([key, value]) => (
