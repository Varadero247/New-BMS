...
import { Search, CheckCircle, XCircle, Clock, AlertTriangle, FlaskConical, ChevronDown, ChevronRight } from 'lucide-react';
...
interface VerificationTest {
  ...
  testName: string;
  product: string;
  category: 'electrical-safety' | 'biocompatibility' | 'software' | 'mechanical' | 'emc' | 'environmental' | 'sterility';
  standard: string;
...
const tests: VerificationTest[] = [
  { id: '1', testId: 'VER-CMX3-001', testName: 'Electrical Safety — Leakage Current', product: 'CardioMonitor Pro X3', category: 'electrical-safety', standard: 'IEC 60601-1:2005+A2', result: 'pass', testDate: '2026-01-12', tester: 'E. Rodriguez', acceptanceCriteria: '≤ 500µA patient leakage (normal)', actualResult: '120µA measured', deviations: '' },
  { id: '2', testId: 'VER-CMX3-002', testName: 'EMC — Radiated Emissions', product: 'CardioMonitor Pro X3', category: 'emc', standard: 'IEC 60601-1-2:2014+A1', result: 'pass', testDate: '2026-01-15', tester: 'External Lab — TUV', acceptanceCriteria: 'Class B limits per CISPR 11', actualResult: 'Within limits at all frequencies', deviations: '' },
  { id: '3', testId: 'VER-CMX3-003', testName: 'Biocompatibility — Cytotoxicity', product: 'CardioMonitor Pro X3', category: 'biocompatibility', standard: 'ISO 10993-5:2009', result: 'pass', testDate: '2025-12-20', tester: 'External Lab — Nelson', acceptanceCriteria: 'Grade 0-1 reactivity', actualResult: 'Grade 0 — no reactivity', deviations: '' },
  { id: '4', testId: 'VER-NSV2-001', testName: 'Software Unit Testing Coverage', product: 'NeuroStim Controller V2', category: 'software', standard: 'IEC 62304:2006+A1', result: 'conditional', testDate: '2026-02-01', tester: 'J. Wilson', acceptanceCriteria: '≥ 95% MC/DC coverage for Class C', actualResult: '92% coverage — 3 modules below target', deviations: 'DEV-2026-004: Coverage gap in alarm module' },
  { id: '5', testId: 'VER-NSV2-002', testName: 'Software Integration Testing', product: 'NeuroStim Controller V2', category: 'software', standard: 'IEC 62304:2006+A1', result: 'pending', testDate: '2026-02-15', tester: 'J. Wilson', acceptanceCriteria: 'All integration tests pass, no P1/P2 defects open', actualResult: 'Scheduled', deviations: '' },
  { id: '6', testId: 'VER-SVE-001', testName: 'Mechanical Strength — Drop Test', product: 'SurgiView Endoscope', category: 'mechanical', standard: 'IEC 60601-1 Clause 15', result: 'pass', testDate: '2026-01-08', tester: 'R. Kim', acceptanceCriteria: '1m drop, 3 orientations, no functional failure', actualResult: 'All 3 drops passed, minor cosmetic mark only', deviations: '' },
  { id: '7', testId: 'VER-SVE-002', testName: 'Environmental — Temperature Cycling', product: 'SurgiView Endoscope', category: 'environmental', standard: 'IEC 60068-2-14', result: 'pass', testDate: '2026-01-10', tester: 'R. Kim', acceptanceCriteria: '-20°C to +55°C, 10 cycles, functional after', actualResult: 'All 10 cycles completed, full functionality confirmed', deviations: '' },
  { id: '8', testId: 'VER-OFK-001', testName: 'Sterility Assurance — Bioburden', product: 'OrthoFix Implant Kit', category: 'sterility', standard: 'ISO 11737-1:2018', result: 'pass', testDate: '2025-10-05', tester: 'External Lab — Nelson', acceptanceCriteria: '< 1000 CFU per device', actualResult: '12 CFU measured (3 samples)', deviations: '' },
  { id: '9', testId: 'VER-DSP-001', testName: 'Electrical Safety — Dielectric Strength', product: 'DiagnosScan Portable', category: 'electrical-safety', standard: 'IEC 60601-1:2005+A2', result: 'fail', testDate: '2026-02-05', tester: 'E. Rodriguez', acceptanceCriteria: '4000V AC, 1 min, no breakdown', actualResult: 'Breakdown at 3200V — insulation failure', deviations: 'NCR-2026-012: Insulation material non-conformance' },
];
...
const categoryLabels: Record<string, string> = {
  'electrical-safety': 'Electrical Safety', biocompatibility: 'Biocompatibility', software: 'Software', mechanical: 'Mechanical', emc: 'EMC', environmental: 'Environmental', sterility: 'Sterility',
};
...
export default function VerificationClient() {
  ...
  const passRate = Math.round((passCount / tests.filter((t) => t.result !== 'pending').length) * 100);

  const filtered = useMemo(() => {
    return tests.filter((t) => {
      const matchesSearch = !search || t.testName.toLowerCase().includes(search.toLowerCase()) || t.testId.toLowerCase().includes(search.toLowerCase()) || t.product.toLowerCase().includes(search.toLowerCase());
