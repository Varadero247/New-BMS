...
const records: TraceabilityRecord[] = [
  {
    id: '1',
    udiNumber: '(01)00884838000018(21)SN20260115001',
    lotNumber: 'LOT-2026-0115',
    product: 'CardioMonitor Pro X3',
    serialNumber: 'CM-X3-0001',
    manufacturingDate: '2026-01-15',
    expiryDate: '2029-01-15',
    status: 'distributed',
    currentLocation: 'St. James Hospital, London',
    chain: [
      {
        date: '2026-01-15',
    ...
    ] },
  {
    ...
    lotNumber: 'LOT-2026-0201',
    product: 'SurgiView Endoscope',
    serialNumber: 'SV-END-0198',
    manufacturingDate: '2026-02-01',
    expiryDate: '2031-02-01',
    status: 'in-production',
    currentLocation: 'Plant A, Line 5',
    chain: [
      { date: '2026-02-01', event: 'Assembly started', location: 'Plant A, Line 5', by: 'System' },
      {
        date: '2026-02-05',
    ...
    ] },
  {
    ...
    serialNumber: 'OF-IK-0567',
    manufacturingDate: '2025-10-05',
    expiryDate: '2028-10-05',
    status: 'implanted',
    currentLocation: 'Royal London Hospital',
    chain: [
      {
        date: '2025-10-05',
        event: 'Manufacturing complete',
    ...
    ] },
  {
    ...
    lotNumber: 'LOT-2026-0115B',
    product: 'VitalSign Monitor M5',
    serialNumber: 'VS-M5-1234',
    manufacturingDate: '2026-01-15',
    expiryDate: '2029-01-15',
    status: 'recalled',
    currentLocation: 'Recall Processing',
    chain: [
      {
        date: '2026-01-15',
        event: 'Manufacturing complete',
  ...
  const filtered = useMemo(() => {
    return records.filter((r) => {
      const matchesSearch =
        ...
        r.udiNumber.toLowerCase().includes(search.toLowerCase());
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      <div>
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
          UDI tracking and chain of custody — FDA 21 CFR 830 / EU MDR 2017/745
        </p>
      </div>
      ...
      <div className="flex items-center gap-3">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            type="text"
            aria-label="Search by UDI, lot, serial, or product..."
            placeholder="Search by UDI, lot, serial, or product..."
            value={search}
          ...
          return (
            ...
            >
              <button
                ...
                className="w-full text-left px-4 py-3 hover:bg-gray-50 dark:bg-gray-800"
              >
              ...
              {isExpanded && (
                <div className="px-4 py-3 bg-gray-50 dark:bg-gray-800 border-t border-gray-100 dark:border-gray-700 space-y-3">
                  <div className="text-xs text-gray-500 dark:text-gray-400 font-mono break-all">
                    UDI: {record.udiNumber}
                  </div>
                  <div className="grid grid-cols-3 gap-3 text-xs">
                    <div>
                      <span className="text-gray-500 dark:text-gray-400">Manufactured:</span>{' '}
                      <span className="font-medium">{record.manufacturingDate}</span>
                    </div>
                    <div>
                      <span className="text-gray-500 dark:text-gray-400">Expires:</span>{' '}
                      <span className="font-medium">{record.expiryDate}</span>
                    </div>
                    <div>
                      <span className="text-gray-500 dark:text-gray-400">Events:</span>{' '}
                      <span className="font-medium">{record.chain.length}</span>
                    </div>
                  </div>
                    ...
                    {record.chain.map((event, i) => (
                      <div key={i} className="flex items-start gap-3 relative">
                        <div className="flex flex-col items-center">
                          <div
                            className={`w-2.5 h-2.5 rounded-full ${i === record.chain.length - 1 ? 'bg-blue-500' : 'bg-gray-300'}`}
                          />
