...
const capas: CAPA[] = [
  ...
  },
  {
    ...
    title: 'Labelling non-conformance — missing UDI on packaging',
    type: 'corrective',
    source: 'ncr',
    severity: 'major',
    status: 'closed',
    product: 'SurgiView Endoscope',
    rootCause: 'Label template not updated after UDI regulation change',
  ...
  },
  {
    ...
    title: 'Sterilization process deviation — EtO residuals above limit',
    type: 'corrective',
    source: 'ncr',
    severity: 'critical',
    status: 'investigation',
    product: 'OrthoFix Implant Kit',
    rootCause: 'Under investigation — aeration cycle parameters suspect',
  ...
  },
  {
    ...
    title: 'Risk management file incomplete for design transfer',
    type: 'preventive',
  ...
  },
  {
    ...
    title: 'Customer complaint — intermittent display failure',
    type: 'corrective',
    source: 'complaint',
    severity: 'major',
    status: 'closed',
    product: 'VitalSign Monitor M5',
    rootCause: 'Solder joint fatigue due to thermal cycling',
    owner: 'Robert Kim',
...
export default function CAPAClient() {
  ...
  const overdue = capas.filter(
    (c) => c.status !== 'closed' && new Date(c.targetDate) < new Date('2026-02-13')
  ).length;
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      <div>
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
          Corrective and Preventive Actions — FDA 21 CFR 820.90 / ISO 13485:2016
        </p>
      </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span
                          className={`px-2 py-0.5 rounded-full text-xs font-medium ${sev.color}`}
                        >
                          {capa.severity}
                        </span>
                        <span
                          className={`px-2 py-0.5 rounded-full text-xs font-medium ${capa.type === 'corrective' ? 'bg-red-50 text-red-600' : 'bg-blue-50 text-blue-600'}`}
                        >
                ...
                <div className="flex items-center gap-0.5 mt-2 ml-7">
                  {steps.map((s, i) => (
                    <div key={s} className="flex-1">
                      <div
                        className={`h-1.5 rounded-full ${i + 1 <= sc.step ? 'bg-blue-500' : 'bg-gray-200'}`}
                      />
                    </div>
                  ))}
                </div>
              ...
              {isExpanded && (
                <div className="px-4 py-3 bg-gray-50 dark:bg-gray-800 border-t border-gray-100 dark:border-gray-700 ml-7 space-y-2">
                  ...
                  <div className="text-xs">
                    ...
                    <span className="text-gray-700 dark:text-gray-300">{capa.rootCause}</span>
                  </div>
