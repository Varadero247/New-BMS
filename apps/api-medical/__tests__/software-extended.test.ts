...
jest.mock('../src/prisma', () => ({
  prisma: {
    softwareProject: { findMany: jest.fn(), findUnique: jest.fn(), create: jest.fn(), update: jest.fn(), count: jest.fn() },
...
describe('Software Validation Routes (IEC 62304)', () => {
  ...
  describe('POST /api/software/projects', () => {
    ...
    it('should return 400 for missing title', async () => {
      const res = await request(app).post('/api/software/projects').send({ safetyClass: 'CLASS_A' });
    ...
    it('should return 400 for invalid safetyClass', async () => {
      const res = await request(app).post('/api/software/projects').send({
        title: 'Test', safetyClass: 'INVALID',
      });
      expect(res.status).toBe(400);
    });
    ...
    it('should accept CLASS_A', async () => {
      ...
      const res = await request(app).post('/api/software/projects').send({
        title: 'Dashboard', safetyClass: 'CLASS_A',
      });
      expect(res.status).toBe(201);
    });
    ...
    it('should accept CLASS_B', async () => {
      ...
      const res = await request(app).post('/api/software/projects').send({
        title: 'Monitor', safetyClass: 'CLASS_B',
      });
      expect(res.status).toBe(201);
    });
