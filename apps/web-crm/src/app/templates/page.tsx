...
export default function TemplatesPage() {
  ...
  async function loadTemplates() {
    try {
      setError(null);
      const res = await gatewayApi.get('/templates?module=CRM');
    ...
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-1">CRM document templates and forms</p>
        </div>
        ...
        <Card className="mb-6">
          <CardContent className="pt-6">
            <div className="relative">
              ...
              <input type="text" aria-label="Search templates..." placeholder="Search templates..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-violet-500" />
            </div>
          </CardContent>
        </Card>
              ...
              <Card key={template.id} className="hover:shadow-md transition-shadow">
                <CardContent className="pt-6">
                  ...
                  <div className="flex items-center justify-between text-xs text-gray-400 dark:text-gray-500 mb-4">
                    ...
                    {template.isBuiltIn && <Badge className="bg-violet-50 text-violet-600">Built-in</Badge>}
                    {template.category && <span>{template.category}</span>}
                  </div>
                  <div className="flex gap-2">
                    <button onClick={() => viewTemplateDetails(template)} className="flex items-center gap-1 text-sm text-violet-600 hover:text-violet-700">
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="text-center py-12 text-gray-500 dark:text-gray-400">
                ...
                <p>No templates found for CRM module</p>
              </div>
            </CardContent>
          </Card>
        )}
