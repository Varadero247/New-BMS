...
export default function QuotesPage() {
  ...
  async function handleCreate() {
    setFormError('');
    if (!formTitle.trim()) { setFormError('Quote title is required'); return; }
  ...
  return (
    <div className="p-8">
            ...
            <div className="flex flex-wrap gap-4 items-center">
              <div className="flex-1 min-w-[200px]">
                <div className="relative">
                  ...
                  <input type="text" aria-label="Search quotes..." placeholder="Search quotes..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-violet-500" />
                </div>
              </div>
      ...
      {/* Create Modal */}
      <Modal isOpen={createModalOpen} onClose={() => setCreateModalOpen(false)} title="New Quote" size="full">
        <div className="max-h-[70vh] overflow-y-auto pr-2 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <Input id="q-title" value={formTitle} onChange={(e) => setFormTitle(e.target.value)} placeholder="Quote title" />
            </div>
            <div>
              <Label htmlFor="q-deal">Deal ID</Label>
              <Input id="q-deal" value={formDealId} onChange={(e) => setFormDealId(e.target.value)} placeholder="Optional deal reference" />
            </div>
            <div>
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
              ...
              <button onClick={addLine} className="text-sm text-violet-600 hover:text-violet-700 font-medium">+ Add Line</button>
            </div>
                ...
                {lines.map((line, idx) => (
                  <tr key={idx} className="border-t">
                    <td className="py-2 px-3">
                      <input value={line.description} onChange={(e) => updateLine(idx, 'description', e.target.value)} placeholder="Item description" className="w-full border rounded px-2 py-1 text-sm" />
                    </td>
                    <td className="py-2 px-3">
                      <input type="number" min="1" value={line.quantity} onChange={(e) => updateLine(idx, 'quantity', parseFloat(e.target.value) || 0)} className="w-full border rounded px-2 py-1 text-sm text-right" />
                    </td>
                    <td className="py-2 px-3">
                      <input type="number" step="0.01" min="0" value={line.unitPrice || ''} onChange={(e) => updateLine(idx, 'unitPrice', parseFloat(e.target.value) || 0)} className="w-full border rounded px-2 py-1 text-sm text-right" placeholder="0.00" />
                    </td>
                    <td className="py-2 px-3 text-right font-medium">{formatCurrency(line.amount)}</td>
                    <td className="py-2 px-3">
                      {lines.length > 1 && (
                        <button onClick={() => removeLine(idx)} className="text-red-400 hover:text-red-600"><Trash2 className="h-4 w-4" /></button>
                      )}
                    </td>
                  </tr>
                ))}
        ...
        <ModalFooter>
          ...
          <Button onClick={handleCreate} disabled={submitting}>{submitting ? 'Creating...' : 'Create Quote'}</Button>
        </ModalFooter>
      </Modal>

      {/* View Modal */}
      <Modal isOpen={viewModalOpen} onClose={() => setViewModalOpen(false)} title={`Quote: ${viewQuote?.reference || ''}`} size="lg">
        {viewQuote && (
