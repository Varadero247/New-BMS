...
async function generateRef(orgId: string): Promise<string> {
  ...
  const c = await prisma.suppScorecard.count({
    where: { orgId, referenceNumber: { startsWith: `SSC-${y}` } },
  });
  return `SSC-${y}-${String(c + 1).padStart(4, '0')}`;
}
router.get('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch scorecards' },
    });
  }
});
router.get('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    const orgId = ((req as AuthRequest).user as any)?.orgId || 'default';
    const item = await prisma.suppScorecard.findFirst({
...
router.post('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const data = await prisma.suppScorecard.create({
      data: {
        supplierId,
        period,
        quality,
...
router.put('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const orgId = ((req as AuthRequest).user as any)?.orgId || 'default';
    const existing = await prisma.suppScorecard.findFirst({
      where: { id: req.params.id, orgId, deletedAt: null } as any,
    });
    if (!existing)
      return res
    ...
    const data = await prisma.suppScorecard.update({
  ...
  try {
    ...
    if (!existing)
      return res
        .status(404)
        .json({ success: false, error: { code: 'NOT_FOUND', message: 'scorecard not found' } });
    await prisma.suppScorecard.update({
      where: { id: req.params.id },
      data: { deletedAt: new Date(), updatedBy: (req as AuthRequest).user?.id },
    });
    res.json({ success: true, data: { message: 'scorecard deleted successfully' } });
  } catch (error: unknown) {
    logger.error('Failed to process request', { error: (error as Error).message });
    res.status(500).json({
      success: false,
