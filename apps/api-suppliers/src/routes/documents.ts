...
async function generateRef(orgId: string): Promise<string> {
  ...
  const c = await prisma.suppDocument.count({
    where: { orgId, referenceNumber: { startsWith: `SDC-${y}` } } as any,
  });
  return `SDC-${y}-${String(c + 1).padStart(4, '0')}`;
}
...
router.get('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    const orgId = ((req as AuthRequest).user as { orgId?: string })?.orgId || 'default';
    const item = await prisma.suppDocument.findFirst({
...
router.put('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const orgId = ((req as AuthRequest).user as { orgId?: string })?.orgId || 'default';
    const existing = await prisma.suppDocument.findFirst({
      where: { id: req.params.id, orgId, deletedAt: null } as any,
    });
    if (!existing)
      return res
    ...
    const { supplierId, type, title, fileUrl, expiryDate, isVerified, verifiedBy, notes } =
      parsed.data;
    const data = await prisma.suppDocument.update({
  ...
  try {
    ...
    if (!existing)
      return res
        .status(404)
        .json({ success: false, error: { code: 'NOT_FOUND', message: 'document not found' } });
    await prisma.suppDocument.update({
      where: { id: req.params.id },
