...
async function generateRef(orgId: string): Promise<string> { const y = new Date().getFullYear(); const c = await prisma.suppDocument.count({ where: { orgId, referenceNumber: { startsWith: `SDC-${y}` } } }); return `SDC-${y}-${String(c + 1).padStart(4, '0')}`; }
