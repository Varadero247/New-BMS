...
describe('GET /api/suppliers', () => {
  it('should return suppliers', async () => {
    (prisma as any).suppSupplier.findMany.mockResolvedValue([{ id: '1', title: 'Test' }]);
...
describe('PUT /api/suppliers/:id', () => {
  it('should update', async () => {
    (prisma as any).suppSupplier.findFirst.mockResolvedValue({ id: '1' });
    (prisma as any).suppSupplier.update.mockResolvedValue({ id: '1', title: 'Updated' });
    const res = await request(app).put('/api/suppliers/1').send({ title: 'Updated' });
    expect(res.status).toBe(200);
  });
});
