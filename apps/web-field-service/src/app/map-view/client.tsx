...
const MOCK_JOBS: Job[] = [
  { id: 'JOB-001', title: 'HVAC Quarterly Service', customer: 'ABC Manufacturing', site: 'Unit 5, Industrial Park', status: 'On Site', priority: 'Medium', technician: 'T. Harrison', scheduledDate: '2026-02-13', estimatedHours: 3, x: 25, y: 30, region: 'North' },
  { id: 'JOB-002', title: 'Emergency Generator Repair', customer: 'City Hospital', site: '100 High Street', status: 'En Route', priority: 'Emergency', technician: 'M. Chen', scheduledDate: '2026-02-13', estimatedHours: 4, x: 55, y: 20, region: 'Central' },
  { id: 'JOB-003', title: 'Fire Alarm Annual Inspection', customer: 'Retail Park Ltd', site: 'Retail Park, Zone B', status: 'Scheduled', priority: 'High', technician: 'S. Patel', scheduledDate: '2026-02-13', estimatedHours: 2, x: 70, y: 45, region: 'East' },
  { id: 'JOB-004', title: 'Lift Maintenance Service', customer: 'Central Tower Office', site: '25 Queen Street', status: 'Completed', priority: 'Medium', technician: 'T. Harrison', scheduledDate: '2026-02-13', estimatedHours: 2, x: 45, y: 55, region: 'Central' },
  { id: 'JOB-005', title: 'Plumbing Emergency', customer: 'Green Park School', site: 'Green Park Road', status: 'Scheduled', priority: 'Emergency', technician: 'A. Williams', scheduledDate: '2026-02-13', estimatedHours: 2, x: 15, y: 65, region: 'West' },
  { id: 'JOB-006', title: 'Electrical Panel Upgrade', customer: 'DataSys Ltd', site: 'Tech Campus, Building 3', status: 'On Site', priority: 'High', technician: 'R. Brown', scheduledDate: '2026-02-13', estimatedHours: 6, x: 80, y: 25, region: 'East' },
  { id: 'JOB-007', title: 'AC Unit Replacement', customer: 'Sunset Hotel', site: '45 Beach Road', status: 'Scheduled', priority: 'Medium', technician: 'S. Patel', scheduledDate: '2026-02-14', estimatedHours: 5, x: 35, y: 80, region: 'South' },
  { id: 'JOB-008', title: 'Security System Check', customer: 'Metro Bank', site: '10 Market Square', status: 'Completed', priority: 'Low', technician: 'M. Chen', scheduledDate: '2026-02-12', estimatedHours: 1.5, x: 50, y: 40, region: 'Central' },
  { id: 'JOB-009', title: 'Boiler Service', customer: 'Riverside Apartments', site: 'Riverside Drive', status: 'Scheduled', priority: 'Medium', technician: 'A. Williams', scheduledDate: '2026-02-14', estimatedHours: 3, x: 60, y: 70, region: 'South' },
  { id: 'JOB-010', title: 'Compressor Rebuild', customer: 'Northern Industries', site: 'Industrial Estate North', status: 'En Route', priority: 'High', technician: 'R. Brown', scheduledDate: '2026-02-13', estimatedHours: 8, x: 30, y: 15, region: 'North' },
];

const MOCK_TECHS: Technician[] = [
  { id: 'T1', name: 'T. Harrison', status: 'On Job', currentJobId: 'JOB-001', x: 25, y: 30, jobsCompleted: 3, hoursWorked: 6.5 },
  { id: 'T2', name: 'M. Chen', status: 'En Route', currentJobId: 'JOB-002', x: 48, y: 25, jobsCompleted: 2, hoursWorked: 4.0 },
  { id: 'T3', name: 'S. Patel', status: 'Available', x: 65, y: 50, jobsCompleted: 1, hoursWorked: 2.0 },
  { id: 'T4', name: 'A. Williams', status: 'Available', x: 20, y: 60, jobsCompleted: 2, hoursWorked: 5.0 },
  { id: 'T5', name: 'R. Brown', status: 'On Job', currentJobId: 'JOB-006', x: 80, y: 25, jobsCompleted: 4, hoursWorked: 7.5 },
];
...
export default function MapViewClient() {
  ...
  const emergencies = MOCK_JOBS.filter(j => j.priority === 'Emergency' && j.status !== 'Completed').length;
        ...
        <div className="col-span-2 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg p-4">
          <div className="relative bg-gray-100 dark:bg-gray-800 rounded-lg overflow-hidden" style={{ height: 500 }}>
            ...
            {filteredJobs.map(job => (
              ...
              >
                <div className={`relative h-6 w-6 rounded-full ${statusColors[job.status]} flex items-center justify-center shadow-md ${job.priority === 'Emergency' ? 'animate-pulse ring-2 ring-red-400' : ''}`}>
            ...
            {/* Technician markers */}
            {showTechnicians && MOCK_TECHS.map(tech => (
              <div
                key={tech.id}
              ...
              >
                <div className={`h-5 w-5 rounded-sm ${techStatusColors[tech.status]} flex items-center justify-center shadow border border-white`}>
