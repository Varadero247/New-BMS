...
const emptyForm = {
  title: '', technicianName: '', scheduledDate: '', scheduledTime: '',
  duration: '', type: 'PREVENTIVE', recurrence: 'NONE', status: 'PENDING', notes: '',
};
...
export default function SchedulesPage() {
  ...
  const load = async () => {
    try { const r = await api.get('/schedules'); setItems(r.data.data || []); }
  ...
  const openEdit = (item: Schedule) => {
    setEditItem(item);
    setForm({ title: item.title || item.jobName || '', technicianName: item.technicianName || '',
      scheduledDate: item.scheduledDate ? item.scheduledDate.split('T')[0] : '',
      scheduledTime: item.scheduledTime || item.timeSlot || '',
      duration: item.duration || '', type: item.type || 'PREVENTIVE',
      recurrence: item.recurrence || 'NONE', status: item.status || 'PENDING', notes: item.notes || '' });
  ...
  const handleDelete = async () => {
    ...
    try { await api.delete(`/schedules/${deleteItem.id}`); setDeleteItem(null); await load(); }
  ...
  return (
    <div className="flex min-h-screen">
        ...
        <div className="max-w-7xl mx-auto space-y-6">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-gray-500 dark:text-gray-400 mt-1">Service scheduling and calendar management</p>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { label: 'Total Schedules', value: stats.total, icon: Calendar, bg: 'bg-sky-50', color: 'text-sky-600', border: 'border-sky-200' },
              { label: 'Confirmed', value: stats.confirmed, icon: CheckCircle, bg: 'bg-green-50', color: 'text-green-600', border: 'border-green-200' },
              { label: 'Pending', value: stats.pending, icon: Clock, bg: 'bg-yellow-50', color: 'text-yellow-600', border: 'border-yellow-200' },
              { label: 'Cancelled', value: stats.cancelled, icon: XCircle, bg: 'bg-red-50', color: 'text-red-600', border: 'border-red-200' },
            ].map(s => {
              const Icon = s.icon;
              return (
          ...
          <div className="flex flex-wrap gap-3">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input type="text" aria-label="Search schedules..." placeholder="Search schedules..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b bg-gray-50 dark:bg-gray-800">
                        ...
                        <th className="text-left py-3 px-4 font-semibold text-gray-600">Recurrence</th>
                    ...
                    <tbody>
                      {filtered.map(item => (
                        <tr key={item.id} className="border-b hover:bg-sky-50 transition-colors">
                          <td className="py-3 px-4 font-medium text-gray-900 dark:text-gray-100">{item.title || item.jobName || '-'}</td>
                          ...
                          <td className="py-3 px-4 text-gray-600">{item.scheduledTime || item.timeSlot || '-'}</td>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editItem ? 'Edit Schedule' : 'New Schedule'} size="lg">
        <div className="space-y-4">
          {error && <div className="bg-red-50 border border-red-200 text-red-700 px-3 py-2 rounded text-sm">{error}</div>}
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Title / Job Name *</label>
              <input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))}
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500" placeholder="e.g. Quarterly HVAC Inspection" />
            </div>
            <div>
            ...
            <div>
              ...
              <select value={form.type} onChange={e => setForm(f => ({ ...f, type: e.target.value }))}
                ...
                {['PREVENTIVE', 'REACTIVE', 'INSPECTION', 'INSTALLATION', 'EMERGENCY'].map(t => <option key={t} value={t}>{t}</option>)}
              </select>
            </div>
            <div>
            ...
            <div>
              ...
              <select value={form.recurrence} onChange={e => setForm(f => ({ ...f, recurrence: e.target.value }))}
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500">
                {['NONE', 'DAILY', 'WEEKLY', 'MONTHLY', 'QUARTERLY', 'ANNUALLY'].map(r => <option key={r} value={r}>{r}</option>)}
              </select>
            </div>
            <div>
            ...
            <div className="col-span-2">
              ...
              <textarea value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))}
                rows={2} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500" placeholder="Additional notes..." />
            </div>
          </div>
        </div>
        <ModalFooter>
      ...
      <Modal isOpen={!!deleteItem} onClose={() => setDeleteItem(null)} title="Delete Schedule" size="sm">
        <p className="text-sm text-gray-600">Delete <span className="font-semibold">{deleteItem?.title || deleteItem?.jobName}</span>? This action cannot be undone.</p>
        <ModalFooter>
