...
export default function TimeEntriesPage() {
  ...
  const openEdit = (item: TimeEntry) => {
    ...
    setForm({
      ...
      type: item.type || item.entryType || 'REGULAR',
  ...
  const handleSave = async () => {
    if (!form.technicianName || !form.hours) {
      setError('Technician and hours are required.');
      return;
    }
    setSaving(true);
    setError('');
  ...
  const handleDelete = async () => {
    ...
    try {
      await api.delete(`/time-entries/${deleteItem.id}`);
      setDeleteItem(null);
      await load();
    } catch (e) {
      console.error(e);
  ...
  return (
    <div className="flex min-h-screen">
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-gray-500 dark:text-gray-400 mt-1">
                Track technician time and labour hours
              </p>
            </div>
            <button
              onClick={openCreate}
                    ...
                    <tbody>
                      {filtered.map((item) => (
                        <tr key={item.id} className="border-b hover:bg-sky-50 transition-colors">
                          ...
                          <td className="py-3 px-4 text-gray-600">
                            {item.type || item.entryType || '-'}
                          </td>
                          <td className="py-3 px-4">
                            <span
                              className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${item.billable ? 'bg-teal-100 text-teal-700' : 'bg-gray-100 dark:bg-gray-800 text-gray-600'}`}
                            >
      ...
      <Modal
        ...
        title={editItem ? 'Edit Time Entry' : 'Log Time'}
        size="lg"
      >
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                ...
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500"
              >
                {['REGULAR', 'OVERTIME', 'TRAVEL', 'TRAINING', 'ADMIN'].map((t) => (
      ...
      >
        <p className="text-sm text-gray-600">
          Delete time entry for <span className="font-semibold">{deleteItem?.technicianName}</span>?
