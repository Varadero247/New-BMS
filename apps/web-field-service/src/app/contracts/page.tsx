...
export default function ContractsPage() {
  ...
  const openEdit = (item: Contract) => {
    ...
    setForm({
      ...
      expiresAt:
        item.expiresAt || item.endDate ? (item.expiresAt || item.endDate)!.split('T')[0] : '',
      value: item.value || '',
  ...
  const handleDelete = async () => {
    ...
    try {
      await api.delete(`/contracts/${deleteItem.id}`);
      setDeleteItem(null);
      await load();
    } catch (e) {
      console.error(e);
  ...
  return (
    <div className="flex min-h-screen">
                      ...
                      {filtered.map((item) => (
                        <tr key={item.id} className="border-b hover:bg-sky-50 transition-colors">
                          ...
                          <td className="py-3 px-4 text-gray-600">
                            {(item.type || item.contractType || '-').replace('_', ' ')}
                          </td>
                          <td className="py-3 px-4 text-right font-medium">
                            {item.value ? `$${Number(item.value).toLocaleString()}` : '-'}
                          </td>
                          <td className="py-3 px-4 text-gray-600">
                            {item.slaHours ? `${item.slaHours}h` : '-'}
                          </td>
                          <td className="py-3 px-4 text-gray-600">
      ...
      <Modal
        ...
        title={editItem ? 'Edit Contract' : 'New Contract'}
        size="lg"
      >
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
              ...
              <textarea
                ...
                rows={2}
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500"
                placeholder="Contract terms and notes..."
              />
            </div>
          </div>
        </div>
      ...
      >
        <p className="text-sm text-gray-600">
          Delete <span className="font-semibold">{deleteItem?.name || deleteItem?.title}</span>?
