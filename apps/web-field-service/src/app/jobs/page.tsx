...
import { Plus, Search, Truck, AlertCircle, Clock, CheckCircle, PlayCircle, Calendar } from 'lucide-react';
...
const emptyForm = {
  title: '', description: '', customerName: '', technicianName: '',
  scheduledDate: '', priority: 'MEDIUM', status: 'SCHEDULED',
  siteAddress: '', estimatedHours: '',
};
...
export default function JobsPage() {
  ...
  const handleSave = async () => {
    if (!form.title || !form.customerName) { setError('Title and customer are required.'); return; }
    setSaving(true); setError('');
    try {
      if (editItem) { await api.put(`/jobs/${editItem.id}`, form); }
  ...
  const handleDelete = async () => {
    ...
    try { await api.delete(`/jobs/${deleteItem.id}`); setDeleteItem(null); await load(); }
    ...
    <div className="flex min-h-screen">
      ...
      <main className="flex-1 p-8">
        <div className="animate-pulse space-y-4">
          ...
          <div className="grid grid-cols-5 gap-4">{[...Array(5)].map((_, i) => <div key={i} className="h-24 bg-gray-200 rounded" />)}</div>
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <main className="flex-1 p-8 bg-gray-50 dark:bg-gray-800">
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            {[
              { label: 'Total Jobs', value: stats.total, icon: Truck, bg: 'bg-sky-50', color: 'text-sky-600', border: 'border-sky-200' },
              { label: 'Scheduled', value: stats.scheduled, icon: Calendar, bg: 'bg-purple-50', color: 'text-purple-600', border: 'border-purple-200' },
              { label: 'In Progress', value: stats.inProgress, icon: PlayCircle, bg: 'bg-blue-50', color: 'text-blue-600', border: 'border-blue-200' },
              { label: 'Completed', value: stats.completed, icon: CheckCircle, bg: 'bg-green-50', color: 'text-green-600', border: 'border-green-200' },
              { label: 'Urgent', value: stats.urgent, icon: AlertCircle, bg: 'bg-red-50', color: 'text-red-600', border: 'border-red-200' },
            ].map(s => {
              const Icon = s.icon;
              return (
          ...
          <div className="flex flex-wrap gap-3">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input type="text" aria-label="Search jobs..." placeholder="Search jobs..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
      ...
      {/* Create/Edit Modal */}
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editItem ? 'Edit Job' : 'New Job'} size="lg">
