        ...
        <div className="max-w-7xl mx-auto space-y-6">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-gray-500 mt-1">Key performance indicators and metrics tracking</p>
            </div>
                    ...
                    <CardContent className="p-5">
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex-1">
                          <p className="font-semibold text-gray-900">{item.name || item.metric}</p>
                          <p className="text-xs text-gray-500 mt-0.5">{item.category} · {item.period || 'MONTHLY'}</p>
                        </div>
                        <div className="flex items-center gap-1">
                          ...
                          {(!item.trend || item.trend === 'FLAT' || item.trend === 'STABLE') && <Minus className="h-4 w-4 text-gray-400" />}
                        </div>
                      </div>
                      <div className="flex items-end justify-between mb-2">
                        <div>
                          <span className="text-2xl font-bold text-gray-900">{curr}</span>
                          <span className="text-sm text-gray-400 ml-1">{item.unit || ''}</span>
                        </div>
                        {tgt > 0 && <span className="text-sm text-gray-500">Target: {tgt}{item.unit || ''}</span>}
                      </div>
                      {tgt > 0 && (
                        <div>
                      ...
                      <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-100">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${trendColors[item.trend || ''] || 'bg-gray-100 text-gray-600'}`}>
                          {item.trend || 'STABLE'}
                        </span>
