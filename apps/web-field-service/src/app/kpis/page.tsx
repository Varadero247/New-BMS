...
const emptyForm = {
  name: '', category: 'OPERATIONS', currentValue: '', target: '',
  unit: '%', trend: 'STABLE', period: 'MONTHLY', description: '',
};
...
export default function KPIsPage() {
  ...
  const load = async () => {
    try { const r = await api.get('/kpis'); setItems(r.data.data || []); }
  ...
  const openEdit = (item: KPI) => {
    setEditItem(item);
    setForm({ name: item.name || item.metric || '', category: item.category || 'OPERATIONS',
      currentValue: item.currentValue ?? item.value ?? '', target: item.target ?? '',
      unit: item.unit || '%', trend: item.trend || 'STABLE',
      period: item.period || 'MONTHLY', description: item.description || '' });
    setError(''); setModalOpen(true);
  };

  const handleSave = async () => {
    if (!form.name) { setError('KPI name is required.'); return; }
  ...
  const handleDelete = async () => {
    ...
    try { await api.delete(`/kpis/${deleteItem.id}`); setDeleteItem(null); await load(); }
  ...
  return (
    <div className="flex min-h-screen">
        ...
        <div className="max-w-7xl mx-auto space-y-6">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-gray-500 dark:text-gray-400 mt-1">Key performance indicators and metrics tracking</p>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { label: 'Total KPIs', value: stats.total, icon: BarChart3, bg: 'bg-sky-50', color: 'text-sky-600', border: 'border-sky-200' },
              { label: 'On Target', value: stats.onTarget, icon: TrendingUp, bg: 'bg-green-50', color: 'text-green-600', border: 'border-green-200' },
              { label: 'Off Target', value: stats.offTarget, icon: TrendingDown, bg: 'bg-red-50', color: 'text-red-600', border: 'border-red-200' },
              { label: 'Trending Up', value: stats.trending, icon: TrendingUp, bg: 'bg-blue-50', color: 'text-blue-600', border: 'border-blue-200' },
            ].map(s => {
              const Icon = s.icon;
              return (
          ...
          <div className="flex flex-wrap gap-3">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input type="text" aria-label="Search KPIs..." placeholder="Search KPIs..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
                    ...
                    <CardContent className="p-5">
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex-1">
                          <p className="font-semibold text-gray-900 dark:text-gray-100">{item.name || item.metric}</p>
                          <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{item.category} · {item.period || 'MONTHLY'}</p>
                        </div>
                        <div className="flex items-center gap-1">
                          ...
                          {(!item.trend || item.trend === 'FLAT' || item.trend === 'STABLE') && <Minus className="h-4 w-4 text-gray-400 dark:text-gray-500" />}
                        </div>
                      </div>
                      <div className="flex items-end justify-between mb-2">
                        <div>
                          <span className="text-2xl font-bold text-gray-900 dark:text-gray-100">{curr}</span>
                          <span className="text-sm text-gray-400 dark:text-gray-500 ml-1">{item.unit || ''}</span>
                        </div>
                        {tgt > 0 && <span className="text-sm text-gray-500 dark:text-gray-400">Target: {tgt}{item.unit || ''}</span>}
                      </div>
                      {tgt > 0 && (
                        <div>
                          <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                            <div className={`h-2 rounded-full transition-all ${onTgt ? 'bg-green-500' : progress > 75 ? 'bg-yellow-400' : 'bg-sky-500'}`}
                              style={{ width: `${progress}%` }} />
                          </div>
                          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{progress}% of target</p>
                        </div>
                      )}
                      <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-100 dark:border-gray-700">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${trendColors[item.trend || ''] || 'bg-gray-100 dark:bg-gray-800 text-gray-600'}`}>
                          {item.trend || 'STABLE'}
                        </span>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editItem ? 'Edit KPI' : 'Add KPI'} size="lg">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              ...
              <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500" placeholder="e.g. First-Time Fix Rate" />
            </div>
            <div>
            ...
            <div>
              ...
              <select value={form.period} onChange={e => setForm(f => ({ ...f, period: e.target.value }))}
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500">
                {['DAILY', 'WEEKLY', 'MONTHLY', 'QUARTERLY', 'ANNUALLY'].map(p => <option key={p} value={p}>{p}</option>)}
              </select>
            </div>
            <div>
              ...
              <input type="number" value={form.currentValue} onChange={e => setForm(f => ({ ...f, currentValue: e.target.value }))}
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500" placeholder="0" step="any" />
            </div>
            <div>
            ...
            <div>
              ...
              <input value={form.unit} onChange={e => setForm(f => ({ ...f, unit: e.target.value }))}
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500" placeholder="%, hrs, $" />
            </div>
            <div>
              ...
              <select value={form.trend} onChange={e => setForm(f => ({ ...f, trend: e.target.value }))}
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500">
                {['UP', 'DOWN', 'FLAT', 'STABLE'].map(t => <option key={t} value={t}>{t}</option>)}
              </select>
            </div>
            <div className="col-span-2">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Description</label>
              <textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
                rows={2} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500" placeholder="What this KPI measures..." />
            </div>
          </div>
        </div>
        <ModalFooter>
      ...
      <Modal isOpen={!!deleteItem} onClose={() => setDeleteItem(null)} title="Delete KPI" size="sm">
        <p className="text-sm text-gray-600">Delete <span className="font-semibold">{deleteItem?.name || deleteItem?.metric}</span>? This action cannot be undone.</p>
        <ModalFooter>
