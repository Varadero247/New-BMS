...
const emptyForm = {
  partName: '', partNumber: '', jobNumber: '', technicianName: '',
  quantity: '1', unitCost: '', category: '', supplier: '', date: '',
};
...
export default function PartsUsedPage() {
  ...
  const load = async () => {
    try { const r = await api.get('/parts-used'); setItems(r.data.data || []); }
  ...
  const openEdit = (item: PartUsed) => {
    setEditItem(item);
    setForm({ partName: item.partName || item.name || '', partNumber: item.partNumber || '',
      jobNumber: item.jobNumber || '', technicianName: item.technicianName || '',
      quantity: item.quantity || '1', unitCost: item.unitCost || item.cost || '',
  ...
  const handleSave = async () => {
    if (!form.partName) { setError('Part name is required.'); return; }
  ...
  const handleDelete = async () => {
    ...
    try { await api.delete(`/parts-used/${deleteItem.id}`); setDeleteItem(null); await load(); }
  ...
  if (loading) return (
    <div className="flex min-h-screen"><Sidebar />
      <main className="flex-1 p-8"><div className="animate-pulse space-y-4">
        ...
        <div className="grid grid-cols-3 gap-4">{[...Array(3)].map((_, i) => <div key={i} className="h-24 bg-gray-200 rounded" />)}</div>
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <main className="flex-1 p-8 bg-gray-50 dark:bg-gray-800">
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              { label: 'Total Records', value: stats.total, icon: Package, bg: 'bg-sky-50', color: 'text-sky-600', border: 'border-sky-200' },
              { label: 'Total Units Used', value: stats.totalQty, icon: Hash, bg: 'bg-blue-50', color: 'text-blue-600', border: 'border-blue-200' },
              { label: 'Total Cost', value: `$${stats.totalCost.toLocaleString()}`, icon: DollarSign, bg: 'bg-teal-50', color: 'text-teal-600', border: 'border-teal-200' },
            ].map(s => {
              const Icon = s.icon;
              return (
          ...
          <div className="flex flex-wrap gap-3">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input type="text" aria-label="Search parts..." placeholder="Search parts..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
                    ...
                    <tbody>
                      {filtered.map(item => (
                        <tr key={item.id} className="border-b hover:bg-sky-50 transition-colors">
                          ...
                          <td className="py-3 px-4 text-right font-medium">{(item.totalCost || item.cost) ? `$${Number(item.totalCost || item.cost).toLocaleString()}` : '-'}</td>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editItem ? 'Edit Parts Record' : 'Log Parts Used'} size="lg">
