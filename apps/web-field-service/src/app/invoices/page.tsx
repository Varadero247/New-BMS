    ...
    <div className="flex min-h-screen">
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-gray-500 dark:text-gray-400 mt-1">
                Service invoices and billing management
              </p>
            </div>
            <button
              onClick={openCreate}
          ...
          {stats.outstanding > 0 && (
            <div className="bg-amber-50 border border-amber-200 rounded-lg px-4 py-3 flex items-center gap-2">
              ...
              <p className="text-sm text-amber-700">
                <span className="font-semibold">${stats.outstanding.toLocaleString()}</span>{' '}
                outstanding across sent and overdue invoices.
              </p>
            </div>
          )}
                        ...
                        <tr key={item.id} className="border-b hover:bg-sky-50 transition-colors">
                          ...
                          <td className="py-3 px-4 text-right font-semibold text-gray-900 dark:text-gray-100">
                            {item.totalAmount || item.amount
                              ? `$${Number(item.totalAmount || item.amount).toLocaleString()}`
                              : '-'}
                          </td>
      ...
      >
        <p className="text-sm text-gray-600">
          Delete invoice <span className="font-semibold">{deleteItem?.invoiceNumber}</span>? This
