...
const emptyForm = {
  ...
  certifications: '', region: '', status: 'AVAILABLE',
};
...
export default function TechniciansPage() {
  ...
  const load = async () => {
    try { const r = await api.get('/technicians'); setItems(r.data.data || []); }
  ...
  const openEdit = (item: Technician) => {
    ...
    setForm({ name: item.name || '', email: item.email || '', phone: item.phone || '',
      specialization: item.specialization || item.skills || '', certifications: item.certifications || '',
      region: item.region || '', status: item.status || 'AVAILABLE' });
  ...
  const handleDelete = async () => {
    ...
    try { await api.delete(`/technicians/${deleteItem.id}`); setDeleteItem(null); await load(); }
  ...
  return (
    <div className="flex min-h-screen">
        ...
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="flex flex-wrap gap-3">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input type="text" aria-label="Search technicians..." placeholder="Search technicians..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b bg-gray-50 dark:bg-gray-800">
                        ...
                        <th className="text-left py-3 px-4 font-semibold text-gray-600">Active Jobs</th>
                      ...
                      {filtered.map(item => (
                        <tr key={item.id} className="border-b hover:bg-sky-50 transition-colors">
                          ...
                          <td className="py-3 px-4 text-gray-600">
                            {item.phone ? <a href={`tel:${item.phone}`} className="flex items-center gap-1 text-sky-600 hover:underline"><Phone className="h-3 w-3" />{item.phone}</a> : '-'}
                          </td>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editItem ? 'Edit Technician' : 'Add Technician'} size="lg">
