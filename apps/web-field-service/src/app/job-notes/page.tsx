...
const emptyForm = {
  jobNumber: '', technicianName: '', note: '', type: 'GENERAL', isInternal: false,
};
...
export default function JobNotesPage() {
  ...
  const load = async () => {
    try { const r = await api.get('/job-notes'); setItems(r.data.data || []); }
  ...
  const openEdit = (item: JobNote) => {
    setEditItem(item);
    setForm({ jobNumber: item.jobNumber || '', technicianName: item.technicianName || '',
      note: item.note || item.content || '', type: item.type || 'GENERAL',
      isInternal: item.isInternal || false });
    setError(''); setModalOpen(true);
  };

  const handleSave = async () => {
    if (!form.note) { setError('Note content is required.'); return; }
  ...
  const handleDelete = async () => {
    ...
    try { await api.delete(`/job-notes/${deleteItem.id}`); setDeleteItem(null); await load(); }
  ...
  return (
    <div className="flex min-h-screen">
        ...
        <div className="max-w-7xl mx-auto space-y-6">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-gray-500 dark:text-gray-400 mt-1">Technician notes and service observations</p>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { label: 'Total Notes', value: stats.total, icon: StickyNote, bg: 'bg-sky-50', color: 'text-sky-600', border: 'border-sky-200' },
              { label: 'Issues', value: stats.issues, icon: AlertCircle, bg: 'bg-red-50', color: 'text-red-600', border: 'border-red-200' },
              { label: 'Resolutions', value: stats.resolutions, icon: CheckCircle, bg: 'bg-green-50', color: 'text-green-600', border: 'border-green-200' },
              { label: 'General', value: stats.general, icon: MessageSquare, bg: 'bg-gray-50 dark:bg-gray-800', color: 'text-gray-600', border: 'border-gray-200 dark:border-gray-700' },
            ].map(s => {
              const Icon = s.icon;
              return (
          ...
          <div className="flex flex-wrap gap-3">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input type="text" aria-label="Search notes..." placeholder="Search notes..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
            ...
            <select aria-label="Filter by type" value={typeFilter} onChange={e => setTypeFilter(e.target.value)}
              ...
              {['ISSUE', 'RESOLUTION', 'OBSERVATION', 'GENERAL', 'HANDOVER', 'CUSTOMER_FEEDBACK'].map(t => <option key={t} value={t}>{t.replace('_', ' ')}</option>)}
            </select>
          </div>
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1 flex-wrap">
                            <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${typeColors[item.type || 'GENERAL'] || 'bg-gray-100 dark:bg-gray-800 text-gray-600'}`}>
                              {(item.type || 'GENERAL').replace('_', ' ')}
                            </span>
                            {item.jobNumber && <span className="text-xs font-mono text-gray-500 dark:text-gray-400">Job: {item.jobNumber}</span>}
                            {item.technicianName && <span className="text-xs text-gray-500 dark:text-gray-400">by {item.technicianName}</span>}
                            {item.isInternal && <span className="text-xs bg-yellow-100 text-yellow-700 px-1.5 py-0.5 rounded">Internal</span>}
                            <span className="text-xs text-gray-400 dark:text-gray-500 ml-auto">{item.createdAt ? new Date(item.createdAt).toLocaleDateString() : '-'}</span>
                          </div>
                          <p className="text-sm text-gray-700 dark:text-gray-300 line-clamp-2 cursor-pointer hover:text-gray-900 dark:text-gray-100"
                            onClick={() => setViewItem(item)}>
                            {item.note || item.content || '-'}
                          </p>
                        </div>
                        <div className="flex gap-2 flex-shrink-0">
                          <button onClick={() => setViewItem(item)} className="text-sky-600 hover:text-sky-800 text-xs font-medium">View</button>
                          <button onClick={() => openEdit(item)} className="text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:text-gray-300 text-xs font-medium">Edit</button>
        ...
        {viewItem && (
          <div className="space-y-3">
            <div className="flex flex-wrap gap-2">
              <span className={`inline-flex items-center px-2 py-1 rounded text-xs font-medium ${typeColors[viewItem.type || 'GENERAL'] || 'bg-gray-100 dark:bg-gray-800 text-gray-600'}`}>
                {(viewItem.type || 'GENERAL').replace('_', ' ')}
              </span>
              {viewItem.jobNumber && <span className="text-sm font-mono text-gray-500 dark:text-gray-400">Job: {viewItem.jobNumber}</span>}
              {viewItem.isInternal && <span className="text-xs bg-yellow-100 text-yellow-700 px-1.5 py-0.5 rounded">Internal</span>}
            </div>
            {viewItem.technicianName && <p className="text-sm text-gray-500 dark:text-gray-400">By: <span className="font-medium text-gray-700 dark:text-gray-300">{viewItem.technicianName}</span></p>}
            <div className="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4">
              <p className="text-sm text-gray-700 dark:text-gray-300 whitespace-pre-wrap">{viewItem.note || viewItem.content}</p>
            </div>
            {viewItem.createdAt && <p className="text-xs text-gray-400 dark:text-gray-500">{new Date(viewItem.createdAt).toLocaleString()}</p>}
          </div>
        )}
        <ModalFooter>
          <button onClick={() => setViewItem(null)} className="px-4 py-2 text-sm border border-gray-300 rounded-lg hover:bg-gray-50 dark:bg-gray-800">Close</button>
          {viewItem && <button onClick={() => { openEdit(viewItem); setViewItem(null); }} className="px-4 py-2 text-sm bg-sky-600 text-white rounded-lg hover:bg-sky-700">Edit</button>}
        </ModalFooter>
      </Modal>

      {/* Create/Edit Modal */}
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editItem ? 'Edit Note' : 'Add Job Note'} size="lg">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select value={form.type} onChange={e => setForm(f => ({ ...f, type: e.target.value }))}
                ...
                {['GENERAL', 'ISSUE', 'RESOLUTION', 'OBSERVATION', 'HANDOVER', 'CUSTOMER_FEEDBACK'].map(t => <option key={t} value={t}>{t.replace('_', ' ')}</option>)}
              </select>
            </div>
            <div className="flex items-center gap-2 pt-6">
              <input type="checkbox" id="internal" checked={form.isInternal} onChange={e => setForm(f => ({ ...f, isInternal: e.target.checked }))}
                className="h-4 w-4 rounded border-gray-300 text-sky-600" />
              <label htmlFor="internal" className="text-sm font-medium text-gray-700 dark:text-gray-300">Internal note (not visible to customer)</label>
            </div>
            <div className="col-span-2">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Note *</label>
              <textarea value={form.note} onChange={e => setForm(f => ({ ...f, note: e.target.value }))}
                rows={5} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500" placeholder="Enter your note here..." />
            </div>
          </div>
        </div>
        <ModalFooter>
      ...
      <Modal isOpen={!!deleteItem} onClose={() => setDeleteItem(null)} title="Delete Note" size="sm">
