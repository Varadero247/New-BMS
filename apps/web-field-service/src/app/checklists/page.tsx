...
const emptyForm = {
  name: '', category: 'MAINTENANCE', description: '', status: 'ACTIVE',
  items: [] as string[],
};

const CATEGORIES = ['MAINTENANCE', 'INSPECTION', 'SAFETY', 'INSTALLATION', 'HANDOVER', 'COMMISSIONING'];
...
export default function ChecklistsPage() {
  ...
  const load = async () => {
    try { const r = await api.get('/checklists'); setItems(r.data.data || []); }
  ...
  const stats = {
    ...
    totalItems: items.reduce((s, i) => s + (i.itemCount ?? i.items?.length ?? i.checklistItems?.length ?? 0), 0),
  };

  const openCreate = () => { setEditItem(null); setForm({ ...emptyForm, items: [] }); setNewItem(''); setError(''); setModalOpen(true); };
  const openEdit = (item: Checklist) => {
    setEditItem(item);
    setForm({ name: item.name || '', category: item.category || 'MAINTENANCE',
      description: item.description || '', status: item.status || 'ACTIVE',
      items: [...(item.items || item.checklistItems || [])] });
    setNewItem(''); setError(''); setModalOpen(true);
  };
  ...
  const handleSave = async () => {
    if (!form.name) { setError('Checklist name is required.'); return; }
  ...
  const handleDelete = async () => {
    ...
    try { await api.delete(`/checklists/${deleteItem.id}`); setDeleteItem(null); await load(); }
  ...
  return (
    <div className="flex min-h-screen">
        ...
        <div className="max-w-7xl mx-auto space-y-6">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-gray-500 dark:text-gray-400 mt-1">Service checklists and inspection forms</p>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { label: 'Total Checklists', value: stats.total, icon: ClipboardCheck, bg: 'bg-sky-50', color: 'text-sky-600', border: 'border-sky-200' },
              { label: 'Active', value: stats.active, icon: CheckSquare, bg: 'bg-green-50', color: 'text-green-600', border: 'border-green-200' },
              { label: 'Categories', value: stats.categories, icon: ClipboardCheck, bg: 'bg-blue-50', color: 'text-blue-600', border: 'border-blue-200' },
              { label: 'Total Items', value: stats.totalItems, icon: CheckSquare, bg: 'bg-teal-50', color: 'text-teal-600', border: 'border-teal-200' },
            ].map(s => {
              const Icon = s.icon;
              return (
          ...
          <div className="flex flex-wrap gap-3">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input type="text" placeholder="Search checklists..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b bg-gray-50 dark:bg-gray-800">
                        ...
                        <th className="text-left py-3 px-4 font-semibold text-gray-600">Last Used</th>
                    ...
                    <tbody>
                      {filtered.map(item => (
                        <tr key={item.id} className="border-b hover:bg-sky-50 transition-colors">
                          ...
                          <td className="py-3 px-4 text-gray-600">{item.itemCount ?? item.items?.length ?? item.checklistItems?.length ?? '-'}</td>
                          <td className="py-3 px-4 text-gray-600">{item.lastUsed ? new Date(item.lastUsed).toLocaleDateString() : 'Never'}</td>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editItem ? 'Edit Checklist' : 'New Checklist'} size="lg">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              ...
              <input value={form.name} onChange={e => setForm((f: any) => ({ ...f, name: e.target.value }))}
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500" placeholder="e.g. HVAC Maintenance Checklist" />
            </div>
            <div>
            ...
            <div className="col-span-2">
              ...
              <textarea value={form.description} onChange={e => setForm((f: any) => ({ ...f, description: e.target.value }))}
                rows={2} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500" placeholder="What this checklist covers..." />
            </div>
            <div className="col-span-2">
              ...
              <div className="flex gap-2 mb-2">
                <input value={newItem} onChange={e => setNewItem(e.target.value)}
                  ...
                  className="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500" placeholder="Add a checklist item..." />
                <button onClick={addItem} className="px-3 py-2 bg-sky-100 text-sky-700 rounded-lg hover:bg-sky-200 flex items-center gap-1 text-sm">
              ...
              {(form.items || []).length > 0 ? (
                <div className="space-y-1 max-h-40 overflow-y-auto border border-gray-200 dark:border-gray-700 rounded-lg p-2">
                  {(form.items || []).map((it: string, idx: number) => (
                    <div key={idx} className="flex items-center gap-2 py-1 px-2 bg-gray-50 dark:bg-gray-800 rounded text-sm">
                      ...
                      <button onClick={() => removeItem(idx)} className="text-gray-400 dark:text-gray-500 hover:text-red-500">
                ...
                <p className="text-xs text-gray-400 dark:text-gray-500 text-center py-2">No items added yet</p>
              )}
            </div>
          </div>
        </div>
      ...
      <Modal isOpen={!!deleteItem} onClose={() => setDeleteItem(null)} title="Delete Checklist" size="sm">
