...
export default function CustomersPage() {
  ...
  const load = async () => {
    try { const r = await api.get('/customers'); setItems(r.data.data || []); }
  ...
  const openEdit = (item: Customer) => {
    setEditItem(item);
    setForm({ name: item.name || item.companyName || '', contactName: item.contactName || '',
      email: item.email || '', phone: item.phone || '', address: item.address || '',
      industry: item.industry || '', status: item.status || 'ACTIVE', notes: item.notes || '' });
  ...
  const handleDelete = async () => {
    ...
    try { await api.delete(`/customers/${deleteItem.id}`); setDeleteItem(null); await load(); }
  ...
  return (
    <div className="flex min-h-screen">
        ...
        <div className="max-w-7xl mx-auto space-y-6">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-gray-500 dark:text-gray-400 mt-1">Manage customer accounts and service history</p>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { label: 'Total Customers', value: stats.total, icon: Building2, bg: 'bg-sky-50', color: 'text-sky-600', border: 'border-sky-200' },
              { label: 'Active', value: stats.active, icon: UserCheck, bg: 'bg-green-50', color: 'text-green-600', border: 'border-green-200' },
              { label: 'Inactive', value: stats.inactive, icon: Globe, bg: 'bg-gray-50 dark:bg-gray-800', color: 'text-gray-600', border: 'border-gray-200 dark:border-gray-700' },
              { label: 'Total Jobs', value: stats.totalJobs, icon: Briefcase, bg: 'bg-blue-50', color: 'text-blue-600', border: 'border-blue-200' },
            ].map(s => {
              const Icon = s.icon;
              return (
          ...
          <div className="flex flex-wrap gap-3">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input type="text" aria-label="Search customers..." placeholder="Search customers..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b bg-gray-50 dark:bg-gray-800">
                        ...
                        <th className="text-left py-3 px-4 font-semibold text-gray-600">Total Jobs</th>
                    ...
                    <tbody>
                      {filtered.map(item => (
                        <tr key={item.id} className="border-b hover:bg-sky-50 transition-colors">
                          <td className="py-3 px-4 font-medium text-gray-900 dark:text-gray-100">{item.name || item.companyName || '-'}</td>
                          <td className="py-3 px-4 text-gray-600">{item.contactName || item.email || '-'}</td>
                          ...
                          <td className="py-3 px-4">
                            <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${item.status === 'ACTIVE' ? 'bg-green-100 text-green-700' : item.status === 'INACTIVE' ? 'bg-gray-100 dark:bg-gray-800 text-gray-600' : 'bg-red-100 text-red-700'}`}>
                              {item.status || '-'}
                            </span>
                          </td>
                          <td className="py-3 px-4">
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editItem ? 'Edit Customer' : 'Add Customer'} size="lg">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              ...
              <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500" placeholder="e.g. ABC Manufacturing Ltd" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Contact Name</label>
              <input value={form.contactName} onChange={e => setForm(f => ({ ...f, contactName: e.target.value }))}
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500" placeholder="Primary contact" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Email</label>
              <input type="email" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))}
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500" placeholder="contact@company.com" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Phone</label>
              <input value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))}
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500" placeholder="+44 20 0000 0000" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Industry</label>
              <input value={form.industry} onChange={e => setForm(f => ({ ...f, industry: e.target.value }))}
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500" placeholder="e.g. Manufacturing, Healthcare" />
            </div>
            <div>
            ...
            <div className="col-span-2">
              ...
              <textarea value={form.address} onChange={e => setForm(f => ({ ...f, address: e.target.value }))}
                rows={2} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500" placeholder="Full address..." />
            </div>
            <div className="col-span-2">
      ...
      <Modal isOpen={!!deleteItem} onClose={() => setDeleteItem(null)} title="Delete Customer" size="sm">
        <p className="text-sm text-gray-600">Delete <span className="font-semibold">{deleteItem?.name || deleteItem?.companyName}</span>? This action cannot be undone.</p>
        <ModalFooter>
