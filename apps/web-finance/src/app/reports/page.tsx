...
export default function ReportsPage() {
  ...
  const [pnlData, setPnlData] = useState<{ revenue: ReportData; expenses: ReportData; netIncome: number } | null>(null);
  const [balanceData, setBalanceData] = useState<{ assets: ReportData; liabilities: ReportData; equity: ReportData } | null>(null);
  const [cashflowData, setCashflowData] = useState<{ operating: ReportData; investing: ReportData; financing: ReportData; netChange: number } | null>(null);
  ...
  async function loadReport() {
    ...
    try {
      ...
      if (activeTab === 'pnl') setPnlData({ revenue: { rows: [], total: 0, period: '' }, expenses: { rows: [], total: 0, period: '' }, netIncome: 0 });
      if (activeTab === 'balance') setBalanceData({ assets: { rows: [], total: 0, period: '' }, liabilities: { rows: [], total: 0, period: '' }, equity: { rows: [], total: 0, period: '' } });
      if (activeTab === 'cashflow') setCashflowData({ operating: { rows: [], total: 0, period: '' }, investing: { rows: [], total: 0, period: '' }, financing: { rows: [], total: 0, period: '' }, netChange: 0 });
    } finally {
      setLoading(false);
    }
  }
        ...
        {loading ? (
          ...
          <>
            ...
            {activeTab === 'pnl' && pnlData && (
              <>
                <ReportSection title="Revenue" data={pnlData.revenue} icon={TrendingUp} iconColor="text-green-600" />
                <ReportSection title="Expenses" data={pnlData.expenses} icon={TrendingDown} iconColor="text-red-600" />
                <Card className="bg-indigo-50 border-indigo-200">
                  <CardContent className="pt-6">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm text-indigo-600 font-medium">Net Income</p>
                        <p className={`text-3xl font-bold ${pnlData.netIncome >= 0 ? 'text-green-700' : 'text-red-700'}`}>
            ...
            {activeTab === 'balance' && balanceData && (
              <>
                <ReportSection title="Assets" data={balanceData.assets} icon={TrendingUp} iconColor="text-blue-600" />
                <ReportSection title="Liabilities" data={balanceData.liabilities} icon={TrendingDown} iconColor="text-red-600" />
                <ReportSection title="Equity" data={balanceData.equity} icon={DollarSign} iconColor="text-purple-600" />
              </>
            )}
            ...
            {activeTab === 'cashflow' && cashflowData && (
              <>
                <ReportSection title="Operating Activities" data={cashflowData.operating} icon={BarChart3} iconColor="text-blue-600" />
                <ReportSection title="Investing Activities" data={cashflowData.investing} icon={TrendingDown} iconColor="text-orange-600" />
                <ReportSection title="Financing Activities" data={cashflowData.financing} icon={DollarSign} iconColor="text-purple-600" />
                <Card className="bg-indigo-50 border-indigo-200">
                  <CardContent className="pt-6">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm text-indigo-600 font-medium">Net Change in Cash</p>
                        <p className={`text-3xl font-bold ${cashflowData.netChange >= 0 ? 'text-green-700' : 'text-red-700'}`}>
