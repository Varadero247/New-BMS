      ...
      setIntegrations((prev) =>
        prev.map((i) => {
          if (i.provider === selectedIntegration.provider) {
            if (actionType === 'connect')
              return { ...i, status: 'CONNECTED', connectedAt: new Date().toISOString() };
            if (actionType === 'disconnect')
              return { ...i, status: 'DISCONNECTED', connectedAt: undefined };
  ...
  return (
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-1">
            Connect your finance tools and payment providers
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {integrations.map((integration) => {
            ...
            return (
              <Card
                key={integration.id}
                className={`border ${isConnected ? info.bgColor : 'border-gray-200 dark:border-gray-700'}`}
              >
                <CardContent className="pt-6">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <div
                        className={`p-3 rounded-lg ${isConnected ? 'bg-white dark:bg-gray-900 shadow-sm' : 'bg-gray-100 dark:bg-gray-800'}`}
                      >
      ...
      <Modal
        ...
        title={`${actionType.charAt(0).toUpperCase() + actionType.slice(1)} ${selectedIntegration?.name || ''}`}
        size="sm"
      >
        <div className="py-4">
          <p className="text-gray-600">
            {actionType === 'connect' &&
              `Are you sure you want to connect to ${selectedIntegration?.name}? This will initiate an OAuth flow to link your account.`}
            {actionType === 'disconnect' &&
              `Are you sure you want to disconnect ${selectedIntegration?.name}? Synced data will be retained but no new data will be imported.`}
            {actionType === 'sync' &&
              `Manually sync data from ${selectedIntegration?.name}? This may take a few minutes.`}
          </p>
        </div>
        <ModalFooter>
          <Button
