...
export default function TaxPage() {
  ...
  async function handleCreateRate() {
    setFormError('');
    if (!rateForm.name.trim()) { setFormError('Name is required'); return; }
    if (!rateForm.code.trim()) { setFormError('Code is required'); return; }
    if (!rateForm.rate || parseFloat(rateForm.rate) < 0) { setFormError('Valid rate is required'); return; }
  ...
  async function handleCreateReturn() {
    setFormError('');
    if (!returnForm.period.trim()) { setFormError('Period is required'); return; }
    if (!returnForm.startDate) { setFormError('Start date is required'); return; }
    if (!returnForm.endDate) { setFormError('End date is required'); return; }
    if (!returnForm.dueDate) { setFormError('Due date is required'); return; }
    ...
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <Card className="mb-8">
          <CardHeader>
            <div className="flex justify-between items-center">
              <CardTitle className="flex items-center gap-2"><Calculator className="h-5 w-5 text-indigo-600" />Tax Rates</CardTitle>
              <Button variant="outline" className="flex items-center gap-2" onClick={() => { setRateForm({ name: '', code: '', rate: '', type: 'SALES_TAX', description: '' }); setFormError(''); setCreateRateOpen(true); }}>
        ...
        <Card>
          <CardHeader>
            <div className="flex justify-between items-center">
              <CardTitle className="flex items-center gap-2"><FileText className="h-5 w-5 text-indigo-600" />Tax Returns</CardTitle>
              <Button variant="outline" className="flex items-center gap-2" onClick={() => { setReturnForm({ period: '', startDate: '', endDate: '', dueDate: '', taxCollected: '', taxPaid: '' }); setFormError(''); setCreateReturnOpen(true); }}>
      ...
      {/* Create Rate Modal */}
      <Modal isOpen={createRateOpen} onClose={() => setCreateRateOpen(false)} title="Add Tax Rate" size="lg">
        <div className="space-y-4">
          {formError && <div className="p-3 bg-red-50 border border-red-200 rounded-md text-red-700 text-sm">{formError}</div>}
          <div className="grid grid-cols-2 gap-4">
            <div><Label htmlFor="tr-name">Name *</Label><Input id="tr-name" value={rateForm.name} onChange={(e) => setRateForm(p => ({ ...p, name: e.target.value }))} placeholder="e.g. Standard VAT" /></div>
            <div><Label htmlFor="tr-code">Code *</Label><Input id="tr-code" value={rateForm.code} onChange={(e) => setRateForm(p => ({ ...p, code: e.target.value }))} placeholder="e.g. VAT-STD" /></div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <Label htmlFor="tr-type">Type</Label>
              <select id="tr-type" value={rateForm.type} onChange={(e) => setRateForm(p => ({ ...p, type: e.target.value }))} className="w-full border rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500">
                <option value="SALES_TAX">Sales Tax</option><option value="VAT">VAT</option><option value="GST">GST</option><option value="INCOME_TAX">Income Tax</option><option value="WITHHOLDING">Withholding</option><option value="OTHER">Other</option>
              </select>
            </div>
            <div><Label htmlFor="tr-rate">Rate (%) *</Label><Input id="tr-rate" type="number" step="0.01" min="0" value={rateForm.rate} onChange={(e) => setRateForm(p => ({ ...p, rate: e.target.value }))} placeholder="e.g. 10" /></div>
          </div>
          <div><Label htmlFor="tr-desc">Description</Label><textarea id="tr-desc" value={rateForm.description} onChange={(e) => setRateForm(p => ({ ...p, description: e.target.value }))} rows={2} className="w-full border rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500" /></div>
        </div>
        <ModalFooter>
          <Button variant="outline" onClick={() => setCreateRateOpen(false)} disabled={submitting}>Cancel</Button>
          <Button onClick={handleCreateRate} disabled={submitting}>{submitting ? 'Creating...' : 'Create Rate'}</Button>
        </ModalFooter>
      </Modal>

      {/* Create Return Modal */}
      <Modal isOpen={createReturnOpen} onClose={() => setCreateReturnOpen(false)} title="Add Tax Return" size="lg">
        <div className="space-y-4">
          {formError && <div className="p-3 bg-red-50 border border-red-200 rounded-md text-red-700 text-sm">{formError}</div>}
          <div><Label htmlFor="ret-period">Period *</Label><Input id="ret-period" value={returnForm.period} onChange={(e) => setReturnForm(p => ({ ...p, period: e.target.value }))} placeholder="e.g. Q1 2026" /></div>
          <div className="grid grid-cols-3 gap-4">
            <div><Label htmlFor="ret-start">Start Date *</Label><Input id="ret-start" type="date" value={returnForm.startDate} onChange={(e) => setReturnForm(p => ({ ...p, startDate: e.target.value }))} /></div>
            <div><Label htmlFor="ret-end">End Date *</Label><Input id="ret-end" type="date" value={returnForm.endDate} onChange={(e) => setReturnForm(p => ({ ...p, endDate: e.target.value }))} /></div>
            <div><Label htmlFor="ret-due">Due Date *</Label><Input id="ret-due" type="date" value={returnForm.dueDate} onChange={(e) => setReturnForm(p => ({ ...p, dueDate: e.target.value }))} /></div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div><Label htmlFor="ret-collected">Tax Collected</Label><Input id="ret-collected" type="number" step="0.01" value={returnForm.taxCollected} onChange={(e) => setReturnForm(p => ({ ...p, taxCollected: e.target.value }))} placeholder="0.00" /></div>
            <div><Label htmlFor="ret-paid">Tax Paid</Label><Input id="ret-paid" type="number" step="0.01" value={returnForm.taxPaid} onChange={(e) => setReturnForm(p => ({ ...p, taxPaid: e.target.value }))} placeholder="0.00" /></div>
          </div>
        </div>
        <ModalFooter>
          <Button variant="outline" onClick={() => setCreateReturnOpen(false)} disabled={submitting}>Cancel</Button>
          <Button onClick={handleCreateReturn} disabled={submitting}>{submitting ? 'Creating...' : 'Create Return'}</Button>
        </ModalFooter>
      </Modal>
    </div>
  );
