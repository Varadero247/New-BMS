...
export default function BudgetsPage() {
  ...
  async function handleCreate() {
    setFormError('');
    if (!formData.accountId) { setFormError('Account is required'); return; }
    if (!formData.budgetAmount || parseFloat(formData.budgetAmount) < 0) { setFormError('Valid budget amount is required'); return; }
  ...
  return (
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><PiggyBank className="h-5 w-5 text-indigo-600" />Budget Entries ({filteredBudgets.length})</CardTitle>
          </CardHeader>
          <CardContent>
                    ...
                    {filteredBudgets.map((item) => {
                      ...
                      return (
                        <tr key={item.id} className="border-b hover:bg-gray-50 dark:bg-gray-800">
                          ...
                          <td className="py-3 px-4 text-gray-600">{months[(item.month || 1) - 1]}</td>
                          <td className="py-3 px-4 text-right font-medium">{formatCurrency(item.budgetAmount || 0)}</td>
                          <td className="py-3 px-4 text-right font-medium">{formatCurrency(item.actualAmount || 0)}</td>
                          <td className={`py-3 px-4 text-right font-bold ${variance >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                            {formatCurrency(variance)}
                          </td>
                          <td className={`py-3 px-4 text-right ${varPercent >= 0 ? 'text-green-600' : 'text-red-600'}`}>
      ...
      {/* Create Modal */}
      <Modal isOpen={createModalOpen} onClose={() => setCreateModalOpen(false)} title="Add Budget Entry" size="lg">
        <div className="space-y-4">
          ...
          <div>
            ...
            <select id="bud-account" value={formData.accountId} onChange={(e) => setFormData(p => ({ ...p, accountId: e.target.value }))} className="w-full border rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <select id="bud-year" value={formData.fiscalYear} onChange={(e) => setFormData(p => ({ ...p, fiscalYear: parseInt(e.target.value) }))} className="w-full border rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500">
                {[2024, 2025, 2026, 2027].map(y => <option key={y} value={y}>{y}</option>)}
              </select>
            </div>
            <div>
              <Label htmlFor="bud-month">Month</Label>
              <select id="bud-month" value={formData.month} onChange={(e) => setFormData(p => ({ ...p, month: parseInt(e.target.value) }))} className="w-full border rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500">
                {months.map((m, i) => <option key={i} value={i + 1}>{m}</option>)}
              </select>
            </div>
            <div>
              <Label htmlFor="bud-amount">Budget Amount *</Label>
              <Input id="bud-amount" type="number" step="0.01" min="0" value={formData.budgetAmount} onChange={(e) => setFormData(p => ({ ...p, budgetAmount: e.target.value }))} placeholder="0.00" />
            </div>
          </div>
          <div>
            <Label htmlFor="bud-notes">Notes</Label>
            <textarea id="bud-notes" value={formData.notes} onChange={(e) => setFormData(p => ({ ...p, notes: e.target.value }))} rows={2} className="w-full border rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500" placeholder="Budget notes..." />
          </div>
        </div>
        <ModalFooter>
