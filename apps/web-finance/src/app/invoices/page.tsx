...
export default function InvoicesPage() {
  ...
  async function handleCreate() {
    setFormError('');
    if (!formCustomerId) { setFormError('Customer is required'); return; }
    if (!formIssueDate) { setFormError('Issue date is required'); return; }
  ...
  const filteredInvoices = invoices.filter(inv => {
    const matchesSearch = !searchTerm ||
      ...
      (inv.customer?.name || inv.customerName || '').toLowerCase().includes(searchTerm.toLowerCase());
  ...
  return (
    <div className="p-8">
            ...
            <div className="flex flex-wrap gap-4 items-center">
              <div className="flex-1 min-w-[200px]">
                <div className="relative">
                  ...
                  <input type="text" aria-label="Search invoices..." placeholder="Search invoices..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                </div>
              </div>
      ...
      {/* Create Modal */}
      <Modal isOpen={createModalOpen} onClose={() => setCreateModalOpen(false)} title="New Invoice" size="full">
        <div className="max-h-[70vh] overflow-y-auto pr-2 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <select id="inv-customer" value={formCustomerId} onChange={(e) => setFormCustomerId(e.target.value)} className="w-full border rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500">
            ...
            <div>
              ...
              <Input id="inv-issue" type="date" value={formIssueDate} onChange={(e) => setFormIssueDate(e.target.value)} />
            </div>
            <div>
          ...
          <div>
            ...
            <textarea id="inv-notes" value={formNotes} onChange={(e) => setFormNotes(e.target.value)} rows={2} className="w-full border rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500" placeholder="Invoice notes..." />
          </div>
          <div>
        ...
        <ModalFooter>
          ...
          <Button onClick={handleCreate} disabled={submitting}>{submitting ? 'Creating...' : 'Create Invoice'}</Button>
        </ModalFooter>
      </Modal>

      {/* View Modal */}
      <Modal isOpen={viewModalOpen} onClose={() => setViewModalOpen(false)} title={`Invoice: ${viewInvoice?.reference || ''}`} size="lg">
        {viewInvoice && (
