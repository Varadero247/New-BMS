    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg p-4 text-center flex flex-col justify-center">
          ...
          <p
            className={`text-xl font-bold ${totalVariance >= 0 ? 'text-green-700' : 'text-red-700'}`}
          >
            {totalVariance >= 0 ? '+' : '-'}
      ...
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg p-4">
        ...
        <div className="flex items-end gap-3 h-40">
          {MONTHLY_DATA.map((m) => (
            <div key={m.month} className="flex-1 flex flex-col items-center gap-1">
              ...
              {m.actual > 0 && (
                <span
                  className={`text-[9px] font-medium ${m.actual > m.budget ? 'text-red-600' : 'text-green-600'}`}
                >
                  {m.actual > m.budget ? '+' : ''}
        ...
        <div className="flex items-center gap-4 mt-2 justify-center">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[10px] text-gray-500 dark:text-gray-400">Under Budget</span>
          </div>
          <div className="flex items-center gap-1.5">
            <div className="w-3 h-2 rounded bg-red-400" />
            <span className="text-[10px] text-gray-500 dark:text-gray-400">Over Budget</span>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-5 gap-3">
          {deptSummary.map((d) => (
            ...
            >
              ...
              <p
                className={`text-[10px] font-medium mt-1 ${d.variance >= 0 ? 'text-green-600' : 'text-red-600'}`}
              >
                {d.variance >= 0 ? '+' : '-'}
        ...
        <table className="w-full text-xs">
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
            {filtered.map((line) => (
              <tr key={line.id} className="hover:bg-gray-50 dark:bg-gray-800">
                ...
                <td className="py-2.5 px-3 text-right font-mono text-gray-900 dark:text-gray-100 font-medium">
                  {formatCurrency(line.ytdActual)}
                </td>
                <td
                  className={`py-2.5 px-3 text-right font-mono font-medium ${line.variance >= 0 ? 'text-green-600' : 'text-red-600'}`}
                >
                  {line.variance >= 0 ? '+' : '-'}
                  {formatCurrency(line.variance)}
                </td>
                <td
                  className={`py-2.5 px-3 text-right font-medium ${line.variancePct >= 0 ? 'text-green-600' : 'text-red-600'}`}
                >
          ...
          <tfoot>
            <tr className="bg-gray-50 dark:bg-gray-800 border-t-2 border-gray-300 font-semibold">
              <td colSpan={2} className="py-2.5 px-3 text-gray-700 dark:text-gray-300">
                Total
              </td>
              ...
              <td
                className={`py-2.5 px-3 text-right font-mono ${filtered.reduce((s, b) => s + b.variance, 0) >= 0 ? 'text-green-600' : 'text-red-600'}`}
              >
