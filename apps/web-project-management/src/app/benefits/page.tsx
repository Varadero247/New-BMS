...
const initialForm = {
  title: '', description: '', category: 'FINANCIAL', projectName: '',
  expectedValue: '', currency: 'USD', realizationDate: '', responsible: '', status: 'PLANNED',
};
...
export default function BenefitsPage() {
  ...
  async function handleSave() {
    setFormError('');
    if (!form.title.trim()) { setFormError('Benefit title is required'); return; }
    setSubmitting(true);
    try {
      ...
      if (editing) { await api.put(`/benefits/${editing.id}`, payload); }
  ...
  async function handleDelete(id: string) {
    ...
    try { await api.delete(`/benefits/${id}`); load(); } catch { alert('Failed'); }
  }

  const filtered = items.filter(i => {
    const title = i.title || i.name || '';
    return title.toLowerCase().includes(search.toLowerCase()) || (i.category || '').toLowerCase().includes(search.toLowerCase());
  });

  const totalExpected = items.reduce((s, i) => s + (i.expectedValue || 0), 0);
  const totalRealised = items.filter(i => i.status === 'REALISED').reduce((s, i) => s + (i.actualValue || i.expectedValue || 0), 0);

  const formatCurrency = (val: number, currency = 'USD') => {
    try { return new Intl.NumberFormat('en-US', { style: 'currency', currency, maximumFractionDigits: 0 }).format(val); }
    catch { return `${currency} ${val.toLocaleString()}`; }
  };
  ...
  return (
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <Card className="mb-6"><CardContent className="pt-6">
          <div className="flex gap-4">
            <div className="flex-1 relative">
              ...
              <input type="text" aria-label="Search benefits..." placeholder="Search benefits..." value={search} onChange={e => setSearch(e.target.value)}
        ...
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2"><TrendingUp className="h-5 w-5 text-blue-600" />Benefits ({filtered.length})</CardTitle></CardHeader>
          <CardContent>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Benefit' : 'Add Benefit'} size="lg">
        <div className="space-y-4">
          ...
          <div>
            ...
            <input value={form.title} onChange={e => setForm({...form, title: e.target.value})}
              className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none" placeholder="e.g. Reduced operational costs" />
          </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="col-span-2">
              ...
              <input type="number" value={form.expectedValue} onChange={e => setForm({...form, expectedValue: e.target.value})}
                className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none" placeholder="0" />
            </div>
            <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="date" value={form.realizationDate} onChange={e => setForm({...form, realizationDate: e.target.value})}
                className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none" />
            </div>
            <div>
