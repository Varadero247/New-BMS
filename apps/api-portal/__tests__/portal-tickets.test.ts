...
describe('GET /api/portal/tickets', () => {
  it('should list tickets', async () => {
    const items = [{ id: 't-1', number: 'PTL-TKT-2602-1234', subject: 'Login issue', status: 'OPEN', messages: [] }];
...
describe('POST /api/portal/tickets', () => {
  it('should create a ticket', async () => {
    const ticket = { id: 't-1', number: 'PTL-TKT-2602-1234', subject: 'Login issue', status: 'OPEN' };
    (prisma as any).portalTicket.create.mockResolvedValue(ticket);

    const res = await request(app)
      .post('/api/portal/tickets')
      .send({ subject: 'Login issue', description: 'Cannot log in', category: 'TECHNICAL', priority: 'HIGH', portalType: 'CUSTOMER' });
  ...
  it('should return 400 for missing subject', async () => {
    const res = await request(app)
      .post('/api/portal/tickets')
      .send({ description: 'Cannot log in', category: 'TECHNICAL', priority: 'HIGH', portalType: 'CUSTOMER' });
...
describe('GET /api/portal/tickets/:id', () => {
  it('should return ticket with messages', async () => {
    const ticket = { id: 't-1', subject: 'Login issue', messages: [{ id: 'm-1', message: 'Help' }] };
    (prisma as any).portalTicket.findFirst.mockResolvedValue(ticket);

    const res = await request(app).get('/api/portal/tickets/t-1');
  ...
  it('should return 404 if not found', async () => {
    ...
    const res = await request(app).get('/api/portal/tickets/nonexistent');
...
describe('PUT /api/portal/tickets/:id', () => {
  it('should update a ticket', async () => {
    (prisma as any).portalTicket.findFirst.mockResolvedValue({ id: 't-1' });
    (prisma as any).portalTicket.update.mockResolvedValue({ id: 't-1', priority: 'LOW' });

    const res = await request(app)
      .put('/api/portal/tickets/t-1')
      .send({ priority: 'LOW' });
  ...
  it('should return 404 for update if not found', async () => {
    ...
    const res = await request(app)
      .put('/api/portal/tickets/nonexistent')
      .send({ priority: 'LOW' });
...
describe('POST /api/portal/tickets/:id/messages', () => {
  it('should add a message', async () => {
    const ticket = { id: 't-1', status: 'OPEN' };
    (prisma as any).portalTicket.findFirst.mockResolvedValue(ticket);
    (prisma as any).portalTicketMessage.create.mockResolvedValue({ id: 'm-1', ticketId: 't-1', message: 'Hello' });
    (prisma as any).portalTicket.update.mockResolvedValue({ ...ticket, status: 'IN_PROGRESS' });

    const res = await request(app)
      .post('/api/portal/tickets/t-1/messages')
      .send({ message: 'Hello' });
  ...
  it('should return 404 if ticket not found for message', async () => {
    ...
    const res = await request(app)
      .post('/api/portal/tickets/nonexistent/messages')
      .send({ message: 'Hello' });
...
describe('GET /api/portal/tickets/:id/messages', () => {
  it('should list messages', async () => {
    (prisma as any).portalTicket.findFirst.mockResolvedValue({ id: 't-1' });
    (prisma as any).portalTicketMessage.findMany.mockResolvedValue([{ id: 'm-1', message: 'Hello' }]);

    const res = await request(app).get('/api/portal/tickets/t-1/messages');
  ...
  it('should return 404 if ticket not found for messages', async () => {
    ...
    const res = await request(app).get('/api/portal/tickets/nonexistent/messages');
...
describe('PUT /api/portal/tickets/:id/resolve', () => {
  it('should resolve a ticket', async () => {
    const ticket = { id: 't-1', status: 'IN_PROGRESS' };
    (prisma as any).portalTicket.findFirst.mockResolvedValue(ticket);
    (prisma as any).portalTicket.update.mockResolvedValue({ ...ticket, status: 'RESOLVED', resolvedAt: new Date() });

    const res = await request(app)
      .put('/api/portal/tickets/t-1/resolve')
  ...
  it('should return 400 if ticket already closed', async () => {
    (prisma as any).portalTicket.findFirst.mockResolvedValue({ id: 't-1', status: 'CLOSED' });
  ...
  it('should return 404 if ticket not found for resolve', async () => {
    ...
    const res = await request(app)
      .put('/api/portal/tickets/nonexistent/resolve')
      .send({});
