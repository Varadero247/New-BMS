...
describe('GET /api/customer/documents', () => {
  it('should list shared documents', async () => {
    const items = [
      { id: 'd-1', title: 'Spec Sheet', category: 'SPECIFICATION', visibility: 'PUBLIC' },
...
describe('GET /api/customer/documents/:id', () => {
  it('should return a document', async () => {
    const doc = { id: 'd-1', title: 'Spec Sheet', visibility: 'PUBLIC', portalType: 'CUSTOMER' };
    (prisma as any).portalDocument.findFirst.mockResolvedValue(doc);

    const res = await request(app).get('/api/customer/documents/d-1');
  ...
  it('should return 404 if not found', async () => {
    ...
    const res = await request(app).get('/api/customer/documents/nonexistent');
