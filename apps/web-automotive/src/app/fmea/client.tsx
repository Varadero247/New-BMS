...
const MOCK_FAILURE_MODES: FailureMode[] = [
  {
    id: '1',
    processStep: 'Cylinder Head Torquing',
    failureMode: 'Under-torque of head bolts',
    effect: 'Head gasket failure — coolant/oil contamination',
    cause: 'Torque wrench calibration drift',
    severity: 9,
    occurrence: 3,
    detection: 4,
    currentControls: 'Calibration schedule every 6 months; visual inspection',
    recommendedAction:
      'Reduce calibration interval to 3 months; add electronic torque verification',
    status: 'in-progress',
    owner: 'K. Schulz',
  ...
  {
    id: '2',
    processStep: 'Crankshaft Bearing Assembly',
    failureMode: 'Incorrect bearing clearance',
    effect: 'Premature bearing wear, engine seizure',
    cause: 'Micrometer reading error by operator',
    severity: 10,
    occurrence: 2,
    detection: 5,
    currentControls: 'Measurement recorded on work order; supervisor sign-off',
    recommendedAction: 'Introduce automated CMM verification; poka-yoke gauging fixture',
    status: 'open',
    owner: 'M. Bauer',
  ...
  {
    id: '3',
    processStep: 'Fuel Injector Seating',
    failureMode: 'Injector O-ring not seated',
    effect: 'Fuel leak at injector, fire hazard',
    cause: 'O-ring omitted or rolled during assembly',
    severity: 8,
    occurrence: 2,
    detection: 3,
    currentControls: 'Visual inspection checklist; test leak check at 6 bar',
    recommendedAction: 'Camera vision system for O-ring presence check',
    status: 'completed',
    owner: 'R. Weber',
  ...
  {
    id: '4',
    processStep: 'Transmission Gear Selection',
    failureMode: 'Incorrect gear selected during assembly',
    effect: 'Wrong gear ratio, transmission noise',
    cause: 'Similar-looking gears not differentiated',
    severity: 6,
    occurrence: 4,
    detection: 6,
    currentControls: 'Part number label check; operator visual confirmation',
    recommendedAction: 'Colour coding of gear families; bin pick-to-light system',
    status: 'in-progress',
    owner: 'P. Fischer',
  ...
  {
    id: '5',
    processStep: 'Brake Caliper Mounting',
    failureMode: 'Caliper bolt missing',
    effect: 'Caliper detachment during braking — safety critical',
    cause: 'Bolt dropped or not retrieved from work tray',
    severity: 10,
    occurrence: 1,
    detection: 4,
    currentControls: 'Torque verification; final inspection gate',
    recommendedAction: 'Bolt counting system (shadow board); vision system at final gate',
    status: 'verified',
    owner: 'L. Hoffmann',
  ...
  {
    id: '6',
    processStep: 'Camshaft Timing',
    failureMode: 'Timing chain installed 1 tooth off',
    effect: 'Valve timing error — poor performance or bent valves',
    cause: 'Incorrect alignment of timing marks',
    severity: 9,
    occurrence: 3,
    detection: 5,
    currentControls: 'Timing mark checklist; engine rotation test',
    recommendedAction: 'Dowel-pin timing tool to prevent off-tooth installation',
    status: 'open',
    owner: 'T. Meier',
  ...
  {
    id: '7',
    processStep: 'Exhaust Manifold Gasket Fitting',
    failureMode: 'Gasket installed upside-down',
    effect: 'Exhaust gas leak; EGR contamination',
    cause: 'Asymmetric gasket, no orientation feature',
    severity: 7,
    occurrence: 3,
    detection: 4,
    currentControls: 'Inspection checklist; leak test at end of line',
    recommendedAction: 'Add directional arrow to gasket; poka-yoke flange orientation pin',
    status: 'completed',
    owner: 'A. Richter',
  ...
  {
    id: '8',
    processStep: 'Wheel Hub Bearing Press-fit',
    failureMode: 'Bearing under-pressed (insufficient depth)',
    effect: 'Bearing movement, wheel wobble, NVH',
    cause: 'Press force not monitored in real-time',
    severity: 8,
    occurrence: 2,
    detection: 6,
    currentControls: 'Depth gauge check after pressing; pull-test sampling',
    recommendedAction:
      'Force-displacement curve monitoring on press; automatic reject for out-of-spec',
    status: 'in-progress',
    owner: 'H. Braun',
...
export default function FMEAClient() {
  ...
  const withRPN = MOCK_FAILURE_MODES.map((fm) => ({
    ...fm,
  ...
  const filtered = useMemo(() => {
    const base = withRPN.filter((fm) => {
      const matchSearch =
        fm.processStep.toLowerCase().includes(search.toLowerCase()) ||
        fm.failureMode.toLowerCase().includes(search.toLowerCase()) ||
        fm.effect.toLowerCase().includes(search.toLowerCase());
  ...
  const openCount = withRPN.filter(
    (fm) => fm.status === 'open' || fm.status === 'in-progress'
  ).length;
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
              IATF 16949 — Failure Mode and Effects Analysis (Process FMEA)
            </p>
          </div>
      ...
      <div className="px-6 py-6 space-y-6">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 p-4 flex flex-wrap gap-3">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              type="text"
              aria-label="Search process step, failure mode, effect..."
                ...
                return (
                  <tr key={fm.id} className="hover:bg-gray-50 dark:bg-gray-800">
                    ...
                    <td className="px-4 py-3 text-center">
                      <span
                        className={`inline-block w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${fm.severity >= 8 ? 'bg-red-100 text-red-800' : fm.severity >= 5 ? 'bg-orange-100 text-orange-800' : 'bg-gray-100 dark:bg-gray-800 text-gray-700'}`}
                      >
                        {fm.severity}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`inline-flex items-center px-2.5 py-1 rounded font-bold text-sm border ${rpnConf.bg} ${rpnConf.text} ${rpnConf.border}`}
                      >
        ...
        <p className="text-xs text-gray-400 dark:text-gray-500">
          SEV = Severity (1-10), OCC = Occurrence (1-10), DET = Detection (1-10), RPN = SEV &times;
          OCC &times; DET
        </p>
      </div>
    </div>
