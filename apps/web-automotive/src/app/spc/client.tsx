...
const SPC_STATUSES = ['ACTIVE', 'IN_CONTROL', 'OUT_OF_CONTROL', 'MONITORING', 'CLOSED'] as const;
...
export default function SpcClient() {
  ...
  return (
    <div className="p-8">
            ...
            <div className="flex flex-wrap gap-4 items-end">
              <div className="flex-1 min-w-[200px]">
                ...
                <div className="relative">
                  ...
                  <input
                    type="text"
                    aria-label="Search by title, reference, part number, characteristic..."
                  ...
                  <tbody>
                    {filtered.map((chart) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-center">
                          <span
                            className={`text-sm font-bold ${(chart.oocCount || 0) > 0 ? 'text-red-600' : 'text-green-600'}`}
                          >
                            {chart.oocCount || 0}
                          </span>
                        </td>
      ...
      {/* CREATE MODAL                                                      */}
      {/* ================================================================= */}
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title="New SPC Chart" size="lg">
        ...
        {detailLoading ? (
          ...
          <div className="max-h-[70vh] overflow-y-auto pr-2 space-y-6">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <Badge
                className={statusColors[selectedChart.status] || 'bg-gray-100 dark:bg-gray-800'}
              >
            ...
            {selectedChart.dataPoints && selectedChart.dataPoints.length > 0 && (
              <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wide mb-3">
                  Recent Data Points ({Math.min(selectedChart.dataPoints.length, 20)} of{' '}
                  {selectedChart.dataPoints.length})
                </h3>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                        ...
                        .map((dp, idx) => (
                          ...
                          >
                            <td className="px-3 py-2 text-xs text-gray-400 dark:text-gray-500">
                              {selectedChart.dataPoints!.length - idx}
                            </td>
                            <td
                              className={`px-3 py-2 font-mono font-medium ${dp.outOfControl ? 'text-red-600' : 'text-gray-900 dark:text-gray-100'}`}
                            >
                              {dp.value.toFixed(3)}
                            </td>
                            ...
                            <td className="px-3 py-2 text-xs text-gray-400 dark:text-gray-500 max-w-[150px] truncate">
                              {dp.notes || '-'}
                            </td>
                          </tr>
                        ))}
