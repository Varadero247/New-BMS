...
const CATEGORIES = ['APQP', 'PPAP', 'FMEA', 'CONTROL_PLAN', 'MSA', 'SPC', 'EIGHT_D', 'LPA', 'SUPPLIER', 'GENERAL'];
const catColor = (c: string) => c === 'FMEA' ? 'bg-orange-100 text-orange-700' : c === 'PPAP' ? 'bg-blue-100 text-blue-700' : c === 'APQP' ? 'bg-purple-100 text-purple-700' : c === 'EIGHT_D' ? 'bg-red-100 text-red-700' : 'bg-gray-100 dark:bg-gray-800 text-gray-700';
...
export default function TemplatesPage() {
  ...
  async function load() {
    setLoading(true);
    try { const res = await api.get('/templates'); setItems(res.data.data || []); } catch (e) { console.error(e); } finally { setLoading(false); }
  }
    ...
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <div><h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">Templates</h1><p className="text-gray-500 dark:text-gray-400 mt-1">IATF 16949 document and form templates</p></div>
        </div>
              ...
              <Card key={item.id} className="hover:shadow-md transition-shadow">
                <CardContent className="pt-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {item.downloadUrl && (
                      <a href={item.downloadUrl} className="flex items-center gap-1 text-xs text-orange-600 hover:text-orange-700">
