...
const MODULE_FILTER = 'AUTOMOTIVE';
...
const CATEGORIES = [
  ...
  { value: 'CONTROL_PLAN', label: 'Control Plan' },
  { value: 'SPC', label: 'SPC' },
  { value: 'MSA', label: 'MSA' },
  { value: '8D', label: '8D Report' },
...
export default function TemplatesClient() {
  ...
  const fetchTemplates = useCallback(async () => {
    ...
    try {
      const response = await gateway.get('/templates', {
        params: { module: MODULE_FILTER, search: search || undefined, category: categoryFilter || undefined, limit: 50 },
      });
  ...
  const renderFieldInput = (field: FieldDefinition) => {
    switch (field.type) {
      case 'section':
        return <h3 key={field.name} className="text-lg font-semibold text-gray-800 mt-4 mb-2 border-b pb-1">{field.label}</h3>;
      case 'textarea':
        return (
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-500 dark:text-gray-400 mt-1">IATF 16949 and APQP/PPAP templates</p>
      </div>
            ...
            <Card key={template.id} className="hover:shadow-md transition-shadow">
              <CardContent className="p-5">
                <div className="flex items-start justify-between mb-2">
                  <Badge className="bg-orange-100 text-orange-700 text-xs">{template.code}</Badge>
                ...
                <div className="flex items-center gap-2 border-t pt-3">
                  <button
                    onClick={() => { setSelectedTemplate(template); setShowUseModal(false); }}
                    className="flex items-center gap-1 text-xs text-gray-600 hover:text-orange-600 transition-colors"
                    title="Preview"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 text-xs text-white bg-orange-600 hover:bg-orange-700 px-2 py-1 rounded transition-colors"
                    title="Use Template"
                  >
      ...
      >
        {selectedTemplate && (
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              <Badge className="bg-orange-100 text-orange-700">{selectedTemplate.code}</Badge>
                ...
                {selectedTemplate.fieldDefinitions?.map((field, idx) => (
                  <li key={idx} className="flex items-start gap-2 text-sm">
                    {field.type === 'section' ? (
                      ...
                      <>
                        <span className="inline-block bg-orange-50 text-orange-600 text-xs px-2 py-0.5 rounded font-mono">{field.type}</span>
        ...
        <ModalFooter>
          ...
          <Button className="bg-orange-600 hover:bg-orange-700 text-white" onClick={() => selectedTemplate && handleOpenUse(selectedTemplate)}>
            Use Template
          </Button>
        </ModalFooter>
      </Modal>
      ...
      >
        ...
        <ModalFooter>
          ...
          <Button className="bg-orange-600 hover:bg-orange-700 text-white" onClick={handleSubmitUse}>
            Submit
          </Button>
        </ModalFooter>
      </Modal>
