...
dotenv.config();
initSentry('api-reg-monitor');
initTracing({ serviceName: 'api-reg-monitor' });
...
const logger = createLogger('api-reg-monitor');
import changesRouter from './routes/changes';
import legalRegisterRouter from './routes/legal-register';
...
app.use(metricsMiddleware('api-reg-monitor'));
...
app.get('/health', createHealthCheck('api-reg-monitor', prisma, '1.0.0'));
...
app.use('/api/legal-register', legalRegisterRouter);
...
const server = app.listen(PORT, () => {
  logger.info(`Regulatory Monitor API running on port ${PORT}`);
});
