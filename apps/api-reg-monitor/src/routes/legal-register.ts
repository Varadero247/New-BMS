...
async function generateRef(orgId: string): Promise<string> {
  ...
  const c = await prisma.regLegalRegister.count({ where: { orgId, referenceNumber: { startsWith: `RLR-${y}` } } });
...
router.get('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const [data, total] = await Promise.all([
      prisma.regLegalRegister.findMany({ where, skip, take: parseInt(limit), orderBy: { createdAt: 'desc' } }),
    ...
    res.status(500).json({ success: false, error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch legal register entries' } });
  }
});
...
router.get('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const item = await prisma.regLegalRegister.findFirst({ where: { id: req.params.id, orgId, deletedAt: null } as any });
    ...
    res.status(500).json({ success: false, error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch legal register entry' } });
  }
});
...
router.put('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    const orgId = ((req as any).user as any)?.orgId || 'default';
    const existing = await prisma.regLegalRegister.findFirst({ where: { id: req.params.id, orgId, deletedAt: null } as any });
