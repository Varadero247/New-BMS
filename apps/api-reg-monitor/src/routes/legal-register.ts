import { Router, Request, Response } from 'express';
import { z } from 'zod';
import { authenticate } from '@ims/auth';
import { createLogger } from '@ims/monitoring';
import { prisma } from '../prisma';

const router = Router();
const logger = createLogger('reg-monitor-legal-register');

const createSchema = z.object({
  title: z.string().min(1, 'Title is required'),
  legislation: z.string().optional(),
  jurisdiction: z.string().optional(),
  applicability: z.string().optional(),
  requirements: z.string().optional(),
  complianceStatus: z.string().optional(),
  responsiblePerson: z.string().optional(),
  reviewDate: z.string().optional(),
  lastReviewDate: z.string().optional(),
  notes: z.string().optional(),
});

const updateSchema = createSchema.partial();

async function generateRef(orgId: string): Promise<string> {
  const y = new Date().getFullYear();
  const c = await (prisma as any).regLegalRegister.count({ where: { orgId, referenceNumber: { startsWith: `RLR-${y}` } } });
  return `RLR-${y}-${String(c + 1).padStart(4, '0')}`;
}

router.get('/', authenticate, async (req: Request, res: Response) => {
  try {
    const orgId = (req as any).user?.orgId || 'default';
    const { status, search, page = '1', limit = '20' } = req.query as Record<string, string>;
    const where: any = { orgId, deletedAt: null };
    if (status) where.status = status;
    if (search) where.title = { contains: search, mode: 'insensitive' };
    const skip = (parseInt(page) - 1) * parseInt(limit);
    const [data, total] = await Promise.all([
      (prisma as any).regLegalRegister.findMany({ where, skip, take: parseInt(limit), orderBy: { createdAt: 'desc' } }),
      (prisma as any).regLegalRegister.count({ where }),
    ]);
    res.json({ success: true, data, pagination: { page: parseInt(page), limit: parseInt(limit), total, pages: Math.ceil(total / parseInt(limit)) } });
  } catch (error: any) {
    logger.error('Fetch failed', { error: error.message });
    res.status(500).json({ success: false, error: { code: 'FETCH_ERROR', message: 'Failed to fetch legal register entrys' } });
  }
});

router.get('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    const item = await (prisma as any).regLegalRegister.findFirst({ where: { id: req.params.id, deletedAt: null } });
    if (!item) return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'legal register entry not found' } });
    res.json({ success: true, data: item });
  } catch (error: any) {
    res.status(500).json({ success: false, error: { code: 'FETCH_ERROR', message: 'Failed to fetch legal register entry' } });
  }
});

router.post('/', authenticate, async (req: Request, res: Response) => {
  try {
    const parsed = createSchema.safeParse(req.body);
    if (!parsed.success) {
      return res.status(400).json({ success: false, error: { code: 'VALIDATION_ERROR', message: parsed.error.errors[0].message } });
    }
    const orgId = (req as any).user?.orgId || 'default';
    const referenceNumber = await generateRef(orgId);
    const { title, legislation, jurisdiction, applicability, requirements, complianceStatus, responsiblePerson, reviewDate, lastReviewDate, notes } = parsed.data;
    const data = await (prisma as any).regLegalRegister.create({
      data: {
        title, legislation, jurisdiction, applicability, requirements, complianceStatus, responsiblePerson,
        reviewDate: reviewDate ? new Date(reviewDate) : undefined,
        lastReviewDate: lastReviewDate ? new Date(lastReviewDate) : undefined,
        notes,
        orgId, referenceNumber, createdBy: (req as any).user?.id, updatedBy: (req as any).user?.id,
      },
    });
    res.status(201).json({ success: true, data });
  } catch (error: any) {
    res.status(400).json({ success: false, error: { code: 'CREATE_ERROR', message: error.message } });
  }
});

router.put('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    const existing = await (prisma as any).regLegalRegister.findFirst({ where: { id: req.params.id, deletedAt: null } });
    if (!existing) return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'legal register entry not found' } });
    const parsed = updateSchema.safeParse(req.body);
    if (!parsed.success) {
      return res.status(400).json({ success: false, error: { code: 'VALIDATION_ERROR', message: parsed.error.errors[0].message } });
    }
    const { title, legislation, jurisdiction, applicability, requirements, complianceStatus, responsiblePerson, reviewDate, lastReviewDate, notes } = parsed.data;
    const updateData: any = { updatedBy: (req as any).user?.id };
    if (title !== undefined) updateData.title = title;
    if (legislation !== undefined) updateData.legislation = legislation;
    if (jurisdiction !== undefined) updateData.jurisdiction = jurisdiction;
    if (applicability !== undefined) updateData.applicability = applicability;
    if (requirements !== undefined) updateData.requirements = requirements;
    if (complianceStatus !== undefined) updateData.complianceStatus = complianceStatus;
    if (responsiblePerson !== undefined) updateData.responsiblePerson = responsiblePerson;
    if (reviewDate !== undefined) updateData.reviewDate = new Date(reviewDate);
    if (lastReviewDate !== undefined) updateData.lastReviewDate = new Date(lastReviewDate);
    if (notes !== undefined) updateData.notes = notes;
    const data = await (prisma as any).regLegalRegister.update({ where: { id: req.params.id }, data: updateData });
    res.json({ success: true, data });
  } catch (error: any) {
    res.status(500).json({ success: false, error: { code: 'UPDATE_ERROR', message: error.message } });
  }
});

router.delete('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    const existing = await (prisma as any).regLegalRegister.findFirst({ where: { id: req.params.id, deletedAt: null } });
    if (!existing) return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'legal register entry not found' } });
    await (prisma as any).regLegalRegister.update({ where: { id: req.params.id }, data: { deletedAt: new Date(), updatedBy: (req as any).user?.id } });
    res.json({ success: true, data: { message: 'legal register entry deleted successfully' } });
  } catch (error: any) {
    res.status(500).json({ success: false, error: { code: 'DELETE_ERROR', message: error.message } });
  }
});

export default router;
