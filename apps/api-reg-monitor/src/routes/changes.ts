...
async function generateRef(orgId: string): Promise<string> {
  ...
  const c = await prisma.regChange.count({
    where: { orgId, referenceNumber: { startsWith: `RGC-${y}` } },
  });
...
router.get('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch regulatory changes' },
    });
  }
});
  ...
  try {
    ...
    if (!item)
      return res.status(404).json({
        success: false,
        error: { code: 'NOT_FOUND', message: 'regulatory change not found' },
      });
    ...
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch regulatory change' },
    });
  }
});
