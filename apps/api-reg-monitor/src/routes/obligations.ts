...
router.get('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch obligations' },
    });
  }
});
...
router.get('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    if (!item)
      return res
        .status(404)
        .json({ success: false, error: { code: 'NOT_FOUND', message: 'obligation not found' } });
    ...
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch obligation' },
    });
  }
});
...
router.post('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const { title, description, source, dueDate, frequency, responsible, status, evidence, notes } =
      parsed.data;
...
router.put('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const data = await prisma.regObligation.update({
  ...
  try {
    ...
    if (!existing)
      return res
        .status(404)
        .json({ success: false, error: { code: 'NOT_FOUND', message: 'obligation not found' } });
    await prisma.regObligation.update({
      where: { id: req.params.id },
