...
async function generateRef(orgId: string): Promise<string> {
  ...
  const c = await prisma.compCommunication.count({
    where: { orgId, referenceNumber: { startsWith: `CMC-${y}` } } as any,
  });
  return `CMC-${y}-${String(c + 1).padStart(4, '0')}`;
}
router.get('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const [data, total] = await Promise.all([
      prisma.compCommunication.findMany({
        where,
        skip,
        take: Math.min(Math.max(1, parseInt(limit, 10) || 20), 100),
        orderBy: { createdAt: 'desc' },
      }),
      prisma.compCommunication.count({ where }),
    ]);
    res.json({
      success: true,
      data,
    ...
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch communications' },
    });
  }
});
router.get('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    const orgId = ((req as AuthRequest).user as { orgId?: string })?.orgId || 'default';
    const item = await prisma.compCommunication.findFirst({
      where: { id: req.params.id, orgId, deletedAt: null } as any,
    });
    if (!item)
      return res
        .status(404)
        .json({ success: false, error: { code: 'NOT_FOUND', message: 'communication not found' } });
    res.json({ success: true, data: item });
  } catch (error: unknown) {
    logger.error('Failed to process request', { error: (error as Error).message });
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch communication' },
    });
  }
});
router.post('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const { complaintId, direction, channel, subject, content, sentAt, sentBy } = parsed.data;
    const data = await prisma.compCommunication.create({
      data: {
        complaintId,
        direction,
        channel,
...
router.put('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const orgId = ((req as AuthRequest).user as { orgId?: string })?.orgId || 'default';
    const existing = await prisma.compCommunication.findFirst({
      where: { id: req.params.id, orgId, deletedAt: null } as any,
    });
    if (!existing)
      return res
        ...
        .json({ success: false, error: { code: 'NOT_FOUND', message: 'communication not found' } });
    const { complaintId, direction, channel, subject, content, sentAt, sentBy } = parsed.data;
    const data = await prisma.compCommunication.update({
  ...
  try {
    ...
    if (!existing)
      return res
        .status(404)
        .json({ success: false, error: { code: 'NOT_FOUND', message: 'communication not found' } });
    await prisma.compCommunication.update({
      where: { id: req.params.id },
      data: { deletedAt: new Date(), updatedBy: (req as AuthRequest).user?.id } as any,
    });
    res.json({ success: true, data: { message: 'communication deleted successfully' } });
  } catch (error: unknown) {
    logger.error('Failed to process request', { error: (error as Error).message });
    res.status(500).json({
      success: false,
