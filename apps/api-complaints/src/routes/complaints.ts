...
async function generateRef(orgId: string): Promise<string> {
  ...
  const c = await prisma.compComplaint.count({
    where: { orgId, referenceNumber: { startsWith: `CMP-${y}` } },
  });
  return `CMP-${y}-${String(c + 1).padStart(4, '0')}`;
}
...
router.get('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    const orgId = (req as any).user?.orgId || 'default';
    const item = await prisma.compComplaint.findFirst({
...
router.put('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const orgId = (req as any).user?.orgId || 'default';
    const existing = await prisma.compComplaint.findFirst({
      where: { id: req.params.id, orgId, deletedAt: null } as any,
    });
    if (!existing)
      return res
    ...
    const data = await prisma.compComplaint.update({
  ...
  try {
    ...
    if (!existing)
      return res
        .status(404)
        .json({ success: false, error: { code: 'NOT_FOUND', message: 'complaint not found' } });
    await prisma.compComplaint.update({
      where: { id: req.params.id },
