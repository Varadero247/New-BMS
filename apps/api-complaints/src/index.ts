...
dotenv.config();
initSentry('api-complaints');
initTracing({ serviceName: 'api-complaints' });
...
const logger = createLogger('api-complaints');
...
app.use(metricsMiddleware('api-complaints'));
...
app.get('/health', createHealthCheck('api-complaints', prisma, '1.0.0'));
...
const server = app.listen(PORT, () => {
  logger.info(`Complaints API server running on port ${PORT}`);
});
