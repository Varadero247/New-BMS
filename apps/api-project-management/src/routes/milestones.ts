...
router.get('/', scopeToUser, async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to list milestones' },
    });
  }
});
  ...
  try {
    ...
    const milestone = await prisma.projectMilestone.create({
      data: {
        ...
        status: (data.status || 'UPCOMING') as import('@ims/database/project-management').PmMilestoneStatus,
...
router.put(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      res.status(500).json({
        success: false,
        error: { code: 'INTERNAL_ERROR', message: 'Failed to approve milestone' },
      });
    }
  }
);
