...
router.post('/', async (req: Request, res: Response) => {
  try {
    const schema = z.object({
      ...
      milestonesAchieved: z.string().trim().optional(),
      upcomingMilestones: z.string().trim().optional(),
      ...
      keyIssues: z.string().trim().optional(),
      keyRisks: z.string().trim().optional(),
      accomplishments: z.string().trim().optional(),
      nextPeriodPlans: z.string().trim().optional(),
    ...
    const report = await prisma.projectStatusReport.create({
      data: {
        ...
        reportType: (data.reportType || 'WEEKLY') as import('@ims/database/project-management').ReportType,
        reportDate: new Date(),
