...
export default function DashboardPage() {
  ...
  return (
    <div className="flex min-h-screen"><Sidebar /><main className="flex-1 p-8"><div className="max-w-7xl mx-auto"><div className="mb-8"><h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">Incidents Dashboard</h1><p className="text-gray-500 dark:text-gray-400 mt-1">Overview and key metrics</p></div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{Object.entries(stats).map(([key, value]) => (<Card key={key} className="hover:shadow-md transition-shadow"><CardContent className="p-6"><div className="flex items-center justify-between"><div><p className="text-sm text-gray-500 dark:text-gray-400">{key.replace(/([A-Z])/g, ' $1').trim()}</p><p className="text-2xl font-bold mt-1">{String(value)}</p></div><div className="p-3 rounded-lg bg-red-50 dark:bg-red-900/20"><LayoutDashboard className="h-6 w-6 text-red-600" /></div></div></CardContent></Card>))}</div>
    </div></main></div>
  );
}
