          ...
          {group.items.map((item) => {
            ...
            return (
              <li key={item.name}>
                ...
                >
                  <Icon className={`h-4 w-4 shrink-0 ${isActive ? 'text-red-600 dark:text-red-400' : 'text-gray-400 dark:text-gray-500'}`} />
  ...
  return (
    <aside className="w-64 bg-card border-r border-border flex flex-col h-full min-h-screen">
      <div className="p-4 border-b border-border bg-red-50 dark:bg-red-900/10">
        <h1 className="text-lg font-bold font-display text-red-900 dark:text-red-100">Incidents</h1>
        <p className="text-xs text-red-600 dark:text-red-400 mt-0.5">Incident Management System</p>
      </div>
      <nav className="flex-1 p-3 overflow-y-auto" aria-label="Incidents module navigation">
        {navGroups.map((group) => (
