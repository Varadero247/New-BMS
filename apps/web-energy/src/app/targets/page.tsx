...
const empty: Partial<EnergyTarget> = {
  name: '',
  description: '',
  targetValue: 0,
  currentValue: 0,
...
export default function TargetsPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEditing && editItem.id) {
        await api.put(`/targets/${editItem.id}`, editItem);
      } else {
        await api.post('/targets', editItem);
      }
      setModalOpen(false);
      await load();
    } catch (e) {
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      await api.delete(`/targets/${deleteId}`);
  ...
  return (
    <div className="p-8">
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">
              Energy reduction targets and objectives (ISO 50001 §6.2)
            </p>
          </div>
          <button
            onClick={openCreate}
            ...
            <CardContent className="pt-5">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-emerald-700">{stats.achieved}</p>
                </div>
            ...
            <CardContent className="pt-5">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-red-700">{stats.atRisk}</p>
                </div>
                      ...
                      return (
                        <tr key={item.id} className="border-b hover:bg-yellow-50 transition-colors">
                          ...
                          <td className="py-3 px-4 text-right font-mono font-medium">
                            {Number(item.targetValue).toLocaleString()}{' '}
                          ...
                          <td className="py-3 px-4">
                            <div className="flex items-center gap-2">
                              <div className="flex-1 bg-gray-200 rounded-full h-2">
                                <div
                                  className={`h-2 rounded-full ${pct >= 100 ? 'bg-emerald-500' : pct >= 70 ? 'bg-green-500' : pct >= 40 ? 'bg-yellow-500' : 'bg-red-500'}`}
                                  style={{ width: `${pct}%` }}
                                />
                              </div>
      ...
      <Modal
        ...
        title={isEditing ? 'Edit Target' : 'Set Energy Target'}
        size="lg"
      >
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="e.g. Reduce electricity consumption by 15%"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                type="date"
                value={editItem.targetDate || ''}
                onChange={(e) => setEditItem((p) => ({ ...p, targetDate: e.target.value }))}
                className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400"
              />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                value={editItem.status || 'ON_TRACK'}
