...
export default function BaselinesPage() {
  ...
  const load = async () => {
    try {
      const r = await api.get('/baselines');
  ...
  const stats = {
    ...
    avgVariance: items.length
      ? Math.round(items.reduce((s, i) => s + (i.variance || 0), 0) / items.length)
      : 0,
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEditing && editItem.id) {
        await api.put(`/baselines/${editItem.id}`, editItem);
      } else {
        await api.post('/baselines', editItem);
      }
      setModalOpen(false);
      await load();
    } catch (e) {
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      await api.delete(`/baselines/${deleteId}`);
  ...
  return (
    <div className="p-8">
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">
              Energy performance baselines (EnB) per ISO 50001
            </p>
          </div>
          <button
            onClick={openCreate}
          ...
          <Card>
            <CardContent className="pt-5">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-gray-500 dark:text-gray-400">Total Baselines</p>
            ...
            <CardContent className="pt-5">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-blue-700">{stats.avgVariance}%</p>
                </div>
            ...
            {filtered.length > 0 ? (
                      ...
                      return (
                        <tr key={item.id} className="border-b hover:bg-yellow-50 transition-colors">
                          ...
                          <td className="py-3 px-4 text-right">
                            <span
                              className={`font-medium ${variance < 0 ? 'text-green-600' : variance > 0 ? 'text-red-600' : 'text-gray-500 dark:text-gray-400'}`}
                            >
                              {variance > 0 ? '+' : ''}
                              {variance}%
                            </span>
                          ...
                          <td className="py-3 px-4">
                            <span
                              className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${sc.className}`}
                            >
                              {sc.label}
                            </span>
                          </td>
              ...
              <div className="text-center py-12 text-gray-500 dark:text-gray-400">
                ...
                <p className="text-sm mt-1">
                  Create your first energy baseline to track performance
                </p>
              </div>
            )}
          </CardContent>
      ...
      <Modal
        ...
        title={isEditing ? 'Edit Baseline' : 'Create Energy Baseline'}
        size="lg"
      >
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="e.g. 2024 Electricity Baseline"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                onChange={(e) => setEditItem((p) => ({ ...p, startDate: e.target.value }))}
                className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400"
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) => setEditItem((p) => ({ ...p, endDate: e.target.value }))}
                className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400"
              />
            </div>
          </div>
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              ...
              <select
                value={editItem.energySource || 'ELECTRICITY'}
                onChange={(e) => setEditItem((p) => ({ ...p, energySource: e.target.value }))}
                className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400"
              >
      ...
      >
        <p className="text-gray-600 text-sm">
          Are you sure you want to delete this baseline? This cannot be undone.
        </p>
        <ModalFooter>
          <button
