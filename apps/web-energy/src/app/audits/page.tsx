  ...
  const openEdit = (item: Audit) => {
    setEditItem({
      ...
      completedDate: item.completedDate
        ? new Date(item.completedDate).toISOString().slice(0, 10)
        : '',
    });
    setIsEditing(true);
    setModalOpen(true);
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEditing && editItem.id) {
        await api.put(`/audits/${editItem.id}`, editItem);
      } else {
        await api.post('/audits', editItem);
      }
      setModalOpen(false);
      await load();
    } catch (e) {
  ...
  return (
    <div className="p-8">
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">
              Internal and external energy audit management (ISO 50001 §9.2)
            </p>
          </div>
          <button
            onClick={openCreate}
                      ...
                      return (
                        <tr key={item.id} className="border-b hover:bg-yellow-50 transition-colors">
                          ...
                          <td className="py-3 px-4 text-center">
                            <span className="font-medium">{item.findings ?? '-'}</span>
                          </td>
                          <td className="py-3 px-4 text-center">
                            <span
                              className={`font-medium ${(item.nonConformities || 0) > 0 ? 'text-red-600' : 'text-gray-500 dark:text-gray-400'}`}
                            >
      ...
      <Modal
        ...
        title={isEditing ? 'Edit Audit' : 'Schedule Energy Audit'}
        size="lg"
      >
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                placeholder="e.g. Annual Internal Energy Audit 2026"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
          ...
          <div>
            ...
            <input
              ...
              className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400"
              placeholder="Areas or systems to be audited..."
            />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                onChange={(e) => setEditItem((p) => ({ ...p, auditor: e.target.value }))}
                className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400"
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) => setEditItem((p) => ({ ...p, scheduledDate: e.target.value }))}
                className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400"
              />
            </div>
          </div>
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <input
                ...
                onChange={(e) => setEditItem((p) => ({ ...p, findings: Number(e.target.value) }))}
                className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400"
              />
            </div>
            <div>
              ...
              <input
                type="number"
                value={editItem.nonConformities ?? ''}
                onChange={(e) =>
                  setEditItem((p) => ({ ...p, nonConformities: Number(e.target.value) }))
                }
                className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400"
              />
            </div>
            <div>
              ...
              <select
                value={editItem.status || 'SCHEDULED'}
