    ...
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Energy Audits</h1>
            <p className="text-gray-500 mt-1">Internal and external energy audit management (ISO 50001 §9.2)</p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <Card><CardContent className="pt-5"><div className="flex items-center justify-between"><div><p className="text-sm text-gray-500">Total Audits</p><p className="text-2xl font-bold text-gray-900">{stats.total}</p></div><div className="p-3 bg-yellow-50 rounded-full"><ClipboardCheck className="h-6 w-6 text-yellow-600" /></div></div></CardContent></Card>
          <Card><CardContent className="pt-5"><div className="flex items-center justify-between"><div><p className="text-sm text-gray-500">Scheduled</p><p className="text-2xl font-bold text-blue-700">{stats.scheduled}</p></div><div className="p-3 bg-blue-50 rounded-full"><Calendar className="h-6 w-6 text-blue-600" /></div></div></CardContent></Card>
          ...
          <Card><CardContent className="pt-5"><div className="flex items-center justify-between"><div><p className="text-sm text-gray-500">Overdue</p><p className="text-2xl font-bold text-red-700">{stats.overdue}</p></div><div className="p-3 bg-red-50 rounded-full"><AlertTriangle className="h-6 w-6 text-red-600" /></div></div></CardContent></Card>
        </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead><tr className="border-b bg-gray-50">
                    ...
                    <th className="text-center py-3 px-4 font-medium text-gray-500">NCs</th>
                      ...
                      return (
                        <tr key={item.id} className="border-b hover:bg-yellow-50 transition-colors">
                          <td className="py-3 px-4">
                            ...
                            {item.scope && <p className="text-xs text-gray-400 truncate max-w-[180px]">{item.scope}</p>}
                          </td>
                          ...
                          <td className="py-3 px-4 text-center"><span className={`font-medium ${(item.nonConformities || 0) > 0 ? 'text-red-600' : 'text-gray-500'}`}>{item.nonConformities ?? '-'}</span></td>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={isEditing ? 'Edit Audit' : 'Schedule Energy Audit'} size="lg">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Non-Conformities</label>
