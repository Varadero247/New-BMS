...
import { Plus, Search, Activity, TrendingUp, TrendingDown, Minus, CheckCircle, AlertCircle, Edit2, Trash2 } from 'lucide-react';
...
const CATEGORIES = ['CONSUMPTION', 'INTENSITY', 'COST', 'RENEWABLE', 'EMISSION', 'EFFICIENCY', 'OTHER'];
...
const empty: Partial<EnPI> = { name: '', description: '', formula: '', currentValue: 0, baselineValue: 0, targetValue: 0, unit: 'kWh/unit', trend: 'IMPROVING', status: 'ACTIVE', category: 'INTENSITY', reportingFrequency: 'MONTHLY' };
...
export default function EnPIsPage() {
  ...
  const load = async () => {
    try { const r = await api.get('/enpis'); setItems(r.data.data || []); }
  ...
  const openEdit = (item: EnPI) => { setEditItem({ ...item }); setIsEditing(true); setModalOpen(true); };

  const handleSave = async () => {
    setSaving(true);
    try {
      if (isEditing && editItem.id) { await api.put(`/enpis/${editItem.id}`, editItem); }
      else { await api.post('/enpis', editItem); }
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try { await api.delete(`/enpis/${deleteId}`); setDeleteModal(false); await load(); } catch (e) { console.error(e); }
  };
  ...
  return (
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex gap-3 mb-4 flex-wrap">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input type="text" aria-label="Search EnPIs..." placeholder="Search EnPIs..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10 pr-4 py-2 border rounded-lg w-full text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400" />
          </div>
          <select aria-label="Filter by trend" value={filterTrend} onChange={e => setFilterTrend(e.target.value)} className="px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400">
        ...
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2"><Activity className="h-5 w-5 text-yellow-600" />EnPIs ({filtered.length})</CardTitle></CardHeader>
          <CardContent>
            {filtered.length > 0 ? (
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {filtered.map(item => {
                      ...
                      const TIcon = tc.icon;
                      const improvePct = item.baselineValue > 0 ? Math.round(((item.baselineValue - item.currentValue) / item.baselineValue) * 100) : 0;
                      const onTarget = item.targetValue != null && item.currentValue <= item.targetValue;
                      return (
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={isEditing ? 'Edit EnPI' : 'Add Energy Performance Indicator'} size="lg">
