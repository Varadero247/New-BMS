...
export default function ProjectsPage() {
  ...
  const totalSavings = items
    ...
    .reduce((s, i) => s + (i.actualSavings || i.estimatedSavings || 0), 0);
  const stats = {
    total: items.length,
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEditing && editItem.id) {
        await api.put(`/projects/${editItem.id}`, editItem);
      } else {
        await api.post('/projects', editItem);
      }
      setModalOpen(false);
      await load();
    } catch (e) {
  ...
  return (
    <div className="p-8">
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">
              Energy improvement projects and initiatives
            </p>
          </div>
          <button
            onClick={openCreate}
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-blue-700">
                    {stats.totalSavings.toLocaleString()} kWh
                  </p>
                </div>
                    ...
                    {filtered.map((item) => {
                      ...
                      return (
                        <tr key={item.id} className="border-b hover:bg-yellow-50 transition-colors">
                          ...
                          <td className="py-3 px-4 text-right font-mono text-green-700 font-medium">
                            {item.estimatedSavings
                              ? Number(item.estimatedSavings).toLocaleString()
                              : '-'}{' '}
      ...
      <Modal
        ...
        title={isEditing ? 'Edit Project' : 'Add Energy Project'}
        size="lg"
      >
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="e.g. LED Lighting Upgrade - Building A"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                value={editItem.type || 'LED_LIGHTING'}
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <input
                type="number"
                value={editItem.estimatedSavings || ''}
                onChange={(e) =>
                  setEditItem((p) => ({ ...p, estimatedSavings: Number(e.target.value) }))
                }
                className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400"
              />
            </div>
            ...
            <div>
              ...
              <input
                type="number"
                value={editItem.estimatedCost || ''}
                onChange={(e) =>
                  setEditItem((p) => ({ ...p, estimatedCost: Number(e.target.value) }))
                }
                className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400"
              />
            </div>
