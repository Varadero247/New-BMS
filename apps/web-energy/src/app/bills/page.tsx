  ...
  const openEdit = (item: Bill) => {
    setEditItem({
      ...
      billingPeriodStart: item.billingPeriodStart
        ? new Date(item.billingPeriodStart).toISOString().slice(0, 10)
        : '',
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEditing && editItem.id) {
        await api.put(`/bills/${editItem.id}`, editItem);
      } else {
        await api.post('/bills', editItem);
      }
      setModalOpen(false);
      await load();
    } catch (e) {
  ...
  return (
    <div className="p-8">
            ...
            <CardContent className="pt-5">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-red-700">${totalUnpaid.toLocaleString()}</p>
                </div>
                        ...
                        <tr key={item.id} className="border-b hover:bg-yellow-50 transition-colors">
                          ...
                          <td className="py-3 px-4 text-gray-600 text-xs">
                            {item.billingPeriodStart
                              ? new Date(item.billingPeriodStart).toLocaleDateString()
                              : '-'}
                            {item.billingPeriodEnd
                              ? ` – ${new Date(item.billingPeriodEnd).toLocaleDateString()}`
                              : ''}
                          </td>
                          ...
                          <td className="py-3 px-4 text-right font-mono text-gray-600">
                            {item.usage
                              ? `${Number(item.usage).toLocaleString()} ${item.unit || ''}`
                              : '-'}
                          </td>
                          <td className="py-3 px-4 text-gray-600 text-xs">
      ...
      <Modal
        ...
        title={isEditing ? 'Edit Bill' : 'Add Energy Bill'}
        size="lg"
      >
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                onChange={(e) => setEditItem((p) => ({ ...p, provider: e.target.value }))}
            ...
            <div>
              ...
              <select
                value={editItem.energyType || 'ELECTRICITY'}
                onChange={(e) => setEditItem((p) => ({ ...p, energyType: e.target.value }))}
                className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400"
              >
                {ENERGY_TYPES.map((t) => (
                  <option key={t} value={t}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                onChange={(e) => setEditItem((p) => ({ ...p, invoiceNumber: e.target.value }))}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                type="date"
                value={editItem.billingPeriodStart || ''}
                onChange={(e) => setEditItem((p) => ({ ...p, billingPeriodStart: e.target.value }))}
                className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400"
              />
            </div>
            <div>
              ...
              <input
                type="date"
                value={editItem.billingPeriodEnd || ''}
                onChange={(e) => setEditItem((p) => ({ ...p, billingPeriodEnd: e.target.value }))}
                className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400"
              />
            </div>
          </div>
          <div className="grid grid-cols-4 gap-3">
            <div className="col-span-2">
              ...
              <input
                ...
                onChange={(e) => setEditItem((p) => ({ ...p, amount: Number(e.target.value) }))}
                className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400"
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) => setEditItem((p) => ({ ...p, usage: Number(e.target.value) }))}
                className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400"
              />
            </div>
            <div>
        ...
        <ModalFooter>
          ...
          <button
            onClick={handleSave}
            disabled={saving || !editItem.provider}
