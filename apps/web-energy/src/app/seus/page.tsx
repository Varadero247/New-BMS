...
export default function SEUsPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEditing && editItem.id) {
        await api.put(`/seus/${editItem.id}`, editItem);
      } else {
        await api.post('/seus', editItem);
      }
      setModalOpen(false);
      await load();
    } catch (e) {
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      await api.delete(`/seus/${deleteId}`);
  ...
  return (
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">
              Identify and manage significant energy uses (SEUs) per ISO 50001 §6.3
            </p>
          </div>
          <button
            onClick={openCreate}
          ...
          <Card>
            <CardContent className="pt-5">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-gray-500 dark:text-gray-400">Total SEUs</p>
            ...
            <CardContent className="pt-5">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-700">{stats.optimized}</p>
                </div>
        ...
        <div className="flex gap-3 mb-4 flex-wrap">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              type="text"
              aria-label="Search SEUs..."
              placeholder="Search SEUs..."
              value={searchTerm}
            ...
            {filtered.length > 0 ? (
                  ...
                  <tbody>
                    {filtered.map((item) => {
                      ...
                      const share =
                        ...
                        (totalConsumption > 0
                          ? Math.round((item.consumption / totalConsumption) * 100)
                          : 0);
                      return (
                        <tr key={item.id} className="border-b hover:bg-yellow-50 transition-colors">
                          ...
                          <td className="py-3 px-4">
                            <span className="inline-flex px-2 py-0.5 rounded text-xs font-medium bg-yellow-50 text-yellow-700">
              ...
              <div className="text-center py-12 text-gray-500 dark:text-gray-400">
                ...
                <p className="text-sm mt-1">
                  Add your significant energy uses to identify improvement opportunities
                </p>
              </div>
            )}
          </CardContent>
      ...
      <Modal
        ...
        title={isEditing ? 'Edit SEU' : 'Add Significant Energy Use'}
        size="lg"
      >
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <input
                type="number"
                value={editItem.consumption || ''}
                onChange={(e) =>
                  setEditItem((p) => ({ ...p, consumption: Number(e.target.value) }))
                }
                className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400"
              />
            </div>
            ...
            <div>
              ...
              <input
                type="number"
                value={editItem.percentageOfTotal || ''}
                onChange={(e) =>
                  setEditItem((p) => ({ ...p, percentageOfTotal: Number(e.target.value) }))
                }
                className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400"
                placeholder="%"
              />
          ...
          <div>
            ...
            <textarea
              value={editItem.improvementOpportunities || ''}
              onChange={(e) =>
                setEditItem((p) => ({ ...p, improvementOpportunities: e.target.value }))
              }
              rows={2}
              className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400"
              placeholder="Describe potential improvements..."
            />
          </div>
        </div>
        <ModalFooter>
