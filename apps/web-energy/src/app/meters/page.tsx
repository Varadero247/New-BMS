    ...
    <div className="p-8">
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">
              Manage energy metering infrastructure
            </p>
          </div>
          <button
            onClick={openCreate}
                      ...
                      return (
                        <tr key={item.id} className="border-b hover:bg-yellow-50 transition-colors">
                          ...
                          <td className="py-3 px-4">
                            <span
                              className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${typeColors[item.meterType] || 'bg-gray-100 dark:bg-gray-800 text-gray-700'}`}
                            >
      ...
      >
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                value={editItem.meterType || 'ELECTRICITY'}
                onChange={(e) => setEditItem((p) => ({ ...p, meterType: e.target.value }))}
                className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400"
              >
                {METER_TYPES.map((t) => (
                  <option key={t} value={t}>
        ...
        <ModalFooter>
          ...
          <button
            ...
            className="px-4 py-2 text-sm bg-yellow-600 text-white rounded-lg hover:bg-yellow-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
          >
            {saving && (
      ...
      >
        <p className="text-gray-600 text-sm">
          Are you sure you want to delete this meter? This action cannot be undone and will remove
