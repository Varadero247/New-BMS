...
describe('GET /api/notices', () => {
  it('should return notices list', async () => {
    (prisma as any).contNotice.findMany.mockResolvedValue([{ id: '1', title: 'Notice A', dueDate: '2026-03-01' }]);
...
describe('GET /api/notices/:id', () => {
  it('should return notice by id', async () => {
    (prisma as any).contNotice.findFirst.mockResolvedValue({ id: '1', title: 'Notice A' });
  ...
  it('should return 404 if notice not found', async () => {
    ...
    const res = await request(app).get('/api/notices/nope');
...
describe('POST /api/notices', () => {
  it('should create a notice', async () => {
    (prisma as any).contNotice.count.mockResolvedValue(0);
    (prisma as any).contNotice.create.mockResolvedValue({ id: '1', contractId: 'c-1', title: 'New Notice', dueDate: '2026-03-01' });
...
describe('PUT /api/notices/:id', () => {
  it('should update a notice', async () => {
    (prisma as any).contNotice.findFirst.mockResolvedValue({ id: '1', title: 'Old Title' });
    (prisma as any).contNotice.update.mockResolvedValue({ id: '1', title: 'Updated Title' });
    const res = await request(app).put('/api/notices/1').send({ title: 'Updated Title' });
    expect(res.status).toBe(200);
    expect(res.body.success).toBe(true);
  });
  ...
  it('should return 404 if notice not found', async () => {
    (prisma as any).contNotice.findFirst.mockResolvedValue(null);
    const res = await request(app).put('/api/notices/nope').send({ title: 'Updated' });
    expect(res.status).toBe(404);
    expect(res.body.success).toBe(false);
    expect(res.body.error.code).toBe('NOT_FOUND');
  });
  ...
  it('should return 500 on update error', async () => {
    (prisma as any).contNotice.findFirst.mockResolvedValue({ id: '1' });
...
describe('DELETE /api/notices/:id', () => {
  it('should soft delete a notice', async () => {
    (prisma as any).contNotice.findFirst.mockResolvedValue({ id: '1' });
    (prisma as any).contNotice.update.mockResolvedValue({ id: '1', deletedAt: new Date() });
  ...
  it('should return 404 if notice not found', async () => {
    (prisma as any).contNotice.findFirst.mockResolvedValue(null);
    const res = await request(app).delete('/api/notices/nope');
    expect(res.status).toBe(404);
    expect(res.body.success).toBe(false);
    expect(res.body.error.code).toBe('NOT_FOUND');
  });
