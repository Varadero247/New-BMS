...
async function generateRef(orgId: string): Promise<string> {
  ...
  const c = await prisma.contClause.count({
    where: { orgId, referenceNumber: { startsWith: `CCL-${y}` } } as any,
  });
  return `CCL-${y}-${String(c + 1).padStart(4, '0')}`;
}
router.get('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const [data, total] = await Promise.all([
      prisma.contClause.findMany({
        where,
        skip,
        take: Math.min(Math.max(1, parseInt(limit, 10) || 20), 100),
        orderBy: { createdAt: 'desc' },
      }),
      prisma.contClause.count({ where }),
    ]);
    res.json({
      success: true,
      data,
    ...
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch clauses' },
    });
  }
});
router.get('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    const orgId = (req as any).user?.orgId || 'default';
    const item = await prisma.contClause.findFirst({
      where: { id: req.params.id, orgId, deletedAt: null } as any,
    });
    if (!item)
      return res
        .status(404)
        .json({ success: false, error: { code: 'NOT_FOUND', message: 'clause not found' } });
    res.json({ success: true, data: item });
  } catch (error: unknown) {
    logger.error('Failed to process request', { error: (error as Error).message });
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch clause' },
    });
  }
});
router.post('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const { contractId, title, content, clauseNumber, category, isKey } = parsed.data;
    const data = await prisma.contClause.create({
      data: {
        contractId,
        title,
        content,
...
router.put('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const orgId = (req as any).user?.orgId || 'default';
    const existing = await prisma.contClause.findFirst({
      where: { id: req.params.id, orgId, deletedAt: null } as any,
    });
    if (!existing)
      return res
        ...
        .json({ success: false, error: { code: 'NOT_FOUND', message: 'clause not found' } });
    const { contractId, title, content, clauseNumber, category, isKey } = parsed.data;
    const data = await prisma.contClause.update({
  ...
  try {
    ...
    if (!existing)
      return res
        .status(404)
        .json({ success: false, error: { code: 'NOT_FOUND', message: 'clause not found' } });
    await prisma.contClause.update({
      where: { id: req.params.id },
      data: { deletedAt: new Date(), updatedBy: (req as AuthRequest).user?.id } as any,
    });
    res.json({ success: true, data: { message: 'clause deleted successfully' } });
  } catch (error: unknown) {
    logger.error('Failed to process request', { error: (error as Error).message });
    res.status(500).json({
      success: false,
