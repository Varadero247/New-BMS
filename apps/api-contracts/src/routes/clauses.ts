...
async function generateRef(orgId: string): Promise<string> { const y = new Date().getFullYear(); const c = await prisma.contClause.count({ where: { orgId, referenceNumber: { startsWith: `CCL-${y}` } } as any }); return `CCL-${y}-${String(c + 1).padStart(4, '0')}`; }
