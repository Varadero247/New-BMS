...
router.get('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const [data, total] = await Promise.all([
      prisma.contApproval.findMany({
        where,
        skip,
        take: Math.min(Math.max(1, parseInt(limit, 10) || 20), 100),
        orderBy: { createdAt: 'desc' },
      }),
      prisma.contApproval.count({ where }),
    ]);
    res.json({
      success: true,
      data,
...
router.post('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const { contractId, approver, approverName, status, comments, decidedAt } = parsed.data;
    const data = await prisma.contApproval.create({
      data: {
        contractId,
        approver,
        approverName,
  ...
  try {
    ...
    if (!existing)
      return res
        ...
        .json({ success: false, error: { code: 'NOT_FOUND', message: 'approval not found' } });
    const { contractId, approver, approverName, status, comments, decidedAt } = parsed.data;
    const data = await prisma.contApproval.update({
