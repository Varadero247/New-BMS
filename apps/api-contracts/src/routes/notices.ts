...
router.get('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch notices' },
    });
  }
});
router.get('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch notice' },
    });
  }
});
router.post('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const data = await prisma.contNotice.create({
      data: {
        contractId,
        title,
        description,
...
router.put('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const data = await prisma.contNotice.update({
