...
async function generateRef(orgId: string): Promise<string> {
  ...
  const c = await (prisma as any).incIncident.count({
...
router.get('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const [data, total] = await Promise.all([
      (prisma as any).incIncident.findMany({ where, skip, take: parseInt(limit), orderBy: { createdAt: 'desc' } }),
      (prisma as any).incIncident.count({ where }),
    ]);
    res.json({
      success: true,
      data,
...
router.get('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    const item = await (prisma as any).incIncident.findFirst({ where: { id: req.params.id, deletedAt: null } });
...
router.post('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const data = await (prisma as any).incIncident.create({
      data: {
        ...parsed.data,
        orgId,
        referenceNumber,
...
router.put('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const existing = await (prisma as any).incIncident.findFirst({ where: { id: req.params.id, deletedAt: null } });
    if (!existing) return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'incident not found' } });
    const data = await (prisma as any).incIncident.update({
...
router.delete('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    const existing = await (prisma as any).incIncident.findFirst({ where: { id: req.params.id, deletedAt: null } });
    if (!existing) return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'incident not found' } });
    await (prisma as any).incIncident.update({
      where: { id: req.params.id },
