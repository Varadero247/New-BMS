...
router.get('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const [data, total] = await Promise.all([
      prisma.incIncident.findMany({ where, skip, take: Math.min(Math.max(1, parseInt(limit, 10) || 20), 100), orderBy: { createdAt: 'desc' } }),
      prisma.incIncident.count({ where }),
    ]);
    res.json({
      success: true,
      data,
      pagination: { page: parseInt(page, 10) || 1, limit: Math.max(1, parseInt(limit, 10) || 20), total, totalPages: Math.ceil(total / Math.max(1, parseInt(limit, 10) || 20)) },
    });
  } catch (error: unknown) {
