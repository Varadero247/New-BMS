...
describe('GET /api/dashboard/stats', () => {
  it('should return stats with totalIncidents', async () => {
    (prisma as any).incIncident.count.mockResolvedValue(42);
    const res = await request(app).get('/api/dashboard/stats');
    expect(res.status).toBe(200);
    expect(res.body.success).toBe(true);
    expect(res.body.data).toHaveProperty('totalIncidents', 42);
    expect((prisma as any).incIncident.count).toHaveBeenCalledWith({
      where: { orgId: 'org-1', deletedAt: null },
    });
  });

  it('should return 500 on database error', async () => {
    (prisma as any).incIncident.count.mockRejectedValue(new Error('DB error'));
    const res = await request(app).get('/api/dashboard/stats');
