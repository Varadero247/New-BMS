...
function getTypeColor(type: string) {
  return (
    CAPA_TYPES.find((t) => t.value === type)?.color || 'bg-gray-100 dark:bg-gray-800 text-gray-800'
  );
}
...
function getStatusColor(status: string) {
  return (
    STATUSES.find((s) => s.value === status)?.color || 'bg-gray-100 dark:bg-gray-800 text-gray-800'
  );
}
  ...
  async function generateAiAnalysis() {
    ...
    try {
      ...
      if (data) {
        ...
        if (data.effectivenessCriteria)
          updateForm('effectivenessCriteria', data.effectivenessCriteria);
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      await api.post('/capa', {
        ...
        containmentDate: form.containmentDate
          ? new Date(form.containmentDate).toISOString()
          : undefined,
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">
              ISO 14001 Clause 10.2 -- Corrective Action & Preventive Action
            </p>
          </div>
          <Button
            onClick={openModal}
          ...
          <div className="space-y-4">
            {filtered.map((capa) => {
              ...
              return (
                <Card
                  key={capa.id}
                  className={`hover:shadow-md transition-shadow ${overdue ? 'border-red-300 bg-red-50/30' : ''}`}
                >
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <span
                            className={`px-2 py-0.5 rounded-full text-xs font-medium ${getTypeColor(capa.capaType)}`}
                          >
                            {capa.capaType}
                          </span>
                          <span
                            className={`px-2 py-0.5 rounded-full text-xs font-medium ${getSeverityColor(capa.severity)}`}
                          >
                            {capa.severity}
                          </span>
                          {capa.rcaMethod && (
                          ...
                          <span
                            className={`px-2 py-0.5 rounded-full text-xs font-medium ${getStatusColor(capa.status)}`}
                          >
                        ...
                        <div className="flex items-center gap-4 mt-3 text-xs text-gray-400 dark:text-gray-500">
                          <span>
                            Responsible:{' '}
                            <strong className="text-gray-600">{capa.responsiblePerson}</strong>
                          </span>
                          <span>
                ...
                {form.rcaMethod === 'BOWTIE' && (
                  <div className="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-2 gap-6">
                      ...
                      <div className="space-y-3">
                        <div className="text-center text-xs font-semibold text-red-600 uppercase tracking-wider">
                          Threats (Left)
                        </div>
                        <div>
                          <Label>Threats</Label>
                      ...
                      <div className="space-y-3">
                        <div className="text-center text-xs font-semibold text-orange-600 uppercase tracking-wider">
                          Consequences (Right)
                        </div>
                        <div>
                ...
                <div className="flex items-center justify-between mb-2">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                      Define actions to address the root cause and prevent recurrence
                    </p>
                  </div>
                  <Button
                    size="sm"
                ...
                {form.capaActions.length > 0 ? (
                  ...
                  <div className="text-center py-8 border border-dashed border-gray-300 rounded-lg">
                    ...
                    <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">
                      Add actions manually or use AI Analysis to generate suggestions.
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                  ...
                  <p className="text-xs text-green-600 mb-4">
                    Use AI to generate root cause analysis, suggest actions, and validate your CAPA
                    approach. Ensure description is at least 20 characters for full analysis.
                  </p>
