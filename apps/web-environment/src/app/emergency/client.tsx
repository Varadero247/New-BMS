...
import { ShieldAlert, Plus, Search, AlertTriangle, CheckCircle, Clock, FileText } from 'lucide-react';
...
export default function EmergencyPreparednessPage() {
  ...
  const [planForm, setPlanForm] = useState({
    title: '', scenario: '', triggerConditions: '', immediateResponse: '',
    notificationReqs: '', containmentProcs: '', impactMitigation: '', recoveryActions: '',
  });

  const [showDrillModal, setShowDrillModal] = useState(false);
  const [drillForm, setDrillForm] = useState({
    planId: '', drillDate: '', drillType: 'TABLETOP', participants: '',
    scenario: '', outcome: 'SATISFACTORY', lessonsLearned: '', actionsRequired: '', conductedBy: '',
  });
  ...
  const handleCreatePlan = async () => {
    try {
      ...
      setPlanForm({ title: '', scenario: '', triggerConditions: '', immediateResponse: '', notificationReqs: '', containmentProcs: '', impactMitigation: '', recoveryActions: '' });
      fetchPlans();
      fetchDashboard();
  ...
  const handleCreateDrill = async () => {
    try {
      await api.post('/emergency/drills', {
        ...drillForm,
        participants: drillForm.participants.split(',').map(s => s.trim()).filter(Boolean),
      });
      setShowDrillModal(false);
      setDrillForm({ planId: '', drillDate: '', drillType: 'TABLETOP', participants: '', scenario: '', outcome: 'SATISFACTORY', lessonsLearned: '', actionsRequired: '', conductedBy: '' });
      fetchDrills();
      fetchDashboard();
    } catch (err) { console.error('Failed to create drill', err); }
  };
  ...
  return (
    <div className="space-y-6">
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <Button onClick={() => setShowPlanModal(true)}><Plus className="h-4 w-4 mr-2" /> New Plan</Button>
            </div>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <Button onClick={() => setShowDrillModal(true)}><Plus className="h-4 w-4 mr-2" /> Log Drill</Button>
            </div>
          </CardHeader>
          <CardContent>
      ...
      <Modal isOpen={showPlanModal} onClose={() => setShowPlanModal(false)} title="Create Emergency Plan" size="lg">
        <div className="space-y-4">
          <div><Label>Title *</Label><Input value={planForm.title} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setPlanForm({ ...planForm, title: e.target.value })} /></div>
          <div><Label>Scenario *</Label><Textarea rows={2} value={planForm.scenario} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setPlanForm({ ...planForm, scenario: e.target.value })} /></div>
          <div><Label>Trigger Conditions *</Label><Textarea rows={2} value={planForm.triggerConditions} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setPlanForm({ ...planForm, triggerConditions: e.target.value })} /></div>
          <div><Label>Immediate Response *</Label><Textarea rows={2} value={planForm.immediateResponse} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setPlanForm({ ...planForm, immediateResponse: e.target.value })} /></div>
          <div className="grid grid-cols-2 gap-4">
            <div><Label>Notification Requirements</Label><Textarea rows={2} value={planForm.notificationReqs} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setPlanForm({ ...planForm, notificationReqs: e.target.value })} /></div>
            <div><Label>Containment Procedures</Label><Textarea rows={2} value={planForm.containmentProcs} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setPlanForm({ ...planForm, containmentProcs: e.target.value })} /></div>
          </div>
        </div>
        <ModalFooter>
          <Button variant="outline" onClick={() => setShowPlanModal(false)}>Cancel</Button>
          <Button onClick={handleCreatePlan} disabled={!planForm.title || !planForm.scenario || !planForm.triggerConditions || !planForm.immediateResponse}>Create Plan</Button>
        </ModalFooter>
      </Modal>

      <Modal isOpen={showDrillModal} onClose={() => setShowDrillModal(false)} title="Log Emergency Drill" size="lg">
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <Label>Emergency Plan *</Label>
              <Select value={drillForm.planId} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setDrillForm({ ...drillForm, planId: e.target.value })}>
                <option value="">Select plan...</option>
                {plans.filter(p => p.status === 'ACTIVE').map(p => (<option key={p.id} value={p.id}>{p.refNumber} - {p.title}</option>))}
              </Select>
            </div>
            <div><Label>Drill Date *</Label><Input type="date" value={drillForm.drillDate} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setDrillForm({ ...drillForm, drillDate: e.target.value })} /></div>
            <div>
              <Label>Drill Type</Label>
              <Select value={drillForm.drillType} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setDrillForm({ ...drillForm, drillType: e.target.value })}>
            ...
            <div>
              <Label>Outcome</Label>
              <Select value={drillForm.outcome} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setDrillForm({ ...drillForm, outcome: e.target.value })}>
          ...
          <div><Label>Participants (comma-separated)</Label><Input value={drillForm.participants} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setDrillForm({ ...drillForm, participants: e.target.value })} /></div>
          <div><Label>Lessons Learned</Label><Textarea rows={2} value={drillForm.lessonsLearned} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setDrillForm({ ...drillForm, lessonsLearned: e.target.value })} /></div>
          <div><Label>Conducted By</Label><Input value={drillForm.conductedBy} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setDrillForm({ ...drillForm, conductedBy: e.target.value })} /></div>
        </div>
        <ModalFooter>
          <Button variant="outline" onClick={() => setShowDrillModal(false)}>Cancel</Button>
          <Button onClick={handleCreateDrill} disabled={!drillForm.planId || !drillForm.drillDate}>Log Drill</Button>
        </ModalFooter>
      </Modal>
    </div>
  );
