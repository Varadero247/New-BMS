...
export default function LifeCycleAssessmentPage() {
  ...
  const handleCreate = async () => {
    try {
      ...
      console.error('Failed to create LCA', err);
    }
  };
  ...
  const handleUpdateStage = async () => {
    ...
    try {
      await api.put(
        `/lifecycle/assessments/${selectedAssessment.id}/stages/${editingStage.stageName}`,
        stageForm
      );
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            ISO 14001 Clause 8.1 - Life cycle perspective
          </p>
        </div>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                  {selectedAssessment.refNumber} - {selectedAssessment.productProcess}
                </p>
              </div>
          ...
          <CardContent>
              ...
              {(selectedAssessment.stages || []).map((stage, i) => (
                <div key={stage.id} className="flex items-center gap-2">
                  ...
                  >
                    ...
                    {stage.severity && (
                      <Badge className={`mt-1 text-xs ${SEVERITY_COLORS[stage.severity] || ''}`}>
                        Severity: {stage.severity}
                      </Badge>
                    )}
                  </button>
                  {i < (selectedAssessment.stages?.length || 0) - 1 && (
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 text-center">
              Click a stage to edit its environmental aspects, impacts, and controls.
            </p>
          </CardContent>
        </Card>
      ) : (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b text-left text-gray-500 dark:text-gray-400">
                      ...
                      <th className="pb-2 pr-4">Product/Process</th>
                    ...
                    {assessments.map((a) => (
                      ...
                      >
                        ...
                        <td className="py-3 pr-4">
                          <Badge
                            className={STATUS_COLORS[a.status] || 'bg-gray-100 dark:bg-gray-800'}
                          >
      ...
      <Modal
        ...
        title="New Life Cycle Assessment"
        size="lg"
      >
        <div className="space-y-4">
          ...
          <div>
            <Label>Product/Process *</Label>
            <Input
              value={createForm.productProcess}
              onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                setCreateForm({ ...createForm, productProcess: e.target.value })
              }
            />
          </div>
          <div>
        ...
        <ModalFooter>
          ...
          <Button onClick={handleCreate} disabled={!createForm.title || !createForm.productProcess}>
            Create Assessment
          </Button>
        </ModalFooter>
      </Modal>
      ...
      <Modal
        ...
        title={
          editingStage
            ? `Edit Stage: ${STAGE_LABELS[editingStage.stageName] || editingStage.stageName}`
            : ''
        }
        size="lg"
      >
        <div className="space-y-4">
          <div>
            <Label>Environmental Aspects</Label>
            <Textarea
              rows={2}
              value={stageForm.aspects}
              onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) =>
                setStageForm({ ...stageForm, aspects: e.target.value })
              }
              placeholder="Energy use, emissions, waste generation..."
            />
          </div>
          <div>
            ...
            <Textarea
              rows={2}
              value={stageForm.impacts}
              onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) =>
                setStageForm({ ...stageForm, impacts: e.target.value })
              }
              placeholder="Climate change, resource depletion, pollution..."
            />
          </div>
          <div>
            <Label>Severity (1-5)</Label>
            <Select
              value={String(stageForm.severity)}
              onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
                setStageForm({ ...stageForm, severity: parseInt(e.target.value) })
              }
            >
          ...
          <div>
            ...
            <Textarea
              rows={2}
              value={stageForm.controls}
              onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) =>
                setStageForm({ ...stageForm, controls: e.target.value })
              }
            />
          </div>
          <div>
            <Label>Supplier Requirements</Label>
            <Textarea
              rows={2}
              value={stageForm.supplierReqs}
              onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) =>
                setStageForm({ ...stageForm, supplierReqs: e.target.value })
              }
            />
          </div>
          <div>
