...
export default function OrgChartPage() {
  ...
  const getName = (e: Employee) => e.name || `${e.firstName || ''} ${e.lastName || ''}`.trim() || '—';
  ...
  const filtered = activeEmployees.filter(e => {
    const matchSearch = getName(e).toLowerCase().includes(search.toLowerCase()) ||
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex gap-2">
            ...
            <Button variant={viewMode === 'list' ? 'default' : 'outline'} onClick={() => setViewMode('list')} className={viewMode === 'list' ? 'bg-violet-600 hover:bg-violet-700' : ''}>List View</Button>
          </div>
        </div>
        ...
        <Card className="mb-6"><CardContent className="pt-6">
          <div className="flex gap-4">
            <div className="flex-1 relative">
              ...
              <input type="text" aria-label="Search by name or job title..." placeholder="Search by name or job title..." value={search} onChange={e => setSearch(e.target.value)}
        ...
        {loading ? (
          ...
          <Card>
            <CardHeader><CardTitle className="flex items-center gap-2"><Users className="h-5 w-5 text-violet-600" />Organisation Tree ({filtered.length})</CardTitle></CardHeader>
            <CardContent>
              <div className="space-y-2">
                {topLevel.map(emp => <EmployeeCard key={emp.id} emp={emp} depth={0} />)}
                {/* Show employees not in the hierarchy */}
                {filtered.filter(e => e.managerId && !filteredIds.has(e.managerId)).map(emp => (
          ...
          <div className="space-y-4">
            {Object.entries(byDept).map(([dept, emps]) => (
              <Card key={dept}>
                <CardHeader><CardTitle className="flex items-center gap-2 text-base"><Building2 className="h-4 w-4 text-violet-600" />{dept} <Badge variant="outline" className="ml-2">{emps.length}</Badge></CardTitle></CardHeader>
                <CardContent>
