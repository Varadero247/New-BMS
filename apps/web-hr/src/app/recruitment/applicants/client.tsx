          ...
          {stages.map((stage, i) => {
            ...
            return (
              <div key={stage} className="flex-1 text-center">
                <div
                  className={`h-8 rounded flex items-center justify-center text-xs font-bold text-white ${count > 0 ? 'bg-blue-500' : 'bg-gray-300'}`}
                >
                  {count}
                </div>
                  ...
                  <div className="flex items-center gap-3">
                    <div className="flex items-center gap-0.5">
                      {[1, 2, 3, 4, 5].map((s) => (
                        <Star
                          key={s}
                          className={`h-3 w-3 ${s <= applicant.rating ? 'text-amber-400 fill-amber-400' : 'text-gray-300'}`}
                        />
                      ))}
                    </div>
                ...
                <div className="px-4 py-3 bg-gray-50 dark:bg-gray-800 border-t border-gray-100 dark:border-gray-700 ml-12 space-y-2">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                    ...
                    <div>
                      ...
                      <span className="font-medium">{applicant.source}</span>
                    </div>
                  </div>
                  <div className="text-xs">
                    <span className="text-gray-500 dark:text-gray-400">Experience:</span>{' '}
                    <span className="text-gray-700 dark:text-gray-300">{applicant.experience}</span>
                  </div>
                  <div className="text-xs">
                    <span className="text-gray-500 dark:text-gray-400">Notes:</span>{' '}
                    <span className="text-gray-700 dark:text-gray-300">{applicant.notes}</span>
                  </div>
                  {/* Pipeline progress */}
