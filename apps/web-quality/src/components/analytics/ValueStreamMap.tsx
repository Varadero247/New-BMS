...
export function ValueStreamMap({
  ...
  const metrics = useMemo(() => {
    ...
    const bottleneck = steps.reduce(
      (max, step) => (step.cycleTime > max.cycleTime ? step : max),
      steps[0]
    );
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-purple-50 rounded-lg p-4 text-center">
          <p className="text-2xl font-bold text-purple-700">
            {metrics.processEfficiency.toFixed(1)}%
          </p>
          ...
          {steps.map((step, index) => {
            ...
            return (
              <div key={step.id} className="flex items-start gap-4">
                ...
                {/* Process box */}
                <div
                  className={`w-36 rounded-lg border-2 ${
                    isBottleneck
                ...
                >
                  {/* Step name */}
                  <div
                    className={`px-3 py-2 text-center font-medium text-sm rounded-t-md ${
                  ...
                  <div className="p-2 text-xs space-y-1">
                    <div className="flex justify-between">
                      ...
                      <span
                        className={`font-medium ${exceedsTakt ? 'text-red-600' : 'text-gray-900 dark:text-gray-100'}`}
                      >
                        {step.cycleTime}m
                      </span>
                    </div>
                    ...
                    {step.uptime !== undefined && (
                      <div className="flex justify-between">
                        ...
                        <span
                          className={`font-medium ${step.uptime < 90 ? 'text-orange-600' : 'text-green-600'}`}
                        >
                          {step.uptime}%
                        </span>
                      </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-medium w-16 text-right">
              {metrics.valueAddedTime.toFixed(1)}m
            </span>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex-1 h-6 bg-gray-200 rounded overflow-hidden">
              <div
                ...
                style={{
                  width: `${((metrics.leadTime - metrics.valueAddedTime) / metrics.leadTime) * 100}%`,
                }}
              />
            </div>
            <span className="text-xs font-medium w-16 text-right">
              {(metrics.leadTime - metrics.valueAddedTime).toFixed(1)}m
            </span>
          </div>
        </div>
      </div>
