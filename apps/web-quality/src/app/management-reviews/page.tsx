...
const statusColors: Record<string, string> = { PLANNED: 'info', IN_PROGRESS: 'warning', COMPLETED: 'success', CANCELLED: 'default' };
...
export default function ManagementReviewsPage() {
  ...
  const [form, setForm] = useState({
    title: '', meetingDate: '', chairperson: '', attendees: '',
    previousActions: '', changesInContext: '', customerFeedback: '', qualityObjectives: '',
    processPerformance: '', nonconformanceSummary: '', auditResults: '', supplierPerformance: '',
    resourceAdequacy: '', riskAssessment: '',
    improvements: '', resourceNeeds: '', decisions: '', actionItems: '',
    nextReviewDate: '', minutes: '', notes: '',
  });
  ...
  const openCreate = () => {
    setEditItem(null);
    setForm({ title: '', meetingDate: '', chairperson: '', attendees: '', previousActions: '', changesInContext: '', customerFeedback: '', qualityObjectives: '', processPerformance: '', nonconformanceSummary: '', auditResults: '', supplierPerformance: '', resourceAdequacy: '', riskAssessment: '', improvements: '', resourceNeeds: '', decisions: '', actionItems: '', nextReviewDate: '', minutes: '', notes: '' });
    setModalOpen(true);
  };
  ...
  const openEdit = (item: ManagementReview) => {
    setEditItem(item);
    setForm({
      title: item.title, meetingDate: item.meetingDate?.slice(0, 10) || '',
      chairperson: item.chairperson || '', attendees: item.attendees || '',
      previousActions: item.previousActions || '', changesInContext: item.changesInContext || '',
      customerFeedback: item.customerFeedback || '', qualityObjectives: item.qualityObjectives || '',
      processPerformance: item.processPerformance || '', nonconformanceSummary: item.nonconformanceSummary || '',
      auditResults: item.auditResults || '', supplierPerformance: item.supplierPerformance || '',
      resourceAdequacy: item.resourceAdequacy || '', riskAssessment: item.riskAssessment || '',
      improvements: item.improvements || '', resourceNeeds: item.resourceNeeds || '',
      decisions: item.decisions || '', actionItems: item.actionItems || '',
      nextReviewDate: item.nextReviewDate?.slice(0, 10) || '', minutes: item.minutes || '', notes: item.notes || '',
    });
    setModalOpen(true);
  };
  ...
  const handleSave = async () => {
    try {
      if (editItem) { await api.put(`/management-reviews/${editItem.id}`, form); }
      else { await api.post('/management-reviews', form); }
      setModalOpen(false);
      fetchItems();
    } catch { /* empty */ }
  };
  ...
  const handleComplete = async (id: string) => {
    try { await api.put(`/management-reviews/${id}/complete`); fetchItems(); } catch { /* empty */ }
  };

  const handleDelete = async (id: string) => {
    if (!confirm('Delete this management review?')) return;
    try { await api.delete(`/management-reviews/${id}`); fetchItems(); } catch { /* empty */ }
  };
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex gap-3">
        <div className="relative flex-1">
          ...
          <Input aria-label="Search reviews..." placeholder="Search reviews..." value={search} onChange={e => { setSearch(e.target.value); setPagination(p => ({ ...p, page: 1 })); }} className="pl-10" />
        </div>
                    ...
                    <td className="p-3" onClick={e => e.stopPropagation()}>
                      <div className="flex gap-1">
                        {item.status !== 'COMPLETED' && item.status !== 'CANCELLED' && (
                          <Button size="sm" onClick={() => handleComplete(item.id)}><CheckCircle className="h-3 w-3 mr-1" />Complete</Button>
                        )}
      ...
      {/* Create/Edit Modal */}
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editItem ? 'Edit Management Review' : 'Schedule Management Review'} size="lg">
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div><Label>Title *</Label><Input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} /></div>
            <div><Label>Meeting Date *</Label><Input type="date" value={form.meetingDate} onChange={e => setForm(f => ({ ...f, meetingDate: e.target.value }))} /></div>
            <div><Label>Chairperson</Label><Input value={form.chairperson} onChange={e => setForm(f => ({ ...f, chairperson: e.target.value }))} /></div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div><Label>Customer Feedback</Label><Textarea value={form.customerFeedback} onChange={e => setForm(f => ({ ...f, customerFeedback: e.target.value }))} rows={2} /></div>
            <div><Label>Quality Objectives</Label><Textarea value={form.qualityObjectives} onChange={e => setForm(f => ({ ...f, qualityObjectives: e.target.value }))} rows={2} /></div>
            <div><Label>Process Performance</Label><Textarea value={form.processPerformance} onChange={e => setForm(f => ({ ...f, processPerformance: e.target.value }))} rows={2} /></div>
            <div><Label>NC Summary</Label><Textarea value={form.nonconformanceSummary} onChange={e => setForm(f => ({ ...f, nonconformanceSummary: e.target.value }))} rows={2} /></div>
            <div><Label>Audit Results</Label><Textarea value={form.auditResults} onChange={e => setForm(f => ({ ...f, auditResults: e.target.value }))} rows={2} /></div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div><Label>Decisions</Label><Textarea value={form.decisions} onChange={e => setForm(f => ({ ...f, decisions: e.target.value }))} rows={2} /></div>
            <div><Label>Action Items</Label><Textarea value={form.actionItems} onChange={e => setForm(f => ({ ...f, actionItems: e.target.value }))} rows={2} /></div>
          </div>
      ...
      {/* Detail Modal */}
      <Modal isOpen={detailOpen} onClose={() => setDetailOpen(false)} title={detailItem?.title || 'Review Details'} size="lg">
        {detailItem && (
          <div className="space-y-4 text-sm">
            <div className="grid grid-cols-2 gap-4">
              <div><strong>Reference:</strong> {detailItem.referenceNumber}</div>
              <div><strong>Status:</strong> <Badge variant={statusColors[detailItem.status] as any}>{detailItem.status}</Badge></div>
              <div><strong>Meeting Date:</strong> {new Date(detailItem.meetingDate).toLocaleDateString()}</div>
              <div><strong>Chairperson:</strong> {detailItem.chairperson || '—'}</div>
            </div>
            {detailItem.attendees && <div><strong>Attendees:</strong><p className="text-gray-600 mt-1">{detailItem.attendees}</p></div>}
            {detailItem.decisions && <div><strong>Key Decisions:</strong><p className="text-gray-600 mt-1">{detailItem.decisions}</p></div>}
            {detailItem.actionItems && <div><strong>Action Items:</strong><p className="text-gray-600 mt-1">{detailItem.actionItems}</p></div>}
            {detailItem.improvements && <div><strong>Improvements:</strong><p className="text-gray-600 mt-1">{detailItem.improvements}</p></div>}
          </div>
        )}
        <ModalFooter>
