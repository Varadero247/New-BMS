...
export default function CompetencesPage() {
  ...
  const openEdit = (item: Competence) => {
    ...
    setForm({
      ...
      assessmentDate: item.assessmentDate?.slice(0, 10) || '',
  ...
  const handleSave = async () => {
    try {
      if (editItem) {
        await api.put(`/competences/${editItem.id}`, form);
      } else {
        await api.post('/competences', form);
      }
      setModalOpen(false);
      fetchItems();
    } catch {
    ...
    <div className="p-6 space-y-6">
        ...
        <Card>
          <CardContent className="p-4">
            <div className="text-2xl font-bold text-green-600">
              {items.filter((i) => i.status === 'COMPETENT').length}
            </div>
            <div className="text-sm text-gray-500 dark:text-gray-400">Competent</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            <div className="text-2xl font-bold text-blue-600">
              {items.filter((i) => i.status === 'IN_TRAINING').length}
            </div>
            <div className="text-sm text-gray-500 dark:text-gray-400">In Training</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            <div className="text-2xl font-bold text-red-600">
              {items.filter((i) => i.status === 'NOT_COMPETENT' || i.status === 'EXPIRED').length}
            </div>
            <div className="text-sm text-gray-500 dark:text-gray-400">Gaps</div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="flex gap-3">
        <div className="relative flex-1">
          ...
          <Input
            aria-label="Search employees or competency areas..."
            placeholder="Search employees or competency areas..."
            value={search}
            onChange={(e) => {
      ...
      <Modal
        ...
        title={editItem ? 'Edit Competence Record' : 'Add Competence Record'}
        size="lg"
      >
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <Input
              value={form.competencyArea}
              onChange={(e) => setForm((f) => ({ ...f, competencyArea: e.target.value }))}
            />
          </div>
          <div>
            <Label>Status</Label>
          ...
          <div className="col-span-2">
            ...
            <Textarea
              ...
              onChange={(e) => setForm((f) => ({ ...f, trainingCompleted: e.target.value }))}
            />
          </div>
          <div className="col-span-2">
            ...
            <Textarea
              ...
              onChange={(e) => setForm((f) => ({ ...f, gapAnalysis: e.target.value }))}
            />
          </div>
          <div className="col-span-2">
