...
export default function PolicyPage() {
  ...
  const handleExport = useCallback(() => {
    ...
    policy.communicationPlan.forEach((c) =>
      lines.push(`  ${c.method} | Audience: ${c.audience} | Frequency: ${c.frequency}`)
    );
    lines.push('');
    lines.push('=== SIGNATORIES ===');
    if (policy.signatories.length === 0) lines.push('  (none)');
    policy.signatories.forEach((s) =>
      lines.push(`  ${s.name} (${s.role}) -- Signed: ${formatDate(s.signedAt)}`)
    );
    lines.push('');
    ...
    policy.acknowledgements.forEach((a) =>
      lines.push(
        `  [${a.acknowledged ? 'YES' : 'NO'}] ${a.name} (${a.department})${a.acknowledged ? ' -- ' + formatDate(a.acknowledgedAt) : ''}`
      )
    );
    ...
    <div className="p-6 space-y-6 print:p-0">
      ...
      <div className="flex items-start justify-between gap-4 print:hidden">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            ISO 9001:2015 &mdash; Clause 5.2 &mdash; Quality policy establishment, communication and
            maintenance
          </p>
        </div>
          ...
          {versions.length === 0 ? (
            ...
            <div className="space-y-2 max-h-48 overflow-y-auto">
              {versions.map((v, idx) => (
                ...
                >
                  <span>
                    <strong className="text-foreground">v{v.version}</strong>{' '}
                    <span className="text-muted-foreground ml-2">{formatDate(v.updatedAt)}</span>
                  </span>
                  <button
      ...
      <div className="hidden print:block mb-8">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400">
          ISO 9001:2015 Clause 5.2 | Version {policy.version} | Status: {policy.status} |{' '}
      ...
      {activeSection === 'statement' && (
        <div className="bg-card border border-border rounded-xl p-6 space-y-4">
          ...
          <p className="text-xs text-muted-foreground">
            The quality policy shall be appropriate to the purpose and context of the organisation,
            provide a framework for setting quality objectives, include a commitment to satisfy
            applicable requirements, and include a commitment to continual improvement of the QMS.
          </p>
          <textarea
      ...
      {activeSection === 'objectives' && (
        <div className="bg-card border border-border rounded-xl p-6 space-y-4">
          ...
          <p className="text-xs text-muted-foreground">
            Demonstrate how the quality policy provides a framework for setting and reviewing
        ...
        <div className="bg-card border border-border rounded-xl p-6 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                How the quality policy is communicated, understood, and applied within the
                organisation (5.2.2).
              </p>
            </div>
            <button
          ...
          {policy.communicationPlan.length === 0 ? (
            <p className="text-sm text-muted-foreground italic py-4">
              No communication methods defined yet.
            </p>
          ) : (
                  ...
                  {policy.communicationPlan.map((entry) => (
                    <tr key={entry.id} className="group">
                      <td className="p-2">
                        <input
                          type="text"
                          className="w-full text-sm border border-border rounded px-2 py-1.5 bg-background text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-amber-500"
                          value={entry.method}
                          onChange={(e) => updateCommEntry(entry.id, 'method', e.target.value)}
                          placeholder="e.g., Noticeboard, Induction, Email"
                        />
                      </td>
                      <td className="p-2">
                        <input
                          ...
                          onChange={(e) => updateCommEntry(entry.id, 'audience', e.target.value)}
                          placeholder="e.g., All staff, New starters"
                        />
                      </td>
                      <td className="p-2">
                        <input
                          ...
                          onChange={(e) => updateCommEntry(entry.id, 'frequency', e.target.value)}
                          placeholder="e.g., On change, Annually"
                        />
                      </td>
                      <td className="p-2">
                        <button
                          onClick={() => removeCommEntry(entry.id)}
      ...
      {activeSection === 'signatories' && (
        <div className="bg-card border border-border rounded-xl p-6 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-semibold text-foreground flex items-center gap-2">
                <UserCheck className="h-5 w-5 text-green-500" /> Signatories (Top Management
                Sign-off)
              </h2>
              <p className="text-xs text-muted-foreground mt-1">
                Track top management approval of the quality policy.
              </p>
            </div>
            <button
          ...
          {policy.signatories.length === 0 ? (
            <p className="text-sm text-muted-foreground italic py-4">
              No signatories added. Add top management representatives.
            </p>
          ) : (
            <div className="space-y-3">
              {policy.signatories.map((sig) => (
                <div
                  key={sig.id}
                  className="group flex items-center gap-3 p-3 rounded-lg border border-border bg-background"
                >
                  <div
                    className={`p-2 rounded-full ${sig.signedAt ? 'bg-green-100 dark:bg-green-900/30' : 'bg-gray-100 dark:bg-gray-800'}`}
                  >
                    {sig.signedAt ? (
                  ...
                  <input
                    type="text"
                    className="flex-1 text-sm border border-border rounded px-2 py-1.5 bg-card text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-green-500"
                    value={sig.name}
                  ...
                  <input
                    type="text"
                    className="w-48 text-sm border border-border rounded px-2 py-1.5 bg-card text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-green-500"
                    value={sig.role}
                  ...
                  {sig.signedAt ? (
                    ...
                    <button
                      onClick={() => markSigned(sig.id)}
                      className="text-xs px-2 py-1 rounded bg-green-100 text-green-700 hover:bg-green-200 dark:bg-green-900/30 dark:text-green-300 transition-colors whitespace-nowrap"
                    >
                      Mark Signed
                    </button>
                  )}
            ...
            <div className="flex items-center gap-2 pt-2 border-t border-border text-xs text-muted-foreground">
              <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 flex-1">
                <div
                  ...
                  style={{ width: `${totalSigs > 0 ? (signedCount / totalSigs) * 100 : 0}%` }}
                />
              </div>
              <span>
        ...
        <div className="bg-card border border-border rounded-xl p-6 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Track which personnel have acknowledged and understood the quality policy.
              </p>
            </div>
            <button
              onClick={addAcknowledgement}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium bg-purple-600 text-white hover:bg-purple-700 transition-colors"
            >
            ...
            <div className="flex items-center gap-4 p-3 rounded-lg bg-background border border-border">
              <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5 flex-1">
                <div
                  ...
                  style={{ width: `${totalAck > 0 ? (ackedCount / totalAck) * 100 : 0}%` }}
                />
              </div>
              ...
              <span className="inline-flex items-center gap-1 text-xs text-muted-foreground">
                <UserX className="h-3 w-3" /> {totalAck - ackedCount} pending
              </span>
            </div>
          )}
          ...
          {policy.acknowledgements.length === 0 ? (
            <p className="text-sm text-muted-foreground italic py-4">
              No acknowledgements tracked. Add personnel who should acknowledge the policy.
            </p>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                  ...
                  {policy.acknowledgements.map((ack) => (
                    <tr key={ack.id} className="group">
                      <td className="p-2">
                        <input
                          type="text"
                          className="w-full text-sm border border-border rounded px-2 py-1.5 bg-background text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-purple-500"
                          value={ack.name}
                          onChange={(e) => updateAcknowledgement(ack.id, 'name', e.target.value)}
                          placeholder="Full name"
                        />
                      </td>
                      <td className="p-2">
                        <input
                          type="text"
                          className="w-full text-sm border border-border rounded px-2 py-1.5 bg-background text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-purple-500"
                          value={ack.department}
                          onChange={(e) =>
                            updateAcknowledgement(ack.id, 'department', e.target.value)
                          }
                      ...
                      <td className="p-2 text-center">
                        <button
                          onClick={() => toggleAcknowledged(ack.id)}
                      ...
                      <td className="p-2 text-xs text-muted-foreground">
                        {ack.acknowledgedAt ? formatDate(ack.acknowledgedAt) : '--'}
                      </td>
                      <td className="p-2">
                        <button
      ...
      <div className="flex items-center gap-6 text-xs text-muted-foreground bg-card border border-border rounded-xl px-4 py-3 print:hidden">
        <span>
          Version: <strong className="text-foreground">{policy.version}</strong>
        </span>
        <span>
          Status: <strong className="text-foreground">{policy.status}</strong>
        </span>
        <span>
          Signatories:{' '}
