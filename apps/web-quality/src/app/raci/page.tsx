...
const raciColors: Record<string, string> = { RESPONSIBLE: 'info', ACCOUNTABLE: 'danger', CONSULTED: 'warning', INFORMED: 'default' };
const raciLetters: Record<string, string> = { RESPONSIBLE: 'R', ACCOUNTABLE: 'A', CONSULTED: 'C', INFORMED: 'I' };

type MatrixData = Record<string, Record<string, Array<{ roleName: string; personName: string | null; raciType: string }>>>;
...
export default function RaciPage() {
  ...
  const [form, setForm] = useState({
    processName: '', processId: '', activityName: '', roleName: '', personName: '', raciType: 'RESPONSIBLE', notes: '',
  });
  ...
  const openCreate = () => {
    setEditItem(null);
    setForm({ processName: '', processId: '', activityName: '', roleName: '', personName: '', raciType: 'RESPONSIBLE', notes: '' });
    setModalOpen(true);
  };
  ...
  const openEdit = (item: RaciEntry) => {
    setEditItem(item);
    setForm({
      processName: item.processName, processId: item.processId || '', activityName: item.activityName,
      roleName: item.roleName, personName: item.personName || '', raciType: item.raciType, notes: item.notes || '',
    });
    setModalOpen(true);
  };
  ...
  const handleSave = async () => {
    try {
      if (editItem) { await api.put(`/raci/${editItem.id}`, form); }
      else { await api.post('/raci', form); }
      setModalOpen(false);
      fetchItems();
    } catch { /* empty */ }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try { await api.delete(`/raci/${id}`); fetchItems(); } catch { /* empty */ }
  };
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <div className="flex rounded-lg border overflow-hidden">
            <button className={`px-3 py-1.5 text-sm ${viewMode === 'matrix' ? 'bg-blue-100 text-blue-700' : 'text-gray-600'}`} onClick={() => setViewMode('matrix')}>Matrix</button>
            <button className={`px-3 py-1.5 text-sm ${viewMode === 'list' ? 'bg-blue-100 text-blue-700' : 'text-gray-600'}`} onClick={() => setViewMode('list')}>List</button>
          </div>
                  ...
                  <tbody className="divide-y">
                    {loading ? (
                      ...
                      <tr key={item.id} className="hover:bg-gray-50 dark:bg-gray-800">
                        ...
                        <td className="p-3"><Badge variant={raciColors[item.raciType] as any}>{item.raciType}</Badge></td>
                        <td className="p-3">
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editItem ? 'Edit RACI Entry' : 'Add RACI Entry'} size="lg">
        <div className="grid grid-cols-2 gap-4">
          <div><Label>Process Name *</Label><Input value={form.processName} onChange={e => setForm(f => ({ ...f, processName: e.target.value }))} /></div>
          <div><Label>Activity Name *</Label><Input value={form.activityName} onChange={e => setForm(f => ({ ...f, activityName: e.target.value }))} /></div>
          <div><Label>Role Name *</Label><Input value={form.roleName} onChange={e => setForm(f => ({ ...f, roleName: e.target.value }))} /></div>
          <div><Label>Person Name</Label><Input value={form.personName} onChange={e => setForm(f => ({ ...f, personName: e.target.value }))} /></div>
          <div><Label>RACI Type *</Label><Select value={form.raciType} onChange={e => setForm(f => ({ ...f, raciType: e.target.value }))}>{RACI_TYPES.map(t => <option key={t} value={t}>{t}</option>)}</Select></div>
