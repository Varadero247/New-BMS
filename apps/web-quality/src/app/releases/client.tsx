...
const decisionColors: Record<string, string> = { APPROVED: 'success', REJECTED: 'danger', CONDITIONAL: 'warning', ON_HOLD: 'default' };
...
export default function ReleasesPage() {
  ...
  const [form, setForm] = useState({
    productName: '', productId: '', batchNumber: '', releaseStage: '',
    inspectionCriteria: '', testResults: '', conditions: '', nonconformanceRef: '',
    evidence: '', notes: '',
  });
  ...
  const openCreate = () => {
    setEditItem(null);
    setForm({ productName: '', productId: '', batchNumber: '', releaseStage: '', inspectionCriteria: '', testResults: '', conditions: '', nonconformanceRef: '', evidence: '', notes: '' });
    setModalOpen(true);
  };
  ...
  const openEdit = (item: Release) => {
    setEditItem(item);
    setForm({
      productName: item.productName, productId: item.productId || '', batchNumber: item.batchNumber || '',
      releaseStage: item.releaseStage || '', inspectionCriteria: item.inspectionCriteria || '',
      testResults: item.testResults || '', conditions: item.conditions || '',
      nonconformanceRef: item.nonconformanceRef || '', evidence: item.evidence || '', notes: item.notes || '',
    });
    setModalOpen(true);
  };
  ...
  const handleSave = async () => {
    try {
      if (editItem) { await api.put(`/releases/${editItem.id}`, form); }
  ...
  const handleDelete = async (id: string) => {
    ...
    try { await api.delete(`/releases/${id}`); fetchItems(); } catch { /* empty */ }
  };
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex gap-3">
        <div className="relative flex-1">
          ...
          <Input aria-label="Search products or batches..." placeholder="Search products or batches..." value={search} onChange={e => { setSearch(e.target.value); setPagination(p => ({ ...p, page: 1 })); }} className="pl-10" />
        </div>
              ...
              <tbody className="divide-y">
                {loading ? (
                  ...
                  <tr key={item.id} className="hover:bg-gray-50 dark:bg-gray-800">
                    ...
                    <td className="p-3"><Badge variant={decisionColors[item.decision] as any}>{item.decision.replace(/_/g, ' ')}</Badge></td>
                    <td className="p-3 text-gray-600">{item.authorisedBy || '—'}</td>
                    <td className="p-3 text-gray-600">{item.authorisedAt ? new Date(item.authorisedAt).toLocaleDateString() : '—'}</td>
                    <td className="p-3">
                      <div className="flex gap-1">
                        {item.decision === 'ON_HOLD' && <Button size="sm" onClick={() => openAuthorise(item)}>Authorise</Button>}
      ...
      {/* Create/Edit Modal */}
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editItem ? 'Edit Release' : 'New Release Record'} size="lg">
        <div className="grid grid-cols-2 gap-4">
          <div><Label>Product Name *</Label><Input value={form.productName} onChange={e => setForm(f => ({ ...f, productName: e.target.value }))} /></div>
          <div><Label>Product ID</Label><Input value={form.productId} onChange={e => setForm(f => ({ ...f, productId: e.target.value }))} /></div>
          <div><Label>Batch Number</Label><Input value={form.batchNumber} onChange={e => setForm(f => ({ ...f, batchNumber: e.target.value }))} /></div>
          <div><Label>Release Stage</Label><Input value={form.releaseStage} onChange={e => setForm(f => ({ ...f, releaseStage: e.target.value }))} placeholder="e.g. Final Inspection" /></div>
          <div className="col-span-2"><Label>Inspection Criteria</Label><Textarea value={form.inspectionCriteria} onChange={e => setForm(f => ({ ...f, inspectionCriteria: e.target.value }))} /></div>
          <div className="col-span-2"><Label>Test Results</Label><Textarea value={form.testResults} onChange={e => setForm(f => ({ ...f, testResults: e.target.value }))} /></div>
      ...
      {/* Authorise Modal */}
      <Modal isOpen={authModalOpen} onClose={() => setAuthModalOpen(false)} title="Authorise Release" size="md">
        <div className="space-y-4">
          <p className="text-sm text-gray-600">Authorise release for: <strong>{authItem?.productName}</strong> (Batch: {authItem?.batchNumber || 'N/A'})</p>
          <div><Label>Decision *</Label>
            <Select value={authForm.decision} onChange={e => setAuthForm(f => ({ ...f, decision: e.target.value }))}>
              <option value="APPROVED">Approved</option>
              <option value="REJECTED">Rejected</option>
              <option value="CONDITIONAL">Conditional</option>
            </Select>
          </div>
          <div><Label>Conditions</Label><Textarea value={authForm.conditions} onChange={e => setAuthForm(f => ({ ...f, conditions: e.target.value }))} placeholder="Enter any conditions for release..." /></div>
        </div>
        <ModalFooter>
          <Button variant="outline" onClick={() => setAuthModalOpen(false)}>Cancel</Button>
