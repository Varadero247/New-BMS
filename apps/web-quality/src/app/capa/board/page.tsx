...
const MOCK_CAPAS: CapaItem[] = [
  {
    id: 'CAPA-2026-001',
    title: 'Recurring dimensional non-conformance on Line 3',
    status: 'Open',
    priority: 'Critical',
  ...
  {
    id: 'CAPA-2026-002',
    title: 'Supplier material batch variance exceeding spec',
  ...
  {
    id: 'CAPA-2026-003',
    title: 'Documentation gap in work instruction WI-045',
    status: 'Investigation',
    priority: 'Medium',
    source: 'Internal Audit',
    assignee: 'A. Patel',
    dueDate: '2026-03-10',
    rootCause: 'Incomplete revision after process change',
  },
  {
    id: 'CAPA-2026-004',
    title: 'Calibration overdue on 3 torque wrenches',
  ...
  {
    id: 'CAPA-2026-005',
    title: 'Training records missing for 2 new operators',
    status: 'Action Required',
    priority: 'Medium',
  ...
  {
    id: 'CAPA-2026-006',
    title: 'Packaging damage during transit — 3 incidents',
    status: 'Action Required',
    priority: 'High',
  ...
  {
    id: 'CAPA-2026-007',
    title: 'SPC chart out-of-control on coating thickness',
    status: 'Verification',
    priority: 'Critical',
    source: 'SPC Alert',
    assignee: 'M. Jones',
    dueDate: '2026-02-18',
    rootCause: 'Spray nozzle wear pattern',
  },
  {
    id: 'CAPA-2026-008',
    title: 'Label printing error — wrong revision on 50 units',
    status: 'Verification',
    priority: 'Medium',
  ...
  {
    id: 'CAPA-2026-009',
    title: 'Corrective action for audit finding AF-2025-034',
    status: 'Closed',
    priority: 'Low',
    source: 'External Audit',
    assignee: 'R. Brown',
    dueDate: '2026-01-30',
    rootCause: 'Procedure not updated after ISO transition',
  },
  {
    id: 'CAPA-2026-010',
    title: 'Root cause: incorrect fixture alignment jig',
    status: 'Closed',
    priority: 'High',
    ...
    dueDate: '2026-02-01',
    ncr: 'NCR-2026-012',
    rootCause: 'Fixture wear beyond tolerance',
  },
  {
    id: 'CAPA-2026-011',
    title: 'Preventive action for environmental storage conditions',
    status: 'Open',
    priority: 'Medium',
  ...
  {
    id: 'CAPA-2026-012',
    title: 'Customer feedback — specification clarification needed',
    status: 'Investigation',
    priority: 'Low',
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Kanban view of corrective and preventive actions
          </p>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-xs text-gray-500 dark:text-gray-400">
            {capas.length} CAPAs total
          </span>
          <a
            href="/capa"
        ...
        {STATUSES.map((status) => {
          ...
          return (
            <div key={status} className="flex-shrink-0 w-72">
              {/* Column Header */}
              <div
                className={`rounded-t-lg px-3 py-2 flex items-center justify-between ${config.header}`}
              >
              ...
              {/* Column Body */}
              <div
                className={`rounded-b-lg border ${config.border} ${config.bg} p-2 min-h-[400px] space-y-2`}
              >
                {items.map((capa) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <span
                          className={`text-[10px] font-medium rounded-full px-1.5 py-0.5 ${priorityConfig[capa.priority]}`}
                        >
                          {capa.priority}
                        </span>
                      </div>
                      <p className="text-xs font-medium text-gray-900 dark:text-gray-100 mt-1 line-clamp-2">
                        {capa.title}
                      </p>
                      <div className="flex items-center justify-between mt-2">
                        ...
                        <span
                          className={`text-[10px] font-mono ${overdue ? 'text-red-600 font-bold' : 'text-gray-400'}`}
                        >
                      ...
                      {selectedCapa?.id === capa.id && (
                        <div className="mt-3 pt-3 border-t border-gray-100 dark:border-gray-700 space-y-1.5 text-[10px]">
                          ...
                          {capa.rootCause && (
                            <div>
                              <span className="text-gray-500 dark:text-gray-400">Root Cause:</span>{' '}
                              <span className="text-gray-700 dark:text-gray-300 font-medium">
                                {capa.rootCause}
                              </span>
                            </div>
