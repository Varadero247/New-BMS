...
async function generateRef(orgId: string): Promise<string> {
  ...
  const c = await prisma.docDocument.count({
    where: { orgId, referenceNumber: { startsWith: `DOC-${y}` } },
  });
  return `DOC-${y}-${String(c + 1).padStart(4, '0')}`;
}
...
router.get('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    const orgId = ((req as AuthRequest).user as { orgId?: string })?.orgId || 'default';
    const item = await prisma.docDocument.findFirst({
...
router.post('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const data = await prisma.docDocument.create({
      data: {
        title,
        description,
        category,
...
router.put('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const data = await prisma.docDocument.update({
