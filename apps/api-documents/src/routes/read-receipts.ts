...
router.get('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const [data, total] = await Promise.all([
      prisma.docReadReceipt.findMany({
        where,
        skip,
        take: Math.min(Math.max(1, parseInt(limit, 10) || 20), 100),
        orderBy: { createdAt: 'desc' },
      }),
      prisma.docReadReceipt.count({ where }),
    ]);
    res.json({
      success: true,
      data,
    ...
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch read receipts' },
    });
  }
});
router.get('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    if (!item)
      return res
        .status(404)
        .json({ success: false, error: { code: 'NOT_FOUND', message: 'read receipt not found' } });
    res.json({ success: true, data: item });
  } catch (error: unknown) {
    logger.error('Failed to process request', { error: (error as Error).message });
    res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch read receipt' },
    });
  }
});
  ...
  try {
    ...
    if (!existing)
      return res
        ...
        .json({ success: false, error: { code: 'NOT_FOUND', message: 'read receipt not found' } });
    const { documentId, userId, userName, status, readAt, acknowledgedAt } = parsed.data;
    const data = await prisma.docReadReceipt.update({
  ...
  try {
    ...
    if (!existing)
      return res
        .status(404)
        .json({ success: false, error: { code: 'NOT_FOUND', message: 'read receipt not found' } });
    await prisma.docReadReceipt.update({
      where: { id: req.params.id },
      data: { deletedAt: new Date(), updatedBy: (req as AuthRequest).user?.id },
    });
    res.json({ success: true, data: { message: 'read receipt deleted successfully' } });
  } catch (error: unknown) {
    logger.error('Failed to process request', { error: (error as Error).message });
    res.status(500).json({
      success: false,
