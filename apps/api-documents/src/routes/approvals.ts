...
router.get('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    if (search) where.approverName = { contains: search, mode: 'insensitive' };
    const skip =
      (Math.max(1, parseInt(page, 10) || 1) - 1) * Math.max(1, parseInt(limit, 10) || 20);
    const [data, total] = await Promise.all([
      prisma.docApproval.findMany({
        where,
        skip,
...
router.get('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    const orgId = (req as any).user?.orgId || 'default';
    const item = await prisma.docApproval.findFirst({
...
router.post('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const orgId = (req as any).user?.orgId || 'default';
    const { documentId, approver, approverName, status, comments, decidedAt } = parsed.data;
    const data = await prisma.docApproval.create({
      data: {
        documentId,
        approver,
        approverName,
...
router.put('/:id', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const orgId = (req as any).user?.orgId || 'default';
    const existing = await prisma.docApproval.findFirst({
      where: { id: req.params.id, orgId, deletedAt: null } as any,
    });
    if (!existing)
      return res
        ...
        .json({ success: false, error: { code: 'NOT_FOUND', message: 'approval not found' } });
    const { documentId, approver, approverName, status, comments, decidedAt } = parsed.data;
    const data = await prisma.docApproval.update({
  ...
  try {
    ...
    if (!existing)
      return res
        .status(404)
        .json({ success: false, error: { code: 'NOT_FOUND', message: 'approval not found' } });
    await prisma.docApproval.update({
      where: { id: req.params.id },
