...
export default function LegalRegisterClient() {
  ...
  function openEdit(item: LegalItem) {
    setForm({
      ...
      complianceStatus: item.complianceStatus || 'NOT_ASSESSED',
  ...
  async function handleSubmit() {
    ...
    try {
      const payload: any = {
        title: form.title,
        legislation: form.legislation || undefined,
      ...
      console.error('Failed to save legal item:', err);
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleDelete(id: string) {
    ...
    try {
      await api.delete(`/legal-register/${id}`);
      loadItems();
    } catch (err) {
      console.error(err);
    }
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">
              Legislation and regulatory requirements tracking
            </p>
          </div>
          ...
          <Card>
            <CardContent className="pt-6 text-center">
              <p className="text-3xl font-bold text-green-600">
                {items.filter((r) => r.complianceStatus === 'COMPLIANT').length}
              </p>
          ...
          <Card>
            <CardContent className="pt-6 text-center">
              <p className="text-3xl font-bold text-yellow-600">
                {items.filter((r) => r.complianceStatus === 'PARTIALLY_COMPLIANT').length}
              </p>
          ...
          <Card>
            <CardContent className="pt-6 text-center">
              <p className="text-3xl font-bold text-red-600">
                {items.filter((r) => r.complianceStatus === 'NON_COMPLIANT').length}
              </p>
        ...
        <div className="flex gap-4 mb-6 flex-wrap items-center">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              type="text"
              aria-label="Search legal register"
                  ...
                  <TableBody>
                    {items.map((item) => (
                      <TableRow key={item.id}>
                        ...
                        <TableCell>
                          <span
                            className={`px-2 py-1 rounded-full text-xs font-medium ${getComplianceColor(item.complianceStatus)}`}
                          >
        ...
        {modalOpen && (
          <Modal
            ...
            title={editId ? 'Edit Legal Register Entry' : 'Add Legal Register Entry'}
            size="lg"
          >
            <div className="space-y-4">
              <div>
                ...
                <Input
                  ...
                  placeholder="Legal requirement title"
                />
              </div>
              ...
              <div>
                ...
                <Textarea
                  ...
                  rows={2}
                  placeholder="How does this apply to the organisation?"
                />
              </div>
              <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setForm((p) => ({ ...p, lastReviewDate: e.target.value }))}
                  />
                </div>
              </div>
              <div>
