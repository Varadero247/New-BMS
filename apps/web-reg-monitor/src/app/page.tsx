...
export default function DashboardPage() {
  ...
  const kpis = [
    ...
    { label: 'Obligations', value: stats.totalObligations ?? 0, icon: ClipboardList, color: 'indigo' },
    { label: 'Compliance Rate', value: stats.complianceRate ?? '-', icon: TrendingUp, color: 'green' },
  ];
    ...
    <div className="flex min-h-screen">
      ...
      <main className="flex-1 p-8">
        <div className="max-w-7xl mx-auto">
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">Regulatory Monitor Dashboard</h1>
            <p className="text-gray-500 dark:text-gray-400 mt-1">Regulatory change tracking and compliance overview</p>
          </div>
          ...
          {Object.keys(stats).length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {Object.entries(stats).filter(([k]) => !['totalChanges', 'totalLegalItems', 'totalObligations', 'complianceRate'].includes(k)).map(([key, value]) => (
