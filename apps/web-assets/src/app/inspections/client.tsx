...
const emptyForm: InspectionForm = {
  assetId: '', inspector: '', inspectorName: '', scheduledDate: '',
  completedDate: '', condition: 'GOOD', passed: 'true', findings: '',
  recommendations: '', nextInspection: '', notes: '',
};
...
export default function InspectionsClient() {
  ...
  function openEdit(item: Inspection) {
    setForm({
      assetId: item.assetId || '', inspector: item.inspector || '',
      ...
      condition: item.condition || 'GOOD', passed: item.passed ? 'true' : 'false',
  ...
  async function handleDelete(id: string) {
    ...
    try { await api.delete(`/inspections/${id}`); loadItems(); } catch (err) { console.error(err); }
  }
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">Asset inspection records and scheduling</p>
          </div>
          <Button onClick={openCreate} className="flex items-center gap-2"><Plus className="h-4 w-4" />Add Inspection</Button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          ...
          <Card><CardContent className="pt-6 text-center"><p className="text-3xl font-bold text-green-600">{items.filter(i => i.passed).length}</p><p className="text-sm text-gray-500 dark:text-gray-400">Passed</p></CardContent></Card>
          <Card><CardContent className="pt-6 text-center"><p className="text-3xl font-bold text-red-600">{items.filter(i => i.passed === false).length}</p><p className="text-sm text-gray-500 dark:text-gray-400">Failed</p></CardContent></Card>
          <Card><CardContent className="pt-6 text-center"><p className="text-3xl font-bold text-orange-600">{items.filter(i => i.condition === 'CRITICAL' || i.condition === 'POOR').length}</p><p className="text-sm text-gray-500 dark:text-gray-400">Poor/Critical</p></CardContent></Card>
        </div>

        <div className="flex gap-4 mb-6 flex-wrap items-center">
          <div className="relative flex-1 max-w-md">
            <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-gray-400" />
            <input type="text" aria-label="Search inspections" placeholder="Search inspections..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
          ...
          <select aria-label="Filter by condition" value={conditionFilter} onChange={e => setConditionFilter(e.target.value)}
          ...
          <CardContent className="p-0">
            {loading ? (
                  ...
                  <TableBody>
                    {items.map(item => (
                      <TableRow key={item.id}>
                        ...
                        <TableCell><Badge variant={item.passed ? 'default' : 'destructive'}>{item.passed ? 'PASSED' : 'FAILED'}</Badge></TableCell>
                        <TableCell className="text-sm">{item.scheduledDate ? new Date(item.scheduledDate).toLocaleDateString() : '-'}</TableCell>
                        <TableCell className="text-sm">{item.nextInspection ? new Date(item.nextInspection).toLocaleDateString() : '-'}</TableCell>
                        <TableCell>
              ...
              <div className="text-center py-12">
                ...
                <Button variant="outline" className="mt-4" onClick={openCreate}><Plus className="h-4 w-4 mr-2" />Add First Inspection</Button>
              </div>
            )}
          </CardContent>
        </Card>
        ...
        {modalOpen && (
          <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editId ? 'Edit Inspection' : 'Add Inspection'} size="lg">
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><Label>Inspector ID</Label><Input value={form.inspector} onChange={e => setForm(p => ({ ...p, inspector: e.target.value }))} placeholder="Inspector ID" /></div>
                <div><Label>Inspector Name</Label><Input value={form.inspectorName} onChange={e => setForm(p => ({ ...p, inspectorName: e.target.value }))} placeholder="Inspector name" /></div>
              </div>
              <div className="grid grid-cols-3 gap-4">
                ...
                <div><Label>Next Inspection</Label><Input type="date" value={form.nextInspection} onChange={e => setForm(p => ({ ...p, nextInspection: e.target.value }))} /></div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div><Label>Result</Label><Select value={form.passed} onChange={e => setForm(p => ({ ...p, passed: e.target.value }))}><option value="true">PASSED</option><option value="false">FAILED</option></Select></div>
              </div>
              <div><Label>Findings</Label><Textarea value={form.findings} onChange={e => setForm(p => ({ ...p, findings: e.target.value }))} rows={2} placeholder="Inspection findings..." /></div>
            ...
            <ModalFooter>
              ...
              <Button onClick={handleSubmit} disabled={saving || !form.assetId}>
                {saving ? <><Loader2 className="h-4 w-4 animate-spin mr-2" />Saving...</> : editId ? 'Update Inspection' : 'Create Inspection'}
              </Button>
            </ModalFooter>
          </Modal>
        )}
