      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">Asset value tracking and depreciation analysis</p>
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <Card><CardContent className="pt-6 text-center"><p className="text-3xl font-bold">{items.length}</p><p className="text-sm text-gray-500 dark:text-gray-400">Tracked Assets</p></CardContent></Card>
          <Card><CardContent className="pt-6 text-center"><p className="text-3xl font-bold text-cyan-600">{formatCurrency(totalPurchaseCost)}</p><p className="text-sm text-gray-500 dark:text-gray-400">Total Purchase Cost</p></CardContent></Card>
          <Card><CardContent className="pt-6 text-center"><p className="text-3xl font-bold text-green-600">{formatCurrency(totalCurrentValue)}</p><p className="text-sm text-gray-500 dark:text-gray-400">Current Value</p></CardContent></Card>
          <Card><CardContent className="pt-6 text-center"><p className="text-3xl font-bold text-red-600">{formatCurrency(totalDepreciation)}</p><p className="text-sm text-gray-500 dark:text-gray-400">Total Depreciation</p></CardContent></Card>
        </div>
          ...
          <CardContent className="p-0">
            {loading ? (
                ...
                <Table>
                  ...
                  <TableBody>
                    {filteredItems.map(item => {
                      const depPercent = getDepreciationPercent(item.purchaseCost, item.currentValue);
                      return (
                        <TableRow key={item.id}>
                          <TableCell className="font-medium">{item.name}</TableCell>
                          <TableCell className="text-sm">{item.purchaseDate ? new Date(item.purchaseDate).toLocaleDateString() : '-'}</TableCell>
                          <TableCell className="text-sm font-mono">{formatCurrency(item.purchaseCost || 0)}</TableCell>
                          <TableCell className="text-sm font-mono">{formatCurrency(item.currentValue || 0)}</TableCell>
                          <TableCell className="text-sm font-mono text-red-600 dark:text-red-400">{formatCurrency((item.purchaseCost || 0) - (item.currentValue || 0))}</TableCell>
                          <TableCell>
                            <div className="flex items-center gap-2">
                              ...
                              <span className={`text-sm font-medium ${getDepreciationColor(depPercent)}`}>
              ...
              <div className="text-center py-12">
                ...
                <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">Assets with purchase cost values will appear here</p>
              </div>
            )}
          </CardContent>
        </Card>
