...
const STATUS_CONFIG = {
  ...
  superseded: { label: 'Superseded', bg: 'bg-gray-100',   text: 'text-gray-500',   border: 'border-gray-200',   icon: Clock },
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Configuration Baselines</h1>
            <p className="text-sm text-gray-500 mt-0.5">AS9100D Clause 8.3 — Design & Development Baselines</p>
          </div>
      ...
      <div className="px-6 py-6 space-y-6">
        ...
        <div className="grid grid-cols-4 gap-4">
          <div className="bg-white rounded-lg border border-gray-200 p-4">
            <p className="text-xs text-gray-500 uppercase tracking-wide">Current Baselines</p>
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-4">
            <p className="text-xs text-gray-500 uppercase tracking-wide">Superseded</p>
            <p className="text-3xl font-bold text-gray-500 mt-1">{counts.superseded}</p>
            <p className="text-xs text-gray-500 mt-1">Historical record</p>
          </div>
          <div className="bg-white rounded-lg border border-gray-200 p-4">
            <p className="text-xs text-gray-500 uppercase tracking-wide">Current Config Items</p>
            <p className="text-3xl font-bold text-blue-600 mt-1">{counts.totalItems.toLocaleString()}</p>
            <p className="text-xs text-gray-500 mt-1">Across active baselines</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          {filtered.map(baseline => {
            ...
            return (
              <div key={baseline.id} className={`bg-white rounded-lg border ${sc.border} overflow-hidden hover:shadow-md transition-shadow`}>
                ...
                <div className="px-5 py-4">
                  <div className="flex items-center gap-3 mb-3">
                    <span className="text-2xl font-bold text-gray-900 font-mono">{baseline.version}</span>
                  ...
                  <div className="grid grid-cols-3 gap-3 text-center border-t border-gray-100 pt-3">
                    <div>
                      ...
                      <p className="text-xs text-gray-500">Config Items</p>
                    </div>
                    <div>
                      <div className="flex items-center justify-center gap-1 text-gray-400 mb-0.5">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">{baseline.approvedBy ? `By ${baseline.approvedBy}` : 'Pending'}</p>
                    </div>
                  </div>
                </div>
              </div>
