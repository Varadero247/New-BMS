...
const MOCK_EVENTS: FODEvent[] = [
  { id: '1', area: 'Wing Assembly Bay — Station 4', type: 'incident', severity: 'critical', date: '2026-02-05', description: 'Metal drill bit fragment (12mm) found inside sealed wing rib cavity during final inspection. Production halted for full cavity inspection.', status: 'investigating', correctiveAction: 'Full sweep of assembly area, all tools inventoried, toolbox accountability check initiated', reportedBy: 'T. Brooks', closedDate: null },
  { id: '2', area: 'Avionics Installation Bay', type: 'near-miss', severity: 'major', date: '2026-01-30', description: 'Torque wrench slipped from tech platform; landed 30cm from open avionics rack. No contact made.', status: 'open', correctiveAction: 'Tool tethering requirement added to Work Order; tooling SOP revision in progress', reportedBy: 'P. Nakamura', closedDate: null },
  { id: '3', area: 'Engine Run Pad', type: 'inspection', severity: 'minor', date: '2026-01-22', description: 'Pre-run FOD walk found 2x safety lock-wire segments and 1x cable tie near intake exclusion zone.', status: 'closed', correctiveAction: 'Items removed. Refresher FOD walk training completed for all Engine Run crew.', reportedBy: 'M. Chen', closedDate: '2026-01-23' },
  { id: '4', area: 'Final Assembly Line — Fuselage Mating', type: 'incident', severity: 'major', date: '2026-01-18', description: 'Safety pin from hydraulic test fixture discovered inside main fuselage structure after mating.', status: 'closed', correctiveAction: 'Red-tagged safety pin accounting system implemented; 100% verification required before fuselage closure', reportedBy: 'J. Harrison', closedDate: '2026-01-25' },
  { id: '5', area: 'Parts Storage — Bond Store', type: 'inspection', severity: 'minor', date: '2026-01-10', description: 'Routine FOD inspection of bond store. No FOD found. All seals intact.', status: 'closed', correctiveAction: 'No action required. Inspection record filed.', reportedBy: 'R. Patel', closedDate: '2026-01-10' },
  { id: '6', area: 'Landing Gear Bay', type: 'near-miss', severity: 'major', date: '2026-02-08', description: 'Safety inspector observed rag used for wipe-down left on top of retracted gear bay structure by technician.', status: 'investigating', correctiveAction: 'Verbal correction issued. Material control procedures review scheduled.', reportedBy: 'A. Kumar', closedDate: null },
  { id: '7', area: 'Composite Layup Room', type: 'inspection', severity: 'minor', date: '2026-01-05', description: 'Weekly FOD walk — 3 aluminium shavings and 1x screw found on floor outside marked work zone.', status: 'closed', correctiveAction: 'Housekeeping frequency increased to twice daily. FOD bins repositioned.', reportedBy: 'L. Morgan', closedDate: '2026-01-06' },
  { id: '8', area: 'Flight Line — Aircraft 007', type: 'incident', severity: 'critical', date: '2026-02-10', description: 'Post-ground-test inspection revealed foreign object impact marks on fan blade leading edge. Source TBD.', status: 'open', correctiveAction: 'Aircraft grounded, engine borescope inspection ordered, flight line full FOD sweep', reportedBy: 'D. Foster', closedDate: null },
];
...
const STATUS_CONFIG = {
  ...
  investigating: { label: 'Investigating', bg: 'bg-yellow-50', text: 'text-yellow-700', icon: Clock },
...
export default function FODPreventionClient() {
  ...
  const filtered = useMemo(() => {
    return MOCK_EVENTS.filter(e => {
      const matchSearch = e.area.toLowerCase().includes(search.toLowerCase()) || e.description.toLowerCase().includes(search.toLowerCase()) || e.reportedBy.toLowerCase().includes(search.toLowerCase());
      ...
      <div className="px-6 py-6 space-y-6">
        ...
        <div className="grid grid-cols-4 gap-4">
          ...
          <div className="bg-yellow-50 rounded-lg border border-yellow-200 p-4">
            <p className="text-xs text-yellow-700 uppercase tracking-wide font-medium">Under Investigation</p>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 p-4 flex flex-wrap gap-3">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              type="text"
              aria-label="Search area, description, reporter..." placeholder="Search area, description, reporter..."
              value={search}
          ...
          <select aria-label="Filter by severity" value={severityFilter} onChange={e => setSeverityFilter(e.target.value)} className="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
              ...
              {filtered.map(event => {
                ...
                return (
                  <tr key={event.id} className={`hover:bg-gray-50 dark:bg-gray-800 ${event.status === 'open' && event.severity === 'critical' ? 'bg-red-50' : ''}`}>
                    ...
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium ${sc2.bg} ${sc2.text}`}>
