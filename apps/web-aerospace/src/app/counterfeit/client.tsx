...
const MOCK_VERIFICATIONS: SupplierVerification[] = [
  {
    id: '1',
    partNumber: 'MS27500C22RC3',
    partDescription: 'Mil-Spec Aircraft Wire Harness',
    supplier: 'TrueSource Aviation Parts',
    ...
    riskScore: 12,
    quantity: 500,
    notes: 'DLA CAGE verified, CoC on file, lot traceability confirmed',
  },
  {
    id: '2',
    partNumber: 'AN960-416L',
    partDescription: 'Flat Washer — Aluminium',
    supplier: 'GlobalFasteners LLC',
    verificationMethod: 'xrf',
    status: 'suspect',
    ...
    quantity: 2000,
    notes:
      'XRF analysis shows atypical alloy composition vs. AMS 2770 spec. Quarantine recommended.',
  },
  {
    id: '3',
    partNumber: 'NAS1149F0332P',
    partDescription: 'Flat Washer — Steel',
    supplier: 'AeroFasten Direct',
    verificationMethod: 'certificate',
    status: 'verified',
    lastCheck: '2026-01-10',
    nextCheck: '2026-07-10',
    riskScore: 8,
    quantity: 1000,
    notes: 'Full lot traceability from authorized distributor with DFARS compliance',
  },
  {
    id: '4',
    partNumber: 'MS21042L3',
    partDescription: 'Self-Locking Nut',
    supplier: 'Pacific Aerospace Dist.',
    verificationMethod: 'dimensional',
    status: 'quarantined',
    lastCheck: '2026-02-02',
    nextCheck: '—',
    riskScore: 91,
    quantity: 350,
    notes:
      'Dimensional non-conformance detected. Thread pitch irregular. Counterfeit suspected. DHS notification pending.',
  },
  {
    id: '5',
    partNumber: 'AN3-6A',
    partDescription: 'Hex Head Bolt — AN Standard',
    supplier: 'VerifiedParts Inc.',
    verificationMethod: 'xrf',
    status: 'verified',
    ...
    riskScore: 15,
    quantity: 800,
    notes: 'XRF confirms 4130 steel alloy. CoC batch matched, head markings consistent.',
  },
  {
    id: '6',
    partNumber: 'MS9021-08',
    partDescription: 'O-Ring — Hydraulic Fitting',
    supplier: 'SealTech Suppliers',
    ...
    riskScore: 22,
    quantity: 200,
    notes: 'Visual inspection and shore hardness testing within MIL-P-25732 tolerances',
  },
];
...
export default function CounterfeitPartsClient() {
  ...
  const filtered = useMemo(() => {
    return MOCK_VERIFICATIONS.filter((v) => {
      const matchSearch =
        v.partNumber.toLowerCase().includes(search.toLowerCase()) ||
        v.partDescription.toLowerCase().includes(search.toLowerCase()) ||
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
              AS9100D Clause 8.1.4 — Prevention of Counterfeit/Suspect Unapproved Parts
            </p>
          </div>
      ...
      <div className="px-6 py-6 space-y-6">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 p-4 flex flex-wrap gap-3">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              type="text"
              aria-label="Search part number, description, supplier..."
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
              {filtered.map((v) => {
                ...
                return (
                  <tr
                    key={v.id}
                    className={`hover:bg-gray-50 dark:bg-gray-800 ${v.status === 'quarantined' ? 'bg-red-50' : v.status === 'suspect' ? 'bg-yellow-50' : ''}`}
                  >
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium ${mc.bg} ${mc.text}`}
                      >
