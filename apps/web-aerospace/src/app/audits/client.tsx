...
import { Search, Plus, ClipboardCheck, AlertTriangle, Info, CheckCircle2, Clock, XCircle } from 'lucide-react';
...
const MOCK_AUDITS: Audit[] = [
  { id: '1', auditNumber: 'AUD-2026-001', type: 'FCA', status: 'completed', findings: { major: 0, minor: 1, observations: 3 }, auditor: 'P. Nakamura', auditDate: '2026-01-08', scope: 'Airframe Assembly — Functional Configuration Audit', area: 'Production' },
  { id: '2', auditNumber: 'AUD-2026-002', type: 'PCA', status: 'in-progress', findings: { major: 1, minor: 2, observations: 1 }, auditor: 'S. Williams', auditDate: '2026-01-15', scope: 'Avionics Suite — Physical Configuration Audit against drawing package', area: 'Avionics' },
  { id: '3', auditNumber: 'AUD-2026-003', type: 'internal', status: 'planned', findings: { major: 0, minor: 0, observations: 0 }, auditor: 'R. Patel', auditDate: '2026-02-20', scope: 'Document control and configuration management procedures', area: 'Quality' },
  { id: '4', auditNumber: 'AUD-2026-004', type: 'internal', status: 'completed', findings: { major: 0, minor: 3, observations: 5 }, auditor: 'M. Chen', auditDate: '2026-01-22', scope: 'Supplier configuration data packages — AS9100D Clause 8.4', area: 'Supply Chain' },
  { id: '5', auditNumber: 'AUD-2026-005', type: 'FCA', status: 'closed', findings: { major: 0, minor: 0, observations: 2 }, auditor: 'L. Morgan', auditDate: '2025-12-10', scope: 'Propulsion System — all functional requirements validated against test data', area: 'Propulsion' },
  { id: '6', auditNumber: 'AUD-2026-006', type: 'PCA', status: 'planned', findings: { major: 0, minor: 0, observations: 0 }, auditor: 'J. Harrison', auditDate: '2026-03-05', scope: 'Landing Gear assembly — drawing versus physical configuration verification', area: 'Structures' },
  { id: '7', auditNumber: 'AUD-2025-098', type: 'internal', status: 'closed', findings: { major: 2, minor: 4, observations: 6 }, auditor: 'T. Brooks', auditDate: '2025-11-15', scope: 'Configuration Management System full-scope internal audit — AS9100D readiness review', area: 'Quality' },
];
...
export default function ConfigAuditsClient() {
  ...
  const filtered = useMemo(() => {
    return MOCK_AUDITS.filter(a => {
      const matchSearch = a.auditNumber.toLowerCase().includes(search.toLowerCase()) || a.scope.toLowerCase().includes(search.toLowerCase()) || a.auditor.toLowerCase().includes(search.toLowerCase());
      ...
      <div className="px-6 py-6 space-y-6">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 p-4 flex flex-wrap gap-3">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              type="text"
              aria-label="Search audit number, scope, auditor..." placeholder="Search audit number, scope, auditor..."
              value={search}
                      ...
                      <div className="flex items-center gap-2">
                        {audit.findings.major > 0 && (
                          <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-xs font-bold bg-red-100 text-red-700">
                            <AlertTriangle className="w-3 h-3" />{audit.findings.major}M
                          </span>
                        )}
                        {audit.findings.minor > 0 && (
                          <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-xs font-bold bg-orange-100 text-orange-700">
                            <AlertTriangle className="w-3 h-3" />{audit.findings.minor}m
                          </span>
                        )}
                        {audit.findings.observations > 0 && (
                          <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-xs font-medium bg-blue-50 text-blue-600">
                            <Info className="w-3 h-3" />{audit.findings.observations}
                          </span>
                        )}
