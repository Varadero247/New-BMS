...
const MOCK_ECRS: ECR[] = [
  {
    ...
    title: 'Update tolerance spec on Frame Assembly Bracket',
    type: 'major',
    status: 'review',
    impactLevel: 3,
  ...
  },
  {
    ...
    title: 'Material substitution for Wing Spar Rib',
    type: 'emergency',
    status: 'approved',
    impactLevel: 4,
    requestor: 'M. Chen',
  ...
  },
  {
    ...
    title: 'Fastener torque spec revision — Fuselage Panel',
    type: 'minor',
    status: 'implemented',
    impactLevel: 1,
    requestor: 'R. Patel',
  ...
  },
  {
    ...
    title: 'Surface treatment process change — Landing Gear',
    type: 'major',
    status: 'draft',
    impactLevel: 3,
  ...
  },
  {
    ...
    title: 'Weld inspection criteria update — Engine Mount',
    type: 'minor',
    status: 'review',
    impactLevel: 2,
    requestor: 'T. Brooks',
  ...
  },
  {
    ...
    title: 'Drawing revision — Hydraulic Manifold Block',
    type: 'major',
    status: 'rejected',
    impactLevel: 2,
    requestor: 'A. Kumar',
  ...
  },
  {
    ...
    title: 'Emergency re-design of Oxygen System Valve seat',
    type: 'emergency',
    status: 'approved',
    impactLevel: 4,
    requestor: 'L. Morgan',
  ...
  },
  {
    ...
    title: 'NDT procedure update — Rotor Blade attachment',
    type: 'minor',
    status: 'implemented',
    impactLevel: 1,
    requestor: 'D. Foster',
...
export default function EngineeringChangesClient() {
  ...
  const filtered = useMemo(() => {
    return MOCK_ECRS.filter((e) => {
      const matchSearch =
        e.ecrNumber.toLowerCase().includes(search.toLowerCase()) ||
        e.title.toLowerCase().includes(search.toLowerCase()) ||
        e.requestor.toLowerCase().includes(search.toLowerCase());
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
              AS9100D Clause 8.3.6 — Engineering Change Requests
            </p>
          </div>
      ...
      <div className="px-6 py-6 space-y-6">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 p-4 flex flex-wrap gap-3">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              type="text"
              aria-label="Search ECR number, title, requestor..."
                ...
                return (
                  <>
                    <tr
                      ...
                      onClick={() => setExpandedId(isExpanded ? null : ecr.id)}
                    >
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${type.bg} ${type.text}`}
                        >
                          {type.label}
                        </span>
                      </td>
                        ...
                        <div className="flex items-center gap-2">
                          <div className="flex gap-0.5">
                            {[1, 2, 3, 4].map((n) => (
                              <div
                                key={n}
                                className={`w-2 h-4 rounded-sm ${n <= ecr.impactLevel ? impact.color : 'bg-gray-200'}`}
                              />
                            ))}
                          </div>
