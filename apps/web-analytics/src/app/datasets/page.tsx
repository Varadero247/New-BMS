...
import { Plus, Search, Database, RefreshCw, XCircle, CheckCircle, AlertTriangle, Clock } from 'lucide-react';
...
const MOCK_DATASETS: DatasetItem[] = [
  { id: '1', name: 'H&S Incidents', source: 'api-health-safety', type: 'INTERNAL', recordCount: 1247, lastRefreshed: '2026-02-14T06:00:00Z', refreshFrequency: 'Hourly', status: 'ACTIVE', owner: 'Bob Smith', description: 'All incident records including near-misses, LTIs and observations', sizeKb: 842 },
  { id: '2', name: 'CAPA Register', source: 'api-quality', type: 'INTERNAL', recordCount: 389, lastRefreshed: '2026-02-14T05:30:00Z', refreshFrequency: 'Hourly', status: 'ACTIVE', owner: 'Ivan Quality', description: 'Corrective and preventive action records across all modules', sizeKb: 294 },
  { id: '3', name: 'GHG Emissions', source: 'api-esg', type: 'INTERNAL', recordCount: 2160, lastRefreshed: '2026-02-01T00:00:00Z', refreshFrequency: 'Monthly', status: 'ACTIVE', owner: 'Eve Green', description: 'Scope 1, 2 and 3 emissions data with emission factors', sizeKb: 1830 },
  { id: '4', name: 'Supplier Performance', source: 'api-quality + api-inventory', type: 'COMPOSITE', recordCount: 156, lastRefreshed: '2026-02-13T12:00:00Z', refreshFrequency: 'Daily', status: 'ACTIVE', owner: 'Karl Procurement', description: 'Supplier scorecard combining delivery, quality and audit data', sizeKb: 128 },
  { id: '5', name: 'Energy Metering', source: 'External API (Utility Co.)', type: 'EXTERNAL', recordCount: 8760, lastRefreshed: '2026-02-14T01:00:00Z', refreshFrequency: 'Hourly', status: 'ACTIVE', owner: 'Heidi Energy', description: 'Smart meter readings for all site buildings', sizeKb: 3420 },
  { id: '6', name: 'HR Headcount', source: 'api-hr', type: 'INTERNAL', recordCount: 312, lastRefreshed: '2026-02-10T00:00:00Z', refreshFrequency: 'Weekly', status: 'ACTIVE', owner: 'Jane HR', description: 'Employee records, department breakdown and training status', sizeKb: 215 },
  { id: '7', name: 'Financial KPIs', source: 'api-finance', type: 'INTERNAL', recordCount: 48, lastRefreshed: null, refreshFrequency: 'Monthly', status: 'STALE', owner: 'Jane Finance', description: 'Revenue, cost and profitability data by department', sizeKb: 34 },
  { id: '8', name: 'ISO Compliance Scores', source: 'api-gateway', type: 'COMPOSITE', recordCount: 24, lastRefreshed: '2026-02-14T04:00:00Z', refreshFrequency: 'Daily', status: 'ACTIVE', owner: 'Alice Johnson', description: 'Cross-module compliance scoring against ISO standards', sizeKb: 18 },
];
...
export default function DatasetsPage() {
  ...
  async function refreshDataset(id: string) {
    ...
    try {
      ...
      setItems(prev => prev.map(d => d.id === id ? { ...d, lastRefreshed: new Date().toISOString(), status: 'ACTIVE' } : d));
    } catch {
      setItems(prev => prev.map(d => d.id === id ? { ...d, lastRefreshed: new Date().toISOString(), status: 'ACTIVE' } : d));
    } finally {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-wrap gap-3 mb-6">
          <div className="relative flex-1 min-w-48">
            ...
            <input
              type="text"
              aria-label="Search datasets..." placeholder="Search datasets..."
              value={searchTerm}
          ...
          <select aria-label="Filter by type" value={typeFilter} onChange={e => setTypeFilter(e.target.value)} className="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500">
                      ...
                      <tr key={ds.id} className="border-b hover:bg-gray-50 dark:bg-gray-800">
                        ...
                        <td className="py-3 px-4 text-right">
                          <button
                            ...
                            title="Refresh now"
                          >
                            <RefreshCw className={`h-3.5 w-3.5 ${refreshingId === ds.id ? 'animate-spin' : ''}`} />
                          </button>
                        </td>
                      </tr>
                    ))}
