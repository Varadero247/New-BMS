...
const MOCK_SCHEDULES: ScheduleItem[] = [
  {
    id: '1',
    name: 'Weekly H&S KPI Report',
    reportName: 'H&S Performance Dashboard',
    frequency: 'Weekly',
    cronExpression: '0 8 * * MON',
    nextRun: '2026-02-17T08:00:00Z',
    lastRun: '2026-02-10T08:00:00Z',
    recipients: ['bob.smith@ims.local', 'alice.johnson@ims.local'],
    format: 'PDF',
    status: 'ACTIVE',
    owner: 'Bob Smith',
    successCount: 12,
  ...
  },
  {
    ...
    reportName: 'Q1 2026 Compliance Summary',
    frequency: 'Monthly',
    cronExpression: '0 9 1 * *',
    nextRun: '2026-03-01T09:00:00Z',
    lastRun: '2026-02-01T09:00:00Z',
    recipients: ['alice.johnson@ims.local', 'ceo@company.com'],
    format: 'PDF',
    status: 'ACTIVE',
    owner: 'Alice Johnson',
    successCount: 6,
  ...
  {
    id: '3',
    name: 'Fortnightly CAPA Effectiveness',
    reportName: 'CAPA Effectiveness Review',
    frequency: 'Fortnightly',
    cronExpression: '0 8 1,15 * *',
    nextRun: '2026-03-01T08:00:00Z',
    lastRun: '2026-02-14T08:00:00Z',
    recipients: ['ivan.quality@ims.local'],
    format: 'PDF',
    status: 'ACTIVE',
    owner: 'Ivan Quality',
    successCount: 8,
  ...
  {
    id: '4',
    name: 'Daily ESG Emissions Digest',
    reportName: 'ESG Emissions Report',
    frequency: 'Daily',
    cronExpression: '0 7 * * *',
    nextRun: '2026-02-15T07:00:00Z',
    lastRun: '2026-02-14T07:00:00Z',
    recipients: ['eve.green@ims.local'],
    format: 'XLSX',
    status: 'ACTIVE',
    owner: 'Eve Green',
    successCount: 45,
  ...
  {
    id: '5',
    name: 'Monthly Energy Management Report',
    reportName: 'Energy Management Report',
    frequency: 'Monthly',
    cronExpression: '0 9 1 * *',
    nextRun: '2026-03-01T09:00:00Z',
    lastRun: '2026-02-01T09:00:00Z',
    recipients: ['heidi.energy@ims.local', 'facilities@company.com'],
    format: 'PDF',
    status: 'ACTIVE',
    owner: 'Heidi Energy',
    successCount: 3,
  ...
  {
    id: '7',
    name: 'Weekly Supplier Scorecard',
    reportName: 'Supplier Quality Report',
    frequency: 'Weekly',
    cronExpression: '0 8 * * FRI',
    nextRun: null,
    lastRun: null,
    recipients: ['karl@ims.local'],
    format: 'XLSX',
    status: 'PAUSED',
...
function SuccessRate({ success, total }: { success: number; total: number }) {
  if (total === 0)
    return <span className="text-xs text-gray-400 dark:text-gray-500">No runs yet</span>;
  const pct = Math.round((success / total) * 100);
  return (
    <span
      className={`text-xs font-medium ${pct >= 90 ? 'text-green-600' : pct >= 70 ? 'text-yellow-600' : 'text-red-600'}`}
    >
      {pct}% ({success}/{total})
    </span>
  );
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">
              Automate report generation and delivery to recipients
            </p>
          </div>
          <button
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {filtered.map((sched) => (
                      <tr
                        key={sched.id}
                        className={`border-b hover:bg-gray-50 dark:bg-gray-800 ${sched.status === 'PAUSED' ? 'opacity-70' : ''}`}
                      >
                        <td className="py-3 px-4">
                          ...
                          <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">
                            {sched.reportName} · {sched.format}
                          </p>
                        </td>
                        <td className="py-3 px-4 text-center">
                          <span
                            className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${FREQ_COLORS[sched.frequency] || 'bg-gray-100 dark:bg-gray-800 text-gray-700'}`}
                          >
                            {sched.frequency}
                          </span>
                        </td>
                        ...
                        <td className="py-3 px-4 text-center">
                          <SuccessRate
                            success={sched.successCount}
                            total={sched.successCount + sched.failureCount}
                          />
                        </td>
                        <td className="py-3 px-4 text-xs text-gray-500 dark:text-gray-400">
                          <p>
                            {sched.recipients.length} recipient
                            {sched.recipients.length !== 1 ? 's' : ''}
                          </p>
                          <p className="text-gray-400 dark:text-gray-500 truncate max-w-[140px]">
                            {sched.recipients[0]}
                          </p>
                        </td>
                        <td className="py-3 px-4 text-center">
                          <span
                            className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${STATUS_STYLES[sched.status] || 'bg-gray-100 dark:bg-gray-800 text-gray-700'}`}
                          >
                            {sched.status}
                          </span>
                        </td>
            ...
            <div className="bg-white dark:bg-gray-900 rounded-xl shadow-xl w-full max-w-lg p-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    type="text"
                    placeholder="e.g. Weekly Safety Digest"
