...
import { BrainCircuit, AlertTriangle, CheckCircle, Clock, TrendingUp, TrendingDown, Minus, Info } from 'lucide-react';
...
interface NcrForecast {
  ...
  nextMonthForecast: { month: string; forecastCount: number; confidenceInterval: { low: number; high: number }; trend: string; trendReason: string };
  topRiskCategories: { category: string; forecastCount: number; percentOfTotal: number; trend: string }[];
...
export default function PredictionsPage() {
  ...
  const [capaData, setCapaData] = useState<{ predictions: CapaOverrun[]; summary: any; aiDisclosure: any } | null>(null);
  const [auditData, setAuditData] = useState<{ clauses: AuditClause[]; summary: any; aiDisclosure: any; standard: string; auditDate: string } | null>(null);
  ...
  return (
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">Predictive Analytics</h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1">AI-powered predictions for CAPA overruns, audit outcomes, and NCR rates</p>
        </div>
        ...
        <div className="mb-6 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-3 flex items-start gap-3">
          ...
          <div className="text-sm text-blue-700 dark:text-blue-300">
            <span className="font-medium">AI Disclosure:</span> Predictions are generated by IMS Predictive Engine (ims-predict-v1).
            Results should be reviewed by a qualified professional before making decisions.
          </div>
        </div>
                    ...
                    <tbody>
                      {auditData.clauses.map(clause => (
                        <tr key={clause.clause} className="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/30">
                          ...
                          <td className="py-3 px-4 text-center">
                            <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${findingColor(clause.likelyFinding)}`}>
          ...
          <>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              <div className="rounded-lg p-4 bg-purple-50 text-purple-700 dark:bg-purple-900/20 dark:text-purple-300">
                ...
                <p className="text-sm font-medium mt-0.5">{ncrData.nextMonthForecast.month} Forecast</p>
              </div>
              <div className="rounded-lg p-4 bg-blue-50 text-blue-700 dark:bg-blue-900/20 dark:text-blue-300">
                <p className="text-2xl font-bold">{ncrData.nextMonthForecast.confidenceInterval.low}-{ncrData.nextMonthForecast.confidenceInterval.high}</p>
                <p className="text-sm font-medium mt-0.5">Confidence Range</p>
              </div>
              <div className="rounded-lg p-4 bg-orange-50 text-orange-700 dark:bg-orange-900/20 dark:text-orange-300">
                <p className="text-2xl font-bold flex items-center gap-1">
                  <TrendIcon trend={ncrData.nextMonthForecast.trend} /> {ncrData.nextMonthForecast.trend}
                </p>
                      ...
                      <div key={sup.supplier} className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${
                            sup.trend === 'HIGH_RISK' ? 'bg-red-100 text-red-700' :
                            sup.trend === 'MODERATE_RISK' ? 'bg-orange-100 text-orange-700' :
