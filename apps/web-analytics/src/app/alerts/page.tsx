...
const MOCK_ALERTS: AlertItem[] = [
  {
    id: '1',
    name: 'NCR Rate Exceeded',
    condition: 'NCR rate > 5 per week',
    metric: 'NCR Rate',
    threshold: 5,
    severity: 'CRITICAL',
    status: 'FIRING',
    lastTriggered: '2026-02-14T08:30:00Z',
    triggerCount: 3,
    owner: 'Ivan Quality',
    module: 'Quality',
    enabled: true,
  ...
  {
    id: '2',
    name: 'CAPA Overdue Threshold',
    condition: 'Overdue CAPAs > 10',
    metric: 'Overdue CAPAs',
    threshold: 10,
    severity: 'WARNING',
    status: 'FIRING',
    lastTriggered: '2026-02-13T14:00:00Z',
    triggerCount: 7,
    owner: 'Alice Johnson',
    module: 'Quality',
    enabled: true,
  ...
  {
    id: '3',
    name: 'Carbon Intensity Spike',
    condition: 'Carbon intensity > 30 tCO2/£m',
    metric: 'Carbon Intensity',
    threshold: 30,
    severity: 'WARNING',
    status: 'OK',
    lastTriggered: '2026-01-20T09:00:00Z',
    triggerCount: 2,
    owner: 'Eve Green',
    module: 'ESG',
    enabled: true,
  ...
  {
    id: '4',
    name: 'Lost Time Injury Recorded',
    condition: 'LTI count increases',
    metric: 'LTI Count',
    threshold: 0,
    severity: 'CRITICAL',
    status: 'FIRING',
    lastTriggered: '2026-02-12T11:15:00Z',
    triggerCount: 1,
    owner: 'Bob Smith',
    module: 'H&S',
    enabled: true,
  },
  {
    ...
    severity: 'WARNING',
    status: 'OK',
    lastTriggered: '2026-01-28T16:00:00Z',
    triggerCount: 4,
  ...
  },
  {
    ...
    condition: 'Monthly energy > 1500 MWh',
    metric: 'Energy MWh',
    threshold: 1500,
    severity: 'INFO',
    status: 'OK',
  ...
  {
    id: '7',
    name: 'Training Completion Below Target',
    condition: 'Completion rate < 90%',
    metric: 'Training Completion',
    threshold: 90,
    severity: 'INFO',
    status: 'OK',
    lastTriggered: '2026-02-01T08:00:00Z',
    triggerCount: 1,
    owner: 'Jane HR',
    module: 'HR',
    enabled: true,
  ...
  {
    id: '8',
    name: 'High-Risk Assessment Created',
    condition: 'Risk score > 20 on new assessment',
    metric: 'Risk Score',
    threshold: 20,
    severity: 'WARNING',
    status: 'FIRING',
    lastTriggered: '2026-02-14T10:45:00Z',
    triggerCount: 2,
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">
              Data-driven threshold alerts and notifications
            </p>
          </div>
          <button
          ...
          <div className="mb-6 rounded-lg border border-red-200 bg-red-50 p-4 flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-red-800">
                {firing} alert{firing > 1 ? 's are' : ' is'} currently firing
              </p>
              <p className="text-xs text-red-600 mt-0.5">
                Review and acknowledge the conditions below to resolve.
              </p>
            </div>
          </div>
        )}
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {filtered.map((alert) => (
                      <tr
                        key={alert.id}
                        className={`border-b hover:bg-gray-50 dark:bg-gray-800 ${!alert.enabled ? 'opacity-60' : ''}`}
                      >
                        ...
                        <td className="py-3 px-4 text-gray-500 dark:text-gray-400 text-xs max-w-xs">
                          {alert.condition}
                        </td>
                        <td className="py-3 px-4">
                        ...
                        <td className="py-3 px-4 text-center">
                          <span
                            className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${SEVERITY_STYLES[alert.severity] || 'bg-gray-100 dark:bg-gray-800 text-gray-700'}`}
                          >
                            {alert.severity}
                          </span>
                        </td>
                        <td className="py-3 px-4 text-center">
                          <span
                            className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${STATUS_STYLES[alert.status] || 'bg-gray-100 dark:bg-gray-800 text-gray-700'}`}
                          >
                            {alert.status}
                          </span>
                        </td>
                        ...
                        <td className="py-3 px-4 text-center">
                          ...
                          >
                            <span
                              className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white dark:bg-gray-900 shadow transition-transform ${alert.enabled ? 'translate-x-4' : 'translate-x-1'}`}
                            />
                          </button>
                        </td>
                      </tr>
            ...
            <div className="bg-white dark:bg-gray-900 rounded-xl shadow-xl w-full max-w-lg p-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    type="text"
                    placeholder="e.g. NCR Rate Exceeded"
                ...
                <div>
                  ...
                  <input
                    type="text"
                    placeholder="e.g. NCR rate > 5 per week"
                    className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"
                  />
                </div>
                <div className="grid grid-cols-2 gap-3">
