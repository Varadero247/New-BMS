...
import { Plus, Search, PieChart, LayoutDashboard, Lock, Globe, XCircle, Edit, Copy, Star } from 'lucide-react';
    ...
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">Custom Dashboards</h1>
            <p className="text-gray-500 dark:text-gray-400 mt-1">Create, share and manage data visualisation dashboards</p>
          </div>
          <button
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          {[
            { label: 'Total Dashboards', value: items.length, color: 'bg-purple-50 text-purple-700' },
            { label: 'Featured', value: featured.length, color: 'bg-yellow-50 text-yellow-700' },
            { label: 'Total Views', value: totalViews.toLocaleString(), color: 'bg-blue-50 text-blue-700' },
          ].map(s => (
        ...
        {featured.length > 0 && (
          <div className="mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {featured.map(d => (
                <Card key={d.id} className="border-yellow-200 bg-yellow-50/30 hover:shadow-md transition-shadow cursor-pointer">
                  <CardContent className="pt-4 pb-4">
                    <div className="flex items-start justify-between mb-2">
                      <div className={`flex items-center gap-1 text-xs px-2 py-0.5 rounded font-medium ${VISIBILITY_STYLES[d.visibility]}`}>
                        <VisibilityIcon v={d.visibility} />
                        {d.visibility}
                      </div>
                      <Star className="h-3.5 w-3.5 text-yellow-500 fill-yellow-500" />
                    </div>
                    <h3 className="font-semibold text-gray-900 dark:text-gray-100 mb-1">{d.name}</h3>
              ...
              <Card key={d.id} className="hover:shadow-md transition-shadow cursor-pointer">
                <CardContent className="pt-5">
                  <div className="flex items-start justify-between mb-2">
                    ...
                    {d.isFeatured && <Star className="h-3.5 w-3.5 text-yellow-500 fill-yellow-500" />}
                  </div>
                  <h3 className="font-semibold text-gray-900 dark:text-gray-100 mt-2 mb-1">{d.name}</h3>
                  <p className="text-xs text-gray-500 dark:text-gray-400 line-clamp-2">{d.description}</p>

                  <div className="mt-3 flex flex-wrap gap-1">
                    {d.tags.slice(0, 3).map(t => (
                      <span key={t} className="text-[10px] px-1.5 py-0.5 bg-purple-50 text-purple-600 rounded">{t}</span>
                    ))}
                  </div>

                  <div className="mt-3 pt-3 border-t border-gray-100 dark:border-gray-700 flex items-center justify-between">
                    <div className="text-xs text-gray-400 dark:text-gray-500">
                      <span>{d.widgetCount} widgets · {d.viewCount} views</span>
                    </div>
                    <div className="flex gap-1">
                      <button className="p-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-400 dark:text-gray-500" title="Edit">
                  ...
                  <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">{d.owner} · Updated {d.lastUpdated}</p>
                </CardContent>
              </Card>
            ))}
          </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div className="bg-white dark:bg-gray-900 rounded-xl shadow-xl w-full max-w-lg p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">New Dashboard</h2>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input type="text" placeholder="e.g. Operations Weekly Review" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Description</label>
                  <textarea rows={2} placeholder="What is this dashboard for?" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input type="text" placeholder="e.g. Quality, KPIs, Weekly" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500" />
                </div>
              </div>
              <div className="flex justify-end gap-3 mt-6">
                ...
                <button className="px-4 py-2 text-sm bg-purple-600 text-white rounded-lg hover:bg-purple-700">Create Dashboard</button>
              </div>
            </div>
          </div>
        )}
