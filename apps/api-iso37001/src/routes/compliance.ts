...
router.get('/stats', async (_req: Request, res: Response) => {
  try {
    const [total, compliant, nonCompliant, partial, byCategory] = await Promise.all([
      (prisma as any).abCompliance.count({ where: { deletedAt: null } }),
      (prisma as any).abCompliance.count({ where: { deletedAt: null, status: 'COMPLIANT' } }),
      (prisma as any).abCompliance.count({ where: { deletedAt: null, status: 'NON_COMPLIANT' } }),
      (prisma as any).abCompliance.count({ where: { deletedAt: null, status: 'PARTIALLY_COMPLIANT' } }),
      (prisma as any).abCompliance.groupBy({ by: ['category'], where: { deletedAt: null }, _count: { id: true } }),
    ]);
...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    const [items, total] = await Promise.all([
      (prisma as any).abCompliance.findMany({ where, skip, take: limit, orderBy: { createdAt: 'desc' } }),
      (prisma as any).abCompliance.count({ where }),
    ]);
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const record = await (prisma as any).abCompliance.create({
      data: {
        ...parsed.data,
        referenceNumber,
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    const record = await (prisma as any).abCompliance.findFirst({ where: { id: req.params.id, deletedAt: null } });
...
router.put('/:id', async (req: Request, res: Response) => {
  try {
    ...
    const existing = await (prisma as any).abCompliance.findFirst({ where: { id: req.params.id, deletedAt: null } });
    ...
    const record = await (prisma as any).abCompliance.update({ where: { id: req.params.id }, data });
    res.json({ success: true, data: record });
  } catch (error: unknown) {
...
router.delete('/:id', async (req: Request, res: Response) => {
  try {
    ...
    await (prisma as any).abCompliance.update({ where: { id: req.params.id }, data: { deletedAt: new Date() } });
