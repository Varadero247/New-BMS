...
```bash
pnpm test                                # All Jest tests (~1,223,283 across 1,149 suites / 489 projects — all passing)
...
./scripts/check-services.sh              # Service health checks (89 services)
...
./scripts/typecheck-all.sh               # TypeScript check all 480 projects
...
- **Tests**: **~1,223,283 Jest tests (1,149 suites / 489 projects)** + 40 integration test scripts (~1,800+ assertions) + 111 Jest integration tests — ALL PASSING, 0 failures
...
- Login pages built for all 44 web apps (Training Portal uses activation-key gate instead of login page)
